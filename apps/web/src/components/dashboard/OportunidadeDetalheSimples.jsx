    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col">
        <div className="sticky top-0 bg-white border-b p-6 z-10 flex-shrink-0 flex items-center justify-between">
          ...
          <button onClick={() => onOpenChange(false)} className="ml-4 p-2 hover:bg-slate-100 rounded-lg">
        ...
        <div className="p-6 flex-1 overflow-y-auto space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="mt-1 text-lg font-bold text-green-600">{formatCurrency(op.valor_estimado)}</p>
            </div>
            <div>
              <Label className="text-slate-500">Status</Label>
              <p className="mt-1">
                {op.status_nome ? (
                  <Badge variant="outline">{op.status_nome}</Badge>
                ) : '-'}
              </p>
            </div>
            <div>
              <Label className="text-slate-500">Probabilidade</Label>
              <p className="mt-1 font-medium text-slate-800">{op.probabilidade != null ? `${op.probabilidade}%` : '-'}</p>
            </div>
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {op.licitacao_data && (
                  <div>
                    ...
                    <p className="mt-1 font-medium text-slate-800">{formatDate(op.licitacao_data)}</p>
                  </div>
                )}
                ...
                {op.licitacao_data_proposta && (
                  <div>
                    ...
                    <p className="mt-1 font-medium text-slate-800">{formatDate(op.licitacao_data_proposta)}</p>
                  </div>
                )}
                {op.licitacao_data_impugnacao && (
                  <div>
                    <Label className="text-slate-500">Data Impugnação</Label>
                    <p className="mt-1 font-medium text-slate-800">{formatDate(op.licitacao_data_impugnacao)}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {op.data_fechamento_prevista && (
            <div className="border-t pt-4">
              ...
              <p className="mt-1 font-medium text-slate-800">{formatDate(op.data_fechamento_prevista)}</p>
            </div>
          )}
          ...
          {(op.cidade || op.estado || op.endereco) && (
            <div className="border-t pt-4">
              ...
              <p className="mt-1 text-slate-800">
                {[op.endereco, op.numero, op.bairro, op.cidade, op.estado].filter(Boolean).join(', ')}
              </p>
            </div>
          )}
          ...
          {op.descricao && (
            <div className="border-t pt-4">
              ...
              <p className="mt-2 text-sm text-slate-700 whitespace-pre-wrap">{op.descricao.replace(/<[^>]*>/g, '')}</p>
            </div>
          )}
        </div>
