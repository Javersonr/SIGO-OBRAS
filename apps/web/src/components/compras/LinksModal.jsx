...
import { Copy, Mail, MessageSquare, ExternalLink, RefreshCw, Clock, CheckCircle2, AlertCircle, Filter, ArrowUpDown, X, Trash2 } from 'lucide-react';
...
export default function LinksModal({ open, onOpenChange, cotacao, empresaAtiva, onDelete }) {
  ...
  const loadFornecedores = async () => {
    setLoading(true);
    try {
      const cotFornecedores = await base44.entities.CotacaoFornecedor.filter({ 
...
${empresaAtiva.nome_fantasia || empresaAtiva.razao_social || empresaAtiva.nome}
      ...
      await base44.entities.CotacaoFornecedor.update(fornecedor.id, {
  ...
  const reenviarParaTodos = async () => {
    const naoRespondidos = fornecedores.filter(f => 
      f.status === 'Enviada' || f.status === 'Visualizada' || f.status === 'Respondida Parcialmente'
    );
    ...
    if (!confirm(`Enviar lembrete para ${naoRespondidos.length} fornecedor(es) que ainda não responderam?`)) {
      return;
    }
  ...
  const handleExcluirCotacao = async () => {
    if (!confirm(`Tem certeza que deseja excluir a cotação ${cotacao.numero}? Esta ação não pode ser desfeita.`)) return;
    setExcluindo(true);
    try {
      const cotRespostas = await base44.entities.CotacaoResposta.filter({ cotacao_id: cotacao.id });
      await sleep(300);
      const cotItens = await base44.entities.CotacaoItem.filter({ cotacao_id: cotacao.id });
      await sleep(300);
      const cotFornecedores = await base44.entities.CotacaoFornecedor.filter({ cotacao_id: cotacao.id });
      await sleep(300);

      await deleteSequential(cotRespostas, (id) => base44.entities.CotacaoResposta.delete(id));
      await deleteSequential(cotItens, (id) => base44.entities.CotacaoItem.delete(id));
      await deleteSequential(cotFornecedores, (id) => base44.entities.CotacaoFornecedor.delete(id));
  ...
  if (loading) {
    return (
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col w-full sm:w-[90%] md:w-[80%] lg:w-[70%]">
        ...
        <div className="space-y-4 mt-6">
          {fornecedores.length === 0 ? (
            ...
            fornecedores
              ...
              .sort((a, b) => {
                if (ordenacao === 'nome') return a.fornecedor_nome.localeCompare(b.fornecedor_nome);
                  ...
                  <CardContent className="p-4 space-y-3">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="font-semibold text-slate-800">{fornecedor.fornecedor_nome}</h3>
                        </div>
                        ...
                        {fornecedor.ultima_notificacao && fornecedor.status === 'Aguardando' && (
                          <p className="text-xs text-slate-400 mt-1 ml-7">
                            Último envio: {new Date(fornecedor.ultima_notificacao).toLocaleDateString('pt-BR')} às {new Date(fornecedor.ultima_notificacao).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                          </p>
                        )}
                      </div>
                      <Badge className={
                    ...
                    {credenciais[fornecedor.fornecedor_id] && (
                      <div className="p-2 bg-slate-50 border border-slate-200 rounded text-xs space-y-0.5">
                        ...
                        <p className="text-slate-600">🔑 Login: <span className="font-mono">{credenciais[fornecedor.fornecedor_id].email}</span></p>
                        <p className="text-slate-600">🔑 Senha: <span className="font-mono font-bold">{credenciais[fornecedor.fornecedor_id].senha}</span></p>
                      </div>
                    )}
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={(e) => { e.preventDefault(); e.stopPropagation(); window.open(link, '_blank', 'noopener,noreferrer'); }}
                    ...
                    <div className="flex gap-2 pt-2">
                     {(fornecedor.status === 'Enviada' || fornecedor.status === 'Visualizada' || fornecedor.status === 'Respondida Parcialmente') && fornecedor.fornecedor_email && (
                        <Button
                          size="sm"
                          variant="outline"
                      ...
                      {fornecedor.status === 'Respondida Totalmente' && fornecedor.fornecedor_email && (
                        <Button
                          size="sm"
                          variant="outline"
                    ...
                    {/* Info da Resposta */}
                    {(fornecedor.status === 'Respondida Totalmente' || fornecedor.status === 'Respondida Parcialmente') && (
                      <div className="bg-green-50 border border-green-200 rounded p-2 text-xs text-green-700">
                        ✅ Respondido em {fornecedor.data_resposta ? new Date(fornecedor.data_resposta).toLocaleDateString('pt-BR') : '-'}
                        {fornecedor.total_cotado > 0 && ` • Total: R$ ${fornecedor.total_cotado.toFixed(2)}`}
                      </div>
                    )}
                  </CardContent>
                </Card>
