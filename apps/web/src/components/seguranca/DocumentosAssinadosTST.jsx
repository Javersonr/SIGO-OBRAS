...
import { Shield, Wrench, FileText, List, Upload, Eye, Trash2, History, X, ChevronDown, ChevronUp, BookOpen } from 'lucide-react';
...
const DOCUMENTOS = [
  { key: 'ferramentais_anexos',       label: 'Lista de Ferramentas',   icon: Wrench,   onVisualizar: 'onVisualizarFerramentas' },
  { key: 'epis_anexos',               label: 'Lista de EPIs',          icon: Shield,   onVisualizar: 'onVisualizarEPI' },
  { key: 'autorizacao_formal_anexos', label: 'Autorização Formal',     icon: FileText, onVisualizar: 'onVisualizarAutorizacaoFormal' },
  { key: 'direito_recusa_anexos',     label: 'Direito de Recusa',      icon: FileText, onVisualizar: 'onVisualizarDireitoRecusa' },
  { key: 'ordem_servicos_anexos',     label: 'Ordem de Serviço',       icon: List,     onVisualizar: 'onVisualizarOrdemServico' },
];
...
export default function DocumentosAssinadosTST({
  ...
  const callbacks = { onVisualizarFerramentas, onVisualizarEPI, onVisualizarAutorizacaoFormal, onVisualizarDireitoRecusa, onVisualizarOrdemServico };
  ...
  const abrirHistoricoCompleto = async (key) => {
    try {
      ...
      setHistoricoData(sorted.map(r => ({ nome: r.nome_arquivo, url: r.url, data_upload: r.data_upload })));
  ...
  const getAnexos = (key) => {
    try { return JSON.parse(funcionarioForm[key] || '[]'); } catch { return []; }
  };
  ...
  const handleUpload = async (e, key) => {
    ...
    try {
      ...
      if (selectedFuncionario) {
        await base44.entities.Funcionario.update(selectedFuncionario.id, { [key]: JSON.stringify(anexos) });
      }
  ...
  return (
              ...
              <div key={key} className="bg-white rounded-lg border border-slate-200 overflow-hidden">
                ...
                {aberto && anexos.length > 0 && (
                  <div className="border-t border-slate-100 px-2.5 pb-2.5 pt-2 space-y-1.5 bg-slate-50">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Histórico de anexos</p>
                    {anexos.map((anexo, idx) => (
                      <div key={idx} className="flex items-center gap-2 bg-white rounded border border-slate-200 p-2">
                        <FileText className="w-3.5 h-3.5 text-blue-500 flex-shrink-0" />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-slate-700 truncate">{anexo.nome}</p>
                          {anexo.data_upload && (
                            <p className="text-xs text-slate-400">
                              {format(new Date(anexo.data_upload), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}
                            </p>
                          )}
                        </div>
                        <Button
    ...
    <HistoricoDocumentosModal
      ...
      onOpenChange={(v) => { if (!v) setShowHistoricoKey(null); }}
