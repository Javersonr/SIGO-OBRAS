...
export default function CalendarioFinanceiro() {
  ...
  const handleAbrirTransacao = async (t) => {
    ...
    try {
      const anexos = await base44.entities.TransacaoAnexo.filter({ transacao_id: t.id });
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all(
      empresas.map(e => base44.entities.TransacaoFinanceira.filter({ empresa_id: e.id }))
    )
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className={cn(
          ...
          <div>
            <p className={cn("text-xs", resumoMes.saldo >= 0 ? "text-blue-700" : "text-orange-700")}>Saldo</p>
            <p className={cn("text-sm font-bold", resumoMes.saldo >= 0 ? "text-blue-700" : "text-orange-700")}>
        ...
        <CardContent className="p-3">
          ...
          <div className="grid grid-cols-7 gap-1 mb-1">
            {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map(dia => (
              <div key={dia} className="text-center text-xs font-semibold text-slate-500 py-1">{dia}</div>
            ))}
          </div>
                  ...
                  <div className="space-y-0.5 mt-0.5">
                    {showItems.map(t => (
                      <div
                        ...
                        title={t.descricao}
                        onClick={(e) => { e.stopPropagation(); handleAbrirTransacao(t); }}
                      >
                        {t.descricao || '-'}
                      </div>
                    ))}
                    {extra > 0 && (
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          setPopoverDia(prev => prev?.dateStr === dateStr ? null : { dateStr, rect, items });
                        }}
                      >
                        +{extra} mais
                      </button>
        ...
        >
          <div className="sticky top-0 bg-white border-b px-4 py-3 flex items-center justify-between">
            <p className="text-sm font-semibold text-slate-800 capitalize">
              {new Date(popoverDia.dateStr + 'T12:00:00').toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' })}
            </p>
            ...
            {popoverDia.items.map(t => {
              ...
              return (
                <div
                  ...
                  onClick={() => { setPopoverDia(null); handleAbrirTransacao(t); }}
                >
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-sm font-medium text-slate-800 truncate flex-1">{t.descricao || '-'}</p>
                    <span className={cn("text-sm font-bold shrink-0", isReceita ? "text-green-700" : "text-red-700")}>
                      {isReceita ? '+' : '-'}{formatCurrency(t.valor)}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    {t.categoria_nome && (
                      <Badge variant="outline" className="text-xs py-0">{t.categoria_nome}</Badge>
                    )}
                    {t.projeto_nome && (
                      <span className="text-xs text-slate-500">{t.projeto_nome}</span>
                    )}
                    <Badge className={cn("text-xs py-0",
                      t.status === 'Realizado' || t.status === 'pago' ? "bg-green-100 text-green-700" :
      ...
      <DetalheDespesaModal
        ...
        onBaixar={() => { setShowDespesaDetalhe(false); }}
      />
      <DetalheReceitaModal
