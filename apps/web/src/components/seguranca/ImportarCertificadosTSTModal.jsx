...
import { Upload, CheckCircle2, AlertCircle, Loader2, X, ArrowRight, FileText } from 'lucide-react';
...
export default function ImportarCertificadosTSTModal({
  ...
  const handleIniciarImportacao = async () => {
    ...
    try {
      ...
      const outrosFuncionarios = funcionarios.filter(f => f.id !== funcionarioAtual.id && f.ativo !== false);
  ...
  const handleConfirmarEAvancar = async () => {
    ...
    if (resultadoAtual?.resultados?.length > 0) {
      ...
      for (const novo of novosAnexos) {
        const idx = anexosExistentes.findIndex(a => a.treinamento_id && a.treinamento_id === novo.treinamento_id);
        if (idx >= 0) {
  ...
  const progresso = filaFuncionarios.length > 0 ? Math.round(((indiceAtual + (fase === 'resultado' ? 1 : 0)) / filaFuncionarios.length) * 100) : 0;
          ...
          {fase !== 'upload' && fase !== 'concluido' && (
            <div className="mt-2">
              <div className="flex items-center justify-between text-xs text-slate-500 mb-1">
                <span>Funcionário {indiceAtual + 1} de {filaFuncionarios.length}</span>
                <span>{progresso}%</span>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div
                ...
                className={cn(
                  ...
                  zipFile ? "border-green-400 bg-green-50" : "border-slate-300 hover:border-blue-400 hover:bg-blue-50"
                )}
              >
                <input ref={fileInputRef} type="file" accept=".zip" onChange={handleZipSelect} className="hidden" />
                <Upload className={cn("w-10 h-10 mx-auto mb-3", zipFile ? "text-green-500" : "text-slate-400")} />
                {zipFile ? (
                  <div>
                    <p className="font-semibold text-green-700">{zipFile.name}</p>
                    <p className="text-xs text-green-600 mt-1">{(zipFile.size / 1024 / 1024).toFixed(1)} MB</p>
                  </div>
                ) : (
                  <div>
                    <p className="font-semibold text-slate-700">Clique para selecionar o arquivo ZIP</p>
              ...
              <div className="flex gap-3">
                <Button variant="outline" onClick={handleFechar} className="flex-1">Cancelar</Button>
                <Button
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2 p-3 bg-slate-50 rounded-lg">
                ...
                <div className="ml-auto flex gap-2">
                  <Badge className="bg-green-100 text-green-800 border-green-200">{resultadoAtual.processados} ok</Badge>
                  {resultadoAtual.erros_count > 0 && (
                    <Badge className="bg-red-100 text-red-800 border-red-200">{resultadoAtual.erros_count} erro(s)</Badge>
                  )}
                </div>
              </div>
                  ...
                  {resultadoAtual.resultados.map((r, idx) => (
                    <div key={idx} className="flex items-start gap-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-slate-800 truncate">{r.treinamento_nome}</p>
                        {r.match_encontrado && (
                          <p className="text-xs text-green-700">✓ Casado com: {r.match_nome}</p>
                        )}
                        {!r.match_encontrado && (
                          <p className="text-xs text-amber-600">⚠ Sem correspondência - será salvo como extra</p>
                        )}
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  Confirmar e {filaFuncionarios.length - indiceAtual - 1 > 0 ? <><ArrowRight className="w-4 h-4" /> Próximo</> : 'Finalizar'}
                </Button>
              </div>
            </div>
          )}
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {resumoGeral.map((r, idx) => (
                  <div key={idx} className={cn(
                    ...
                    r.processados > 0 ? "bg-green-50 border-green-200" : "bg-slate-50 border-slate-200"
                  )}>
                    ...
                    <div className="flex gap-2">
                      {r.processados > 0 && <Badge className="bg-green-100 text-green-800 text-xs">{r.processados} importado(s)</Badge>}
                      {r.erros_count > 0 && <Badge className="bg-red-100 text-red-800 text-xs">{r.erros_count} erro(s)</Badge>}
                      {r.processados === 0 && r.erros_count === 0 && <span className="text-xs text-slate-400">Nenhum certificado</span>}
                    </div>
                  </div>
                ))}
              </div>

              <Button onClick={handleFechar} className="w-full bg-blue-600 hover:bg-blue-700 text-white">
                Fechar
              </Button>
            </div>
          )}
