...
export default function BarraAcoesMassa({
  ...
  const ferramentasSel = itensSelecionados.flatMap((itemKey) => {
    const grupo = Object.values(ferramentasAgrupadas).find(
      (f) => `${f.codigo}-${f.descricao}` === itemKey
    );
    if (!grupo) return [];
  ...
  const handleAlterarTipo = async (novoTipo) => {
    ...
    try {
      await Promise.all(ids.map((id) => sigo.entities.Ferramenta.update(id, { tipo: novoTipo })));
    ...
    <>
      <div className="flex items-center gap-3 p-3 bg-amber-50 border border-amber-200 rounded-lg flex-wrap">
        <span className="text-sm font-medium text-amber-800">
          {itensSelecionados.length} grupo(s) selecionado(s)
        </span>
        ...
        <Button
          ...
          onClick={() => abrirObrigatorio("caminhao")}
        >
        ...
        <Button
          ...
          onClick={() => abrirObrigatorio("funcao")}
        >
        ...
        <Button
          ...
          onClick={() => handleAlterarTipo("Ferramenta")}
          disabled={alterandoTipo}
        >
          <RefreshCw className="w-4 h-4" />
