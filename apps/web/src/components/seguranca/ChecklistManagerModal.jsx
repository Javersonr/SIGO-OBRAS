...
import { Plus, Trash2, Edit2, Image, X, Upload, ChevronDown, ChevronUp, Copy, Download, FileSpreadsheet } from 'lucide-react';
...
export default function ChecklistManagerModal({ open, onOpenChange, empresaAtiva }) {
  ...
  const handleSalvar = async () => {
    ...
    try {
      const data = {
        ...
        itens: JSON.stringify(form.itens.map((it, i) => ({ ...it, id: it.id || `item_${i}_${Date.now()}` }))),
  ...
  const handleAdicionarItem = () => {
    if (!novoItem.nome) { toast.error('Informe o nome do item'); return; }
    setForm(prev => ({
      ...prev,
  ...
  const handleImportarItens = async (e) => {
    ...
    try {
      ...
      const novosItens = rows.map((row, i) => ({
        id: `item_${i}_${Date.now()}`,
        nome: row['Nome do Item'] || row['nome'] || row['NOME'] || '',
        descricao: row['Descrição'] || row['descricao'] || '',
        obrigatorio: (row['Obrigatório'] || row['obrigatorio'] || '').toString().toLowerCase() !== 'não',
        foto_referencia_url: row['Foto Referência'] || row['foto_referencia_url'] || '',
      })).filter(it => it.nome);
      if (novosItens.length === 0) { toast.error('Nenhum item válido encontrado'); return; }
      ...
      <DialogContent data-fullscreen-modal className="p-0 flex flex-col !rounded-none !border-0"
        ...
        <DialogHeader className="px-6 py-4 border-b flex-shrink-0">
          <div className="flex items-center justify-between">
            <DialogTitle>
              {editando ? (editando === 'novo' ? 'Novo Checklist' : `Editar: ${editando.nome}`) : 'Modelos de Checklist'}
            </DialogTitle>
            {!editando && (
              ...
              {loading ? <p className="text-center text-slate-400">Carregando...</p> :
                checklists.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-slate-400 text-sm mt-1">Crie um modelo de checklist para usar nas inspeções</p>
                  </div>
                ) : checklists.map(cl => (
                  <div key={cl.id} className="border rounded-lg p-4 flex items-center justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-slate-800">{cl.nome}</p>
                      {cl.categoria && <Badge variant="outline" className="text-xs mt-1">{cl.categoria}</Badge>}
                      <p className="text-sm text-slate-500 mt-1">{cl.total_itens || 0} itens</p>
                      {cl.descricao && <p className="text-xs text-slate-400 mt-1">{cl.descricao}</p>}
                    </div>
                    <div className="flex gap-2 flex-shrink-0">
                      <Button variant="outline" size="icon" onClick={() => handleEditar(cl)} title="Editar">
                        <Edit2 className="w-4 h-4" />
                      </Button>
                      <Button variant="outline" size="icon" onClick={() => handleDuplicarChecklist(cl)} title="Duplicar">
                        <Copy className="w-4 h-4" />
                      </Button>
                      <Button variant="outline" size="icon" onClick={() => handleExportarChecklist(cl)} title="Exportar Excel">
                        <Download className="w-4 h-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleExcluir(cl)} title="Excluir">
            ...
            <div className="space-y-6 max-w-3xl mx-auto">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <Input value={form.nome} onChange={e => setForm(p => ({ ...p, nome: e.target.value }))} placeholder="Ex: Inspeção de EPI" className="mt-1.5" />
                </div>
                <div>
                  <Label>Categoria</Label>
                  <Input value={form.categoria} onChange={e => setForm(p => ({ ...p, categoria: e.target.value }))} placeholder="Ex: EPI, Veículos, Campo" className="mt-1.5" />
                </div>
                <div className="col-span-2">
                  <Label>Descrição</Label>
                  <Textarea value={form.descricao} onChange={e => setForm(p => ({ ...p, descricao: e.target.value }))} rows={2} className="mt-1.5" />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-slate-800">Itens do Checklist ({form.itens.length})</h3>
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" className="gap-2" onClick={async () => {
                      try {
                        const response = await base44.functions.invoke('gerarModeloChecklistExcel');
                    ...
                    <label className="cursor-pointer">
                      ...
                      <input type="file" className="hidden" accept=".xlsx,.xls,.csv" onChange={handleImportarItens} />
                    </label>
                  </div>
                </div>
                ...
                <div className="border rounded-lg p-4 bg-slate-50 mb-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      <Input value={novoItem.nome} onChange={e => setNovoItem(p => ({ ...p, nome: e.target.value }))} placeholder="Nome do item *" />
                    </div>
                    <div className="col-span-2">
                      <Input value={novoItem.descricao} onChange={e => setNovoItem(p => ({ ...p, descricao: e.target.value }))} placeholder="Descrição (opcional)" />
                    </div>
                    <div className="flex items-center gap-2">
                      <input type="checkbox" id="obrig" checked={novoItem.obrigatorio} onChange={e => setNovoItem(p => ({ ...p, obrigatorio: e.target.checked }))} className="w-4 h-4" />
                      <label htmlFor="obrig" className="text-sm text-slate-600">Obrigatório</label>
                    </div>
                    <Button onClick={handleAdicionarItem} className="bg-amber-500 hover:bg-amber-600 gap-2">
                  ...
                  {form.itens.map((item, idx) => (
                    <div key={idx} className="border rounded-lg p-3 flex items-start gap-3">
                      ...
                      <div className="flex-shrink-0">
                        {item.foto_referencia_url ? (
                          <div className="relative w-16 h-16 rounded-lg overflow-hidden border">
                            <img src={item.foto_referencia_url} alt="Ref" className="w-full h-full object-cover" />
                            <button
                              onClick={() => setForm(p => { const itens = [...p.itens]; itens[idx] = { ...itens[idx], foto_referencia_url: '' }; return { ...p, itens }; })}
                          ...
                          <label className="w-16 h-16 border-2 border-dashed rounded-lg flex flex-col items-center justify-center cursor-pointer hover:border-amber-400 bg-slate-50">
                            ...
                            <input type="file" className="hidden" accept="image/*" onChange={e => handleUploadFotoReferencia(e, idx)} />
                          </label>
                        )}
                      </div>
                      {/* Info */}
                      <div className="flex-1 min-w-0">
                        ...
                        {item.descricao && <p className="text-xs text-slate-500">{item.descricao}</p>}
                        {item.obrigatorio && <Badge className="text-xs mt-1 bg-red-100 text-red-700">Obrigatório</Badge>}
                        {item.foto_referencia_url && <p className="text-xs text-green-600 mt-1">✓ Foto referência</p>}
                      </div>
                      {/* Ações */}
                      <div className="flex flex-col gap-1 flex-shrink-0">
                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleMoverItem(idx, -1)} disabled={idx === 0}>
                          <ChevronUp className="w-3 h-3" />
                        </Button>
                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleMoverItem(idx, 1)} disabled={idx === form.itens.length - 1}>
                          <ChevronDown className="w-3 h-3" />
                        </Button>
                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleRemoverItem(idx)}>
        ...
        <div className="px-6 py-4 border-t flex-shrink-0 flex gap-3 justify-end">
          {editando ? (
            <>
              ...
              <Button onClick={handleSalvar} className="bg-amber-500 hover:bg-amber-600">Salvar Checklist</Button>
            </>
          ) : (
