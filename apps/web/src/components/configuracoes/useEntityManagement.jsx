...
export const useEntityManagement = (entityName, empresaAtiva) => {
  ...
  const exportToExcel = (items, columns) => {
    const dados = items.map((item) => columns.map((col) => item[col] || ""));
  ...
  const deletarSelecionados = async (ids, entityObj) => {
    ...
    for (let i = 0; i < ids.length; i += BATCH_SIZE) {
      ...
      for (let j = 0; j < batch.length; j += CONCURRENT_CHUNK) {
        ...
        const results = await Promise.allSettled(chunk.map((id) => entityObj.delete(id)));
  ...
  const limparTodos = async (entityObj) => {
    if (
      !confirm(
        "⚠️ ATENÇÃO: Isso irá apagar TODOS os registros. Esta ação não pode ser desfeita. Deseja continuar?"
      )
    )
      return;
