...
export default function UsuariosTab({ usuarios, user, enviandoReset, setSelectedUser, setShowUserModal, handleDeleteUser, handleGerarLinkReset }) {
  return (
    <>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <p className="text-2xl font-bold text-blue-700">
                {usuarios.filter(u => { const p = u.permissoes ? JSON.parse(u.permissoes) : {}; return Object.keys(p).length === 0; }).length}
              </p>
            ...
            <div className="p-4 bg-green-50 rounded-lg border border-green-200">
              ...
              <p className="text-2xl font-bold text-green-700">{usuarios.filter(u => u.perfil === 'Admin').length}</p>
          ...
          <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            <p className="text-xs text-amber-800 flex items-start gap-2">
              ...
              <span><strong>Importante:</strong> Usuários com perfil "Admin" têm acesso total independente das permissões granulares.</span>
            </p>
          </div>
        </CardContent>
      </Card>
                ...
                <TableRow key={u.id}>
                  ...
                  <TableCell>
                    {u.perfil === 'Admin' ? (
                      ...
                      let total = 0;
                      Object.values(perms).forEach(mod => { if (typeof mod === 'object') Object.values(mod).forEach(aba => { if (typeof aba === 'object') total += Object.keys(aba).length; }); });
                      return total > 0 ? <Badge className="bg-blue-100 text-blue-700">{total} funções</Badge> : <span className="text-xs text-slate-400">Sem permissões</span>;
                    })()}
                  </TableCell>
                  <TableCell>
                    <div className="flex gap-2">
                      <Button variant="ghost" size="icon" onClick={() => { setSelectedUser(u); setShowUserModal(true); }}>
                        <Edit className="w-4 h-4" />
                      </Button>
                      <Button variant="ghost" size="icon" disabled={enviandoReset === u.id} onClick={() => handleGerarLinkReset(u)}>
                        <KeyRound className={`w-4 h-4 text-amber-500 ${enviandoReset === u.id ? 'animate-pulse' : ''}`} />
                      </Button>
