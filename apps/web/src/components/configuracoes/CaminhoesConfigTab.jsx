...
export default function CaminhoesConfigTab({ empresaAtiva }) {
  ...
  const selectedCaminhao = expandedCaminhaoId
    ? caminhoes.find((c) => c.id === expandedCaminhaoId)
    : null;
                            ...
                            {ferramentas.filter(
                              (f) =>
                                ...
                                (!searchFerramentas ||
                                  f.codigo
                                    ?.toLowerCase()
                                    .includes(searchFerramentas.toLowerCase()) ||
                                  f.descricao
                                    ?.toLowerCase()
                                    .includes(searchFerramentas.toLowerCase()))
                            ).length === 0 ? (
                              ...
                              <div className="space-y-1 p-2">
                                {ferramentas
                                  ...
                                  .map((f) => {
                                    const outrosCaminhoes = caminhoes.find(
                                      (cm) => cm.id === f.caminhao_id
                                    );
                                    return (
                                      <div
                                        key={f.id}
                                        ...
                                        <div className="flex-1">
                                          ...
                                          {outrosCaminhoes && (
                                            <p className="text-xs text-slate-500">
                                              Vinculado: {outrosCaminhoes.placa}
                                            </p>
                                          )}
                                          {f.valor_unitario && (
        ...
        <SheetContent side="right" className="w-full sm:w-96 overflow-y-auto">
          <SheetHeader>
            ...
            <p className="text-sm text-slate-600 mt-2">
              Selecione as ferramentas que serão obrigatórias em todos os caminhões
            </p>
          </SheetHeader>
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        ...
                        setFerramentasSelecionadas(
                          ferramentasSelecionadas.filter((id) => id !== f.id)
                        );
                      }
                    }}
                  />
