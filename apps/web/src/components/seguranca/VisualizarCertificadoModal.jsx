...
export default function VisualizarCertificadoModal({ 
  ...
  const instrutor = treinamento ? parseInstrutor(treinamento.instrutor_nome, treinamento.instrutor_cpf) : null;

  const handleImprimir = async () => {
    if (!treinamento || !funcionario || !empresaAtiva) return;
    const doc = await gerarCertificadoDoc({ treinamento, funcionario, empresaAtiva, datasEditadas });
    doc.save(`certificado_${treinamento.codigo || treinamento.nome.replace(/\s+/g, '_')}_${funcionario.nome_completo.replace(/\s+/g, '_')}.pdf`);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="p-0 flex flex-col h-[95vh] max-w-5xl w-full" aria-describedby={undefined}>
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-slate-900">
          ...
          <div className="bg-white rounded-lg shadow-2xl max-w-4xl mx-auto" style={{ aspectRatio: '1.414/1' }}>
            ...
            <div className="p-12 relative h-full flex flex-col justify-between">
              ...
              {empresaAtiva?.logo_url && (
                <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
                  <img src={empresaAtiva.logo_url} alt="Logo" className="w-64 h-64 object-contain" />
                </div>
              )}
              ...
              <div className="relative space-y-4 text-center flex-1 flex flex-col justify-center">
                ...
                {/* Título Principal */}
                <h1 className="text-7xl font-bold text-yellow-600 mb-2">
                  CERTIFICADO
                </h1>
              ...
              <div className="mt-8 pt-4 border-t border-gray-300">
                <div className="grid grid-cols-3 gap-6 text-center">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium text-gray-700">{instrutor.nome || '_____________________'}</p>
                  ...
                  {/* Responsável Técnico */}
                  <div>
                    {(treinamento.responsavel_tecnico_assinatura_url || treinamento.engenheiro_responsavel_assinatura_url) ? (
                      <img 
                        src={treinamento.responsavel_tecnico_assinatura_url || treinamento.engenheiro_responsavel_assinatura_url} 
                    ...
                    <p className="text-xs font-medium text-gray-700">{treinamento.responsavel_tecnico_nome || treinamento.engenheiro_responsavel_nome || '_____________________'}</p>
          ...
          {treinamento.conteudo_programatico && (
            <div className="bg-white rounded-lg shadow-2xl p-12 max-w-4xl mx-auto mt-8">
              ...
              <div className="relative">
                ...
                {empresaAtiva?.logo_url && (
                  <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
                    <img src={empresaAtiva.logo_url} alt="Logo" className="w-48 h-48 object-contain" />
                  </div>
                )}
                ...
                <div className="relative space-y-4 text-center">
                  ...
                  {treinamento.carga_horaria && (
                    <p className="text-sm text-gray-600">Carga Horária: {treinamento.carga_horaria}h</p>
                  )}
