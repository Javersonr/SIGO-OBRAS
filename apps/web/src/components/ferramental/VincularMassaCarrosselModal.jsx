...
import { ChevronLeft, ChevronRight, Link2, SkipForward, CheckCircle2, FileText, Wrench } from 'lucide-react';
...
export default function VincularMassaCarrosselModal({
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      await base44.entities.Ferramenta.update(ferramenta.id, updateData);
      setConcluidos(c => c + 1);
  ...
  const laudoVencido = ferramenta.data_vencimento_laudo && new Date(ferramenta.data_vencimento_laudo) < new Date();
      ...
      <div className="space-y-5 p-1">
        ...
        <div>
          <div className="flex justify-between text-xs text-slate-500 mb-1.5">
            <span>{index + 1} de {total} ferramentas</span>
        ...
        {/* Card da ferramenta */}
        <Card className={`border-2 ${laudoVencido ? 'border-red-300 bg-red-50' : 'border-amber-200 bg-amber-50'}`}>
          <CardContent className="p-4 space-y-2">
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex flex-col gap-1 items-end">
                ...
                {ferramenta.status && (
                  <Badge className={ferramenta.status === 'Disponível' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}>
                    {ferramenta.status}
                  </Badge>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2 border-t border-amber-200">
              {ferramenta.numero_serie && (
                <div><p className="text-xs text-slate-500">Série</p><p className="text-sm font-medium">{ferramenta.numero_serie}</p></div>
              )}
              {ferramenta.numero_laudo && (
                <div>
                  <p className="text-xs text-slate-500 flex items-center gap-1"><FileText className="w-3 h-3" />Nº Laudo</p>
              ...
              {ferramenta.data_vencimento_laudo && (
                <div>
                  ...
                  <p className={`text-sm font-medium ${laudoVencido ? 'text-red-600' : 'text-slate-700'}`}>
              ...
              {ferramenta.localizacao && (
                <div><p className="text-xs text-slate-500">Local atual</p><p className="text-sm font-medium">{ferramenta.localizacao}</p></div>
              )}
              {ferramenta.funcionario_nome && (
                <div><p className="text-xs text-slate-500">Funcionário atual</p><p className="text-sm font-medium">{ferramenta.funcionario_nome}</p></div>
              )}
            </div>
          </CardContent>
        </Card>
          ...
          {tipoVinculo === 'funcionario' ? (
            <div>
              ...
              <Select value={destinoId} onValueChange={setDestinoId}>
                ...
                <SelectContent>
                  {[...funcionarios].sort((a, b) => (a.nome_completo || '').localeCompare(b.nome_completo || '')).map(f => (
        ...
        <div className="flex gap-1 flex-wrap justify-center pt-2">
          {ferramentasComLaudo.map((f, i) => (
            <button
              key={f.id}
              onClick={() => { setIndex(i); setDestinoId(''); }}
