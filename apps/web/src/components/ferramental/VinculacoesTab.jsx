...
export default function VinculacoesTab({ empresaAtiva }) {
  ...
  const ferramentasCaminhao = caminhoes.map(caminhao => {
  ...
  const ferramentasFuncionario = funcionarios.map(funcionario => {
    const ferramentasVinculadas = ferramentas.filter(f => f.funcionario_id === funcionario.id);
    return {
      ...funcionario,
      ferramentasVinculadas
    };
  }).filter(f => searchTerm === '' || f.nome_completo.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="space-y-6">
      <div className="space-y-4">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            placeholder={tipoFiltro === 'caminhao' ? 'Buscar por placa...' : 'Buscar por funcionário...'}
            value={searchTerm}
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {caminhao.modelo && (
                          <p className="text-xs text-slate-500">{caminhao.marca || ''} {caminhao.modelo}</p>
                        )}
                      </div>
                    </div>
                        ...
                        <TableBody>
                          {caminhao.ferramentasVinculadas.map(ferr => (
                            <TableRow key={ferr.id} className="hover:bg-slate-50">
                              <TableCell className="font-mono text-slate-700">{ferr.codigo}</TableCell>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-slate-800">{funcionario.nome_completo}</h3>
                        ...
                        <TableBody>
                          {funcionario.ferramentasVinculadas.map(ferr => (
                            <TableRow key={ferr.id} className="hover:bg-slate-50">
                              <TableCell className="font-mono text-slate-700">{ferr.codigo}</TableCell>
                              <TableCell className="text-slate-800">{ferr.descricao}</TableCell>
                              <TableCell className="font-mono text-slate-600">{ferr.numero_serie || '-'}</TableCell>
                              <TableCell>
                                <Badge
                                  variant="outline"
                                  className={`text-xs ${
