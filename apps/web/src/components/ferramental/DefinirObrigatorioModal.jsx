 ...
 *   empresaAtiva
 */
export default function DefinirObrigatorioModal({ open, onOpenChange, modo, ferramentasSelecionadas = [], empresaAtiva }) {
  ...
  const loadItens = async () => {
    ...
    try {
      if (isCaminhao) {
        const result = await base44.entities.Caminhao.filter({ empresa_id: empresaAtiva.id, ativo: true });
        setItens(result);
      } else {
        const result = await base44.entities.Funcao.filter({ empresa_id: empresaAtiva.id, ativo: true });
        setItens(result);
      }
    } catch (e) {
      ...
      if (isCaminhao) {
        ...
        for (const caminhaoId of selecionados) {
          ...
          for (const ferr of ferramentasSelecionadas) {
            ...
            let campoExistente = campos.find(c =>
              c.nome_campo?.toLowerCase() === ferr.descricao?.toLowerCase()
            );

            if (campoExistente) {
              // Adicionar ferramenta ao campo existente
              let ids = [];
              try { ids = JSON.parse(campoExistente.ferramenta_ids || '[]'); } catch {}
              if (!ids.includes(ferr.id)) {
                ids.push(ferr.id);
                await base44.entities.CaminhaoCampoObrigatorio.update(campoExistente.id, {
        ...
        toast.success(`${ferramentasSelecionadas.length} ferramenta(s) definidas como obrigatórias para ${selecionados.length} caminhão(ões)`);
      } else {
        ...
        for (const funcaoId of selecionados) {
          ...
          let modeloEpi = [];
          try { modeloFerramentas = JSON.parse(funcao?.modelo_ferramentas || '[]'); } catch {}
          try { modeloEpi = JSON.parse(funcao?.modelo_epi || '[]'); } catch {}

          for (const ferr of ferramentasSelecionadas) {
            if (ferr.tipo === 'EPI') {
              // Adicionar ao modelo_epi
              const jaExiste = modeloEpi.some(m => m.ferramenta_id === ferr.id || m.item?.toLowerCase() === ferr.descricao?.toLowerCase());
              if (!jaExiste) {
                modeloEpi.push({
              ...
              const jaExiste = modeloFerramentas.some(m => m.ferramenta_id === ferr.id || m.ferramenta?.toLowerCase() === ferr.descricao?.toLowerCase());
              if (!jaExiste) {
          ...
          await base44.entities.Funcao.update(funcaoId, {
        ...
        toast.success(`Vinculado a ${selecionados.length} função(ões): ${totalEpis} EPI(s) e ${totalFerr} ferramenta(s)`);
      }
  ...
  const todosFiltradosSelecionados = itensFiltrados.length > 0 && itensFiltrados.every(i => selecionados.includes(i.id));
      ...
      <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col" style={{ inset: 'auto 0 0 256px', width: 'calc(100% - 256px)', maxWidth: 'none' }}>
        <SheetHeader className="p-6 border-b sticky top-0 bg-white z-20">
          <SheetTitle className="flex items-center gap-2">
            {isCaminhao ? <Truck className="w-5 h-5 text-blue-600" /> : <HardHat className="w-5 h-5 text-amber-600" />}
            Definir Obrigatório para {isCaminhao ? 'Caminhões' : 'Funções'}
          </SheetTitle>
          <p className="text-sm text-slate-500 mt-1">
            {ferramentasSelecionadas.length} ferramenta(s) selecionada(s) serão adicionadas como obrigatórias.
          </p>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {itensFiltrados.length > 0 && (
            ...
            >
              ...
              <span className="text-sm text-slate-600">Selecionar todos ({itensFiltrados.length})</span>
            </div>
          )}
          ...
          {loading ? (
            ...
            <p className="text-sm text-slate-500 text-center py-8">Nenhum {isCaminhao ? 'caminhão' : 'função'} encontrado</p>
          ) : (
        ...
        <SheetFooter className="p-4 border-t bg-white flex gap-2 justify-end">
          ...
          <Button
            ...
            size="sm"
          >
            {salvando ? 'Salvando...' : `Vincular a ${selecionados.length} ${isCaminhao ? 'caminhão(ões)' : 'função(ões)'}`}
          </Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
