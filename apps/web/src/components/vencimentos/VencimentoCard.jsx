...
import { Eye, Edit, Paperclip, User, Bell } from 'lucide-react';
...
export default function VencimentoCard({ vencimento, onEdit, onVerDocumento, onNotificar }) {
  const diasRestantes = differenceInDays(new Date(vencimento.data_vencimento + 'T00:00:00'), new Date());
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-4 mt-1 flex-wrap">
              <span className="text-sm text-slate-600">
                Vence: <strong>{format(new Date(vencimento.data_vencimento + 'T00:00:00'), 'dd/MM/yyyy', { locale: ptBR })}</strong>
              </span>
              {diasRestantes >= 0 ? (
                <span className={`text-sm font-medium ${diasRestantes <= 7 ? 'text-red-600' : diasRestantes <= 30 ? 'text-yellow-600' : 'text-green-600'}`}>
          ...
          <div className="flex gap-1 flex-shrink-0">
            {vencimento.arquivo_url && (
              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onVerDocumento(vencimento)} title="Ver documento">
                <Eye className="w-4 h-4 text-blue-500" />
              </Button>
            )}
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onEdit(vencimento)} title="Editar">
              <Edit className="w-4 h-4 text-slate-500" />
            </Button>
            {vencimento.responsavel_email && (
              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onNotificar(vencimento)} title="Notificar responsável">
