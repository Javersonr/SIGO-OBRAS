...
import { Plus, Search, Bell, AlertTriangle, CheckCircle2, Clock, Building2, Filter, BarChart3 } from 'lucide-react';
...
export default function Vencimentos() {
   ...
   const isAdminHolding = (perfil === 'Admin Holding' && (grupoIdVinculo || grupoIdAuth)) || !!grupoIdAuth;
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (isAdminHolding) {
        ...
        const todasEmpresas = await base44.entities.Empresa.filter({ grupo_id: grupoId, ativo: true });
        const results = await Promise.all(
          todasEmpresas.map(e => base44.entities.Vencimento.filter({ empresa_id: e.id, ativo: true }))
        );
        todos = results.flat();
        
        // Calcular estatísticas por empresa
        const stats = todasEmpresas.map(emp => {
          ...
          return {
            ...
            vencidos: vencsDaEmpresa.filter(v => differenceInDays(new Date(v.data_vencimento + 'T00:00:00'), new Date()) < 0).length,
        ...
        if (empresaAtiva.grupo_id && empresas?.length > 1) {
          ...
          const results = await Promise.all(
            empresasDoGrupo.map(e => base44.entities.Vencimento.filter({ empresa_id: e.id, ativo: true }))
          );
          todos = results.flat();
        } else {
          todos = await base44.entities.Vencimento.filter({ empresa_id: empresaAtiva.id, ativo: true });
        }
  ...
  const vencimentosFiltrados = vencimentos.filter(v => {
    ...
    if (busca && !v.titulo?.toLowerCase().includes(busca.toLowerCase()) && !v.tipo?.toLowerCase().includes(busca.toLowerCase())) return false;
  ...
  const mostrarFiltroEmpresa = empresaAtiva?.grupo_id && empresas?.filter(e => e.grupo_id === empresaAtiva.grupo_id).length > 1;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            {isAdminHolding ? '📊 Visão consolidada do grupo empresarial' : empresaAtiva?.grupo_id ? 'Visão consolidada do grupo empresarial' : `Empresa: ${empresaAtiva?.nome}`}
          </p>
        </div>
        {!isAdminHolding && (
          <Button onClick={() => { setVencimentoEditando(null); setShowModal(true); }} className="bg-amber-500 hover:bg-amber-600 gap-2">
              ...
              {estatisticasGrupo.map((stat, idx) => (
                <div key={idx} className="bg-white rounded-lg p-2 border border-amber-100">
                  ...
                  <div className="flex gap-2 mt-1 text-xs">
                    ...
                    <span className="text-green-600">✓ {stat.total - stat.vencidos - stat.aVencer}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <Input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar vencimento..." className="pl-9" />
        </div>
        <Select value={filtroStatus} onValueChange={setFiltroStatus}>
          <SelectTrigger className="w-36"><SelectValue /></SelectTrigger>
          <SelectContent>{FILTROS_STATUS.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
        </Select>
        <Select value={filtroPrazo} onValueChange={setFiltroPrazo}>
          <SelectTrigger className="w-36"><SelectValue /></SelectTrigger>
          <SelectContent>{FILTROS_PRAZO.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent>
        </Select>
        ...
        {(mostrarFiltroEmpresa || isAdminHolding) && (
          <Select value={filtroEmpresa} onValueChange={setFiltroEmpresa}>
            ...
            <SelectContent>
              ...
              {isAdminHolding ? (
                empresas?.filter(e => e.grupo_id === vinculo.grupo_id).map(e => (
                  <SelectItem key={e.id} value={e.id}>{e.nome_fantasia || e.nome}</SelectItem>
                ))
              ) : (
                empresas.filter(e => e.grupo_id === empresaAtiva.grupo_id).map(e => (
                  <SelectItem key={e.id} value={e.id}>{e.nome_fantasia || e.nome}</SelectItem>
                ))
              )}
            </SelectContent>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-slate-50 rounded-xl">
          ...
          <p className="text-slate-400 text-sm mt-1">Cadastre certidões, documentos e contratos para monitorar os vencimentos</p>
          {!isAdminHolding && (
            <Button onClick={() => { setVencimentoEditando(null); setShowModal(true); }} className="mt-4 bg-amber-500 hover:bg-amber-600 gap-2">
        ...
        <div className="space-y-3">
          <p className="text-sm text-slate-500">{vencimentosFiltrados.length} vencimento(s) encontrado(s)</p>
                ...
                <div className="space-y-2">
                  {grupo.map(v => (
                    <VencimentoCard
                      key={v.id}
                      vencimento={v}
                      onEdit={(venc) => { setVencimentoEditando(venc); setShowModal(true); }}
