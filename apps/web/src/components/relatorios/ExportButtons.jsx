...
export function exportarCSV(colunas, linhas, nomeArquivo) {
  const csv = [colunas, ...linhas].map(row => row.map(c => `"${c ?? ''}"`).join(';')).join('\n');
...
export async function exportarPDF(titulo, colunas, linhas, nomeArquivo) {
  ...
  doc.text(`Gerado em: ${new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}`, doc.internal.pageSize.width - 14, 14, { align: 'right' });
