...
import { Upload, Plus, Calendar, Trash2, FileText, CheckCircle2, Clock, Copy, Link2Off, X } from 'lucide-react';
...
export default function DespesaModal({
  ...
  const [itemManual, setItemManual] = useState({ descricao: '', codigo: '', unidade: 'UN', quantidade: 0, valor_unitario: 0 });
  ...
  useEffect(() => { setFornecedoresLocais(fornecedores); }, [fornecedores]);
      ...
      reader.onload = async (event) => {
        try {
          ...
          const fornecedorEncontrado = fornecedores.find(f => 
            f.cnpj?.replace(/\D/g, '') === cnpjFornecedor.replace(/\D/g, '')
          );
          ...
          const chaveNFe = infProt?.getElementsByTagName('chNFe')[0]?.textContent ||
  ...
  const fornecedoresFiltrados = useMemo(() => {
    ...
    return fornecedoresOrdenados.filter(f => 
  ...
  const categoriasOrdenadas = useMemo(() => {
    return [...categorias].filter(c => c.tipo === 'Despesa').sort((a, b) => 
  ...
  return (
    <>
    <Sheet open={showModal} onOpenChange={(open) => { if (!open) { setShowModal(false); setVisualizandoAnexoUrl(null); } }}>
                            ...
                            <CommandGroup>
                              {fornecedoresFiltrados.map(f => (
                                <CommandItem
                                  ...
                                  value={f.nome_razao}
                                  onSelect={() => {
                                    setForm({ ...form, fornecedor_id: f.id, fornecedor_nome: f.nome_razao });
                                ...
                                >
                                  <div className="flex flex-col">
                                    ...
                                    {f.cnpj && <span className="text-xs text-slate-500">{f.cnpj}</span>}
                                  </div>
                                </CommandItem>
                              ))}
                            </CommandGroup>
                          ...
                          <CommandGroup>
                            {centrosCustoFiltrados.map(c => (
                              <CommandItem
                                ...
                                value={c.nome}
                                onSelect={() => {
                                  setForm({ ...form, centro_custo_id: c.id, centro_custo_nome: c.nome });
                    ...
                    <Input
                      ...
                      onFocus={(e) => {
                        if (!form.data_competencia) {
                          setForm({ ...form, data_competencia: new Date().toISOString().split('T')[0] });
                        }
                      }}
                      className="pr-10"
                    />
          ...
          <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                {notaFiscal.numero && (
                  <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded text-sm">
                    <p className="text-green-700">NFe #{notaFiscal.numero} importada com sucesso</p>
                  </div>
                )}
              </div>
                      ...
                      <div key={index} className="flex items-center justify-between p-2 bg-white rounded border text-sm">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-slate-500">
                            Qtd: {item.quantidade} {item.unidade} • Valor Unit: R$ {item.valor_unitario?.toFixed(2)}
                          </p>
                        </div>
                        {item.material_nome && (
                ...
                <div>
                  ...
                  <div className="relative mt-1.5">
                    <Input
                      ...
                      onChange={(e) => setNotaFiscal({ ...notaFiscal, dataEmissao: e.target.value })}
                      className="pr-10"
                    />
                    <Calendar className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
                  </div>
                ...
                <div>
                  ...
                  <div className="relative mt-1.5">
                    <Input
                      ...
                      onChange={(e) => setNotaFiscal({ ...notaFiscal, dataEntrada: e.target.value })}
                      className="pr-10"
                    />
                    <Calendar className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div>
                   ...
                   <Input
                     ...
                     onChange={(e) => setNotaFiscal({ ...notaFiscal, valorNota: e.target.value })}
                     placeholder="0,00"
                     className="mt-1.5"
                     step="0.01"
                   />
              ...
              <div>
                ...
                <Select value={notaFiscal.statusAprovacao} onValueChange={(v) => setNotaFiscal({ ...notaFiscal, statusAprovacao: v })}>
                  <SelectTrigger className="mt-1.5">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                  ...
                  {anexos.length > 0 && (
                    <div className="space-y-2">
                      {anexos.map((anexo, index) => {
                        ...
                        return (
                          <div key={anexo.id || index} className="flex items-center justify-between p-3 bg-white rounded-lg border hover:border-slate-300 transition-colors">
                            ...
                            <div className="flex items-center gap-1 ml-2">
                              <Button
                                ...
                                onClick={(e) => { 
                                  ...
                                  if (isPdf) {
                                    window.open(`https://docs.google.com/viewer?url=${encodeURIComponent(anexo.url)}&embedded=true`, '_blank');
                                  } else {
              ...
              <div>
                ...
                <div className="space-y-2 mt-1.5">
                  <div className="flex items-center gap-2">
                    <Checkbox
                      id="parcelamento"
                      checked={permitirParcelamento || (selectedItem?.parcelado && parcelas.length > 0)}
                      onCheckedChange={toggleParcelamento}
                      disabled={selectedItem?.parcelado && parcelas.length > 0}
                    />
                    <label htmlFor="parcelamento" className="text-sm text-slate-600 cursor-pointer">
                  ...
                  {(permitirParcelamento || (selectedItem?.parcelado && parcelas.length > 0)) && (
                    <div className="space-y-3">
                      ...
                      {numeroParcelas > 1 && parcelas.length === 0 && !selectedItem?.parcelado && (
              ...
              {mostrarParcelas && permitirParcelamento && numeroParcelas > 1 && parcelas.length > 0 && (
                      ...
                      >
                        ...
                        <div className="col-span-2 flex items-end">
                         <Button
                           ...
                           className={parcela.status === 'pago' ? 'w-full' : 'w-full bg-green-600 hover:bg-green-700'}
                           onClick={async () => {
          ...
          <div className="flex gap-3">
            ...
            {selectedItem && (
              <Button 
                ...
                className={selectedItem.status === 'pago' ? 'bg-blue-600 hover:bg-blue-700 flex-1' : 'bg-green-600 hover:bg-green-700 flex-1'}
              >
      ...
      <Sheet open={showLancamentoManual} onOpenChange={setShowLancamentoManual}>
        <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col w-full md:w-auto md:inset-auto md:right-0" data-fullscreen-modal>
          ...
          <div className="p-6 flex-1 overflow-y-auto space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setItemManual({ ...itemManual, quantidade: parseFloat(e.target.value) || 0 })}
                  className="mt-1.5"
                  step="0.01"
                />
              </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setItemManual({ ...itemManual, valor_unitario: parseFloat(e.target.value) || 0 })}
                placeholder="0,00"
                className="mt-1.5"
                step="0.01"
              />
          ...
          <div className="sticky bottom-0 bg-white border-t p-6 flex-shrink-0 flex gap-3">
            <Button variant="outline" onClick={() => setShowLancamentoManual(false)} className="flex-1">
              Cancelar
            </Button>
            <Button 
              onClick={() => {
      ...
      <NovoFornecedorConfigSheet
        ...
        onFornecedorCriado={(fornecedor) => {
          ...
          setForm(prev => ({ ...prev, fornecedor_id: fornecedor.id, fornecedor_nome: fornecedor.nome_razao }));
      ...
      <Sheet open={showNovoCentroCusto} onOpenChange={setShowNovoCentroCusto}>
        <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col w-full md:w-auto md:inset-auto md:right-0" data-fullscreen-modal>
          ...
          <div className="sticky bottom-0 bg-white border-t p-6 flex-shrink-0 flex gap-3">
            <Button variant="outline" onClick={() => setShowNovoCentroCusto(false)} className="flex-1">
              Cancelar
            </Button>
            <Button onClick={handleSalvarCentroCusto} disabled={!novoCentroCusto.nome} className="flex-1">
