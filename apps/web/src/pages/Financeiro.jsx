...
export default function Financeiro() {
  const { empresaAtiva, setEmpresaAtiva, empresas, perfil, user, temPermissao, vinculo } = useEmpresa();
  ...
  const temAbaRecorrentes = perfil === 'Admin' || temPermissao('Financeiro', 'Transações Recorrentes');
  ...
  const preLancamentosVerTodos = perfil === 'Admin' || vinculo?.is_owner === true ||
    !!permissoes?.Financeiro?.['Pré-Lançamentos']?.['ver_todos'];
  // ver_proprios: pode ver seus próprios pré-lançamentos
  const preLancamentosVerProprios = preLancamentosVerTodos ||
    !!permissoes?.Financeiro?.['Pré-Lançamentos']?.['ver_proprios'];
  // aprovar: pode aprovar pré-lançamentos
  const preLancamentosPodeAprovar = perfil === 'Admin' || vinculo?.is_owner === true ||
    !!permissoes?.Financeiro?.['Pré-Lançamentos']?.['aprovar'];
  // pagar: pode executar pagamento de pré-lançamentos
  const preLancamentosPodePagar = perfil === 'Admin' || vinculo?.is_owner === true ||
  ...
  }, [temAbaResumo, temAbaReceitas, temAbaDespesas, temAbaContas, temAbaRelatorios, temAbaRecorrentes, temAbaPreLancamentos, temAbaTransferencias]);
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <div className="md:hidden">
          <Select value={activeTab} onValueChange={setActiveTab}>
            ...
            <SelectContent>
              {abasDisponiveis.map(aba => (
                <SelectItem key={aba.value} value={aba.value}>{aba.label}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        ...
        <TabsContent value="receitas" className="mt-6">
          <ReceitasTab
            ...
            onTransacaoInicialConsumed={() => { setTransacaoIdInicial(null); transacaoIdRef.current = null; }}
          />
        </TabsContent>
