  ...
  const carregarDados = async () => {
    ...
    try {
      const [empresa, contsData, catsData, fornsData, projsData, optsData] = await Promise.all([
        base44.entities.Empresa.filter({ id: preLancamento.empresa_id }),
        base44.entities.ContaFinanceira.filter({ empresa_id: preLancamento.empresa_id, ativo: true }),
        base44.entities.CategoriaFinanceira.filter({ empresa_id: preLancamento.empresa_id, ativo: true }),
        base44.entities.Fornecedor.filter({ empresa_id: preLancamento.empresa_id, ativo: true }),
        base44.entities.Projeto.filter({ empresa_id: preLancamento.empresa_id }),
        base44.entities.Oportunidade.filter({ empresa_id: preLancamento.empresa_id })
      ]);
      ...
      setForm({
        ...
        data_pagamento: '',
        descricao: dados.descricao || (dados.fornecedor ? `Compra - ${dados.fornecedor}` : 'Pré-lançamento'),
        status: 'em_aberto',
        forma_pagamento: ''
      });
