...
export default function WidgetMetricasGerais() {
  ...
  const loadMetrics = async () => {
    try {
      const promises = [];
      if (podeVerOportunidades)
        promises.push(sigo.entities.Oportunidade.filter({ empresa_id: empresaAtiva.id }, "", 9999));
      else promises.push(Promise.resolve([]));
      if (podeVerProjetos)
        promises.push(sigo.entities.Projeto.filter({ empresa_id: empresaAtiva.id }, "", 9999));
      else promises.push(Promise.resolve([]));
      if (isAdmin)
        promises.push(
      ...
      if (podeVerSolicitacoes)
        promises.push(
          sigo.entities.SolicitacaoCompra.filter({ empresa_id: empresaAtiva.id }, "", 9999)
        );
      else promises.push(Promise.resolve([]));
  ...
  const allCards = [
    {
      ...
      icon: Target,
      color: "text-blue-600",
      visible: podeVerOportunidades,
    },
    {
      title: "Projetos",
      value: metrics.projetos,
      icon: FolderKanban,
      color: "text-green-600",
      visible: podeVerProjetos,
    },
    {
      title: "Usuários",
    ...
    {
      title: "Solicitações",
      value: metrics.solicitacoes,
      icon: TrendingUp,
      color: "text-amber-600",
      visible: podeVerSolicitacoes,
    },
  ];
