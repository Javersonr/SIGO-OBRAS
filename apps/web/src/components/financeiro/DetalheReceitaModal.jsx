...
export default function DetalheReceitaModal({ open, onOpenChange, receita, podeEditar, onEditar, onBaixar, empresaAtiva }) {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col" style={{ inset: 'auto 0 0 256px', width: 'calc(100% - 256px)', maxWidth: 'none' }}>
        ...
        <div className="p-6 flex-1 overflow-y-auto space-y-6">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-700 uppercase mb-4">Informações Gerais</h3>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="mt-1 text-lg font-bold text-green-600">{formatCurrency(receita.valor)}</p>
                </div>
                <div>
                  <Label className="text-slate-500">Forma de Recebimento</Label>
                  <p className="mt-1 text-slate-800">
                    {receita.forma_pagamento ? (
                      <Badge variant="outline" className="capitalize">{receita.forma_pagamento}</Badge>
                    ) : '-'}
                  </p>
                </div>
              </div>
          ...
          <div className="border-t pt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-slate-800">{receita.centro_custo_nome || receita.centro_custo || '-'}</p>
              </div>
              <div>
          ...
          <div className="border-t pt-4">
            <h3 className="text-sm font-semibold text-slate-700 uppercase mb-4">Informações do Sistema</h3>
            <div className="grid grid-cols-2 gap-4 text-xs">
              <div>
                <Label className="text-slate-500">Criado em</Label>
                <p className="mt-1 text-slate-800">
                  {receita.created_date ? new Date(receita.created_date).toLocaleString('pt-BR') : '-'}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="mt-1">
                  <Badge className={receita.status === 'pago' || receita.status === 'Pago' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}>
                    {receita.status === 'pago' || receita.status === 'Pago' ? 'Recebido' : 'Em aberto'}
                  </Badge>
                </p>
              </div>
            </div>
        ...
        <div className="border-t p-4 space-y-3">
          {onBaixar && (
            <Button
              variant={receita.status === 'pago' || receita.status === 'Pago' ? 'outline' : 'default'}
              size="sm"
              onClick={() => onBaixar(receita)}
              className={receita.status === 'pago' || receita.status === 'Pago' ? 'text-blue-600 w-full' : 'bg-green-600 hover:bg-green-700 w-full'}
            >
              <CheckCircle2 className="w-4 h-4 mr-2" />
              {receita.status === 'pago' || receita.status === 'Pago' ? 'Desfazer Recebimento' : 'Registrar Recebimento'}
            </Button>
          )}
          <div className="flex gap-3">
            <Button variant="outline" onClick={() => onOpenChange(false)} className="flex-1">Fechar</Button>
            {podeEditar && onEditar && (
              <Button onClick={() => { onOpenChange(false); onEditar(receita); }} className="bg-amber-500 hover:bg-amber-600 flex-1">
