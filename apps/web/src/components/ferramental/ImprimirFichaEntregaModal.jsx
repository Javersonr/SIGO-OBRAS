...
export default function ImprimirFichaEntregaModal({ open, onOpenChange, entrega, empresaAtiva }) {
  ...
  const itens = (() => {
    try { return JSON.parse(entrega.itens || '[]'); } catch { return []; }
  })();
  ...
  const handleImprimirFerramentas = () => {
    ...
    const rowsHtml = ferramentas.map(item => `
    ...
    if (imgs.length === 0) {
      ...
      Array.from(imgs).forEach(img => {
        const done = () => { loaded++; if (loaded === imgs.length) { pw.focus(); pw.print(); pw.close(); } };
        if (img.complete) done(); else { img.onload = done; img.onerror = done; }
      });
      setTimeout(() => { if (loaded < imgs.length) { pw.focus(); pw.print(); pw.close(); } }, 3000);
    }
  };
  ...
  const handleImprimirEPIs = () => {
    ...
    const renderPage = (epicsData, isSecondPage = false) => {
      ...
      const rowsHtml = epicsData.map(epi => `
        <tr style="height:20px">
          <td style="border:1px solid #000;padding:4px">__/__/____</td>
          <td style="border:1px solid #000;padding:4px">__/__/____</td>
      ...
      <SheetContent side="right" className="w-full h-full overflow-y-auto p-0" data-fullscreen-modal>
        <SheetHeader className="px-6 py-4 border-b sticky top-0 bg-white z-10 flex flex-row items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button onClick={handleImprimirFerramentas} className="gap-2 bg-blue-600 text-white hover:bg-blue-700">
              <Printer className="w-4 h-4" />
              Ferramentas
            </Button>
            <Button onClick={handleImprimirEPIs} className="gap-2 bg-green-600 text-white hover:bg-green-700">
              <Printer className="w-4 h-4" />
              EPIs
            </Button>
            <Button onClick={handleImprimirAmbos} className="gap-2 bg-slate-800 text-white hover:bg-slate-700">
              <Printer className="w-4 h-4" />
              Ambos
            </Button>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div><span className="text-slate-500">Funcionário:</span> <strong>{entrega.funcionario_nome}</strong></div>
            <div><span className="text-slate-500">Função:</span> <strong>{entrega.funcao_nome}</strong></div>
            <div><span className="text-slate-500">Solicitante:</span> <strong>{entrega.solicitante_nome}</strong></div>
            <div><span className="text-slate-500">Responsável Entrega:</span> <strong>{entrega.responsavel_entrega_nome || '-'}</strong></div>
            <div><span className="text-slate-500">Data Solicitação:</span> <strong>{entrega.data_solicitacao ? format(new Date(entrega.data_solicitacao + 'T12:00:00'), 'dd/MM/yyyy') : '-'}</strong></div>
            <div><span className="text-slate-500">Data Entrega:</span> <strong>{entrega.data_entrega ? format(new Date(entrega.data_entrega + 'T12:00:00'), 'dd/MM/yyyy') : '-'}</strong></div>
            <div><span className="text-slate-500">Biometria:</span> <strong className={entrega.biometria_capturada ? 'text-green-600' : 'text-red-500'}>{entrega.biometria_capturada ? 'Confirmada ✓' : 'Não capturada'}</strong></div>
          </div>
              ...
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-4 py-2 text-left text-xs font-semibold text-slate-600">Item</th>
                               ...
                               <th className="px-4 py-2 text-center text-xs font-semibold text-slate-600">Solicitada</th>
                               <th className="px-4 py-2 text-center text-xs font-semibold text-slate-600">Entregue</th>
                               <th className="px-4 py-2 text-left text-xs font-semibold text-slate-600">Nº Série</th>
                </tr>
              </thead>
              <tbody>
                {itens.map((item, idx) => (
                  <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                    <td className="px-4 py-2 text-slate-800">{item.descricao || item.ferramenta || item.item || ''}</td>
                    <td className="px-4 py-2">
                      <span className={`text-xs px-2 py-0.5 rounded font-medium ${item.tipo === 'EPI' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                        {item.tipo || 'Ferramenta'}
                      </span>
                    </td>
                    <td className="px-4 py-2 text-center text-slate-600">{item.quantidade || 1}</td>
                     <td className="px-4 py-2 text-center font-semibold text-slate-800">{item.quantidade_entregue ?? item.quantidade ?? 1}</td>
