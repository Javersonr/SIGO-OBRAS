...
async function desenharPaginaCertificado(doc, W, H, treinamento, funcionario, empresaAtiva, datasEditadas) {
  // Fundo branco
  ...
  doc.lines([[W, 0], [0, 25], [-(W - 100), 10], [-100, 0]], 0, 0, [1, 1], 'F', true);

  // === FORMA AZUL INFERIOR (reduzida) ===
  // Altura reduzida de 48 para 25 mm
  doc.setFillColor(...NAVY);
  doc.lines([[W, 0], [0, -24], [-(W - 100), -10], [-100, 12]], 0, H, [1, 1], 'F', true);
  ...
  let y = 107;
  linhasT.forEach(l => { doc.text(l, W / 2, y, { align: 'center' }); y += 6; });
  ...
  y += 2;
  doc.text(`Aproveitamento: ${aproveitamento}%  |  Carga Horária: ${cargaHoraria} horas/aula`, W / 2, y, { align: 'center' });
  ...
  y += 6;
  const localEmpresa = empresaAtiva.cidade && empresaAtiva.estado ? `${empresaAtiva.cidade} – ${empresaAtiva.estado}` : '';
  const dataPorExtenso = format(new Date(), "dd 'de' MMMM 'de' yyyy", { locale: ptBR });
  doc.setFontSize(9);
  doc.setTextColor(110, 110, 110);
  doc.text(localEmpresa ? `${localEmpresa}, ${dataPorExtenso}` : dataPorExtenso, W / 2, y, { align: 'center' });
  ...
  const xA3 = W - 58;
  const assinaturaUrl = treinamento.responsavel_tecnico_assinatura_url || treinamento.engenheiro_responsavel_assinatura_url;
  if (assinaturaUrl) {
  ...
  doc.setTextColor(50, 50, 50);
  doc.text(treinamento.responsavel_tecnico_nome || treinamento.engenheiro_responsavel_nome || '_____________________', xA3, yAssin + 5, { align: 'center' });
  ...
  if (treinamento.responsavel_tecnico_criacao || treinamento.engenheiro_responsavel_crea) {
    doc.text(`CREA: ${treinamento.responsavel_tecnico_criacao || treinamento.engenheiro_responsavel_crea}`, xA3, yAssin + 13, { align: 'center' });
  }
}

async function desenharPaginaConteudo(doc, W, H, treinamento, empresaAtiva) {
  const barraWidth = 35;
  const NAVY_R = NAVY[0], NAVY_G = NAVY[1], NAVY_B = NAVY[2];
  ...
  let yInfo = 37;
  if (treinamento.codigo) { doc.text(`Código: ${treinamento.codigo}`, margemEsq, yInfo); yInfo += 5; }
  if (treinamento.carga_horaria) { doc.text(`Carga Horária: ${treinamento.carga_horaria}h`, margemEsq, yInfo); yInfo += 5; }
  ...
  if (restante.length > 0) {
    ...
    for (let i = 0; i < barraWidth; i++) {
      const t = i / barraWidth;
      doc.setFillColor(Math.round(NAVY_R + t * 20), Math.round(NAVY_G + t * 30), Math.round(NAVY_B + t * 50));
      doc.rect(i, 0, 1, H, 'F');
    }
    doc.setDrawColor(...GOLD); doc.setLineWidth(2.5); doc.line(barraWidth, 0, barraWidth, H);
    doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.setTextColor(40, 40, 40);
    let yR = 15;
    restante.forEach(linha => { doc.text(linha, margemEsq, yR); yR += 5; });
  }
}
 ...
 */
export async function gerarCertificadoDoc({ treinamento, funcionario, empresaAtiva, datasEditadas = {} }) {
