...
export default function RelatorioFinanceiro({ empresaId, filtros }) {
  ...
  useEffect(() => {
    if (!empresaId) return;
    sigo.entities.TransacaoFinanceira.filter({ empresa_id: empresaId })
      .then(setTransacoes)
  ...
  const dadosFiltrados = useMemo(() => {
    return transacoes.filter((t) => {
      if (
        filtros?.dataInicio &&
        new Date(t.data_vencimento || t.created_date) < new Date(filtros.dataInicio)
      )
        return false;
      if (
        filtros?.dataFim &&
        new Date(t.data_vencimento || t.created_date) > new Date(filtros.dataFim)
      )
        return false;
      return true;
    });
  ...
  const kpis = useMemo(() => {
    ...
    const pagas = dadosFiltrados.filter(
      (t) => t.status === "Pago" || t.status === "Recebido"
    ).length;
    const pendentes = dadosFiltrados.filter(
      (t) => t.status === "Pendente" || t.status === "A Vencer"
    ).length;
    const atrasadas = dadosFiltrados.filter((t) => {
      if (!t.data_vencimento) return false;
      return (
        new Date(t.data_vencimento) < new Date() && t.status !== "Pago" && t.status !== "Recebido"
      );
    }).length;
  ...
  const handlePDF = () =>
    exportarPDF(
      ...
      dadosFiltrados
        .slice(0, 50)
        .map((t) => [t.tipo, (t.descricao || "").slice(0, 30), fmt(t.valor || 0), t.status || ""]),
      "relatorio_financeiro"
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <KpiCard
          titulo="Saldo"
          valor={fmt(kpis.saldo)}
          icon={Scale}
          cor={kpis.saldo >= 0 ? "green" : "red"}
          tendencia={kpis.saldo >= 0 ? "up" : "down"}
        />
        <KpiCard
          titulo="Pagas/Recebidas"
          valor={kpis.pagas}
        ...
        <KpiCard
          ...
          icon={ArrowDownRight}
          cor="red"
          tendencia={kpis.atrasadas > 0 ? "down" : "up"}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm font-semibold text-slate-700">Saldo Mensal</CardTitle>
          </CardHeader>
          <CardContent>
