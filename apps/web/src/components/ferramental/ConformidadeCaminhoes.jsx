...
import { CheckCircle2, Circle, AlertCircle, Search, ChevronDown, ChevronRight, Truck, User, Eye, Edit, FileText } from 'lucide-react';
  ...
  const loadDados = async () => {
    ...
    try {
      const [cams, camposDb, funcs, ferrs] = await Promise.all([
        ...
        base44.entities.CaminhaoCampoObrigatorio.filter({ empresa_id: empresaAtiva.id, ativo: true }),
  ...
  const autoVincularFerramentes = async (caminhao) => {
    ...
    try {
      ...
      for (let i = 0; i < camposDoCaminhao.length; i++) {
        ...
        const ferramentasDisponiveis = ferramentasDoCaminhao.filter(f => !ferramentasUsadas.has(f.id));
        ...
        const ferrsAVincular = ferramentasParaCampo.length > 0 
          ? ferramentasParaCampo 
  ...
  const AcoesFerramenta = ({ f }) => (
    <td className="px-2 py-1.5 whitespace-nowrap">
      <div className="flex items-center gap-1">
        {f.laudo_url && (
          <a href={f.laudo_url} target="_blank" rel="noopener noreferrer"
            className="p-1 text-blue-600 hover:bg-blue-50 rounded" title="Visualizar Laudo">
            <Eye className="w-3.5 h-3.5" />
          </a>
        )}
        <button
          onClick={() => setEditarLaudo(f)}
          className="p-1 text-slate-500 hover:bg-slate-100 rounded" title="Editar Nº Série / Laudo">
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Visualize as ferramentas vinculadas a caminhões e funcionários.</p>
        </div>
        <Button variant="outline" size="sm" onClick={loadDados}>Atualizar</Button>
      </div>

      {/* Abas */}
      <div className="flex gap-2 border-b border-slate-200">
        <button
          onClick={() => { setAba('caminhoes'); setBusca(''); }}
        ...
        <button
          onClick={() => { setAba('funcionarios'); setBusca(''); }}
        ...
        >
          👤 Funcionários ({funcionarios.filter(f => getFerramentasFuncionario(f.id).length > 0).length})
        </button>
      </div>
        ...
        caminhoesFiltrados.length === 0 ? (
          ...
          <div className="space-y-3">
            {caminhoesFiltrados.map(caminhao => {
              ...
              const ferramentasDoCaminhao = getFerramentasCaminhao(caminhao).sort((a, b) => (a.descricao || '').localeCompare(b.descricao || ''));
              const conformeTotal = total > 0 && ok === total;

              return (
                <Card key={caminhao.id} className={`overflow-hidden transition-all ${conformeTotal ? 'border-green-300' : total === 0 ? 'border-slate-200' : ok === 0 ? 'border-slate-200' : 'border-amber-300'}`}>
                  <div
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-500">
                          ...
                          {caminhao.motorista_padrao_nome && ` · ${caminhao.motorista_padrao_nome}`}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      {total > 0 ? (
                        <Badge className={conformeTotal ? 'bg-green-100 text-green-700' : ok === 0 ? 'bg-slate-100 text-slate-600' : 'bg-amber-100 text-amber-700'}>
                          {ok}/{total} campos ok
                        </Badge>
                      ) : (
                        <Badge className="bg-slate-100 text-slate-500">Sem campos obrigatórios</Badge>
                      )}
                      <Badge variant="outline" className="text-xs">{ferramentasDoCaminhao.length} ferr.</Badge>
                      {ok < total && ferramentasDoCaminhao.length > 0 && (
                        ...
                        >
                          {vinculandoCaminhao === caminhao.id ? `${progressoVinculacao}%` : '🤖 Auto-vincular'}
                        </Button>
                      )}
                    ...
                    <div className="border-t border-slate-200 px-4 pb-4 pt-3 space-y-4">
                      ...
                      {camposDoCaminhao.length > 0 && (
                        <div className="space-y-2">
                          <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Campos Obrigatórios</p>
                          {[...camposDoCaminhao].sort((a, b) => a.nome_campo.localeCompare(b.nome_campo)).map(campo => {
                            const status = getStatusCampo(campo);
                            const idsVinculados = getFerramentasIds(campo);
                            const ferrsVinculadas = ferramentas.filter(f => idsVinculados.includes(f.id));
                            ...
                            return (
                              <div key={campo.id} className={`rounded-lg border p-3 ${statusColor[status]}`}>
                                ...
                                {ferrsVinculadas.length > 0 ? (
                                    ...
                                    <table className="w-full text-xs">
                                      <thead className="bg-white/60">
                                       <tr className="border-b border-slate-200">
                                         <th className="text-left px-2 py-1 font-semibold text-slate-600">Sub-Código</th>
                                         <th className="text-left px-2 py-1 font-semibold text-slate-600">Descrição</th>
                                         <th className="text-left px-2 py-1 font-semibold text-slate-600">Status</th>
                                         <th className="text-left px-2 py-1 font-semibold text-slate-600">Nº Série</th>
                                         <th className="text-left px-2 py-1 font-semibold text-slate-600">Nº Laudo</th>
                                         <th className="text-left px-2 py-1 font-semibold text-slate-600">Venc. Laudo</th>
                                         <th className="text-left px-2 py-1 font-semibold text-slate-600">CA</th>
                                         <th className="text-left px-2 py-1 font-semibold text-slate-600">Ações</th>
                                       </tr>
                                      </thead>
                                      <tbody>
                                       {ferrsVinculadas.map(f => (
                                         <tr key={f.id} className="border-b border-slate-100 hover:bg-white/80">
                                           <td className="px-2 py-1.5 font-mono text-blue-600 font-semibold">{f.codigo}</td>
                                           ...
                                           <td className="px-2 py-1.5 font-mono text-slate-600">{f.numero_serie || '-'}</td>
                                           <td className="px-2 py-1.5 font-mono text-slate-600">{f.numero_laudo || '-'}</td>
                                           <td className="px-2 py-1.5 text-slate-600">
                                             {f.data_vencimento_laudo ? new Date(f.data_vencimento_laudo).toLocaleDateString('pt-BR') : '-'}
                                           </td>
                                  ...
                                  <p className="text-xs text-slate-400 italic mt-1">⚠️ Nenhuma ferramenta vinculada</p>
                                )}
                                </div>
                                );
                                })}
                                ...
                                <p className="text-xs font-semibold text-blue-900 mb-2">📦 Ferramentas Neste Caminhão ({ferramentasDoCaminhao.length})</p>
                                ...
                                return (
                                <span key={f.id} className={`text-xs px-2 py-1 rounded border ${vinculada ? 'bg-green-100 border-green-200 text-green-700' : 'bg-white border-blue-200 text-slate-600'}`}>
                                  {vinculada ? '✓' : '○'} {f.codigo}
                                </span>
                                );
                                })}
                      ...
                      {ferramentasDoCaminhao.length > 0 && (
                        <div>
                          <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Todas as Ferramentas do Caminhão ({ferramentasDoCaminhao.length})</p>
                          <div className="overflow-x-auto rounded border border-slate-200">
                            <table className="w-full text-xs">
                              <thead className="bg-slate-100">
                                <tr>
                                  <th className="text-left px-2 py-1.5 font-semibold text-slate-600">Sub-Código</th>
                                  <th className="text-left px-2 py-1.5 font-semibold text-slate-600">Descrição</th>
                                  <th className="text-left px-2 py-1.5 font-semibold text-slate-600">Status</th>
                                  <th className="text-left px-2 py-1.5 font-semibold text-slate-600">Nº Série</th>
                                  <th className="text-left px-2 py-1.5 font-semibold text-slate-600">Nº Laudo</th>
                                  <th className="text-left px-2 py-1.5 font-semibold text-slate-600">Venc. Laudo</th>
                                  <th className="text-left px-2 py-1.5 font-semibold text-slate-600">CA</th>
                                  <th className="text-left px-2 py-1.5 font-semibold text-slate-600">Ações</th>
                                </tr>
                              </thead>
                              <tbody>
                                {ferramentasDoCaminhao.map(f => (
                      ...
                      {camposDoCaminhao.length === 0 && ferramentasDoCaminhao.length === 0 && (
                        <p className="text-sm text-slate-400 text-center py-4">Nenhuma ferramenta ou campo obrigatório configurado para este caminhão.</p>
                      )}
                    </div>
                  )}
                </Card>
        ...
        funcionariosFiltrados.length === 0 ? (
          ...
          <div className="space-y-3">
            {funcionariosFiltrados.map(func => {
              const ferramentasDoFuncionario = getFerramentasFuncionario(func.id).sort((a, b) => (a.descricao || '').localeCompare(b.descricao || ''));
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-500">{func.funcao_nome || 'Sem função'}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge className="bg-blue-100 text-blue-700">{ferramentasDoFuncionario.length} ferramenta(s)</Badge>
                      {isExpanded ? <ChevronDown className="w-4 h-4 text-slate-400" /> : <ChevronRight className="w-4 h-4 text-slate-400" />}
                    </div>
                  </div>
