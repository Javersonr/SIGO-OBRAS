...
export default function RelatoriosOrcamento({ orcamentoItens = [], nomeOrcamento = '', clienteNome = '', onClose }) {
  ...
  const gerarRelatorioSintetico = () => {
    ...
    const subtotal = orcamentoItens.reduce((s, i) => s + ((i.quantidade || 0) * (i.valor_unitario || 0)), 0);
    const bdiMedio = totalItens > 0 ? orcamentoItens.reduce((s, i) => s + (i.bdi || 0), 0) / totalItens : 0;
    const impostoMedio = totalItens > 0 ? orcamentoItens.reduce((s, i) => s + (i.imposto || 0), 0) / totalItens : 0;
            ...
            <table className="w-full text-sm">
              ...
              <tfoot className="bg-slate-100 sticky bottom-0">
                <tr className="border-t-2">
                  <td colSpan={9} className="px-3 py-3 text-right font-semibold">Total Geral:</td>
  ...
  const calcularCurvaABC = () => {
    ...
    const ordenados = [...itensComPercentual].sort((a, b) => (b.valor_total || 0) - (a.valor_total || 0));
  ...
  const gerarRelatorioCurvaABC = () => {
    ...
    const totalA = comClassificacao.filter(i => i.classe === 'A').reduce((s, i) => s + (i.valor_total || 0), 0);
    const totalB = comClassificacao.filter(i => i.classe === 'B').reduce((s, i) => s + (i.valor_total || 0), 0);
    const totalC = comClassificacao.filter(i => i.classe === 'C').reduce((s, i) => s + (i.valor_total || 0), 0);
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            <div className="p-4 bg-red-50 border-2 border-red-200 rounded-lg">
              ...
              <p className="text-xs text-red-600 mt-1">{totalGeral > 0 ? ((totalA / totalGeral) * 100).toFixed(1) : 0}% do total</p>
            </div>
            ...
            <div className="p-4 bg-yellow-50 border-2 border-yellow-200 rounded-lg">
              ...
              <p className="text-xs text-yellow-600 mt-1">{totalGeral > 0 ? ((totalB / totalGeral) * 100).toFixed(1) : 0}% do total</p>
            </div>
            ...
            <div className="p-4 bg-green-50 border-2 border-green-200 rounded-lg">
              ...
              <p className="text-xs text-green-600 mt-1">{totalGeral > 0 ? ((totalC / totalGeral) * 100).toFixed(1) : 0}% do total</p>
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-100 sticky top-0">
                <tr>
                  ...
                  <th className="text-right px-3 py-2 font-semibold text-slate-700">% Individual</th>
              ...
              <tbody>
                {comClassificacao.map((item, idx) => (
                  <tr key={item.id} className="border-b hover:bg-slate-50">
                    ...
                    <td className="px-3 py-2 font-medium">{item.descricao}</td>
                    <td className="px-3 py-2 text-right">{item.quantidade || 0}</td>
                    <td className="px-3 py-2 text-right font-semibold">{formatCurrency(item.valor_total)}</td>
                    <td className="px-3 py-2 text-right">{item.percentual.toFixed(2)}%</td>
                    <td className="px-3 py-2 text-right font-semibold">{item.percentual_acumulado.toFixed(2)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <ul className="text-sm text-blue-800 space-y-1">
              <li><strong>Classe A:</strong> Itens de alto valor (80% do custo) - Controle rigoroso</li>
              <li><strong>Classe B:</strong> Itens de valor médio (15% do custo) - Controle moderado</li>
              <li><strong>Classe C:</strong> Itens de baixo valor (5% do custo) - Controle simples</li>
            </ul>
          </div>
  ...
  const exportarPDFAnalitico = async () => {
    ...
    doc.setFontSize(10);
    doc.setFont(undefined, 'bold');
    doc.text(`TOTAL: ${formatCurrency(orcamentoItens.reduce((s, i) => s + (i.valor_total || 0), 0))}`, 250, y);
