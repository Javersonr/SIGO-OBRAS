...
export default function InspecaoCampoFormModal({
  open,
  onOpenChange,
  empresaAtiva,
  inspecao,
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      if (inspecao?.id) {
        ...
        await sigo.entities.InspecaoCampo.update(inspecao.id, updateData);
        toast.success("Inspeção atualizada");
      } else {
        await sigo.entities.InspecaoCampo.create(data);
          ...
          <div className="max-w-xl mx-auto space-y-4">
            <div>
                ...
                <SelectContent>
                  {checklists.map((cl) => (
                    <SelectItem key={cl.id} value={cl.id}>
                      {cl.nome} ({cl.total_itens || 0} itens)
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              {checklists.length === 0 && (
                <p className="text-xs text-amber-600 mt-1">
                  Nenhum checklist cadastrado. Crie um em "Gerenciar Checklists".
                </p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setForm((p) => ({ ...p, responsavel_email: e.target.value }))}
                  placeholder="email@..."
                  className="mt-1.5"
                />
              </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setForm((p) => ({ ...p, projeto_nome: e.target.value }))}
