...
export function useBulkOperations() {
  const [deleteProgress, setDeleteProgress] = useState({ show: false, current: 0, total: 0, type: '' });

  const deletarTodos = async (entityService, queryFilter, tipoNome) => {
    if (!confirm(`⚠️ ATENÇÃO: Isso irá apagar TODOS os ${tipoNome} desta empresa. Esta ação não pode ser desfeita. Deseja continuar?`)) return;
    ...
    try {
      ...
      for (let i = 0; i < todosRegistros.length; i += BATCH_SIZE) {
        ...
        for (let j = 0; j < batch.length; j += CONCURRENT_CHUNK) {
          ...
          setDeleteProgress({ show: true, current: totalSucessos + totalErros, total, type: tipoNome });
    ...
    try {
      for (let i = 0; i < ids.length; i += BATCH_SIZE) {
        ...
        for (let j = 0; j < batch.length; j += CONCURRENT_CHUNK) {
          ...
          const results = await Promise.allSettled(
            chunk.map(id => entityService.delete(id))
          );
