...
export default function BuscaAvancada({ oportunidades, onResultsChange, statusList, usuarios }) {
  ...
  const searchOportunidades = useCallback((term) => {
    ...
    return oportunidades.filter(op => {
      ...
      if (op.responsaveis_ids) {
        try {
          ...
          matchResponsavel = usuarios.some(u => 
            ids.includes(u.id) && 
            (u.usuario_email?.toLowerCase().includes(lowerTerm) || u.nome_completo?.toLowerCase().includes(lowerTerm))
          );
        } catch {}
      }
      return matchTitulo || matchCliente || matchStatus || matchCidade || matchValor || matchModalidade || matchResponsavel;
    });
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Badge variant="outline" className="text-xs flex-shrink-0">
                        {suggestion.type === 'history' ? 'Histórico' : 
                         suggestion.type === 'oportunidade' ? 'Oportunidade' :
