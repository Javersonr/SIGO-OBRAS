...
import {
  Wrench, TrendingUp, Clock, DollarSign, Package, FileText,
  Plus, Calendar, AlertCircle, CheckCircle, XCircle, Paperclip, Download, Eye, X, Upload
} from 'lucide-react';
...
export default function FerramentaDetalheModal({ open, onOpenChange, ferramenta, onEdit }) {
  ...
  const calcularCustoTotal = () => {
    return manutencoes.reduce((total, m) => {
      ...
      const custoPecas = m.pecas_substituidas 
        ? JSON.parse(m.pecas_substituidas).reduce((sum, p) => sum + (p.quantidade * p.custo_unitario), 0)
        : 0;
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'Em Andamento': return <Clock className="w-4 h-4 text-blue-600" />;
    ...
    <SheetModalComponent
      ...
      footer={
        <div className="flex justify-end gap-3">
          {onEdit && <Button variant="outline" size="sm" onClick={onEdit}>Editar Ferramenta</Button>}
            ...
            <TabsContent value="manutencoes" className="space-y-3">
              {manutencoes.length === 0 ? (
                ...
                manutencoes.map((manut) => {
                  ...
                  const custoTotal = (parseFloat(manut.custo) || 0) + pecas.reduce((s, p) => s + (p.quantidade * p.custo_unitario), 0);
                      ...
                      <CardContent className="p-4">
                        ...
                        {pecas.length > 0 && (
                          <div className="bg-slate-50 rounded p-3 mb-2">
                            <p className="text-xs font-medium text-slate-700 mb-2">Peças Substituídas:</p>
                        ...
                        <div className="flex items-center justify-between pt-2 border-t">
                          <div className="text-xs text-slate-600">
                            {manut.responsavel_nome && (
                              <span>Responsável: <strong>{manut.responsavel_nome}</strong></span>
                            )}
                          </div>
              ...
              <div className="relative">
                {[...manutencoes, ...movimentacoes]
                  ...
                  .map((evento, idx) => {
                    ...
                    return (
                      <div key={`${isManutencao ? 'm' : 'mov'}-${evento.id}`} className="flex gap-4 mb-4">
                        ...
                        <div className="flex-1 pb-4">
                          ...
                          <p className="text-xs text-slate-500">
                            {new Date(evento.data_manutencao || evento.created_date).toLocaleDateString('pt-BR')} às{' '}
                            {new Date(evento.data_manutencao || evento.created_date).toLocaleTimeString('pt-BR', {
                              hour: '2-digit',
                              minute: '2-digit'
                            })}
                          </p>
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  {['Preventiva', 'Corretiva', 'Preditiva', 'Inspeção'].map(tipo => {
                    ...
                    return (
                      <div key={tipo} className="flex justify-between items-center p-2 bg-slate-50 rounded">
                        <div>
                      ...
                      >
                        {uploadingFiles ? (
                          ...
                          <>
                            ...
                            <span className="text-sm font-medium text-amber-700">Clique para anexar</span>
                          </>
                        )}
                      </label>
