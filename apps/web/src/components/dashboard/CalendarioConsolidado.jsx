...
export default function CalendarioConsolidado() {
  ...
  const loadTodasOportunidades = async () => {
    ...
    try {
      ...
      if (empresasIds.length === 0) { setLoading(false); return; }

      // Buscar oportunidades sequencialmente para evitar rate limit
      const todasOps = [];
      for (const empresaId of empresasIds) {
        try {
          const ops = await base44.entities.Oportunidade.filter({ empresa_id: empresaId, arquivado: false });
          todasOps.push(...ops);
        } catch (e) {
          if (e?.status === 429) {
  ...
  const handleClickOportunidade = async (op) => {
    ...
    try {
      const [atualiz, itens, etapas, arqs] = await Promise.all([
        base44.entities.OportunidadeAtualizacao.filter({ empresa_id: op.empresa_id, oportunidade_id: op.id }),
        base44.entities.OrcamentoItem.filter({ empresa_id: op.empresa_id, oportunidade_id: op.id }),
        base44.entities.CronogramaEtapa.filter({ empresa_id: op.empresa_id, oportunidade_id: op.id }),
        base44.entities.ArquivoOportunidade.filter({ empresa_id: op.empresa_id, oportunidade_id: op.id }),
      ]);
      ...
      const [status, usuarios] = await Promise.all([
        base44.entities.StatusOportunidade.filter({ empresa_id: op.empresa_id }),
        base44.entities.UsuarioEmpresa.filter({ empresa_id: op.empresa_id, ativo: true }),
      ]);
  ...
  const oportunidadesPorData = useMemo(() => {
    ...
    opsFiltradas.forEach(op => {
      ...
      if (op.licitacao_data_impugnacao) entradas.push({ op, tipo: 'impugnacao', data: op.licitacao_data_impugnacao });
      if (op.licitacao_data_proposta) entradas.push({ op, tipo: 'proposta', data: op.licitacao_data_proposta });
  ...
  const empresasCores = useMemo(() => {
    const mapa = {};
    Object.keys(empresasMap).forEach((id, i) => { mapa[id] = CORES[i % CORES.length]; });
    return mapa;
  }, [empresasMap]);
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setCurrentDate(new Date())}>Hoje</Button>
        </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {currentMonth && (
                        <button
                          onClick={(e) => { e.stopPropagation(); handleAbrirModalCriacao(dateStr, e); }}
                      ...
                      {opsNoDia.slice(0, 2).map(op => {
                        ...
                        return (
                          <div
                            ...
                            className={cn("px-1 py-0.5 rounded border text-xs cursor-pointer truncate hover:opacity-75 transition-opacity group relative overflow-visible", corClasse)}
                          >
                            <span className="truncate block">{TIPO_LABELS[op._tipoEvento] || ''} {op.nome}</span>
                            <span className="absolute left-0 right-0 -top-14 z-10 hidden group-hover:flex flex-col items-start bg-slate-800 text-white text-xs rounded px-2 py-1.5 pointer-events-none whitespace-nowrap shadow-lg gap-0.5">
                              ...
                              <span>
                                {op._tipoEvento === 'impugnacao' ? '⚖️ Impugnação' : op._tipoEvento === 'proposta' ? '📋 Limite Proposta' : op._tipoEvento === 'licitacao' ? '🏛️ Licitação' : '📅 Fechamento'}
                                {op.licitacao_modalidade ? ` · ${op.licitacao_modalidade}` : ''}
                              </span>
                              {op.licitacao_garantia_proposta && <span>✅ Com garantia de proposta</span>}
                            </span>
                          </div>
                        );
                      })}
            ...
            <div className="p-2 space-y-1">
              {opsPopover.map(op => {
                ...
                return (
                  <div
                    key={op.id + op._tipoEvento}
                    onClick={() => { handleClickOportunidade(op); setPopoverDia(null); }}
                  ...
                  >
                    <p className="font-medium text-slate-800">{TIPO_LABELS[op._tipoEvento] || ''} {op.nome}</p>
                    {nomeEmpresa && <p className="text-xs text-slate-500 mt-0.5">{nomeEmpresa}</p>}
                    {op.licitacao_modalidade && <p className="text-xs text-slate-500">{op.licitacao_modalidade}</p>}
                  </div>
                );
              })}
            </div>
      ...
      {popoverData && (
        <>
          ...
          >
            <p className="text-xs font-semibold text-slate-500 px-2 py-1 mb-1">Criar oportunidade em:</p>
          ...
          {Object.entries(empresasMap).map(([id, empresa], i) => (
            <button
              ...
              className={cn(
                ...
                filtroEmpresa === id ? "border-slate-400 bg-slate-100 font-semibold" : "border-slate-200 hover:border-slate-300"
              )}
            >
              <div className={cn("w-2.5 h-2.5 rounded-full border", CORES[i % CORES.length])} />
              <span className="text-slate-600">{empresa?.nome_fantasia || empresa?.razao_social || empresa?.nome}</span>
            </button>
          ))}
        </div>
      )}
      ...
      <OportunidadeDetalhe
        open={showDetalhe}
        onOpenChange={(v) => { setShowDetalhe(v); if (!v) { setOportunidadeDetalhe(null); setEmpresaDetalhe(null); } }}
