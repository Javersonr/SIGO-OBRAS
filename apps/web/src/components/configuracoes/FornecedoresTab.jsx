...
export default function FornecedoresTab({ empresaAtiva, fornecedores, loadData }) {
  ...
  const handleExportarFornecedoresPDF = async () => {
    ...
    doc.setFontSize(16);
    doc.text("Lista de Fornecedores", 14, 15);
    doc.setFontSize(10);
    ...
    fornecedores.forEach((f) => {
      ...
      doc.text((f.nome_razao || "").substring(0, 30), 14, y);
      doc.text(f.tipo_pessoa || "-", 80, y);
      doc.text(f.cnpj || "-", 100, y);
      doc.text((f.email || "-").substring(0, 25), 140, y);
      doc.text(f.telefone || "-", 190, y);
      doc.text((f.cidade || "-").substring(0, 20), 230, y);
      y += 6;
    });
  ...
  const handleBaixarModeloFornecedores = () => {
    const csv = [
      "Nome;Nome Fantasia;Tipo;CPF/CNPJ;Insc. Estadual;Insc. Municipal;Email;Telefone;Contato Principal;Categoria;Endereço;Número;Complemento Bairro;Cidade;Estado;CEP;Observações",
      "EXEMPLO FORNECEDOR;;PJ;;;;;;;;;;;;;;",
    ].join("\n");
    const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
  ...
  const handleLimparTodosFornecedores = async () => {
    ...
    const todos = await sigo.entities.Fornecedor.filter({ empresa_id: empresaAtiva.id });
    for (const f of todos) {
      await sigo.entities.Fornecedor.update(f.id, { ativo: false });
    }
    ...
    reader.onload = async (event) => {
      try {
        ...
        const batch = data.map((v) => ({
          ...
          tipo_pessoa: v[2] || "PJ",
          cnpj: v[3] || "",
          inscricao_estadual: v[4] || "",
          inscricao_municipal: v[5] || "",
          email: v[6] || "",
          telefone: v[7] || "",
          contato_principal: v[8] || "",
          categorias: v[9]
            ? v[9]
                .split(",")
                .map((c) => c.trim())
        ...
        for (let i = 0; i < batch.length; i += BATCH_SIZE) {
          await sigo.entities.Fornecedor.bulkCreate(batch.slice(i, i + BATCH_SIZE));
  ...
  const handleDeletarSelecionados = async () => {
    ...
    for (const id of selectedFornecedorIds) {
      await sigo.entities.Fornecedor.update(id, { ativo: false });
    }
          ...
          <div className="flex gap-2">
            {selectedFornecedorIds.length > 0 && (
              <Button variant="destructive" onClick={handleDeletarSelecionados}>
                ...
                Apagar {selectedFornecedorIds.length} Selecionados
              </Button>
            )}
            <input
              id="fileInputFornecedores"
              type="file"
              className="hidden"
              accept=".csv"
              onChange={handleImportarFornecedores}
            />
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuItem onClick={handleExportarFornecedoresExcel}>
                  <FileText className="w-4 h-4 mr-2" />
                  Exportar em Excel
                </DropdownMenuItem>
                <DropdownMenuItem onClick={handleExportarFornecedoresPDF}>
                  <FileText className="w-4 h-4 mr-2" />
                  Exportar em PDF
                </DropdownMenuItem>
                <DropdownMenuItem onClick={handleBaixarModeloFornecedores}>
                  <FileText className="w-4 h-4 mr-2" />
                  Modelo de Importação
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => document.getElementById("fileInputFornecedores")?.click()}
                >
                  <FileText className="w-4 h-4 mr-2" />
                  Importar Fornecedores
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={handleLimparTodosFornecedores} className="text-red-600">
                  <Trash2 className="w-4 h-4 mr-2" />
              ...
              <TableHeader>
                <TableRow>
                  <TableHead className="w-12">
                    <Checkbox
                      checked={
                        selectedFornecedorIds.length === fornecedores.length &&
                        fornecedores.length > 0
                      }
                      onCheckedChange={(checked) => {
                        if (checked) setSelectedFornecedorIds(fornecedores.map((f) => f.id));
                        else setSelectedFornecedorIds([]);
                      }}
                    />
                  </TableHead>
                    ...
                    <TableRow key={f.id}>
                      <TableCell>
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            if (checked) setSelectedFornecedorIds([...selectedFornecedorIds, f.id]);
                            else
                              setSelectedFornecedorIds(
                                selectedFornecedorIds.filter((id) => id !== f.id)
                              );
                          }}
                        />
                      </TableCell>
      ...
      <Sheet open={showFornecedorModal} onOpenChange={setShowFornecedorModal}>
        <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col">
          ...
          <div className="space-y-4 py-4 px-6 flex-1">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <Input
                  value={fornecedorForm.nome_razao || ""}
                  onChange={(e) =>
                    setFornecedorForm({ ...fornecedorForm, nome_razao: e.target.value })
                  }
              ...
              <div className="col-span-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFornecedorForm({ ...fornecedorForm, nome_fantasia: e.target.value })
                  }
              ...
              <div>
                <Label>Tipo *</Label>
                <Select
                  value={fornecedorForm.tipo_pessoa || "PJ"}
                  onValueChange={(v) => setFornecedorForm({ ...fornecedorForm, tipo_pessoa: v })}
                >
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1.5">
                  <Input
                    ...
                    placeholder={
                      fornecedorForm.tipo_pessoa === "PF" ? "000.000.000-00" : "00.000.000/0000-00"
                    }
                  />
                  {fornecedorForm.tipo_pessoa === "PJ" && (
                    <BuscarCnpjButton
            ...
            <div className="border-t pt-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <Input
                    value={fornecedorForm.contato_principal || ""}
                    onChange={(e) =>
                      setFornecedorForm({ ...fornecedorForm, contato_principal: e.target.value })
                    }
          ...
          <div className="flex justify-end gap-3 p-6 border-t">
            <Button variant="outline" onClick={() => setShowFornecedorModal(false)}>
              Cancelar
            </Button>
            <Button onClick={handleSaveFornecedor} className="bg-amber-500 hover:bg-amber-600">
              Salvar
            </Button>
          </div>
        </SheetContent>
