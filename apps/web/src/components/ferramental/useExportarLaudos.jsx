...
export async function exportarLaudos(ferramentas, caminhoes, onProgress) {
  ...
  for (let i = 0; i < ferramentasComLaudo.length; i++) {
    ...
    } else if (ferr.caminhao_id || (ferr.localizacao && caminhoes.some(c => c.placa === ferr.localizacao))) {
      const placa = ferr.localizacao || caminhoes.find(c => c.id === ferr.caminhao_id)?.placa || 'Caminhao';
    ...
    try {
      ...
      const numeroSerie = ferr.numero_serie ? `_${ferr.numero_serie.replace(/[/\\?%*:|"<>]/g, '-')}` : '';
