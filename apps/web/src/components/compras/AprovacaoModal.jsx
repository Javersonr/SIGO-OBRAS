...
export default function AprovacaoModal({ 
      ...
      const loadApprovalData = async () => {
        ...
        try {
          ...
          const itensEnriquecidosData = itensData.map(item => {
            ...
            const orcItem = orcamentoItens.find(o => 
              o.descricao?.toLowerCase() === item.descricao?.toLowerCase()
            );
            return {
              ...item,
          ...
          const itensOrdenados = itensEnriquecidosData.sort((a, b) => (a.descricao || '').localeCompare(b.descricao || ''));
          setItens(itensData);
  ...
  const updateQuantidade = (itemId, novaQuantidade) => {
    setItensEnriquecidos(itensEnriquecidos.map(item => 
      item.id === itemId 
  ...
  const handleDecisao = async (decisao) => {
        ...
        try {
          ...
          const itensAlterados = itensEnriquecidos.filter(item => 
            item.quantidade_editavel !== item.quantidade
          );
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    onClick={async () => {
                      if (!confirm('⚠️ Tem certeza que deseja EXCLUIR esta solicitação?\n\nEsta ação não pode ser desfeita!')) return;
                      try {
                        const delay = (ms) => new Promise(res => setTimeout(res, ms));

                        // Excluir aprovações sequencialmente
                        const aprovs = await base44.entities.AprovacaoSolicitacao.filter({ solicitacao_id: solicitacao.id });
                        ...
                        const its = await base44.entities.SolicitacaoCompraItem.filter({ solicitacao_id: solicitacao.id });
                        for (const i of its) {
              ...
              <CardContent className="p-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <Badge variant="outline" className={
                    solicitacao.prioridade === 'Urgente' ? 'border-red-500 text-red-600' :
                    solicitacao.prioridade === 'Alta' ? 'border-orange-500 text-orange-600' :
                    ...
                    <TableBody>
                      {itensEnriquecidos.map((item) => (
                        <TableRow 
                          key={item.id}
                          className={cn(
                            itensSelecionados.includes(item.id) && 'bg-green-50'
                          )}
                        >
                          ...
                          <TableCell className="text-right">
                            <Badge 
                              variant="outline" 
                              className={item.quantidade_estoque < item.quantidade_editavel ? 'border-red-500 text-red-600' : ''}
                            >
