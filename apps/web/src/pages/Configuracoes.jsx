...
export default function Configuracoes() {
  const { empresaAtiva, perfil, user, reloadEmpresaAtiva, empresas, setEmpresaAtiva } = useEmpresa();
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const todasAssinaturas = await base44.entities.Assinatura.filter({ empresa_id: empresaAtiva.id });
      ...
      if (assinaturas.length > 0) {
        ...
        if (planos.length > 0) {
          ...
          if (plano.modulos_liberados) {
            try { modulos = JSON.parse(plano.modulos_liberados); } catch {}
          }
    ...
    const vinculos = await base44.entities.UsuarioEmpresa.filter({ empresa_id: empresaAtiva.id, ativo: true });
    ...
    const clientesList = await base44.entities.Cliente.filter({ empresa_id: empresaAtiva.id, ativo: true });
    await new Promise(resolve => setTimeout(resolve, 150));
    
    // Batch 4: Fetch fornecedores
    const fornecedoresList = await base44.entities.Fornecedor.filter({ empresa_id: empresaAtiva.id, ativo: true });
    await new Promise(resolve => setTimeout(resolve, 150));
    
    // Batch 5: Fetch materiais
    const materiaisList = await base44.entities.Material.filter({ empresa_id: empresaAtiva.id, ativo: true });
    await new Promise(resolve => setTimeout(resolve, 150));
    
    // Batch 6: Fetch mão de obra
    const maoDeObraList = await base44.entities.MaoDeObra.filter({ empresa_id: empresaAtiva.id, ativo: true });
    await new Promise(resolve => setTimeout(resolve, 150));
    
    // Batch 7: Fetch categorias
    const catsMaterial = await base44.entities.CategoriaMaterial.filter({ empresa_id: empresaAtiva.id, ativo: true });
    await new Promise(resolve => setTimeout(resolve, 150));
    const catsMaoObra = await base44.entities.CategoriaMaoDeObra.filter({ empresa_id: empresaAtiva.id, ativo: true });
    await new Promise(resolve => setTimeout(resolve, 150));
    const unidades = await base44.entities.UnidadeMedida.filter({ empresa_id: empresaAtiva.id, ativo: true });
  ...
  const handleBuscarCnpj = async (cnpj) => {
    ...
    try {
      ...
      toast.error('❌ Erro ao buscar CNPJ - verifique se o número está correto', { duration: 3000 });
    } finally {
      setBuscandoCepEmpresa(false);
    }
  };
  ...
  const handleLimparTodosMaoDeObra = async () => {
    await deletarTodos(base44.entities.MaoDeObra, { empresa_id: empresaAtiva.id, ativo: true }, 'serviços');
    ...
    try {
      ...
      await Promise.all([
        base44.entities.StatusOportunidade.bulkCreate([
          { empresa_id: novaEmpresa.id, nome: 'Novo Lead', cor: '#94a3b8', ordem: 1, tipo: 'aberto' },
          { empresa_id: novaEmpresa.id, nome: 'Em Negociação', cor: '#3b82f6', ordem: 2, tipo: 'aberto' },
          { empresa_id: novaEmpresa.id, nome: 'Ganho', cor: '#10b981', ordem: 3, tipo: 'ganho' },
          { empresa_id: novaEmpresa.id, nome: 'Perdido', cor: '#ef4444', ordem: 4, tipo: 'perdido' }
        ]),
  ...
  const handleExportarMaoObraExcel = () => {
    ...
    const headers = ['nome', 'descricao', 'unidade', 'codigo', 'categoria', 'preco_referencia', 'raior_us', 'observacoes'];
  ...
  const handleBaixarModeloMaoObra = () => {
    const modelo = [
      ['nome', 'descricao', 'unidade', 'codigo', 'categoria', 'preco_referencia', 'raior_us', 'observacoes'],
      ['Alvenaria de Tijolos', 'Execução de alvenaria com tijolos cerâmicos', 'M2', 'SERV001', 'Alvenaria', '150', '1.5', '']
    ];
  ...
  const handleImportarMaoObra = async (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const sepMO = totalTabMO > 0 && totalTabMO >= totalSemiMO && totalTabMO >= totalCommaMO ? '\t'
          : totalSemiMO > 0 ? ';' : ',';
        ...
        const parseCSVFullMO = (rawText, separator) => {
          ...
          for (let i = 0; i < rawText.length; i++) {
            ...
            if (c === '"') {
              ...
              if (inQ) { cur += '\n'; if (c === '\r') i++; }
              else { if (c === '\r') i++; rows.push(cur); cur = ''; }
            } else { cur += c; }
          }
          ...
          for (let i = 0; i < maoObraImportada.length; i += BATCH_SIZE) {
            ...
            const batchLimpo = batch.map(m => {
              ...
              Object.keys(maoObraLimpa).forEach(key => {
                if (maoObraLimpa[key] === '' || maoObraLimpa[key] === null || maoObraLimpa[key] === undefined) {
    ...
    <div className="space-y-6">
      ...
      {importProgress.show && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 w-96 shadow-xl">
            <h3 className="text-lg font-semibold mb-4">Importando {activeTab === 'materiais' ? 'Materiais' : activeTab === 'maodeobra' ? 'Mão de Obra' : activeTab === 'fornecedores' ? 'Fornecedores' : 'Clientes'}...</h3>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            {perfil === 'Admin' && (
              <>
                ...
                {modulosLiberados['Compras'] && (
                  <TabsTrigger value="fornecedores" className="whitespace-nowrap text-xs sm:text-sm">
                    Fornecedores
                  </TabsTrigger>
                )}
                ...
                {(modulosLiberados['Ferramental e EPI'] || modulosLiberados['Segurança do Trabalho']) && (
        ...
        <TabsContent value="fornecedores">
          <FornecedoresTab empresaAtiva={empresaAtiva} fornecedores={fornecedores} loadData={loadData} />
        </TabsContent>
        ...
        <TabsContent value="categorias">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button onClick={() => { setCategoriaForm({ nome: '', tipo: 'Despesa' }); setShowCategoriaModal(true); }}>
                  ...
                  <div className="space-y-2">
                    {categorias.filter(c => c.tipo === 'Despesa').map(c => (
                      <div key={c.id} className="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                        <span>{c.nome}</span>
                        <Button variant="ghost" size="icon" onClick={() => handleDeleteCategoria(c)}>
      ...
      />
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button onClick={handleSaveCategoria} className="bg-amber-500 hover:bg-amber-600">Salvar</Button>
            </div>
            </SheetContent>
            </Sheet>
              ...
              <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col">
                <SheetHeader>
                  ...
                  <p className="text-sm text-slate-500">
                    {selectedMaoObraUS?.nome || `${selectedMaoObraUS?.ids?.length || 0} serviço(s) selecionado(s)`}
                  </p>
                </SheetHeader>
                <div className="space-y-4 py-4">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setPrecoUSForm({ ...precoUSForm, valor_us_global: e.target.value })}
                  ...
                  {selectedMaoObraUS?.id && (
                    <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                      ...
                      <p className="text-2xl font-bold text-amber-700 mt-2">
                        R$ {((parseFloat(precoUSForm.valor_us_global) || 0) * (selectedMaoObraUS?.raior_us || 1)).toFixed(2)}
                      </p>
                      <p className="text-xs text-amber-600 mt-1">
                        {precoUSForm.valor_us_global || '0'} × {selectedMaoObraUS?.raior_us || '1'} (Fator de US)
                      </p>
                    </div>
                  )}
                </div>
                <div className="flex justify-end gap-3">
                  <Button variant="outline" onClick={() => setShowPrecoUSModal(false)}>Cancelar</Button>
            ...
            {linkResetModal && (
              <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
                  <div className="p-6">
                    <h3 className="text-lg font-semibold text-slate-800 mb-1">Link de Redefinição de Senha</h3>
                    <p className="text-sm text-slate-500 mb-5">
                      Copie o link abaixo ou envie direto pelo WhatsApp para <strong>{linkResetModal.nome}</strong>.
                    </p>
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <button
                        onClick={() => { navigator.clipboard.writeText(linkResetModal.link); toast.success('✅ Link copiado!'); }}
            ...
            <Sheet open={showNovaEmpresa} onOpenChange={setShowNovaEmpresa}>
              <SheetContent side="right" className="w-full sm:max-w-lg">
                ...
                <div className="flex justify-end gap-3">
                  <Button variant="outline" onClick={() => setShowNovaEmpresa(false)}>Cancelar</Button>
                  <Button onClick={handleCriarEmpresa} className="bg-amber-500 hover:bg-amber-600">Criar Empresa</Button>
                </div>
              </SheetContent>
            </Sheet>
            </div>
