...
import { Plus, Edit, Trash2, Download, X, ShieldCheck, MoreVertical, Eye, FileText, GraduationCap, Search } from 'lucide-react';
...
export default function FuncaoModal({ open, onClose, funcao, empresaAtiva, onSave, treinamentosDisponiveis = [] }) {
  ...
  const loadTreinamentos = async (funcaoId) => {
    ...
    try {
      ...
      setTreinamentos(result.sort((a, b) => new Date(b.created_date || b.data_inicio) - new Date(a.created_date || a.data_inicio)));
    } catch (error) {
  ...
  const handleImportarFerramentas = async (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const nomesExistentes = ferramentasExistentes.map((f) => f.ferramenta?.toLowerCase().trim());
      ...
      <SheetContent side="right" className="w-full overflow-y-auto" data-fullscreen-modal>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
          ...
          <TabsContent value="treinamentos" className="space-y-6 p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center pb-3 border-b-2 border-slate-200">
                <div>
                  <h3 className="text-lg font-semibold text-slate-800">Treinamentos Realizados</h3>
                  <p className="text-sm text-slate-600">Migre os treinamentos existentes para modelos</p>
                </div>
                  ...
                  <Button
                    ...
                    try {
                      ...
                      for (const treinamento of treinamentos) {
                        const jaExiste = modelosAtuais.some(m => 
                      ...
                      if (novosModelos > 0) {
                        ...
                        setFormData({ ...formData, modelo_treinamentos: JSON.stringify(modelosAtuais) });
                        alert(`✅ ${novosModelos} treinamento(s) migrado(s) para modelos com sucesso!`);
                      } else {
              ...
              {treinamentos.length === 0 ? (
                ...
                <div className="space-y-2">
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <h5 className="font-semibold text-sm text-slate-800">{treinamento.nome}</h5>
                                {treinamento.codigo && (
                                  <Badge variant="outline" className="text-xs">{treinamento.codigo}</Badge>
                                )}
                              </div>
                              <div className="space-y-0.5 text-xs text-slate-600">
                                <p><span className="font-medium">Aluno:</span> {treinamento.aluno_nome}</p>
                                {treinamento.data_inicio && treinamento.data_fim && (
                                  <p>
                                    <span className="font-medium">Período:</span>{' '}
                                    {format(new Date(treinamento.data_inicio), 'dd/MM/yyyy')} a {format(new Date(treinamento.data_fim), 'dd/MM/yyyy')}
                                  </p>
                                )}
                              </div>
                            </div>
                            <div className="flex gap-1 ml-2 items-center">
                              <label className="flex items-center gap-1 cursor-pointer mr-2" title="Usar como modelo">
                                <input
                                  type="checkbox"
                  ...
                  }).length === 0 && buscaTreinamentoRealizado && (
                    <div className="text-center py-8 text-slate-500">
          ...
          <TabsContent value="documentos" className="space-y-4 p-6">
              <EPIListaModelo modeloEpi={formData.modelo_epi} allFerramentas={allFerramentas} onChangeModeloEpi={(val) => setFormData({ ...formData, modelo_epi: val })} onBaixarModelo={handleBaixarModeloEPIs} onImportar={handleImportarEPIs} fileInputRef={fileInputEPIRef} />
              {/* Modelo Ferramentas */}
              <Card>
                    ...
                    <div className="flex gap-2">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => fileInputFerramentasRef.current?.click()}>
                            <FileText className="w-4 h-4 mr-2" />
                            Importar Lista
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={handleLimparTodasFerramentas} className="text-red-600">
                ...
                <div className="text-center py-4 text-slate-500 text-sm">
                      Nenhuma ferramenta adicionada. Digite no campo "Ferramenta" abaixo para adicionar.
                    </div> :
                null}
                  <div className="space-y-2 max-h-96 overflow-y-auto border rounded-lg p-3 bg-white">
                    <div className="grid gap-2 items-center p-2 bg-slate-100 rounded font-semibold text-xs sticky top-0" style={{ gridTemplateColumns: '2.5fr 0.6fr 1fr 0.4fr' }}>
                  ...
                  map((ferr, idx) =>
                  <div key={idx} className="grid gap-2 items-center p-2 bg-slate-50 rounded hover:bg-slate-100 transition-colors" style={{ gridTemplateColumns: '2.5fr 0.6fr 1fr 0.4fr' }}>
                       ...
                       placeholder="Digite para buscar ferramenta..."
                        ...
                        onChange={(e) => {
                          ...
                          setFormData({ ...formData, modelo_ferramentas: JSON.stringify(itens) });

                          // Mostrar sugestões se houver texto
                          if (e.target.value.trim()) {
                            const filtered = allFerramentasFerramenta.filter((f) =>
                        ...
                        onFocus={() => {
                          if (ferr.ferramenta?.trim()) {
                            const filtered = allFerramentasFerramenta.filter((f) =>
                            f.descricao?.toLowerCase().includes(ferr.ferramenta.toLowerCase()) ||
                            f.codigo?.toLowerCase().includes(ferr.ferramenta.toLowerCase())
                            );
                            setFilteredSuggestions(filtered);
                            setShowSuggestions(`ferr-${idx}`);
                          }
                        ...
                        className="text-xs h-8" />
                         ...
                         {showSuggestions === `ferr-${idx}` && filteredSuggestions.length > 0 &&
                        ...
                        <button
                          ...
                          className="w-full text-left px-3 py-2 hover:bg-amber-50 border-b last:border-b-0 flex flex-col gap-1 text-xs">
                                 ...
                                 <p className="text-slate-500">Código: {f.codigo || '-'} | Série: {f.numero_serie || '-'}</p>
                               </button>
                        )}
                           </div>
                      }
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Tab' && !e.shiftKey) {
                          ...
                          if (idx === itens.length - 1 && ferr.ferramenta?.trim()) {
                            ...
                            setTimeout(() => {
                              const inputs = document.querySelectorAll('[placeholder="Digite para buscar ferramenta..."]');
                    ...
                    <div className="grid gap-2 items-center p-2 bg-white rounded border-2 border-dashed border-slate-300" style={{ gridTemplateColumns: '2.5fr 0.6fr 1fr 0.4fr' }}>
                      <div className="relative z-0">
                        <Input
                        ...
                        onFocus={() => {
                          if (searchFerramentaValue.trim()) {
                            const filtered = allFerramentasFerramenta.filter((f) =>
                            f.descricao?.toLowerCase().includes(searchFerramentaValue.toLowerCase()) ||
                            f.codigo?.toLowerCase().includes(searchFerramentaValue.toLowerCase())
                            );
                            setFilteredSuggestions(filtered);
                            setShowSuggestions('ferr-new');
                          }
              ...
              <Card>
                <CardContent className="p-4 space-y-3">
                        ...
                        <>
                          ...
                          <div className="pb-2 border-b space-y-2">
                            ...
                            <div className="ml-6 space-y-2">
                              <div>
                                <Label className="text-xs text-slate-600">Código de Autorização</Label>
                                <Input
                                  type="text"
                                  value={opcoes.codigo_autorizacao || ''}
                                  onChange={(e) => handleCheckboxChange('codigo_autorizacao', e.target.value)}
                          ...
                          <div className="pb-2 border-b">
                            ...
                            <div className="ml-6 mt-2 space-y-1">
                              <div className="flex items-center gap-2">
                                ...
                                />
                                <label className="text-xs text-slate-700 cursor-pointer">Supervisor de Entrada</label>
                              </div>
                              <div className="flex items-center gap-2">
                                <input
                                  type="checkbox"
                                  ...
                                  className="w-3 h-3 cursor-pointer flex-shrink-0"
                                />
                                <label className="text-xs text-slate-700 cursor-pointer">Vigia/Trabalhador Autorizado</label>
                              </div>
                            </div>
                          </div>
                            ...
                            <div className="ml-6 mt-2 space-y-1">
                              <div className="flex items-center gap-2">
                                <input
                                  ...
                                  className="w-3 h-3 cursor-pointer flex-shrink-0"
                                />
                                <label className="text-xs text-slate-700 cursor-pointer">Estruturas de RDA</label>
                              </div>
                              <div className="flex items-center gap-2">
                                <input
                                  type="checkbox"
                                  ...
                                  className="w-3 h-3 cursor-pointer flex-shrink-0"
                                />
                                <label className="text-xs text-slate-700 cursor-pointer">Telhado</label>
                              </div>
                              <div className="flex items-center gap-2">
                                <input
                                  type="checkbox"
                                  ...
                                  className="w-3 h-3 cursor-pointer flex-shrink-0"
                                />
                                <label className="text-xs text-slate-700 cursor-pointer">Plataforma elevatórias ou andaimes</label>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="pb-2 border-b">
                            <div className="flex items-start gap-2">
                              <input
                                ...
                                className="mt-1 w-4 h-4 cursor-pointer flex-shrink-0"
                              />
                              <label className="text-xs text-slate-700 cursor-pointer">Outros</label>
                            </div>
                          </div>
                          ...
                          <div>
                            <div className="flex items-start gap-2">
                              <input
                                ...
                                onChange={(e) => handleCheckboxChange('fase_adaptacao', e.target.checked)}
                                className="mt-1 w-4 h-4 cursor-pointer flex-shrink-0"
                              />
                  ...
                  onChange={(e) => setFormData({ ...formData, modelo_autorizacao_formal: e.target.value })}
              ...
              <div>
                ...
                onChange={(e) => setFormData({ ...formData, modelo_direito_recusa: e.target.value })}
              ...
              <Card>
                <CardContent className="p-4 space-y-3">
                  ...
                  <div className="space-y-3">
                          ...
                          <div>
                            ...
                            onChange={(e) => {
                              try {
                                ...
                                setFormData({ ...formData, modelo_ordem_servicos: JSON.stringify(data) });
                              } catch {}
                            }}
                    ...
                    <div>
                      <Label className="text-xs font-semibold">Agentes Ambientais Inerentes ao Local de Trabalho/Atividades</Label>
                      <div className="space-y-2 mt-1">
                        <div>
                    ...
                    <div>
                      <Label className="text-xs font-semibold">Instruções de Segurança e Saúde Ocupacional</Label>
                      <Textarea
                      value={(() => {
                        try {
                          const data = JSON.parse(formData.modelo_ordem_servicos || '{}');
        ...
        <Sheet open={showVisualizarOrdemServico} onOpenChange={setShowVisualizarOrdemServico}>
          <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col bg-white" style={{ inset: 'auto 0 0 256px', width: 'calc(100% - 256px)', maxWidth: 'none' }}>
              ...
              <div className="p-8">
                ...
                <div className="pb-4 mb-6 border-b-4 border-black">
                  <div className="flex items-start justify-center gap-6 mb-4">
                    ...
                    <img src={empresaAtiva.logo_url} alt="Logo" className="h-16 object-contain" />
                    }
                  </div>
                ...
                <div className="mb-4">
                  <div className="bg-slate-400 text-white p-2 text-xs font-bold mb-0.5">Informações sobre o funcionário</div>
                  <div className="border-2 border-slate-400 bg-slate-100 p-3 text-xs space-y-1">
                    <div><span className="font-bold">Nome:</span> _________________________________________________</div>
                    <div><span className="font-bold">Função:</span> {funcao?.nome || '_______________________________'}</div>
                    <div><span className="font-bold">CPF:</span> _________________________</div>
                    <div><span className="font-bold">DATA:</span> ___/___/______</div>
                  </div>
                </div>

                {/* Parágrafo */}
                <div className="mb-6 text-xs text-justify leading-relaxed">
                  <p>Conforme estabelecido no item 1.7, letra "b", NR-01 da Portaria 3214/MTE, cabe ao empregador elaborar Ordem de Serviço (OS) sobre Segurança e Medicina do Trabalho, dando ciência aos empregados:</p>
                </div>

                {/* Cargo e Função */}
                <div className="mb-6 border-2 border-slate-400">
                  <div className="bg-slate-200 p-2 font-bold text-xs border-b-2 border-slate-400 text-center">Cargo e Função:</div>
                        ...
                        return (
                          <>
                            <div className="grid grid-cols-2 gap-4 border-b border-slate-300 pb-2">
                              <div><span className="font-bold">Função:</span> {data.funcao || ''}</div>
                              <div><span className="font-bold">Setor:</span> {data.setor || ''}</div>
                            </div>
                            <div className="border-b border-slate-300 pb-2">
                              <span className="font-bold">Locais de Trabalho:</span> {data.locais_trabalho || ''}
                            </div>
                            <div className="border-b border-slate-300 pb-2">
                              <span className="font-bold">Descrição Sumária:</span> {data.descricao_sumaria || ''}
                            </div>
                            <div>
                              <span className="font-bold">Atividades Desenvolvidas:</span> {data.atividades_desenvolvidas || ''}
                            </div>
                          </>);
                ...
                {/* Agentes Ambientais */}
                <div className="mb-6 border-2 border-slate-400">
                  <div className="bg-slate-200 p-2 font-bold text-xs border-b-2 border-slate-400 text-center">Agentes ambientais inerentes ao local de trabalho/atividades:</div>
                        ...
                        return (
                          <>
                            <div className="grid grid-cols-2 gap-4 mb-2">
                              <div className="border-b border-slate-300 pb-2">
                                <span className="font-bold">Físicos:</span> {data.agentes_fisicos || ''}
                              </div>
                              <div className="border-b border-slate-300 pb-2">
                                <span className="font-bold">Químicos:</span> {data.agentes_quimicos || ''}
                              </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4 mb-2">
                              <div className="border-b border-slate-300 pb-2">
                                <span className="font-bold">Biológicos:</span> {data.agentes_biologicos || ''}
                              </div>
                              <div className="border-b border-slate-300 pb-2">
                                <span className="font-bold">Ergonômicos:</span> {data.agentes_ergonomicos || ''}
                              </div>
                            </div>
                            <div className="border-b border-slate-300 pb-2">
                              <span className="font-bold">Acidentes:</span> {data.agentes_acidentes || ''}
                            </div>
                          </>);
                ...
                <div className="mb-6 border-2 border-slate-400">
                  <div className="bg-slate-200 p-2 font-bold text-xs border-b-2 border-slate-400 text-center">Tecnologias de proteção:</div>
                  <div className="p-3 text-xs space-y-2">
                    {(() => {
                      try {
                        const data = JSON.parse(formData.modelo_ordem_servicos || '{}');
                ...
                <div className="mb-6 border-2 border-slate-400">
                  <div className="bg-slate-200 p-2 font-bold text-xs border-b-2 border-slate-400 text-center">Instruções de Segurança e Saúde Ocupacional</div>
                  <div className="p-3 text-xs whitespace-pre-wrap leading-relaxed">
                    {(() => {
                      try {
                        const data = JSON.parse(formData.modelo_ordem_servicos || '{}');
                        return data.instrucoes_seguranca || 'Nenhum conteúdo definido';
                      } catch {return 'Nenhum conteúdo definido';}
                    })()}
                  </div>
                </div>
                ...
                {/* Observações */}
                <div className="mb-6 border-2 border-slate-400">
                  <div className="bg-slate-200 p-2 font-bold text-xs border-b-2 border-slate-400 text-center">Observações:</div>
                  <div className="p-3 text-xs whitespace-pre-wrap leading-relaxed">
                    {(() => {
                      try {
                        const data = JSON.parse(formData.modelo_ordem_servicos || '{}');
                ...
                {/* Punições */}
                <div className="mb-6 border-2 border-slate-400">
                  <div className="bg-slate-200 p-2 font-bold text-xs border-b-2 border-slate-400 text-center">Punições</div>
                  <div className="p-3 text-xs whitespace-pre-wrap leading-relaxed">
                    {(() => {
                      try {
                        const data = JSON.parse(formData.modelo_ordem_servicos || '{}');
                ...
                <div className="mb-6 border-2 border-slate-400">
                  <div className="bg-slate-200 p-2 font-bold text-xs border-b-2 border-slate-400 text-center">Ministério do Trabalho</div>
                  <div className="p-3 text-xs whitespace-pre-wrap leading-relaxed">
                    {(() => {
                      try {
                        const data = JSON.parse(formData.modelo_ordem_servicos || '{}');
                ...
                <div className="mb-6 border-2 border-slate-400">
                  <div className="bg-slate-200 p-2 font-bold text-xs border-b-2 border-slate-400 text-center">Medicina do trabalho</div>
                  <div className="p-3 text-xs whitespace-pre-wrap leading-relaxed">
                    {(() => {
                      try {
                        const data = JSON.parse(formData.modelo_ordem_servicos || '{}');
                ...
                <div className="mb-6 bg-slate-800 text-white p-3 text-center text-xs font-bold border-2 border-slate-400">
                  BASES LEGAIS - SEGURANÇA E SAÚDE OCUPACIONAL<br />
                  PORTARIA 3214 DE 8 DE JUNHO DE 1978 – NR - 1
                </div>

                {/* Termo de Responsabilidade */}
                <div className="mb-6 border-2 border-slate-400">
                  <div className="bg-slate-200 p-2 font-bold text-xs border-b-2 border-slate-400 text-center">Termo de Responsabilidade</div>
                ...
                <div className="border-2 border-slate-400 mb-6">
                  <div className="grid grid-cols-4 gap-0">
                    <div className="border-r border-slate-400 p-2 text-xs font-bold text-center bg-slate-100">Data</div>
                    <div className="border-r border-slate-400 p-2 text-xs font-bold text-center bg-slate-100">Nome</div>
                    <div className="border-r border-slate-400 p-2 text-xs font-bold text-center bg-slate-100">CPF</div>
                    <div className="p-2 text-xs font-bold text-center bg-slate-100">Assinatura</div>
                  </div>
        ...
        <VisualizarAutorizacaoFormalModal
          ...
          funcionario={{ funcao_nome: funcao?.nome, funcao_id: funcao?.id, nome_completo: '', cpf: '' }}
          funcao={funcao}
        ...
        <OrdenServicoCEMIGModal
          ...
          funcao={funcao}
          empresaAtiva={empresaAtiva}
          onTreinamentoCriado={() => { if (funcao?.id) loadTreinamentos(funcao.id); }}
        />
              ...
              <div className="p-8 space-y-4">
                ...
                <div className="flex items-start gap-6 pb-4 border-b-2 border-slate-800">
                  ...
                  <div className="flex-1 text-center">
                    <h2 className="text-sm font-bold">FICHA DE CONTROLE E ENTREGA DE EQUIPAMENTO DE PROTEÇÃO INDIVIDUAL (EPI) E UNIFORME</h2>
                  </div>
                </div>
                ...
                <div className="text-xs mb-4 pb-4 border-b-2 border-slate-800">
                  <div className="flex gap-4 mb-3">
                    ...
                    <span className="flex-1">Assinatura do Funcionário:X_____________________________________________</span>
                  </div>
                  <p className="text-justify leading-relaxed">
                    Recebi da Empresa ELETRO E ENERGIA LTDA, CNPJ Nº 30.694.170/0001-84, para meu uso obrigatório os EPI´s (Equipamentos de proteção Individual) constantes nesta ficha, o qual obrigou-me a utilizá-los corretamente durante o tempo que permanecerem ao meu dispor, observando as medidas gerais de disciplina e uso que integram a NR-06 – Equipamento de Proteção Individual – EPI´s – da portaria nº 3.214 de 06/jun/1970. Declaro saber também que terei que devolvê-los no ato de meu desligamento da empresa.
                  </p>
                </div>

                {/* Tabela de EPIs */}
                <div className="border-2 border-slate-800">
                  <div className="grid gap-0 border-b-2 border-slate-800" style={{ gridTemplateColumns: '1fr 1fr 0.6fr 2fr 0.8fr 1.5fr 1fr' }}>
                    <div className="p-2 text-xs font-bold border-r border-slate-800">RETIRADA</div>
                    <div className="p-2 text-xs font-bold border-r border-slate-800">DEVOLUÇÃO</div>
                    <div className="p-2 text-xs font-bold border-r border-slate-800 text-center">QUANT.</div>
                    <div className="p-2 text-xs font-bold border-r border-slate-800">DESCRIÇÃO DO EQUIPAMENTO</div>
                    <div className="p-2 text-xs font-bold border-r border-slate-800 text-center">Nº DO C.A.</div>
                    <div className="p-2 text-xs font-bold border-r border-slate-800 text-center">ASSINATURA DO FUNCIONÁRIO</div>
                  ...
                  <div className="p-4 border-b-2 border-slate-800 bg-slate-50">
                    <Label className="text-xs font-bold">Para onde estão indo? (Destino dos EPIs)</Label>
                    <Input
                      value={destinoEpis}
                  ...
                  episEstoque.
                  map((epi, idx) =>
                  <div key={idx} className="grid gap-0 border-b border-slate-300" style={{ gridTemplateColumns: '1fr 1fr 0.6fr 2fr 0.8fr 1.5fr 1fr' }}>
                        ...
                        <div className="p-2 text-xs border-r border-slate-300 uppercase font-semibold">{epi.descricao || ''}</div>
                        <div className="p-2 text-xs border-r border-slate-300 text-center">{epi.ca || '-'}</div>
                        <div className="p-2 text-xs border-r border-slate-300 text-center">X</div>
                        <div className="p-2 text-xs">{destinoEpis ? `→ ${destinoEpis}` : '-'}</div>
                      </div>
                  )
                  }
                </div>
                ...
                <div className="mt-6 grid grid-cols-2 gap-12 text-xs">
                  <div>
                    ...
                    <div className="text-center font-semibold mt-2">Assinatura do Funcionário</div>
                  </div>
                  <div>
                    <div className="pb-6 border-b border-slate-800"></div>
