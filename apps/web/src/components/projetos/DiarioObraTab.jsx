...
import { Calendar, Plus, Camera, Users, Cloud, Sun, CloudRain, Wind, Trash2, CheckSquare, LayoutGrid, CalendarDays, GanttChartSquare, Settings, FileText, Download, Eye, Edit, Printer, MoreVertical } from 'lucide-react';
import { toast } from 'sonner';
import DiarioCameraModal from './DiarioCameraModal';
...
export default function DiarioObraTab({ projetoId, empresaAtiva, usuariosEmpresa, showOnlyTasks, showOnlyDiary, projeto }) {
  ...
  const loadItensPlanejamento = React.useCallback(async () => {
    try {
      ...
      const todos = tarefas
        .map(t => ({ id: t.id, nome: t.titulo, tipo: t.status || 'Tarefa', percentual: t.progresso || 0 }))
        .filter(i => i.nome)
  ...
  }, [projetoId, showOnlyTasks, loadDiarios, loadFuncoes, loadItensConfiguracao, loadEtapas, loadItensPlanejamento]);
  ...
  const handleSalvar = async () => {
    const atividadesTexto = atividadesSelecionadas.length > 0
      ? buildAtividadesTexto()
      : novoDiario.atividades;
  ...
  const getClimaIcon = (clima) => {
    switch (clima) {
      case 'Sol': return <Sun className="w-5 h-5 text-yellow-500" />;
      case 'Nublado': return <Cloud className="w-5 h-5 text-slate-400" />;
      case 'Chuva': return <CloudRain className="w-5 h-5 text-blue-500" />;
      case 'Vento': return <Wind className="w-5 h-5 text-cyan-500" />;
  ...
  const handleEditarDiario = (diario) => {
    let maoObra = [];
    try { maoObra = diario.mao_de_obra ? JSON.parse(diario.mao_de_obra) : []; } catch (e) {}
    let fotos = [];
    try { fotos = diario.fotos ? JSON.parse(diario.fotos) : []; } catch (e) {}
    setEditForm({
      data: diario.data,
          ...
          <SheetContent>
            ...
            <div className="space-y-6 py-6">
              ...
              <div className="flex justify-between gap-3 pt-4 border-t">
                ...
                <Button onClick={() => setShowConfigColunas(false)}>
                  Confirmar
                </Button>
              </div>
            </div>
  ...
  if (showOnlyDiary) {
    return (
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" onClick={() => setShowRelatorioDiario(true)} className="gap-2">
              <FileText className="w-4 h-4" />
              Relatórios
            </Button>
        ...
        {/* Modal de Edição */}
        <Sheet open={!!diarioEditar} onOpenChange={(open) => { if (!open) { setDiarioEditar(null); setEditForm(null); } }}>
              ...
              <div className="space-y-4 pb-8">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input type="text" value={editForm.obra_nome} onChange={e => setEditForm(f => ({ ...f, obra_nome: e.target.value }))} className="mt-1" />
                    </div>
                    <div>
                      <Label className="text-sm">Contratante</Label>
                      <Input type="text" value={editForm.contratante_nome} onChange={e => setEditForm(f => ({ ...f, contratante_nome: e.target.value }))} className="mt-1" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label className="text-sm">Local da Obra</Label>
                      <Input type="text" value={editForm.obra_local} onChange={e => setEditForm(f => ({ ...f, obra_local: e.target.value }))} className="mt-1" />
                    </div>
                    <div>
                      <Label className="text-sm">Responsável</Label>
                      <Input type="text" value={editForm.responsavel} onChange={e => setEditForm(f => ({ ...f, responsavel: e.target.value }))} className="mt-1" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label className="text-sm">Nº do Contrato</Label>
                      <Input type="text" value={editForm.numero_contrato} onChange={e => setEditForm(f => ({ ...f, numero_contrato: e.target.value }))} className="mt-1" />
                    </div>
                    <div>
                      <Label className="text-sm">Prazo Decorrido</Label>
                      <Input type="text" value={editForm.prazo_decorrido} onChange={e => setEditForm(f => ({ ...f, prazo_decorrido: e.target.value }))} className="mt-1" />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <Label>Início</Label>
                      <Input type="time" value={editForm.horario_inicio} onChange={e => setEditForm(f => ({ ...f, horario_inicio: e.target.value }))} className="mt-1" />
                    </div>
                    <div>
                      <Label>Término</Label>
                      <Input type="time" value={editForm.horario_fim} onChange={e => setEditForm(f => ({ ...f, horario_fim: e.target.value }))} className="mt-1" />
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label>Clima</Label>
                    <Select value={editForm.clima} onValueChange={v => setEditForm(f => ({ ...f, clima: v }))}>
                  ...
                  <div>
                    ...
                    <Input type="number" value={editForm.temperatura} onChange={e => setEditForm(f => ({ ...f, temperatura: e.target.value }))} className="mt-1" />
                  </div>
                </div>
                <div>
                  <Label>Atividades Realizadas *</Label>
                  <Textarea value={editForm.atividades} onChange={e => setEditForm(f => ({ ...f, atividades: e.target.value }))} rows={4} className="mt-1" />
                </div>
                <div>
                  <Label>Observações</Label>
                  <Textarea value={editForm.observacoes} onChange={e => setEditForm(f => ({ ...f, observacoes: e.target.value }))} rows={2} className="mt-1" />
                </div>
                <div>
                  <Label>Problemas / Ocorrências</Label>
                  <Textarea value={editForm.problemas} onChange={e => setEditForm(f => ({ ...f, problemas: e.target.value }))} rows={2} className="mt-1" />
                </div>
                <div>
                  ...
                  <div className="space-y-2 mt-2">
                    {editForm.mao_de_obra.map((m, i) => (
                      <div key={i} className="flex items-center gap-2 p-2 bg-white rounded border">
                        ...
                        <Input type="number" value={m.quantidade} min="1" className="w-20 h-8 text-xs"
                          onChange={e => {
                        ...
                        <Button variant="ghost" size="icon" className="h-8 w-8"
                          onClick={() => setEditForm(f => ({ ...f, mao_de_obra: f.mao_de_obra.filter((_, idx) => idx !== i) }))}>
                    ...
                    <Select onValueChange={id => {
                      ...
                      setEditForm(f => ({ ...f, mao_de_obra: [...f.mao_de_obra, { id: fn.id, nome: fn.nome, quantidade: 1 }] }));
                    }}>
                      <SelectTrigger><SelectValue placeholder="+ Adicionar mão de obra" /></SelectTrigger>
                      <SelectContent>
                        {funcoes.filter(f => !editForm.mao_de_obra.find(m => m.id === f.id)).map(f => (
                    ...
                    <div className="grid grid-cols-4 gap-2 mt-2">
                      {editForm.fotos.map((foto, i) => (
                        <div key={i} className="relative group">
                          ...
                          <button onClick={() => setEditForm(f => ({ ...f, fotos: f.fotos.filter((_, idx) => idx !== i) }))}
                ...
                <div className="flex gap-2 pt-2">
                  <Button onClick={handleSalvarEdicao} disabled={salvandoEdicao || !editForm.atividades} className="flex-1">
                    {salvandoEdicao ? 'Salvando...' : 'Salvar Alterações'}
                  </Button>
                  <Button variant="outline" onClick={() => { setDiarioEditar(null); setEditForm(null); }}>Cancelar</Button>
                </div>
              </div>
            )}
          </SheetContent>
      ...
      {showAdd && (
        <Card className="border-blue-200 bg-blue-50" style={{ left: '256px !important', right: '0 !important', width: 'calc(100% - 256px) !important', maxWidth: 'none !important' }}>
          <CardContent className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNovoDiario({ ...novoDiario, contratante_nome: e.target.value })}
                    disabled
                    className="mt-1 bg-white"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNovoDiario({ ...novoDiario, responsavel: e.target.value })}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNovoDiario({ ...novoDiario, numero_contrato: e.target.value })}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNovoDiario({ ...novoDiario, prazo_decorrido: e.target.value })}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNovoDiario({ ...novoDiario, horario_inicio: e.target.value })}
                    className="mt-1"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNovoDiario({ ...novoDiario, horario_fim: e.target.value })}
                    className="mt-1"
                  />
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Clima</Label>
                <Select value={novoDiario.clima} onValueChange={(v) => setNovoDiario({ ...novoDiario, clima: v })}>
            ...
            <div>
              ...
              {itensPlanejamento.length > 0 ? (
                <div className="mt-2 space-y-2">
                  <p className="text-xs text-slate-500 mb-2">Selecione os itens do planejamento e informe o % executado hoje:</p>
                  <div className="max-h-48 overflow-y-auto space-y-1 border rounded-lg p-2 bg-white">
                    {itensPlanejamento.map(item => {
                      ...
                      return (
                        <div key={item.id} className="flex items-center gap-2 p-1.5 rounded hover:bg-slate-50">
                          <input
                            type="checkbox"
                            checked={!!selecionado}
                          ...
                          <span className="text-xs text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">{item.tipo}</span>
                          {selecionado && (
                            <div className="flex items-center gap-1">
                              <Input
                                ...
                                onChange={(e) => handlePercentualAtividade(item.id, e.target.value)}
                  ...
                  <p className="text-xs text-slate-400">Ou adicione uma atividade manual abaixo:</p>
                  <Textarea
            ...
            <div className="flex gap-2 pt-2">
              <Button onClick={handleSalvar} disabled={atividadesSelecionadas.length === 0 && !novoDiario.atividades} className="flex-1">
                Salvar Registro
              </Button>
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => handleVisualizarDiario(diario)} className="gap-2">
                        <Eye className="w-4 h-4" />
                        Visualizar
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleEditarDiario(diario)} className="gap-2">
                        <Edit className="w-4 h-4" />
                        Editar
                      </DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem onClick={() => handleDelete(diario.id)} className="gap-2 text-red-600">
