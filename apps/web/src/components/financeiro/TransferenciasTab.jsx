...
export default function TransferenciasTab({ empresaAtiva, contas, onReload }) {
  ...
  const loadTransferencias = React.useCallback(async () => {
    ...
    const trans = await base44.entities.TransacaoTransferencia.filter({ empresa_id: empresaAtiva.id });
          ...
          <tbody>
            {transferencias.map(t => (
              <tr key={t.id} className="border-b hover:bg-slate-50">
                ...
                <td className="px-4 py-3 text-sm font-semibold text-blue-600">{formatCurrency(t.valor)}</td>
      ...
      <Sheet open={showModal} onOpenChange={setShowModal}>
        <SheetContent style={{ inset: 'auto 0 0 256px', width: 'calc(100% - 256px)', maxWidth: 'none' }}>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button onClick={handleSave} disabled={!form.valor || !form.conta_origem_id || !form.conta_destino_id}>
              Transferir
            </Button>
          </div>
        </SheetContent>
