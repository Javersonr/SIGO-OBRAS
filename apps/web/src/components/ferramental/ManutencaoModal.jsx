...
export default function ManutencaoModal({ open, onOpenChange, manutencao, ferramentas, onSave }) {
  ...
  const getSugestoes = (busca) => ferramentas.filter(f =>
    !busca ||
  ...
  useEffect(() => {
    if (manutencao) {
      const pecasArray = manutencao.pecas_substituidas
        ...
        : [];
      setPecas(pecasArray);
      setDados({ ...dados, ...manutencao, pecas_substituidas: manutencao.pecas_substituidas || '[]' });
      ...
      try {
        ...
        if (lista.length > 0) {
          ...
          if (ferr) setFerramentasList([{ id: ferr.id, codigo: ferr.codigo, descricao: ferr.descricao, numero_serie: ferr.numero_serie }]);
        }
      } catch { }
    } else {
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      const clickedInAnyPortal = Object.values(portalRefs.current).some(r => r?.contains(e.target));
  ...
  const adicionarFerramentaLinha = () => {
    setFerramentasList(prev => [...prev, { id: '', codigo: '', descricao: '', numero_serie: '' }]);
  };
  ...
  const selecionarFerramentaLinha = (idx, ferr) => {
    ...
    setFerramentasList(prev => prev.map((item, i) => i === idx
      ? { id: ferr.id, codigo: ferr.codigo, descricao: ferr.descricao, numero_serie: ferr.numero_serie, localizacao }
      : item
    ));
    setBuscasPorLinha(prev => { const n = { ...prev }; delete n[idx]; return n; });
    setShowSugestoesPorLinha(prev => ({ ...prev, [idx]: false }));
  };

  const loadFornecedores = async () => {
    try {
      const forns = await base44.entities.Fornecedor.filter({ 
        empresa_id: empresaAtiva.id,
        ativo: true 
      });
  ...
  const loadUsuarios = async () => {
    try {
      const users = await base44.entities.UsuarioEmpresa.filter({ 
        empresa_id: empresaAtiva.id,
        ativo: true 
      });
      setUsuarios(users);
    ...
    <SheetModalComponent
      ...
      footer={
        <div className="flex gap-3">
          ...
          <Button onClick={handleSalvar} disabled={loading} className="flex-1 bg-amber-500 hover:bg-amber-600">
        ...
        <div className="space-y-4">
            ...
            <div className="border rounded-lg overflow-hidden mt-1">
              <table className="w-full" style={{ tableLayout: 'auto' }}>
                <thead className="bg-slate-100 border-b">
                  <tr>
                    <th className="text-center px-3 py-2 text-xs font-semibold text-slate-600 w-8">#</th>
                    <th className="text-left px-3 py-2 text-xs font-semibold text-slate-600">Código / Descrição</th>
                    <th className="text-left px-3 py-2 text-xs font-semibold text-slate-600 w-36">Nº Série</th>
                    <th className="text-left px-3 py-2 text-xs font-semibold text-slate-600 w-40">Localização</th>
                ...
                <tbody>
                  {ferramentasList.map((linha, idx) => (
                    <tr key={idx} className="border-b hover:bg-slate-50">
                      <td className="px-3 py-1.5 text-center text-xs text-slate-400 font-medium">{idx + 1}</td>
                      <td className="px-2 py-1.5 relative" ref={el => inputRefs.current[idx] = el}>
                        <div className="flex items-center gap-1">
                          <Search className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
                          <Input
                            placeholder="Buscar ferramenta..."
                            value={buscasPorLinha[idx] !== undefined ? buscasPorLinha[idx] : (linha.id ? `${linha.codigo} - ${linha.descricao}` : '')}
                            onChange={(e) => {
                              setBuscasPorLinha(prev => ({ ...prev, [idx]: e.target.value }));
                              setShowSugestoesPorLinha(prev => ({ ...prev, [idx]: true }));
                              if (!e.target.value) setFerramentasList(prev => prev.map((item, i) => i === idx ? { id: '', codigo: '', descricao: '', numero_serie: '' } : item));
                            }}
                            onFocus={() => {
                            ...
                            onBlur={() => {
                              setTimeout(() => {
                                if (buscasPorLinha[idx] !== undefined && !ferramentasList[idx]?.id) return;
                                setBuscasPorLinha(prev => { const n = { ...prev }; delete n[idx]; return n; });
                              }, 200);
                            }}
                        ...
                        {showSugestoesPorLinha[idx] && (() => {
                          ...
                          return ReactDOM.createPortal(
                            <div
                              ...
                              style={{ position: 'fixed', top: rect.bottom + 2, left: rect.left, width: Math.max(rect.width, 320), zIndex: 99999, background: 'white', border: '1px solid #e2e8f0', borderRadius: 8, boxShadow: '0 8px 24px rgba(0,0,0,0.15)', maxHeight: 220, overflowY: 'auto' }}
                            >
                              {sugs.map(f => (
                                <button key={f.id} type="button"
                                  style={{ display: 'flex', width: '100%', textAlign: 'left', padding: '8px 12px', gap: 8, alignItems: 'center', background: 'none', border: 'none', cursor: 'pointer' }}
                                  onMouseDown={e => { e.preventDefault(); selecionarFerramentaLinha(idx, f); }}
                                  onMouseEnter={e => e.currentTarget.style.background = '#f1f5f9'}
                                  onMouseLeave={e => e.currentTarget.style.background = 'none'}
                                >
                                  <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                                      <span style={{ fontFamily: 'monospace', fontSize: 11, background: '#f1f5f9', padding: '2px 6px', borderRadius: 4, color: '#475569' }}>{f.codigo}</span>
                                      {f.numero_serie && <span style={{ fontFamily: 'monospace', fontSize: 11, background: '#eff6ff', padding: '2px 6px', borderRadius: 4, color: '#2563eb', border: '1px solid #bfdbfe' }}>S/N: {f.numero_serie}</span>}
                                      <span style={{ color: '#1e293b', fontSize: 13 }}>{f.descricao}</span>
                                    </div>
                                    <div style={{ display: 'flex', gap: 8, marginTop: 2 }}>
                                      {f.status && <span style={{ fontSize: 10, color: f.status === 'Disponível' ? '#16a34a' : '#d97706', fontWeight: 600 }}>{f.status}</span>}
                                      {f.funcionario_nome && <span style={{ fontSize: 10, color: '#64748b' }}>👤 {f.funcionario_nome}</span>}
                                    </div>
                                  </div>
                                </button>
                              ))}
                      ...
                      <td className="px-3 py-1.5 text-xs text-slate-500 font-mono">{linha.numero_serie || '-'}</td>
                      <td className="px-3 py-1.5 text-xs text-slate-500">{linha.localizacao || '-'}</td>
                      <td className="px-2 py-1.5">
                        <Button type="button" variant="ghost" size="icon" className="h-6 w-6" onClick={() => removerFerramentaLinha(idx)}>
                ...
                <tfoot className="bg-slate-50 border-t">
                  <tr>
                    <td colSpan={5} className="px-3 py-2">
                      <Button type="button" variant="ghost" size="sm" className="gap-1 text-xs text-blue-600 hover:text-blue-800" onClick={adicionarFerramentaLinha}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select value={dados.tipo_manutencao} onValueChange={(v) => setDados({ ...dados, tipo_manutencao: v })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {['Preventiva','Corretiva','Preditiva','Inspeção'].map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div>
              ...
              <Select value={dados.status} onValueChange={(v) => setDados({ ...dados, status: v })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {['Agendada','Em Andamento','Concluída','Cancelada'].map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setDados({ ...dados, horas_uso_no_momento: parseFloat(e.target.value) || 0 })}
              />
            </div>
              ...
              <table className="w-full" style={{ tableLayout: 'auto' }}>
                <thead className="bg-slate-100 border-b">
                  <tr>
                    ...
                    <th className="text-right px-3 py-2 text-xs font-semibold text-slate-600 w-24">Qtd</th>
                    <th className="text-right px-3 py-2 text-xs font-semibold text-slate-600 w-32">Custo Unit. (R$)</th>
                    <th className="text-right px-3 py-2 text-xs font-semibold text-slate-600 w-32">Total</th>
                    <th className="w-10"></th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={index} className="border-b hover:bg-slate-50">
                      ...
                      <td className="px-3 py-1.5 text-right">
                        <span className="text-xs font-medium text-green-700">
                          R$ {((parseFloat(peca.quantidade) || 0) * (parseFloat(peca.custo_unitario) || 0)).toFixed(2)}
                        </span>
                      </td>
                      <td className="px-2 py-1.5">
                        <Button type="button" variant="ghost" size="icon" className="h-6 w-6" onClick={() => removerPeca(index)}>
                          <XCircle className="w-3.5 h-3.5 text-red-500" />
                        </Button>
                      </td>
                    </tr>
                ...
                <tfoot className="bg-slate-50 border-t">
                  <tr>
                    <td colSpan={2} className="px-3 py-2">
                      <Button type="button" variant="ghost" size="sm" className="gap-1 text-xs text-blue-600 hover:text-blue-800" onClick={adicionarPeca}>
                        <Plus className="w-3.5 h-3.5" />Adicionar Peça
                      </Button>
                    </td>
                    <td className="px-3 py-2 text-right text-xs font-semibold text-slate-600">Total Peças:</td>
                    <td className="px-3 py-2 text-right font-bold text-green-700 text-xs">
                      R$ {pecas.reduce((t, p) => t + (parseFloat(p.quantidade) || 0) * (parseFloat(p.custo_unitario) || 0), 0).toFixed(2)}
                    </td>
                    <td></td>
                  </tr>
                </tfoot>
