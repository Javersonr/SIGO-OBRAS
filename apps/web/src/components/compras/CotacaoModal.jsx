  ...
  const carregarCotacaoExistente = React.useCallback(async () => {
    ...
    try {
      ...
      if (cotacoes.length > 0) {
        ...
        const creds = {};
        await Promise.all(fornecedoresVinculados.map(async (fv) => {
          try {
  ...
  React.useEffect(() => {
    if (open && solicitacao) {
      ...
      if (solicitacao.projeto_id) {
        base44.entities.OrcamentoItem.filter({ projeto_id: solicitacao.projeto_id }).then(orcItens => {
          const mapa = {};
          orcItens.forEach(o => { if (o.descricao) mapa[o.descricao.toLowerCase()] = o.codigo || ''; });
  ...
  const handleCriarCotacao = async () => {
    ...
    try {
      ...
      if (cotacaoExistente) {
        ...
        const novosFornecedores = fornecedoresSelecionados.filter(fId => !idsExistentes.includes(fId));
        
        // Otimizado: Processar fornecedores em paralelo
         await Promise.all(novosFornecedores.map(async (fId) => {
        ...
        // Otimizado: Processar fornecedores em paralelo
         await Promise.all(fornecedoresSelecionados.map(async (fId) => {
           const fornecedor = fornecedores.find(f => f.id === fId);
      ...
      alert(cotacaoExistente ? 'Atualizado!' : 'Cotação criada! Links gerados para compartilhar por WhatsApp.');
      onSave();
    } catch (error) {
      ...
      <SheetContent side="right" className="h-full p-0 flex flex-col w-full md:w-[calc(100%-256px)] md:inset-auto md:right-0 md:left-256px md:top-16" data-fullscreen-modal>
        <div className="sticky top-0 bg-white border-b p-6 z-10 flex-shrink-0 flex items-center justify-between">
          <SheetHeader className="flex-1">
            <SheetTitle>{cotacaoExistente ? 'Editar' : 'Criar'} Cotação - {solicitacao?.numero}</SheetTitle>
          </SheetHeader>
          <button 
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <div className="space-y-2">
                {itens.map((item, idx) => {
                  const codigo = item.material_codigo || codigosOrcamento[item.descricao?.toLowerCase()] || '';
                  return (
                    <div key={item.id} className="flex items-center gap-3 p-2 bg-slate-50 rounded">
                      ...
                      {codigo && (
                        <span className="font-mono text-xs text-slate-500 w-20 flex-shrink-0">{codigo}</span>
                      )}
                      ...
                      <Badge variant="outline">{item.quantidade} {item.unidade}</Badge>
                    </div>
                  );
                })}
              </div>
            ...
            {fornecedoresSelecionados.length > 0 && (
              <div className="mb-3 space-y-3">
                {fornecedoresSelecionados.map(fornecedorId => {
                  ...
                  return (
                    <div key={fornecedor.id} className="p-3 bg-amber-50 border border-amber-200 rounded-lg space-y-2">
                      <div className="flex items-start gap-2">
                        <div className="flex-1 space-y-2">
                          <p className="font-medium text-sm text-slate-800">{fornecedor.nome_razao}</p>
                          <div className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => setEmailsFornecedores({ 
                                ...emailsFornecedores, 
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-2 p-2 bg-white rounded border border-slate-200">
                            ...
                            {fornecedor.telefone && (
                              <Button
                                ...
                                onClick={() => {
                                  const telefone = fornecedor.telefone?.replace(/\D/g, '');
                                  const creds = credenciaisFornecedores[fornecedor.id];
                                  const mensagem = buildMensagemWhatsApp(fornecedor, linkData.link, creds);
                                  window.open(`https://wa.me/${telefone}?text=${encodeURIComponent(mensagem)}`, '_blank');
                                }}
                            ...
                            <Button
                              ...
                              size="sm"
                              variant="ghost"
                              onClick={() => setMensagemExpandida(prev => ({ ...prev, [fornecedor.id]: !prev[fornecedor.id] }))}
                          ...
                          {mensagemExpandida[fornecedor.id] && (
                            <div className="space-y-1">
                              <Textarea
                                readOnly
                                value={buildMensagemWhatsApp(fornecedor, linkData.link, credenciaisFornecedores[fornecedor.id])}
          ...
          {Object.keys(linksCotacao).length > 0 && (
            <Card className="border-green-200 bg-green-50">
              <CardContent className="p-4 space-y-2">
                <h3 className="font-semibold text-green-800 mb-3">✅ Links Gerados - Compartilhe com os Fornecedores</h3>
                {Object.entries(linksCotacao).map(([fornecedorId, link]) => {
                  const fornecedor = fornecedores.find(f => f.id === fornecedorId);
                  return (
                    <div key={fornecedorId} className="p-3 bg-white rounded border border-green-200 space-y-2">
                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => setMensagemExpandida(prev => ({ ...prev, [`gen_${fornecedorId}`]: !prev[`gen_${fornecedorId}`] }))}
                            className="text-slate-500 text-xs"
                          >
                            {mensagemExpandida[`gen_${fornecedorId}`] ? '▲ Ocultar' : '▼ Mensagem'}
                          </Button>
                        </div>
                      </div>
                      {mensagemExpandida[`gen_${fornecedorId}`] && (
                        <div className="space-y-1">
                          <Textarea
                            readOnly
                            value={buildMensagemWhatsApp(fornecedor, typeof link === 'string' ? link : link.link, credenciaisFornecedores[fornecedorId])}
                            className="text-xs bg-slate-50 resize-none"
                            rows={8}
                          />
                          <Button
                            ...
                            variant="outline"
                            onClick={() => {
                              const mensagem = buildMensagemWhatsApp(fornecedor, typeof link === 'string' ? link : link.link, credenciaisFornecedores[fornecedorId]);
                              navigator.clipboard.writeText(mensagem);
                              alert('Mensagem copiada!');
                            }}
                            className="w-full text-slate-600"
                ...
                <p className="text-xs text-slate-600 mt-2 italic">
                  💡 Use o botão WhatsApp para enviar o link aos fornecedores ou copie o link para compartilhar.
                </p>
              </CardContent>
            </Card>
          )}
        ...
        <div className="sticky bottom-0 bg-white border-t p-6 flex-shrink-0 flex justify-end gap-3">
          ...
          >
            {enviandoEmail ? 'Gerando links...' : cotacaoExistente ? 'Atualizar Cotação' : 'Criar Cotação'}
          </Button>
        </div>
      </SheetContent>
    </Sheet>
