...
export default function WidgetResumoFinanceiro() {
  ...
  const loadResumo = async () => {
    try {
      ...
      const receitasMes = receitasList.filter(
        (t) => t.data_vencimento >= primeiroDiaMes && t.data_vencimento <= ultimoDiaMes
      );
      const despesasMes = despesasList.filter(
        (t) => t.data_vencimento >= primeiroDiaMes && t.data_vencimento <= ultimoDiaMes
      );
    ...
    <Card className="hover:shadow-lg transition-shadow">
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="font-semibold text-green-600">R$ {resumo.receitas.toFixed(2)}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm text-slate-600">Despesas</span>
          <span className="font-semibold text-red-600">R$ {resumo.despesas.toFixed(2)}</span>
        </div>
