...
import { 
  Plus, Eye,
  Calendar, X,
  Download, Settings, FileText,
  ChevronDown, FileSpreadsheet, Upload, User as UserIcon, Trash2
} from 'lucide-react';
...
import ClienteViewModal from '../components/oportunidades/ClienteViewModal';
...
export default function Projetos() {
  ...
  const [importacaoProjetos, setImportacaoProjetos] = useState({ ativo: false, total: 0, processados: 0, erros: 0 });
  ...
  const loadOrcamentoData = async (projId) => {
    ...
    const [itens, etapas, arqs, cols] = await Promise.all([
      ...
      base44.entities.ArquivoOportunidade.filter({ empresa_id: empresaAtiva.id, projeto_id: projId }),
    ...
    setTimeout(async () => {
      try {
        ...
        console.log(`Materiais carregados: ${mats.length} Material, ${maoObra.length} Mão de Obra, ${ferram.length} Ferramental`);
      } catch (error) {
  ...
  const handleDeleteSelecionados = async () => {
    ...
    await Promise.all([...itensSelecionados].map(id => base44.entities.OrcamentoItem.delete(id).catch(() => {})));
  ...
  const handleChangeStatus = async (proj, newStatusId) => {
    ...
    setProjetos(prev => prev.map(p => p.id === proj.id ? { ...p, status_id: newStatusId, status_nome: statusNovo?.nome } : p));
    
    await base44.entities.Projeto.update(proj.id, { status_id: newStatusId, status_nome: statusNovo?.nome });
    await base44.entities.OportunidadeAtualizacao.create({
      empresa_id: empresaAtiva.id,
      projeto_id: proj.id,
      usuario_id: user?.id,
  ...
  const handleExport = () => {
    const csv = [
      ['Nome', 'Cliente', 'Status', 'Valor', 'Probabilidade', 'Data Fechamento', 'Responsável'].join(','),
      ...filteredProjs.map(p => [
        p.nome,
        p.cliente_nome || '',
        p.status_nome,
  ...
  const handleBaixarModeloProjetos = async () => {
    if (!XLSX) return;
    const modelo = [['ID', 'Nome', 'Cliente (Razão)', 'Status', 'Origem', 'Cidade', 'Estado', 'CEP', 'Endereço', 'Valor contrato', 'Modalidade (opcional)', 'Data licitação (opcional)', 'Horário licitação (opcional)'], ['', 'Projeto Exemplo', 'CLIENTE EXEMPLO', 'Iniciado', 'Prospecto', 'São Paulo', 'SP', '01310-100', 'Avenida Paulista', '50000.00', 'Pregão', '01/01/2025', '10:00']];
    const ws = XLSX.utils.aoa_to_sheet(modelo);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Projetos');
    ws['!cols'] = [{wch:10},{wch:30},{wch:25},{wch:15},{wch:15},{wch:20},{wch:10},{wch:12},{wch:30},{wch:15},{wch:18},{wch:18},{wch:18}];
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (let lote = 0; lote < data.length; lote += 10) {
          const resultados = await Promise.all(data.slice(lote, lote + 10).map(async (row, idx) => {
            try {
              if (!row.Nome?.trim()) { setImportacaoProjetos(p => ({...p, processados: lote+idx+1})); return false; }
              let status = statusList.find(s => s.nome?.toLowerCase() === row.Status?.toLowerCase()) || statusList[0];
              let cliente = clientes.find(c => c.nome_razao?.toLowerCase() === row['Cliente (Razão)']?.toLowerCase());
              if (!cliente && row['Cliente (Razão)']?.trim()) { cliente = await base44.entities.Cliente.create({ empresa_id: empresaAtiva.id, nome_razao: row['Cliente (Razão)'], tipo_pessoa: 'PJ', ativo: true }); }
              let origem = origensList.find(o => o.nome?.toLowerCase() === row.Origem?.toLowerCase());
              if (!origem && row.Origem?.trim()) { origem = await base44.entities.OrigemOportunidade.create({ empresa_id: empresaAtiva.id, nome: row.Origem }); }
              const existente = await base44.entities.Projeto.filter({ empresa_id: empresaAtiva.id, nome: row.Nome });
              if (existente.length > 0) { setImportacaoProjetos(p => ({...p, processados: lote+idx+1, erros: p.erros+1})); return false; }
              const valorEstimado = parseFloat(String(row['Valor contrato']||'0').replace(/[^\d,.-]/g,'').replace(',','.')) || 0;
              await base44.entities.Projeto.create({ empresa_id: empresaAtiva.id, nome: row.Nome, cliente_id: cliente?.id||null, cliente_nome: cliente?.nome_razao||null, status_id: status?.id, status_nome: status?.nome, origem_id: origem?.id||null, origem_nome: origem?.nome||null, valor_estimado: valorEstimado, cidade: row.Cidade||null, estado: row.Estado||null, cep: row.CEP||null, endereco: row.Endereço||null, licitacao_modalidade: row['Modalidade (opcional)']||null, licitacao_data: row['Data licitação (opcional)']||null, licitacao_horario: row['Horário licitação (opcional)']||null });
              setImportacaoProjetos(p => ({...p, processados: lote+idx+1}));
              return true;
            } catch (err) { setImportacaoProjetos(p => ({...p, processados: lote+idx+1, erros: p.erros+1})); return false; }
          }));
          importados += resultados.filter(Boolean).length;
          erros += resultados.filter(r=>!r).length;
        }
        setTimeout(() => { setImportacaoProjetos({ ativo: false, total: 0, processados: 0, erros: 0 }); alert(`Importação concluída!\n✅ ${importados} importados\n❌ ${erros} erros`); loadData(); }, 500);
      } catch (error) { setImportacaoProjetos({ ativo: false, total: 0, processados: 0, erros: 0 }); alert(`Erro: ${error.message}`); }
  ...
  const filteredProjs = React.useMemo(() => {
   ...
   if (filterStatus !== 'all') {
    ...
    filtered.sort((a, b) => {
      ...
      if (sortConfig.field === 'created_date' || sortConfig.field === 'data_fechamento_prevista' || sortConfig.field === 'licitacao_data') {
    ...
    return filtered;
  }, [searchResults, filterStatus, sortConfig, filtroMeus, perfil, temPermissoesGranulares, user, usuariosEmpresa]);
    ...
    <div className="space-y-6">
        ...
        <div className="flex gap-2">
           <DropdownMenu>
             ...
             <DropdownMenuContent align="end" className="w-56">
               ...
               <DropdownMenuItem onClick={() => fileInputProjetosRef.current?.click()} className="gap-2">
          ...
          {(perfil === 'Admin' || temPermissao('Projetos', 'Lista', 'criar')) && (
            <Button onClick={() => handleOpenModal()} className="bg-amber-500 hover:bg-amber-600" size="icon">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <Button
          ...
          className="gap-2"
          title={perfil === 'Admin' ? 'Admins veem tudo' : temPermissoesGranulares ? 'Você só pode ver seus projetos' : ''}
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        {projsStatus.length} projetos {podeVerValores && `• ${formatCurrency(totalValor)}`}
                      </p>
                    </div>
                  </div>
                                ...
                                <Card 
                                 ...
                                 >
                                   <CardContent className="p-4">
                                     <h4 className="font-medium text-slate-800 mb-2 line-clamp-2">{proj.nome}</h4>
                                    ...
                                    {proj.licitacao_data && (
                                      <p className="text-xs text-slate-500 flex items-center gap-1 mb-2">
                                        ...
                                        {proj.licitacao_horario && ` às ${proj.licitacao_horario}`}
                                      </p>
                                    )}
                                    
                                    <div className="mb-3">
                                      {(() => {
                                        const emails = (() => { try { return JSON.parse(proj.responsaveis_emails || '[]'); } catch { return []; } })();
                                        return (
                                          <ResponsaveisSelect 
                                            responsaveisEmails={emails}
                                            usuarios={usuariosEmpresa}
                                            onUpdate={async (newEmails) => {
                                              setProjetos(prev => prev.map(p => p.id === proj.id ? { ...p, responsaveis_emails: JSON.stringify(newEmails) } : p));
                                              await base44.entities.Projeto.update(proj.id, { responsaveis_emails: JSON.stringify(newEmails) });
                                            }}
                                          />
                                        );
                                      })()}
                                    ...
                                    <div className="mt-3 pt-3 border-t">
                                                                           <EntityActions
                                                                             ...
                                                                             onArchive={handleArchive}
                                                                              onDelete={(perfil === 'Admin' || temPermissao('Projetos', 'Lista', 'excluir')) ? handleDelete : undefined}
                                                                           />
                                                                         </div>
                                  </CardContent>
                                </Card>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-blue-700 font-semibold text-sm">
                        {proj.licitacao_modalidade ? formatModalidade(proj.licitacao_modalidade) : '-'}
                      </td>
      ...
      <Sheet open={showDetail} onOpenChange={setShowDetail}>
        <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col" data-fullscreen-modal>
          {selectedProj && (
            <>
              <SheetHeader>
                <ProjetoDetailHeader
                  ...
                  onEdit={() => { setShowDetail(false); handleOpenModal(selectedProj); }}
                  onClienteView={() => setShowClienteView(true)}
                  onSalvarTemplate={() => setShowSalvarTemplate(true)}
                  onDuplicar={() => { handleOpenModal({...selectedProj, nome: selectedProj.nome + ' (cópia)'}); setShowDetail(false); }}
                  onStatusConfig={() => setShowStatusConfig(true)}
                  onArchive={handleArchive}
                  onDelete={(proj) => { handleDelete(proj); setShowDetail(false); }}
              ...
              <Tabs defaultValue={getDefaultTab()} className="mt-6">
                <div className="overflow-x-auto -mx-6 px-6 pb-1">
                  <TabsList className="flex flex-nowrap gap-1 h-auto bg-slate-100 p-1 w-max min-w-full">
                    {(perfil === 'Admin' || temPermissao('Projetos', 'Lista')) && <TabsTrigger value="geral" className="flex-shrink-0 text-xs sm:text-sm">Geral</TabsTrigger>}
                    {(perfil === 'Admin' || temPermissao('Projetos', 'Orçamento')) && <TabsTrigger value="orcamento" className="flex-shrink-0 text-xs sm:text-sm">Orçamento</TabsTrigger>}
                    {(perfil === 'Admin' || temPermissao('Projetos', 'Cronograma')) && <TabsTrigger value="obra" className="flex-shrink-0 text-xs sm:text-sm">Cronograma</TabsTrigger>}
                    {(perfil === 'Admin' || temPermissao('Projetos', 'Financeiro')) && <TabsTrigger value="financeiro" className="flex-shrink-0 text-xs sm:text-sm">Financeiro</TabsTrigger>}
                    {(perfil === 'Admin' || temPermissao('Projetos', 'Diário de Obra')) && <TabsTrigger value="diario" className="flex-shrink-0 text-xs sm:text-sm">Diário de Obra</TabsTrigger>}
                    {(perfil === 'Admin' || temPermissao('Projetos', 'Arquivos')) && <TabsTrigger value="arquivos" className="flex-shrink-0 text-xs sm:text-sm">Arquivos</TabsTrigger>}
                    {(perfil === 'Admin' || temPermissao('Projetos', 'Anotações')) && <TabsTrigger value="anotacoes" className="flex-shrink-0 text-xs sm:text-sm">Anotações</TabsTrigger>}
                    {(perfil === 'Admin' || temPermissao('Projetos', 'Chat')) && <TabsTrigger value="chat" className="flex-shrink-0 text-xs sm:text-sm">Chat</TabsTrigger>}
                  </TabsList>
                </div>
                ...
                <TabsContent value="geral" className="space-y-4 mt-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-slate-800 mt-1">{selectedProj.cliente_nome || '-'}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <p className="text-lg font-bold text-green-600 mt-1">{formatCurrency(selectedProj.valor_estimado)}</p>
                    </div>
                    <div>
                      <Label className="text-slate-500">Status</Label>
                      <Badge 
                        style={{ backgroundColor: statusList.find(s => s.id === selectedProj.status_id)?.cor }}
                        className="mt-1"
                      >
                    ...
                    <div>
                      ...
                      <p className="font-medium text-slate-800 mt-1">{selectedProj.origem_nome || '-'}</p>
                    </div>
                  </div>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        {selectedProj.licitacao_modalidade && (
                          <div>
                            ...
                            <p className="font-medium text-blue-700 mt-1">{formatModalidade(selectedProj.licitacao_modalidade)}</p>
                          </div>
                        )}
                        ...
                        {selectedProj.licitacao_horario && (
                          <div>
                            ...
                            <p className="font-medium text-slate-800 mt-1">{selectedProj.licitacao_horario}</p>
                          </div>
                        )}
                      </div>
                    </div>
                          ...
                          <div className="grid grid-cols-3 gap-4">
                            {selectedProj.cep && (
                              <div>
                                ...
                                <p className="font-medium text-slate-800 mt-1">{selectedProj.cep}</p>
                              </div>
                            )}
                            {selectedProj.endereco && (
                              <div className="col-span-2">
                                <Label className="text-slate-500">Endereço</Label>
                                <p className="font-medium text-slate-800 mt-1">{selectedProj.endereco}</p>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="grid grid-cols-3 gap-4">
                            {selectedProj.numero && (
                              <div>
                                ...
                                <p className="font-medium text-slate-800 mt-1">{selectedProj.numero}</p>
                              </div>
                            )}
                            {selectedProj.complemento && (
                              <div className="col-span-2">
                                <Label className="text-slate-500">Complemento</Label>
                                <p className="font-medium text-slate-800 mt-1">{selectedProj.complemento}</p>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="grid grid-cols-3 gap-4">
                            {selectedProj.bairro && (
                              <div>
                                ...
                                <p className="font-medium text-slate-800 mt-1">{selectedProj.bairro}</p>
                              </div>
                            )}
                            {selectedProj.cidade && (
                              <div>
                                <Label className="text-slate-500">Cidade</Label>
                                <p className="font-medium text-slate-800 mt-1">{selectedProj.cidade}</p>
                              </div>
                            )}
                            {selectedProj.estado && (
                              <div>
                                <Label className="text-slate-500">Estado</Label>
                                <p className="font-medium text-slate-800 mt-1">{selectedProj.estado}</p>
                              </div>
                            )}
                          </div>
                        </>
                  ...
                  {selectedProj.descricao && (
                    <div className="border-t pt-4">
                      ...
                      <div className="mt-2 p-4 bg-slate-50 rounded-lg">
                        <p className="text-slate-700 whitespace-pre-wrap">{selectedProj.descricao}</p>
                      </div>
                    </div>
                  )}
                </TabsContent>
                ...
                <TabsContent value="orcamento" className="mt-4">
                  <OrcamentoTab
                    ...
                    onCreateSolicitacao={(itensOrc) => {
                     ...
                     setSolicitacaoCompraForm({ projeto_id: selectedProj.id, projeto_nome: selectedProj.nome, oportunidade_id: '', oportunidade_nome: '', aprovador_id: '', aprovador_nome: '', prioridade: 'Normal', origem: 'Orcamento', data_necessidade: '', observacoes: '', itens: todosItens.map(item => ({ descricao: item.descricao, quantidade: item.quantidade, unidade: item.unidade, especificacoes: `Código: ${item.codigo||'-'} | Valor Ref: R$ ${item.valor_unitario?.toFixed(2)||'0.00'}` })) });
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <div>
                        ...
                        />
                        <Button onClick={() => document.getElementById('upload-arquivo-projeto').click()} className="gap-2">
                          <Upload className="w-4 h-4" />
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {arquivos.map(arquivo => (
                        <div key={arquivo.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-slate-50">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500">
                                {new Date(arquivo.created_date).toLocaleDateString('pt-BR')} • {arquivo.usuario_nome}
                              </p>
                            </div>
                          </div>
                          <div className="flex gap-2">
                            <Button variant="ghost" size="icon" onClick={() => handleViewFile(arquivo)}>
                              <Eye className="w-4 h-4" />
                            </Button>
                            <Button variant="ghost" size="icon" onClick={() => window.open(arquivo.url, '_blank')}>
                              <Download className="w-4 h-4" />
                            </Button>
                            <Button variant="ghost" size="icon" onClick={() => handleDeleteArquivo(arquivo.id)}>
                  ...
                  <div className="space-y-4">
                    <div className="flex gap-2">
                      <Textarea
                        ...
                        rows={3}
                      />
                      <Button onClick={handleAddNota} disabled={!novaNota.trim()} className="shrink-0">
                        <Plus className="w-4 h-4" />
                      </Button>
                    </div>

                    <div className="space-y-3">
                      {atualizacoes.filter(a => a.tipo === 'Nota').map(nota => (
      ...
      <ProjetoFormSheet
        open={showModal}
        onOpenChange={(open) => { setShowModal(open); if (!open && selectedProj) setShowDetail(true); }}
        selectedProj={selectedProj}
        statusList={statusList}
        ...
        onSaved={(projAtualizado, isEdit) => {
          if (isEdit) {
            setProjetos(prev => prev.map(p => p.id === projAtualizado.id ? projAtualizado : p));
