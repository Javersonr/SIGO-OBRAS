...
import { Loader2, AlertCircle, Image, Trash2, MoreVertical, Undo2, CheckCheck, FileText, X, Package, Link2, Pencil } from 'lucide-react';
...
function DraggableComprovante({ url, item, onEditar, onFechar }) {
  ...
  return (
    <div
      ...
      style={{ left: pos.x, top: pos.y, width: size.w, height: size.h, minWidth: 280, minHeight: 200 }}
    >
      {/* Header arrastável */}
      <div
      ...
      >
        <svg width="14" height="14" viewBox="0 0 14 14" className="absolute bottom-1 right-1 text-slate-400">
          <path d="M13 1 L1 13 M13 7 L7 13 M13 13 L13 13" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
        </svg>
      </div>
    </div>
  );
...
export default function PreLancamentosAReconciliar({ empresaId, usuarioEmail, usuarioNome = '', verTodos, verProprios, filtroUsuario = '', onReload, onSelecionadosChange, podeAprovar, contas = [], categorias = [] }) {
  ...
  const listaTotalizador = itensSelecionadosFiltrados.length > 0 ? itensSelecionadosFiltrados : listaFiltrada;
  const totalFiltrado = listaTotalizador.reduce((sum, item) => {
    const d = typeof item.dados_extraidos === 'string' ? JSON.parse(item.dados_extraidos || '{}') : (item.dados_extraidos || {});
    return sum + (parseFloat(d.valor) || 0);
  }, 0);

  const todosSelecionados = listaFiltrada.length > 0 && listaFiltrada.every(item => selecionados.includes(item.id));
  ...
  const itensSelecionadosParaCaixa = preLancamentos.filter(item => selecionados.includes(item.id));
  ...
  const carregarUsuarios = async () => {
    try {
      const data = await base44.entities.UsuarioEmpresa.filter({ empresa_id: empresaId, ativo: true });
      setUsuarios(data || []);
    } catch {}
  };
  ...
  const handleSalvarDescricao = async (item, novaDescricao) => {
    const dados = typeof item.dados_extraidos === 'string' ? JSON.parse(item.dados_extraidos) : item.dados_extraidos || {};
  ...
  const handleFecharCaixaDireto = async () => {
    ...
    try {
      ...
      await Promise.all(
        itensSelecionadosParaCaixa.map(pl => base44.entities.PreLancamento.update(pl.id, { status: 'Em Fechamento' }))
      );
      setSelecionados([]);
    ...
    <>
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
                {(podeAprovar || verTodos) && selecionados.length > 0 && tabAtiva === 'pendentes' && (
                  ...
                  >
                    {fechandoCaixa ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Package className="w-3.5 h-3.5" />}
                    Fechar Caixa ({selecionados.length})
                  </Button>
                )}
                <Button size="sm" variant={tabAtiva === 'pendentes' ? 'default' : 'outline'} onClick={() => setTabAtiva('pendentes')} className="text-xs h-7">
                  Pendentes <Badge className="ml-1 bg-yellow-200 text-yellow-800 text-xs px-1">{filtroUsuario ? preLancamentos.filter(p => p.usuario_email === filtroUsuario).length : preLancamentos.length}</Badge>
                </Button>
                <Button size="sm" variant={tabAtiva === 'conciliados' ? 'default' : 'outline'} onClick={() => setTabAtiva('conciliados')} className="text-xs h-7">
                  Conciliados <Badge className="ml-1 bg-green-200 text-green-800 text-xs px-1">{filtroUsuario ? conciliados.filter(p => p.usuario_email === filtroUsuario).length : conciliados.length}</Badge>
                </Button>
                <Button size="sm" variant={mostrarTabela ? 'default' : 'outline'} onClick={() => setMostrarTabela(!mostrarTabela)} className="text-xs h-7 ml-auto bg-blue-600 hover:bg-blue-700 text-white">
            ...
            <table className="w-full min-w-[800px] text-sm">
              <thead className="bg-slate-100 border-b">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-slate-700">Comprovante</th>
                  ...
                  {verTodos && <th className="text-left px-4 py-3 font-semibold text-slate-700">Usuário</th>}
              ...
              <tbody>
                {listaFiltrada.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className={`border-b hover:bg-slate-50 ${isChecked ? 'bg-blue-50' : ''}`}>
                      <td className="px-3 py-3">
                        <Checkbox checked={isChecked} onCheckedChange={() => toggleSelecionado(item.id)} />
                      </td>
                      <td className="px-4 py-3">
                        {item.comprovante_url ? (
                          (() => {
                            const isPdf = item.comprovante_url.toLowerCase().includes('.pdf') || item.comprovante_url.toLowerCase().includes('pdf');
                            return isPdf ? (
                              <button onClick={() => { setVisualizandoUrl(item.comprovante_url); setVisualizandoItem(item); }}
                              ...
                              <button onClick={() => { setVisualizandoUrl(item.comprovante_url); setVisualizandoItem(item); }} className="cursor-pointer">
                                <img src={item.comprovante_url} alt="Comprovante" className="w-12 h-12 object-cover rounded border border-slate-200 hover:opacity-80 transition-opacity" />
                              </button>
                            );
                          })()
                        ) : (
                      ...
                      <td className="px-4 py-3 font-medium text-slate-900">{dados.fornecedor || '-'}</td>
                      ...
                      <td className="px-4 py-3">
                        {isConciliado ? (
                          ...
                          <Badge variant="outline" className="bg-yellow-50 text-yellow-800 border-yellow-300">Pendente</Badge>
                        )}
                      </td>
                      {verTodos && (
                        <td className="px-4 py-3 text-slate-500 text-xs">{item.usuario_email || '-'}</td>
                      )}
                          ...
                          <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem
                                onClick={async () => {
                                  ...
                                  if (response.data?.sucesso) {
                                    ...
                                    alert('Erro ao conciliar: ' + (response.data?.error || 'Tente novamente'));
                                  }
                                }}
                              ...
                              <DropdownMenuItem
                                onClick={async () => {
                                  ...
                                  if (item.transacao_id) {
                                    const ts = await base44.entities.TransacaoFinanceira.filter({ id: item.transacao_id });
                                    transacao = ts[0] || null;
                                  }
                            ...
                            {podeAcionar && (
                              <>
                                ...
                                <DropdownMenuItem onClick={() => handleDeletar(item)} className="gap-2 text-red-600 focus:text-red-600">
                ...
                {listaFiltrada.length === 0 && (
                  <tr>
                    <td colSpan={verTodos ? 9 : 8} className="text-center py-8 text-slate-500">
                      Nenhum pré-lançamento {tabAtiva === 'pendentes' ? 'pendente' : 'conciliado'}{filtroUsuario ? ' para este usuário' : ''}.
                    </td>
                  </tr>
                )}
                {listaFiltrada.length > 0 && (
                  <tr className="bg-slate-50 border-t-2 border-slate-300">
                    <td colSpan={verTodos ? 8 : 7} className="px-4 py-2 text-xs font-semibold text-slate-600 text-right">
      ...
      {visualizandoUrl && (
        <DraggableComprovante
          ...
          onFechar={() => { setVisualizandoUrl(null); setVisualizandoItem(null); }}
        />
      )}
      ...
      {editandoItem && (
        <EditarPreLancamentoComDespesaModal
          open={!!editandoItem}
          onOpenChange={(v) => { if (!v) setEditandoItem(null); }}
      ...
      {desfazendoItem && (
        <DesfazerConciliacaoModal
          open={!!desfazendoItem}
          onOpenChange={(v) => { if (!v) { setDesfazendoItem(null); setTransacaoDesfazer(null); } }}
