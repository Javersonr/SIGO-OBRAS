...
export default function NotaDevolucaoModal({
  open,
  onOpenChange,
  empresaAtiva,
  user,
  ...
  useEffect(() => {
    ...
    sigo.entities.TransacaoFinanceira.filter(
      { empresa_id: empresaAtiva.id, tipo: "Despesa" },
      "-created_date",
      200
    )
      .then((despesas) => {
        // Filtrar apenas as que têm chave NF-e (numero_documento com 44 dígitos)
        const comNFe = despesas.filter(
          (d) => d.numero_documento && d.numero_documento.replace(/\D/g, "").length === 44
        );
  ...
  const handleEmitir = async () => {
    ...
    try {
      ...
      const nota = await sigo.entities.NotaFiscalDevolucao.create({
      ...
      const res = await sigo.functions.invoke("emitirNotaDevolucao", {
        action: "emitir",
        nota_id: nota.id,
        payload: {
          empresa_id: empresaAtiva.id,
  ...
  return (
    <Sheet open={open} onOpenChange={handleClose}>
      <SheetContent
        side="right"
        className="h-full overflow-y-auto p-0 flex flex-col w-full"
          ...
          {step === 1 && (
            <>
              ...
              {despesasComNFe.length > 0 && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <p className="text-sm font-medium text-blue-800 mb-2">
                    📎 NF-e das Despesas Financeiras
                  </p>
                  <p className="text-xs text-blue-600 mb-3">
                    Selecione uma nota fiscal importada nas despesas para pré-preencher os dados do
                    destinatário e a chave NF-e referenciada.
                  </p>
                  <select
              ...
              <div>
                ...
                <Select
                  value={form.almoxarifado_id}
                  onValueChange={(v) => setForm({ ...form, almoxarifado_id: v })}
                >
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <Input
                        value={form.destinatario_ie}
                        onChange={(e) => setForm({ ...form, destinatario_ie: e.target.value })}
                        placeholder="ISENTO"
                        className="mt-1"
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="col-span-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setForm({ ...form, destinatario_endereco: e.target.value })
                        }
                        className="mt-1"
                      />
                    </div>
                    <div>
                      <Label>Número</Label>
                      <Input
                        value={form.destinatario_numero}
                        onChange={(e) => setForm({ ...form, destinatario_numero: e.target.value })}
                        className="mt-1"
                      />
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <Label>Bairro</Label>
                      <Input
                        value={form.destinatario_bairro}
                        onChange={(e) => setForm({ ...form, destinatario_bairro: e.target.value })}
                        className="mt-1"
                      />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(v) => setForm({ ...form, destinatario_uf: v })}
                      >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label>CEP</Label>
                      <Input
                        value={form.destinatario_cep}
                        onChange={(e) => setForm({ ...form, destinatario_cep: e.target.value })}
                        className="mt-1"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        value={form.nfe_referenciada}
                        onChange={(e) => setForm({ ...form, nfe_referenciada: e.target.value })}
              ...
              <div className="bg-slate-50 rounded-lg p-4 space-y-3 border">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <Input
                      value={novoItem.ncm}
                      onChange={(e) => setNovoItem({ ...novoItem, ncm: e.target.value })}
                  ...
                  <div>
                    ...
                    <Select
                      value={novoItem.cfop}
                      onValueChange={(v) => setNovoItem({ ...novoItem, cfop: v })}
                    >
                      <SelectTrigger className="mt-1 h-8 text-xs">
                        <SelectValue />
                      </SelectTrigger>
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        {["UN", "KG", "M", "M2", "M3", "L", "CX", "PC", "SC", "TN"].map((u) => (
                ...
                <Button
                  ...
                  disabled={!novoItem.descricao || novoItem.quantidade <= 0}
            ...
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-1">
                ...
                <p className="text-sm text-slate-600">
                  {form.destinatario_cidade}/{form.destinatario_uf}
                </p>
                {form.nfe_referenciada && (
                  <p className="text-xs text-slate-500">NF-e ref: {form.nfe_referenciada}</p>
                )}
              </div>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                <p className="text-sm text-amber-800">
                  ⚠️ Esta nota será emitida em <strong>ambiente de homologação</strong> (testes).
                  Para produção, altere a URL na função backend.
                </p>
              </div>
            </div>
          )}
                ...
                <>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {focusResult.chave_nfe && (
                          <p className="text-xs text-slate-600 mt-1 font-mono">
                            {focusResult.chave_nfe}
                          </p>
                        )}
                        {focusResult.protocolo && (
                  ...
                  {focusResult.erros && (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                      ...
                      <pre className="text-xs text-red-600 mt-1 whitespace-pre-wrap">
                        {JSON.stringify(focusResult.erros, null, 2)}
                      </pre>
                    </div>
                  )}
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    {focusResult.caminho_danfe && (
                      <Button
                        variant="outline"
                        onClick={() => window.open(focusResult.caminho_danfe, "_blank")}
                        className="gap-2"
                      >
                    ...
                    {focusResult.caminho_xml_nota_fiscal && (
                      <Button
                        variant="outline"
                        onClick={() => window.open(focusResult.caminho_xml_nota_fiscal, "_blank")}
                        className="gap-2"
                      >
        ...
        <div className="sticky bottom-0 bg-white border-t p-4 flex justify-between gap-3">
          {step === 1 && (
            <>
              ...
              <Button
                onClick={() => setStep(2)}
                disabled={!form.destinatario_cnpj || !form.destinatario_nome}
              >
                Próximo: Itens →
              </Button>
            </>
