    ...
    <Sheet open={open} onOpenChange={(isOpen) => {
      ...
      <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col" style={{ inset: 'auto 0 0 256px', width: 'calc(100% - 256px)', maxWidth: 'none' }}>
        <SheetHeader className="p-6 border-b">
          ...
          <p className="text-sm text-slate-600">Escolha um treinamento existente para adicionar aos modelos</p>
        </SheetHeader>
          ...
          {treinamentosDisponiveis.length === 0 ? (
            ...
            <div className="space-y-2">
              {treinamentosDisponiveis
                ...
                .map(treinamento => {
                  const jaAdicionado = treinamentosVinculados.some(t => 
                    t.nome === treinamento.nome && t.codigo === treinamento.codigo
                  );

                  return (
                    <Card key={treinamento.id} className={cn("cursor-pointer transition-all", jaAdicionado && "opacity-50")}>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {jaAdicionado && (
                                <Badge className="bg-green-100 text-green-700 text-xs">Já adicionado</Badge>
                              )}
                            </div>
                            {treinamento.carga_horaria && (
                              <p className="text-xs text-slate-600">Carga horária: {treinamento.carga_horaria}h</p>
                            )}
                          </div>
                          <Button
                            size="sm"
                            ...
                            onClick={async () => {
                                if (!jaAdicionado && funcao?.id) {
                                   try {
                                     ...
                                     let instrutorAssinaturaUrl = treinamento.instrutor_assinatura_url || '';
                                     try {
                                       const instrutoresParsed = JSON.parse(treinamento.instrutor_nome);
                                     ...
                                     await base44.entities.Treinamento.create({
                                       ...
                                       engenheiro_responsavel_nome: treinamento.engenheiro_responsavel_nome,
                                       engenheiro_responsavel_crea: treinamento.engenheiro_responsavel_crea,
