...
import { ShoppingCart, FileCheck, Package, DollarSign, Clock, CheckCircle } from 'lucide-react';
...
export default function RelatorioCompras({ dados }) {
  ...
  const kpis = useMemo(() => {
    ...
    const solAprovadas = solicitacoes.filter(s => s.status === 'Aprovada' || s.status === 'Pedido Gerado').length;
  ...
  const solPorStatus = useMemo(() => {
    ...
    return Object.entries(map).map(([nome, quantidade]) => ({ nome, quantidade })).sort((a, b) => b.quantidade - a.quantidade);
  }, [solicitacoes]);
  ...
  if (solicitacoes.length === 0 && pedidos.length === 0) return (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <KpiCard titulo="Solicitações" valor={kpis.totalSol} subtitulo="total" icon={FileCheck} cor="blue" />
        <KpiCard titulo="Pendentes" valor={kpis.pendentes} subtitulo="aguardando" icon={Clock} cor="amber" />
        <KpiCard titulo="Aprovadas" valor={kpis.solAprovadas} subtitulo="solicitações" icon={CheckCircle} cor="green" />
        <KpiCard titulo="Cotações" valor={cotacoes.length} subtitulo="total" icon={FileCheck} cor="purple" />
        <KpiCard titulo="Pedidos" valor={kpis.totalPed} subtitulo="total" icon={Package} cor="slate" />
        <KpiCard titulo="Tx. Aprovação" valor={`${kpis.txAprovacao}%`} subtitulo="solicitações" icon={ShoppingCart} cor="green" tendencia={kpis.txAprovacao >= 70 ? 'up' : 'down'} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle className="text-sm font-semibold text-slate-700">Solicitações por Status</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={solPorStatus} layout="vertical">
                ...
                <Bar dataKey="quantidade" name="Solicitações" radius={[0, 4, 4, 0]}>
                  {solPorStatus.map((_, i) => <Cell key={i} fill={CORES[i % CORES.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        ...
        <Card>
          <CardHeader><CardTitle className="text-sm font-semibold text-slate-700">Distribuição de Pedidos</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie data={pedPorStatus} dataKey="quantidade" nameKey="nome" cx="50%" cy="50%" outerRadius={100}
                  label={({ nome, percent }) => `${(percent * 100).toFixed(0)}%`}>
                  {pedPorStatus.map((_, i) => <Cell key={i} fill={CORES[i % CORES.length]} />)}
                </Pie>
                <Tooltip />
                <Legend />
              </PieChart>
      ...
      <Card>
        <CardHeader><CardTitle className="text-sm font-semibold text-slate-700">Evolução Mensal (Solicitações vs Pedidos)</CardTitle></CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={280}>
