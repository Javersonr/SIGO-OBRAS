...
export default function AprovacaoConfigTab({ empresaAtiva }) {
  ...
  const renderNiveis = (tipo) => {
    ...
    return (
      <Card>
        <CardHeader>
          <CardTitle className="capitalize">{tipo.replace('SolicitacaoCompra', 'Solicitações de Compra').replace('OrcamentoProjeto', 'Orçamentos de Projeto')}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {niveisFiltrados.map(nivel => (
            <div key={nivel.id} className="p-4 border rounded-lg space-y-3">
              <div className="flex justify-between items-center">
                <h4 className="font-semibold">{nivel.ordem}. {nivel.nome}</h4>
                <div className="flex gap-2">
                  <Button variant="ghost" size="icon" onClick={() => setEditingNivel(nivel)}><Edit className="w-4 h-4" /></Button>
                  <Button variant="ghost" size="icon" onClick={() => handleDeleteNivel(nivel.id)}><Trash2 className="w-4 h-4 text-red-500" /></Button>
                </div>
              </div>
              <p className="text-sm text-slate-600">
                Aplica-se de {`R$ ${nivel.valor_minimo || 0}`} até {nivel.valor_maximo ? `R$ ${nivel.valor_maximo}` : 'infinito'}
              </p>
              <div className="flex flex-wrap gap-2">
                {Array.isArray(JSON.parse(nivel.perfis_aprovadores || '[]')) && JSON.parse(nivel.perfis_aprovadores || '[]').map(p => <Badge key={p} variant="secondary">{p}</Badge>)}
              </div>
            </div>
          ))}
          ...
          {editingNivel && editingNivel.tipo === tipo ? (
            <div className="p-4 border border-blue-500 rounded-lg space-y-4">
                <Input placeholder="Nome do nível (ex: Gerente)" value={editingNivel.nome} onChange={e => setEditingNivel({...editingNivel, nome: e.target.value})} />
                <Input type="number" placeholder="Ordem" value={editingNivel.ordem} onChange={e => setEditingNivel({...editingNivel, ordem: e.target.value})} />
                <div className="grid grid-cols-2 gap-4">
                    <Input type="number" placeholder="Valor Mínimo" value={editingNivel.valor_minimo} onChange={e => setEditingNivel({...editingNivel, valor_minimo: e.target.value})} />
                    <Input type="number" placeholder="Valor Máximo (opcional)" value={editingNivel.valor_maximo} onChange={e => setEditingNivel({...editingNivel, valor_maximo: e.target.value})} />
                </div>
                <div className="space-y-2">
                 ...
                 <div className="flex gap-2 justify-end">
                    <Button variant="ghost" onClick={() => setEditingNivel(null)}>Cancelar</Button>
                    <Button onClick={() => handleSaveNivel(tipo)}>Salvar</Button>
                </div>
            </div>
          ) : (
            <Button variant="outline" className="w-full" onClick={() => setEditingNivel({ tipo: tipo, nome: '', ordem: niveisFiltrados.length + 1, valor_minimo: 0, perfis_aprovadores: [] })}> 
