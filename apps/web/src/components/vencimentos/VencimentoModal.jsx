...
export default function VencimentoModal({
  open,
  onOpenChange,
  vencimento,
  empresaAtiva,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (vencimento?.id) {
        await sigo.entities.Vencimento.update(vencimento.id, data);
        toast.success("Vencimento atualizado");
      } else {
        await sigo.entities.Vencimento.create(data);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="h-full overflow-y-auto p-0 flex flex-col w-full sm:max-w-xl"
        ...
        <div className="p-6 flex-1 space-y-4">
          <div>
            ...
            <Input
              ...
              placeholder="Ex: CND Federal - Receita Federal"
              className="mt-1.5"
            />
          </div>
          ...
          <div>
            ...
            <Select
              value={String(form.alerta_dias)}
              onValueChange={(v) => setForm((f) => ({ ...f, alerta_dias: Number(v) }))}
            >
              <SelectTrigger className="mt-1.5">
                <SelectValue />
              </SelectTrigger>
              ...
              {form.arquivo_url ? (
                <div className="flex items-center gap-2 flex-1 min-w-0 p-2 bg-slate-50 border rounded-lg">
                  <span className="text-sm text-slate-700 truncate flex-1">
                    {form.arquivo_nome || "Arquivo anexado"}
                  </span>
                  <Button
                    variant="ghost"
                    size="icon"
                  ...
                  <Button
                    ...
                    size="icon"
                    className="h-6 w-6 flex-shrink-0"
                    onClick={() => setForm((f) => ({ ...f, arquivo_url: "", arquivo_nome: "" }))}
                  >
          ...
          <div className="flex items-center gap-3">
            <Switch
              ...
              onCheckedChange={(v) => setForm((f) => ({ ...f, renovacao_automatica: v }))}
            />
