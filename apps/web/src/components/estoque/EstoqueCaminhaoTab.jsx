...
import { Truck, Package, Plus, Edit, Trash2, ChevronDown, ChevronRight, FileText, Archive, AlertTriangle } from 'lucide-react';
...
export default function EstoqueCaminhaoTab({ empresaAtiva, user, materiais, saldos, projetos, onRecarregar }) {
  ...
  const handleExportarPDF = (caminhao) => {
    ...
    if (ferrs.length === 0) { toast.error('Nenhuma ferramenta neste caminhão'); return; }
  ...
  const handleExportarLaudosZip = async (caminhao) => {
    ...
    if (ferrs.length === 0) { toast.error('Nenhuma ferramenta possui laudo'); return; }
    toast.info(`Baixando ${ferrs.length} laudo(s)...`);
    const zip = new JSZip();
    await Promise.all(ferrs.map(async (ferr) => {
      try {
  ...
  const handleSaveCell = async (ferr, field) => {
    try {
      ...
      setFerramentas(prev => prev.map(f => f.id === ferr.id ? { ...f, [field]: editingValue } : f));
  ...
  const EditableCell = ({ ferr, field, type = 'text' }) => {
    ...
    return (
      <div
        onClick={e => { e.stopPropagation(); handleEditCell(ferr, field); }}
            ...
            <Card key={c.id} className="border border-slate-200">
              ...
              >
                <div className="flex items-center gap-4">
                  {c.foto_url ? (
                    <img src={c.foto_url} alt={c.placa} className="w-14 h-14 rounded-lg object-cover border" />
                  ) : (
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {c.modelo && <span className="text-slate-500 text-sm">{c.marca} {c.modelo}</span>}
                      {c.ano && <Badge variant="outline" className="text-xs">{c.ano}</Badge>}
                    </div>
                    {c.motorista_padrao_nome && (
                      <p className="text-sm text-slate-500 mt-0.5">👤 {c.motorista_padrao_nome}</p>
                    )}
                    <div className="flex items-center gap-3 mt-1">
                      <span className="text-xs text-blue-600 font-medium">{ferrsDoC.length} ferramentas</span>
                      {ferrsDoC.length > 0 && (
                ...
                <div className="border-t border-slate-100">
                  ...
                  <div className="flex items-center justify-between px-4 py-2 bg-slate-50">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" className="gap-1 text-xs h-8" onClick={() => handleExportarPDF(c)}>
                        <FileText className="w-3 h-3" /> Exportar PDF
                      </Button>
                      <Button size="sm" variant="outline" className="gap-1 text-xs h-8" onClick={() => handleExportarLaudosZip(c)}>
                  ...
                  {ferrsDoC.length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      ...
                      <div className="grid grid-cols-3 gap-3 p-4">
                        ...
                        <Card><CardContent className="p-3">
                          <p className="text-xl font-bold">{new Set(ferrsDoC.map(f => f.codigo?.split('-').slice(0,2).join('-'))).size}</p>
                          <p className="text-xs text-slate-500">Tipos</p>
                        </CardContent></Card>
                        <Card><CardContent className="p-3">
                          <p className="text-xl font-bold text-green-600">R$ {valorTotal.toFixed(2)}</p>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {ferrsDoC.sort((a, b) => (a.descricao || '').localeCompare(b.descricao || '')).map(ferr => (
                            <TableRow key={ferr.id} className="text-xs hover:bg-slate-50 cursor-pointer"
                              ...
                              <TableCell>
                                <Badge className={`text-xs ${STATUS_COLORS[ferr.status] || 'bg-slate-100 text-slate-700'}`}>
                                  {ferr.status}
                                </Badge>
                              </TableCell>
          ...
          onSave={async (formData) => {
            try {
              ...
              if (formData.destino && formData.ferramenta_id) {
                await base44.entities.Ferramenta.update(formData.ferramenta_id, { localizacao: formData.destino });
              }
