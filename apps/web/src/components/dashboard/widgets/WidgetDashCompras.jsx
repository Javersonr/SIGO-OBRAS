...
import { ShoppingCart, Clock, CheckCircle, AlertCircle, FileText, TrendingUp } from 'lucide-react';
...
export default function WidgetDashCompras({ onDadosCarregados }) {
  ...
  const load = async () => {
    try {
      ...
      const cotAbertas = cotacoes.filter(c => c.status === 'Aberta' || c.status === 'Enviada aos Fornecedores' || c.status === 'Aguardando Respostas');
      const pedPendentes = pedidos.filter(p => p.status === 'Pendente' || p.status === 'Aprovado');
      const pedEntregues = pedidos.filter(p => p.status === 'Entregue' || p.status === 'Concluído');
      ...
      const d = { solPendentes: solPendentes.length, solAprovadas: solAprovadas.length, cotAbertas: cotAbertas.length, pedPendentes: pedPendentes.length, pedEntregues: pedEntregues.length, valorPedidos, valorPendente, meses };
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="border-l-4 border-l-yellow-500">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <div><p className="text-xs text-slate-500">Sol. Pendentes</p><p className="text-2xl font-bold text-yellow-600">{data.solPendentes}</p></div>
        ...
        <Card className="border-l-4 border-l-blue-500">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <div><p className="text-xs text-slate-500">Cotações Abertas</p><p className="text-2xl font-bold text-blue-600">{data.cotAbertas}</p></div>
        ...
        <Card className="border-l-4 border-l-orange-500">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <div><p className="text-xs text-slate-500">Pedidos Abertos</p><p className="text-2xl font-bold text-orange-600">{data.pedPendentes}</p></div>
        ...
        <Card className="border-l-4 border-l-green-500">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <div><p className="text-xs text-slate-500">Entregues</p><p className="text-2xl font-bold text-green-600">{data.pedEntregues}</p></div>
      ...
      <Card>
        <CardHeader className="pb-1 pt-3 px-4"><CardTitle className="text-sm">Pedidos por mês</CardTitle></CardHeader>
