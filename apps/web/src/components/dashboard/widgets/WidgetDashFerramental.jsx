...
import { Wrench, Truck, User, Hammer, AlertTriangle, FileText, FileWarning, CheckCircle, XCircle, TrendingUp, ClipboardList } from 'lucide-react';
  ...
  const load = async () => {
    try {
      ...
      const [ferramentas, movimentacoes, laudos] = await Promise.all([
        ...
        base44.entities.MovimentacaoFerramenta.filter({ empresa_id: empresaAtiva.id }),
      ...
      const noCaminhao = ferramentas.filter(f => f.localizacao?.toLowerCase().includes('caminhão') || f.campo_obrigatorio_id).length;
      const comFuncionario = ferramentas.filter(f => f.status === 'Em Uso' && f.funcionario_id).length;
      ...
      const movMes = movimentacoes.filter(m => {
        ...
        return d >= inicioMes && d <= hoje &&
          (m.tipo_movimentacao === 'Entrega para Funcionário' || m.tipo_movimentacao === 'Empréstimo');
      });
      ...
      const ferramentasComLaudo = ferramentas.filter(f => f.numero_laudo || f.data_vencimento_laudo);
      const laudoVencido = ferramentasComLaudo.filter(f => f.data_vencimento_laudo && new Date(f.data_vencimento_laudo) < hoje);
      ...
      const laudosVencidos = laudos.filter(l => l.data_vencimento && new Date(l.data_vencimento) < hoje).length;
      ...
      const movComObservacao = movEPIMes.filter(m => m.observacoes && m.observacoes.length > 0).length;

      const d = {
        total: ferramentas.length, disponivel, emUso, emManutencao, danificado,
        noCaminhao, comFuncionario, manutencaoProxima, manutencaoVencida, porStatus,
        topSaidas, totalSaidasMes: movMes.length,
        // Laudos
        ...
        laudosVencidos, laudosVencendo, laudosSemDoc,
        // Listas assinadas
        totalMovMes: movMes.length,
        movComLista, movSemLista,
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="border-l-4 border-l-green-500">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <div><p className="text-xs text-slate-500">Disponíveis</p><p className="text-2xl font-bold text-green-600">{data.disponivel}</p></div>
        ...
        <Card className="border-l-4 border-l-blue-500">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <div><p className="text-xs text-slate-500">Nos Caminhões</p><p className="text-2xl font-bold text-blue-600">{data.noCaminhao}</p></div>
        ...
        <Card className="border-l-4 border-l-purple-500">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <div><p className="text-xs text-slate-500">c/ Funcionário</p><p className="text-2xl font-bold text-purple-600">{data.comFuncionario}</p></div>
        ...
        <Card className="border-l-4 border-l-yellow-500">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <div><p className="text-xs text-slate-500">Manutenção</p><p className="text-2xl font-bold text-yellow-600">{data.emManutencao}</p></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <Card>
          <CardHeader className="pb-1 pt-3 px-4"><CardTitle className="text-sm">Distribuição por Status</CardTitle></CardHeader>
          <CardContent className="pb-3">
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                <Pie data={data.porStatus} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={60} label={({ name, value }) => `${name}: ${value}`} labelLine={false} fontSize={9}>
        ...
        <Card>
          ...
          <CardContent className="px-4 pb-3 space-y-2">
            <div className="flex justify-between items-center p-2 bg-red-50 rounded">
              ...
              <Badge className="bg-red-100 text-red-700 border-red-200">{data.laudoVencido + data.laudosVencidos}</Badge>
            </div>
            <div className="flex justify-between items-center p-2 bg-yellow-50 rounded">
              ...
              <Badge className="bg-yellow-100 text-yellow-700 border-yellow-200">{data.laudoVencendo30 + data.laudosVencendo}</Badge>
            </div>
            <div className={`flex justify-between items-center p-2 rounded ${data.semArquivoLaudo + data.laudosSemDoc > 0 ? 'bg-orange-50' : 'bg-green-50'}`}>
              <div className="flex items-center gap-2">
                {data.semArquivoLaudo + data.laudosSemDoc > 0
                  ? <FileWarning className="w-4 h-4 text-orange-500" />
                  : <CheckCircle className="w-4 h-4 text-green-500" />}
                <span className={`text-xs font-medium ${data.semArquivoLaudo + data.laudosSemDoc > 0 ? 'text-orange-700' : 'text-green-700'}`}>
                  Sem arquivo anexado
                </span>
              </div>
              <Badge className={data.semArquivoLaudo + data.laudosSemDoc > 0 ? 'bg-orange-100 text-orange-700 border-orange-200' : 'bg-green-100 text-green-700 border-green-200'}>
            ...
            <div className="flex justify-between items-center p-2 bg-slate-50 rounded">
              ...
              <Badge className="bg-slate-100 text-slate-700 border-slate-200">{data.ferramentasComLaudo}</Badge>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="px-4 pb-3 space-y-2">
            <div className="flex justify-between items-center p-2 bg-blue-50 rounded">
              ...
              <Badge className="bg-blue-100 text-blue-700 border-blue-200">{data.totalMovMes}</Badge>
            </div>
            <div className="flex justify-between items-center p-2 bg-green-50 rounded">
              ...
              <Badge className="bg-green-100 text-green-700 border-green-200">{data.movComLista}</Badge>
            </div>
            <div className={`flex justify-between items-center p-2 rounded ${data.movSemLista > 0 ? 'bg-yellow-50' : 'bg-green-50'}`}>
              <div className="flex items-center gap-2">
                {data.movSemLista > 0
                  ? <AlertTriangle className="w-4 h-4 text-yellow-500" />
                  : <CheckCircle className="w-4 h-4 text-green-500" />}
                <span className={`text-xs font-medium ${data.movSemLista > 0 ? 'text-yellow-700' : 'text-green-700'}`}>
                  Sem referência de lista
                </span>
              </div>
              <Badge className={data.movSemLista > 0 ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 'bg-green-100 text-green-700 border-green-200'}>
                {data.movSemLista}
              </Badge>
            </div>
            <div className="flex justify-between items-center p-2 bg-slate-50 rounded">
              <span className="text-xs text-slate-600 font-medium">EPIs cadastrados</span>
              <Badge className="bg-slate-100 text-slate-700 border-slate-200">{data.epiTotal}</Badge>
            </div>
            ...
            {data.totalMovMes > 0 && (
              <div className="mt-1">
                <div className="flex justify-between text-xs text-slate-500 mb-1">
                  ...
                  <span className="font-medium">{Math.round((data.movComLista / data.totalMovMes) * 100)}%</span>
                </div>
      ...
      <Card>
        <CardHeader className="pb-2 pt-3 px-4"><CardTitle className="text-sm">Alertas de Manutenção</CardTitle></CardHeader>
