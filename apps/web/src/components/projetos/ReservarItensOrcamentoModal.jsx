...
export default function ReservarItensOrcamentoModal({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const todosOsSaldos = await sigo.entities.EstoqueSaldo.filter({
        empresa_id: empresaAtiva.id,
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await sigo.entities.ReservaMaterial.bulkCreate(reservasParaCriar);
      ...
      if (semEstoque.length > 0 || itensComEstoque.some((i) => i.faltam > 0)) {
        ...
        if (itensSolicitacao.length > 0) {
          await sigo.entities.SolicitacaoCompraItem.bulkCreate(itensSolicitacao);
        }
      }
                      ...
                      <div key={item.orcamento_item.id} className="flex items-start gap-2 text-sm">
                        ...
                        <div className="flex-1">
                          <div className="font-medium text-slate-800">
                            {item.orcamento_item.descricao}
                          </div>
                          <div className="text-xs text-slate-600">
                            Código: {item.orcamento_item.codigo || "-"} • Necessário:{" "}
                            {item.quantidade_necessaria} {item.orcamento_item.unidade} • Disponível:{" "}
                            {item.disponivel}
                          </div>
                        </div>
                      </div>
                      ...
                      .map((item) => (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-slate-600">
                              Necessário: {item.quantidade_necessaria} {item.orcamento_item.unidade}{" "}
                              • Faltam: {item.faltam} un.
                            </div>
                          </div>
                        </div>
                      ))}
