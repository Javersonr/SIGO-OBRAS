...
export default function AlertaDocumentosRH({ documentosRH }) {
  ...
  const temAlerta = 
    asoStatus.status !== 'ok' || 
    examesStatus.status !== 'ok' || 
    registroStatus.status !== 'ok';
  ...
  return (
    <div className="space-y-2 mb-4">
      {alertas.map(item => (
        <Alert key={item.nome} className={`${
          item.status.status === 'vencido' ? 'border-red-200 bg-red-50' :
          item.status.status === 'proximaVencimento' || item.status.status === 'proxima_vencimento' ? 'border-amber-200 bg-amber-50' :
            ...
            {item.status.status === 'inconsistencia' && (
              <div className="text-sm space-y-1">
                ...
                {item.status.inconsistencias.map((inc, idx) => (
                  <div key={idx} className="text-xs">• {inc}</div>
                ))}
              </div>
            )}
