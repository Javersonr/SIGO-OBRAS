...
import { Plus, Calendar, Trash2, Edit, CheckCircle2, FileText, FileSpreadsheet, Download, Upload, ChevronDown, Clock, Eye } from 'lucide-react';
...
export default function ReceitasTab({ empresaAtiva, transacoes: transacoesIniciais, contas, categorias, projetos, clientes, onReload, filtroProjetoInicial, ocultarFiltrosProjeto, transacaoIdInicial, transacaoIdRef, transacaoKey, onTransacaoInicialConsumed }) {
  ...
  useEffect(() => { setClientesLocais(clientes || []); }, [clientes]);
  ...
  const categoriasOrdenadas = useMemo(() => {
    return [...categorias].filter(c => c.tipo === 'Receita').sort((a, b) => 
  ...
  const centrosCustoFiltrados = useMemo(() => {
    ...
    return centrosCustoOrdenados.filter(c => 
  ...
  const handleExportarExcel = async () => {
    ...
    const dadosExportacao = transacoesIniciais.filter(t => (t.tipo || '').toLowerCase() === 'receita').map((t, index) => {
      ...
      return {
        ...
        'Status': t.status === 'pago' ? 'Recebido' : t.status === 'em_aberto' ? 'Pendente' : 'Atrasado',
  ...
  const handleBaixarModelo = async () => {
    ...
    const wscols = [
      { wch: 30 }, { wch: 25 }, { wch: 25 }, { wch: 25 }, { wch: 20 },
      { wch: 20 }, { wch: 12 }, { wch: 15 }, { wch: 12 }, { wch: 15 }, { wch: 30 }
    ];
    ws['!cols'] = wscols;
    ...
    reader.onload = async (evt) => {
      try {
        ...
        // Verificar se é NFS-e
          ...
          const clienteNome = getText(tomador, 'RazaoSocial') || getText(tomador, 'razaoSocial') || 'Cliente Desconhecido';
          const clienteCNPJ = getText(tomador, 'Cnpj') || getText(tomador, 'cnpj') || getText(tomador, 'Cpf') || getText(tomador, 'cpf') || '';
          
          const dataEmissao = (getText(infNfse, 'DataEmissao') || getText(infNfse, 'dataEmissao') || new Date().toISOString()).split('T')[0];
  ...
  const handleImportarExcel = async (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const [todosClientes, todasOportunidades, todosProjetos, todasCategorias, todasContas] = await Promise.all([
          ...
          base44.entities.CategoriaFinanceira.filter({ empresa_id: empresaAtiva.id, tipo: 'Receita' }),
        ...
        for (let index = 0; index < data.length; index++) {
          ...
          try {
            ...
            const status = row.Status === 'Recebido' ? 'pago' : row.Status === 'Atrasado' ? 'atrasado' : 'em_aberto';
  ...
  const receitasFiltradas = useMemo(() => aplicarFiltros(transacoesIniciais), [transacoesIniciais, filtros]);
  ...
  const receitas = useMemo(() => {
    const sorted = [...receitasFiltradas].sort((a, b) => {
      ...
      if (sortConfig.field === 'data_vencimento' || sortConfig.field === 'data_pagamento' || sortConfig.field === 'created_date') {
  ...
  const handleExcluirEmLote = async () => {
    ...
    for (let i = 0; i < itensSelecionados.length; i += LOTE_SIZE) {
      ...
      for (const id of lote) {
        ...
        setImportacao(prev => ({ ...prev, processados: i + lote.indexOf(id) + 1, erros: idsComErro.length }));
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              <DropdownMenuItem onClick={() => document.getElementById('importar-xml-receitas')?.click()}>
        ...
        <table className="w-full min-w-[1000px]">
          <thead className="bg-slate-100 border-b group">
            <tr>
              ...
              <SortableTableHeader field="descricao" label="Descrição" currentSort={sortConfig} onSortChange={setSortConfig} />
              <SortableTableHeader field="cliente_nome" label="Cliente" currentSort={sortConfig} onSortChange={setSortConfig} />
              <SortableTableHeader field="categoria_nome" label="Categoria" currentSort={sortConfig} onSortChange={setSortConfig} />
              <SortableTableHeader field="data_vencimento" label="Vencimento" currentSort={sortConfig} onSortChange={setSortConfig} />
              <SortableTableHeader field="valor" label="Valor" currentSort={sortConfig} onSortChange={setSortConfig} align="right" />
          ...
          <tbody>
            {receitas.length === 0 ? (
              ...
              receitas.map(r => (
                 <tr key={r.id} className="border-b hover:bg-slate-50 cursor-pointer" onClick={() => { setReceitaDetalhes(r); setShowDetalhes(true); }}>
                    ...
                    <Checkbox
                      ...
                      onCheckedChange={(checked, event) => {
                        ...
                        if (event?.nativeEvent?.shiftKey && ultimoItemClicado) {
                          ...
                          const indiceUltimo = todasReceitas.findIndex(item => item.id === ultimoItemClicado);
                          ...
                          const idsNoIntervalo = todasReceitas.slice(inicio, fim + 1).map(item => item.id);
                  ...
                  <td className="px-4 py-3 text-sm font-semibold text-green-600">{formatCurrency(r.valor)}</td>
                  <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                   <Badge 
                     className={r.status === 'pago' || r.status === 'Pago' ? 'bg-green-100 text-green-700 cursor-pointer' : 'bg-blue-100 text-blue-700 cursor-pointer'}
          ...
          <div className="text-sm text-slate-600">
           Mostrando {indiceInicio + 1} a {Math.min(indiceFim, receitasFiltradas.length)} de {receitasFiltradas.length} receitas
          </div>
      ...
      <Sheet open={showModal} onOpenChange={setShowModal}>
        <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col lg:!inset-[auto_0_0_256px] lg:!w-[calc(100%-256px)] lg:!max-w-none" data-fullscreen-modal>
          <SheetHeader>
          ...
          <div className="space-y-6 py-4">
            <div>
              <h3 className="text-xs font-semibold text-slate-500 uppercase mb-4">Informações da Receita</h3>
              ...
              <div className="space-y-4">
                            ...
                            <CommandGroup>
                              {categoriasFiltradas.map(c => (
                                <CommandItem
                                  ...
                                  value={c.nome}
                                  onSelect={() => {
                                    setForm({ ...form, categoria_id: c.id, categoria_nome: c.nome });
                            ...
                            <CommandGroup>
                              {clientesFiltrados.map(c => (
                                <CommandItem
                                  ...
                                  value={c.nome_razao}
                                  onSelect={() => {
                                    setForm({ ...form, cliente_id: c.id, cliente_nome: c.nome_razao });
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div>
                     ...
                     <Select value={form.oportunidade_id || ''} onValueChange={(v) => setForm({ ...form, oportunidade_id: v })}>
                       <SelectTrigger className="mt-1.5">
                          ...
                          <div className="col-span-2 flex items-center">
                            <div className="flex flex-col">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-bold">{parcela.numero}/{parcelas.length}</span>
                              </div>
                            </div>
                          </div>
                            ...
                            <Select
                              ...
                              onValueChange={(v) => {
                                ...
                                if (v === 'pago' && !novasParcelas[index].data_pagamento) {
                                  novasParcelas[index].data_pagamento = new Date().toISOString().split('T')[0];
                                }
                                setParcelas(novasParcelas);
                              }}
                            >
          ...
          <div className="flex justify-end gap-3 border-t pt-4">
            ...
            <Button onClick={handleSave} disabled={!form.valor || !form.conta_id} className="bg-green-600 hover:bg-green-700">
              Salvar
            </Button>
          </div>
        </SheetContent>
      ...
      {showNovoCliente && createPortal(
        <NovoClienteModal
          ...
          onClienteCriado={(cliente) => {
            ...
            setForm(prev => ({ ...prev, cliente_id: cliente.id, cliente_nome: cliente.nome_razao }));
            onReload();
          }}
        />,
        document.body
      ...
      <DetalheReceitaModal
        ...
        podeEditar={true}
        empresaAtiva={empresaAtiva}
        onEditar={(r) => { setShowDetalhes(false); handleOpen(r); }}
