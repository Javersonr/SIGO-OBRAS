...
export default function SelecionarFerramentasModal({
  open,
  onClose,
  onConfirm,
  empresaAtiva,
  tipo = "EPI",
  ferramentasJaSelecionadas = [],
}) {
  ...
  const handleSearchFerramenta = (value, rowIndex) => {
    ...
    if (value.trim()) {
      const filtered = allFerramentas.filter(
        (ferr) =>
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-slate-500">
                          Código: {ferr.codigo} | Estoque: {ferr.quantidade_estoque} | R${" "}
                          {(ferr.valor_aquisicao || 0).toFixed(2)}
                        </p>
                      </div>
                    </button>
                  ))}
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b sticky top-0">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-700 min-w-[200px]">
                    Descrição
                  </th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-700 w-16">
                    Tipo
                  </th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-700 w-20">
                    Código
                  </th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-700 w-16">
                    Unidade
                  </th>
                  <th className="text-right px-4 py-3 text-xs font-semibold text-slate-700 w-24">
                    Preço Ref.
                  </th>
                  <th className="text-right px-4 py-3 text-xs font-semibold text-slate-700 w-16">
                    Estoque
                  </th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-700 w-20">
                    Localização
                  ...
                  <th className="text-center px-4 py-3 text-xs font-semibold text-slate-700 w-16">
                    Qtd
                  </th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-slate-700 w-16">
                    Ações
                    ...
                    {editingIndex === index ? (
                      <>
                        ...
                        <td className="px-4 py-2 text-xs text-slate-600">
                          {editingData.tipo || tipo}
                        </td>
                        <td className="px-4 py-2 text-xs text-slate-600">
                          {editingData.codigo || "-"}
                        </td>
                        <td className="px-4 py-2 text-xs text-slate-600">
                          {editingData.unidade || "UN"}
                        </td>
                        <td className="px-4 py-2 text-right text-xs text-slate-600">
                          R$ {(editingData.valor_aquisicao || 0).toFixed(2)}
                        </td>
                        <td className="px-4 py-2 text-right text-xs text-slate-600">
                          {editingData.quantidade_estoque || 0}
                        </td>
                        <td className="px-4 py-2 text-xs text-slate-600">
                          {editingData.localizacao || "-"}
                        </td>
                      ...
                      <>
                        ...
                        <td className="px-4 py-2 text-xs text-slate-600 text-center">
                          {ferramenta.unidade || "UN"}
                        </td>
                        <td className="px-4 py-2 text-right text-xs text-slate-600">
                          R$ {(ferramenta.valor_aquisicao || 0).toFixed(2)}
                        </td>
                        <td className="px-4 py-2 text-right text-xs text-slate-600">
                          {ferramenta.quantidade_estoque || 0}
                        </td>
                        <td className="px-4 py-2 text-xs text-slate-600">
                        ...
                        <td className="px-4 py-2 text-xs text-center font-medium text-slate-700">
