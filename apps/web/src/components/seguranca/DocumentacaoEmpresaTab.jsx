...
import { 
  Plus, Upload, Eye, Trash2, FileText, Clock, Search, 
  AlertCircle, CheckCircle2, Archive, Download, X, Edit, Bell
} from 'lucide-react';
...
export default function DocumentacaoEmpresaTab({ empresaAtiva, temPermissao, perfil }) {
  ...
  const podeEditar = perfil === 'Admin' || temPermissao('Segurança do Trabalho', 'Documentação da Empresa', 'editar');
  const podeCriar = perfil === 'Admin' || temPermissao('Segurança do Trabalho', 'Documentação da Empresa', 'criar');
  const podeDeletar = perfil === 'Admin' || temPermissao('Segurança do Trabalho', 'Documentação da Empresa', 'deletar');
  ...
  const handleNotificarVencimento = async (v) => {
    if (!v.responsavel_email) { toast.error('Responsável sem email cadastrado'); return; }
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Listas de Presença', value: resumo['Lista de Presença'], cor: 'text-green-600', bg: 'bg-green-50' },
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        {podeCriar && (
          <Button
            onClick={() => { setEditando(null); setForm(FORM_INICIAL); setShowModal(true); }}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-slate-50 rounded-xl border-2 border-dashed border-slate-200">
          ...
          <p className="text-xs text-slate-400 mt-1">Crie documentos como PCMSO, PGR e Listas de Presença</p>
        </div>
      ) : (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <Badge className={cn('text-xs border', TIPO_CORES[doc.tipo])}>{doc.tipo}</Badge>
                        <Badge className={cn('text-xs', STATUS_CORES[doc.status])}>{doc.status}</Badge>
                        {statusVenc && StatusIcon && (
                          <span className={cn('flex items-center gap-1 text-xs font-medium', statusVenc.cor)}>
                      ...
                      {doc.treinamento_nome && (
                        <p className="text-sm text-slate-500">Treinamento: {doc.treinamento_nome}</p>
                      )}
                      <div className="flex items-center gap-4 mt-1 text-xs text-slate-400">
                        {doc.data_documento && (
                          <span>📅 {format(new Date(doc.data_documento + 'T00:00:00'), 'dd/MM/yyyy')}</span>
                        )}
                        {doc.data_vencimento && (
                          <span>⏰ Vence: {format(new Date(doc.data_vencimento + 'T00:00:00'), 'dd/MM/yyyy')}</span>
                        )}
                        {doc.responsavel_tecnico && (
                          <span>👤 {doc.responsavel_tecnico}</span>
                        )}
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => { setDocSelecionado(doc); setShowVisualizarModal(true); }}
                        title="Ver detalhes"
                      >
                        <Eye className="w-4 h-4" />
                      </Button>
                      {podeEditar && (
                        <Button variant="ghost" size="sm" onClick={() => handleEditar(doc)} title="Editar">
                          <Edit className="w-4 h-4" />
                        </Button>
                      )}
                      {podeDeletar && (
                        <Button variant="ghost" size="sm" onClick={() => handleExcluir(doc)} title="Excluir">
      ...
      <Sheet open={showModal} onOpenChange={setShowModal}>
        <SheetContent side="right" className="w-full h-full overflow-y-auto !p-0 flex flex-col" data-fullscreen-modal>
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex items-end gap-2">
                  <div className="flex-1">
                    ...
                    <Select value={form.tipo} onValueChange={(v) => setForm({ ...form, tipo: v })}>
                      ...
                      <SelectContent>
                        {tiposCustom.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  <Button 
              ...
              <div className="col-span-2">
                ...
                <Input
                  ...
                  placeholder={form.tipo === 'Lista de Presença' ? 'Ex: Lista de Presença - NR-35 Jan/2026' : 'Ex: PCMSO 2026'}
                  className="mt-1.5"
                />
              </div>
          ...
          <SheetFooter className="border-t p-6 flex-shrink-0 flex gap-3">
            ...
            <Button onClick={handleSalvar} disabled={saving} className="bg-amber-500 hover:bg-amber-600 flex-1">
      ...
      <Dialog open={showNovoTipo} onOpenChange={setShowNovoTipo}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setShowNovoTipo(false); setNovoTipo(''); }}>
              Cancelar
            </Button>
            <Button 
              onClick={() => {
      ...
      <Dialog open={showVisualizarModal} onOpenChange={setShowVisualizarModal}>
        {docSelecionado && (
          <DialogContent className="max-w-xl">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                <Badge className={cn('text-xs border', TIPO_CORES[docSelecionado.tipo])}>{docSelecionado.tipo}</Badge>
                {docSelecionado.nome}
              </DialogTitle>
            </DialogHeader>
            <div className="space-y-3 py-2 text-sm">
              {docSelecionado.treinamento_nome && (
                <div><span className="font-medium text-slate-600">Treinamento:</span> {docSelecionado.treinamento_nome}</div>
              )}
              {docSelecionado.data_documento && (
                <div><span className="font-medium text-slate-600">Data:</span> {format(new Date(docSelecionado.data_documento + 'T00:00:00'), 'dd/MM/yyyy')}</div>
              )}
              ...
              {docSelecionado.responsavel_tecnico && (
                <div><span className="font-medium text-slate-600">Responsável:</span> {docSelecionado.responsavel_tecnico}</div>
              )}
              {docSelecionado.crea_responsavel && (
                <div><span className="font-medium text-slate-600">CREA/CRM:</span> {docSelecionado.crea_responsavel}</div>
              )}
              {docSelecionado.descricao && (
                <div><span className="font-medium text-slate-600">Observações:</span> {docSelecionado.descricao}</div>
              )}
                  ...
                  <div className="space-y-1.5">
                    {JSON.parse(docSelecionado.anexos || '[]').map((anexo, idx) => (
                      <div key={idx} className="flex items-center gap-2 p-2 bg-slate-50 rounded">
                        ...
                        <a href={anexo.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex-1 truncate">
                          {anexo.nome}
                        </a>
                        <a href={anexo.url} download className="text-slate-500 hover:text-slate-700">
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowVisualizarModal(false)}>Fechar</Button>
            </DialogFooter>
          </DialogContent>
        )}
        </Dialog>
         ...
         <TabsContent value="vencimentos" className="space-y-6">
           ...
           {podeCriar && (
             <Button onClick={() => { setVencimentoEditando(null); setShowVencimentoModal(true); }} className="bg-amber-500 hover:bg-amber-600 gap-2">
           ...
           {loading ? (
             ...
             <div className="space-y-3">
               {['Vencido', 'A Vencer', 'OK'].map(status => {
                 ...
                 return (
                   <div key={status}>
                     <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 mt-4">{status} ({grupo.length})</h3>
                             ...
                             <div className="flex items-center justify-between gap-3">
                               <div className="flex-1">
                                 <div className="flex items-center gap-2 mb-1">
                                   <Badge className={status === 'Vencido' ? 'bg-red-100 text-red-700' : status === 'A Vencer' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}>
                                     {status}
                                   </Badge>
                                 ...
                                 <div className="flex items-center gap-4 mt-1 text-xs text-slate-500">
                                   <span>📅 Vence: {format(new Date(v.data_vencimento + 'T00:00:00'), 'dd/MM/yyyy', { locale: ptBR })}</span>
                                   {v.responsavel_nome && <span>👤 {v.responsavel_nome}</span>}
                                 </div>
                               </div>
                               <div className="flex items-center gap-1 flex-shrink-0">
                                 {podeEditar && (
                                   <Button variant="ghost" size="sm" onClick={() => { setVencimentoEditando(v); setShowVencimentoModal(true); }} title="Editar">
                                     <Edit className="w-4 h-4" />
                                   </Button>
                                 )}
                                 {podeDeletar && (
                                   <Button variant="ghost" size="sm" onClick={() => handleExcluirVencimento(v)} title="Excluir">
                                     <Trash2 className="w-4 h-4 text-red-500" />
                                   </Button>
                                 )}
                                 <Button variant="ghost" size="sm" onClick={() => handleNotificarVencimento(v)} title="Notificar responsável">
           ...
           {showVencimentoModal && (
             <Dialog open={showVencimentoModal} onOpenChange={setShowVencimentoModal}>
               <DialogContent className="max-w-xl">
                 <DialogHeader>
                   <DialogTitle>{vencimentoEditando ? 'Editar Vencimento' : 'Novo Vencimento'}</DialogTitle>
                 </DialogHeader>
                 <VencimentoModalContent
                   vencimento={vencimentoEditando}
                   onSave={handleSalvarVencimento}
                   onCancel={() => { setShowVencimentoModal(false); setVencimentoEditando(null); }}
                 />
               </DialogContent>
             </Dialog>
           )}
        ...
        const [form, setForm] = useState(vencimento || {
        tipo: '',
        titulo: '',
        data_vencimento: '',
        data_emissao: '',
        ...
        <div className="space-y-4 py-2">
         ...
         <div>
           <Label>Título *</Label>
           <Input value={form.titulo} onChange={(e) => setForm({ ...form, titulo: e.target.value })} className="mt-1.5" />
         </div>
         <div className="grid grid-cols-2 gap-4">
           <div>
             <Label>Data de Vencimento *</Label>
             <Input type="date" value={form.data_vencimento} onChange={(e) => setForm({ ...form, data_vencimento: e.target.value })} className="mt-1.5" />
           </div>
           <div>
             <Label>Data de Emissão</Label>
             <Input type="date" value={form.data_emissao} onChange={(e) => setForm({ ...form, data_emissao: e.target.value })} className="mt-1.5" />
           </div>
         </div>
         <div className="grid grid-cols-2 gap-4">
           <div>
             <Label>Responsável</Label>
             <Input value={form.responsavel_nome} onChange={(e) => setForm({ ...form, responsavel_nome: e.target.value })} className="mt-1.5" />
           </div>
           <div>
             <Label>Email</Label>
             <Input type="email" value={form.responsavel_email} onChange={(e) => setForm({ ...form, responsavel_email: e.target.value })} className="mt-1.5" />
           </div>
         </div>
         <div>
           <Label>Alerta (dias antes do vencimento)</Label>
           <Input type="number" value={form.alerta_dias} onChange={(e) => setForm({ ...form, alerta_dias: parseInt(e.target.value) || 30 })} className="mt-1.5" />
         </div>
         <div>
           <Label>Observações</Label>
           <Textarea value={form.observacao} onChange={(e) => setForm({ ...form, observacao: e.target.value })} className="mt-1.5" rows={3} />
         </div>
        </div>
        <DialogFooter>
         <Button variant="outline" onClick={onCancel}>Cancelar</Button>
         <Button onClick={() => onSave(form)} disabled={!form.tipo || !form.titulo || !form.data_vencimento} className="bg-amber-500 hover:bg-amber-600">
