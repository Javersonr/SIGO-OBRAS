...
1. Dashboard Cloudflare → conta correta → **SSL/TLS → Custom Hostnames**
   ...
   - Se não aparecer e os UUIDs continuam no DNS: o Mocha está usando *outra* conta Cloudflare como SaaS provider. Nesse caso, os TXT records ficam no nosso DNS mas SEM efeito após removermos a referência.
