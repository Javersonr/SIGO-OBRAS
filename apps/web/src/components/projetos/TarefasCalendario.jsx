...
import { ChevronLeft, ChevronRight, Plus, FileText, Download } from 'lucide-react';
...
export default function TarefasCalendario({ projetoId, empresaAtiva, usuariosEmpresa }) {
  ...
  const getTarefasDoDia = (dia, mes, ano) => {
    ...
    return tarefas.filter(t => {
      ...
      if (t.data_inicio && t.data_fim) {
        return new Date(t.data_inicio) <= new Date(dataStr) && new Date(t.data_fim) >= new Date(dataStr);
      }
      return false;
    });
  };
  ...
  const handleDrop = async (e, dia, mes, ano) => {
    ...
    if (dragData.data_inicio && dragData.data_fim) {
      duracao = Math.ceil((new Date(dragData.data_fim) - new Date(dragData.data_inicio)) / (1000 * 60 * 60 * 24));
    }

    const dataFim = duracao > 0 
      ? new Date(new Date(novaData).getTime() + duracao * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
      : novaData;
                    ...
                    {tarefasDoDia.slice(0, 3).map(tarefa => (
                      <div
                        ...
                        className={cn(
                          ...
                          tarefa.status === 'Concluída' && "bg-green-100 border-green-300 text-green-800",
                          tarefa.status === 'Em Andamento' && "bg-blue-100 border-blue-300 text-blue-800",
                          tarefa.status === 'A Fazer' && "bg-slate-100 border-slate-300 text-slate-800",
