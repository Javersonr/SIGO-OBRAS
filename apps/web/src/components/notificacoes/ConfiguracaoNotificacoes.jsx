...
import { 
  Bell, Mail, Package, FolderKanban, DollarSign, 
  Target, Save, CheckCircle, Loader2, Play, RefreshCw
} from 'lucide-react';
...
export default function ConfiguracaoNotificacoes() {
  ...
  const disparar = async (tipo) => {
    ...
    try {
      ...
      toast.success(`Alertas disparados! ${data.notificacoes_criadas || 0} notificação(ões) criada(s), ${data.emails_enviados || 0} email(s) enviado(s).`);
    } catch (e) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Escolha como deseja ser notificado para cada tipo de alerta</p>
        </div>
      ...
      <div className="flex items-center gap-6 p-3 bg-slate-50 rounded-lg border border-slate-200 text-sm">
        <div className="flex items-center gap-2 text-slate-600">
          ...
          <span><strong>App</strong>: Notificação no sistema</span>
        </div>
        <div className="flex items-center gap-2 text-slate-600">
          <Mail className="w-4 h-4 text-blue-500" />
          <span><strong>E-mail</strong>: Notificação por e-mail</span>
        </div>
      </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3 flex-1">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${alerta.cor}`}>
                      <Icon className="w-5 h-5" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="font-semibold text-slate-800 text-sm">{alerta.label}</p>
                        {!temAcesso && <Badge variant="outline" className="text-xs">Sem permissão</Badge>}
                      </div>
                      <p className="text-xs text-slate-500 mt-0.5">{alerta.desc}</p>
                      <p className="text-xs text-slate-400 mt-1">Para perfis: {alerta.perfis.join(', ')}</p>
                    </div>
                  </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-amber-600 mt-0.5">Disparar todos os alertas agora para todos os usuários da empresa</p>
              </div>
              <Button
