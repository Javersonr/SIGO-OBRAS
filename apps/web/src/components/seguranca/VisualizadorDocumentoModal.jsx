...
export default function VisualizadorDocumentoModal({ open, onOpenChange, documento }) {
  ...
  const isImage = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'].some(ext => urlBase.endsWith('.' + ext) || nomeLower.endsWith('.' + ext));
  ...
  const content = (
    <>
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b flex-shrink-0" style={{ zIndex: 100000, backgroundColor: '#1e293b', borderColor: '#334155' }}>
          <h2 className="text-sm font-medium truncate max-w-lg" style={{ color: '#f1f5f9' }} title={nomeExibicao}>
            {nomeExibicao}
          </h2>
        ...
        {/* Content */}
        <div style={{ flex: 1, overflow: 'hidden', background: '#f1f5f9', minHeight: 0, display: 'flex', flexDirection: 'column' }}>
          {isImage ? (
