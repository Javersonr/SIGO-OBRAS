...
export default function SolicitarEntregaCaminhaoModal({
  open,
  onOpenChange,
  empresaAtiva,
  user,
  ...
  const loadCaminhaos = async () => {
    try {
      const cams = await sigo.entities.Caminhao.filter({
  ...
  const loadItens = async () => {
    ...
    try {
      const [ferr, obrig] = await Promise.all([
        ...
        sigo.entities.CaminhaoCampoObrigatorio.filter({ caminhao_id: caminhaoSelecionado.id }),
      ]);
  ...
  const toggleItem = (idx) => {
    ...
    if (!buscas[idx]) {
      setBuscas((prev) => ({
        ...prev,
        [idx]: ferramentasObrigatorias[idx]?.descricao?.split(" ").slice(0, 3).join(" ") || "",
      }));
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await sigo.entities.EntregaFerramental.create({
        empresa_id: empresaAtiva.id,
        ...
        <SheetHeader className="px-6 py-4 border-b sticky top-0 bg-white z-10">
          ...
          {step === 2 && caminhaoSelecionado && (
            <div className="flex items-center gap-2 mt-1">
              ...
              {caminhaoSelecionado.modelo && (
                <span className="text-sm text-slate-500">— {caminhaoSelecionado.modelo}</span>
              )}
              <button
                onClick={() => setStep(1)}
              ...
              {carregandoItens ? (
                ...
                <>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      className={
                        totalVinculados === ferramentasObrigatorias.length
                  ...
                  <div className="space-y-2">
                    {ferramentasObrigatorias.map((item, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className={`border rounded-lg overflow-hidden transition-all ${item.vinculado ? "border-green-300 bg-green-50" : "border-slate-200 bg-white"}`}
                        >
                          {/* Header do item */}
                          <div
                          ...
                          >
                            <div
                              className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${item.vinculado ? "bg-green-200" : "bg-slate-100"}`}
                            >
                              {item.vinculado ? (
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {item.vinculado ? (
                                <p className="text-xs text-green-700 truncate">
                                  ...
                                  {item.vinculado.numero_serie &&
                                    ` · Série: ${item.vinculado.numero_serie}`}
                                  {item.vinculado.codigo && ` · ${item.vinculado.codigo}`}
                                </p>
                              ) : (
                                <p className="text-xs text-slate-400">
                                  Clique para buscar no estoque
                                </p>
                              )}
                              {/* Indicador de laudo */}
                              {(() => {
                                const fonte =
                                  item.vinculado ||
                                  todasFerramentas.find((f) => f.id === item.ferramenta_id);
                                if (!fonte) return null;
                                const temLaudo = fonte.laudo_url || fonte.numero_laudo;
                                const laudoVencido =
                                  fonte.data_vencimento_laudo &&
                                  new Date(fonte.data_vencimento_laudo) < new Date();
                                return (
                                  <span
                            ...
                            >
                              <div className="relative mb-2">
                                ...
                                <input
                                  ...
                                  value={buscas[idx] || ""}
                                  onChange={(e) =>
                                    setBuscas((prev) => ({ ...prev, [idx]: e.target.value }))
                                  }
