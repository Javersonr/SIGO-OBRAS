...
import { Download, FileSpreadsheet, FileText, Users, Sun, CloudRain, Cloud, Wind, TrendingUp, AlertTriangle, CheckCircle2, Calendar } from 'lucide-react';
...
export default function RelatorioDiarioObra({ open, onOpenChange, diarios = [], projeto, empresaAtiva }) {
  ...
  const dadosLinha = useMemo(() => {
    return diariosFiltrados.slice().reverse().map(d => ({
  ...
  const exportarPDF = () => {
    ...
    diariosFiltrados.forEach((d, idx) => {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(10);
      const dataFmt = new Date(d.data).toLocaleDateString('pt-BR', { weekday: 'short', day: '2-digit', month: '2-digit', year: 'numeric' });
      doc.text(`${idx + 1}. ${dataFmt} | ${d.clima || ''} ${d.temperatura ? `| ${d.temperatura}°C` : ''}`, 20, y);
      y += 6;
  ...
  const exportarExcel = () => {
    ...
    XLSX.writeFile(wb, `Relatorio_Diario_Obra_${(projeto?.nome || 'Obra').replace(/\s/g, '_')}.xlsx`);
  };
        ...
        <SheetHeader className="p-6 border-b sticky top-0 bg-white z-10">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button onClick={exportarExcel} variant="outline" size="sm" className="gap-2 text-green-700 border-green-300 hover:bg-green-50">
                <FileSpreadsheet className="w-4 h-4" /> Excel
              </Button>
              <Button onClick={exportarPDF} size="sm" className="gap-2 bg-red-600 hover:bg-red-700 text-white">
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div>
                  ...
                  <Input type="date" value={filtroDataInicio} onChange={e => setFiltroDataInicio(e.target.value)} className="mt-1 h-8 text-sm" />
                </div>
                <div>
                  <Label className="text-xs">Data fim</Label>
                  <Input type="date" value={filtroDataFim} onChange={e => setFiltroDataFim(e.target.value)} className="mt-1 h-8 text-sm" />
                </div>
                <div>
              ...
              <p className="text-xs text-slate-500 mt-2">{diariosFiltrados.length} de {diarios.length} registros exibidos</p>
            </CardContent>
          </Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={dadosLinha}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Line type="monotone" dataKey="trabalhadores" stroke="#3b82f6" strokeWidth={2} name="Trabalhadores" dot={{ r: 3 }} />
                    <Line type="monotone" dataKey="fotos" stroke="#10b981" strokeWidth={2} name="Fotos" dot={{ r: 3 }} />
                  </LineChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                      <Pie data={dadosClima} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={65} label={({ name, value }) => `${climaIcon(name)} ${name}: ${value}`}>
                        {dadosClima.map((_, i) => <Cell key={i} fill={CORES[i % CORES.length]} />)}
                      </Pie>
                      <Tooltip />
                    </PieChart>
                  </ResponsiveContainer>
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={dadosMaoObra} layout="vertical">
                      ...
                      <Tooltip />
                      <Bar dataKey="total" fill="#3b82f6" name="Trabalhadores" radius={[0, 4, 4, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={dadosOcorrencias}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="total" fill="#3b82f6" name="Dias trabalhados" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="problemas" fill="#ef4444" name="Ocorrências" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
                    ...
                    {diariosFiltrados.map(d => {
                      ...
                      return (
                        <tr key={d.id} className="border-b hover:bg-slate-50 transition-colors">
                          <td className="p-3 whitespace-nowrap font-medium">{new Date(d.data).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: '2-digit' })}</td>
                          <td className="p-3">{climaIcon(d.clima)} {d.clima}</td>
                          <td className="p-3">{totalMO > 0 ? <Badge variant="outline">{totalMO}</Badge> : '-'}</td>
                          <td className="p-3 max-w-xs truncate text-slate-600">{d.atividades?.substring(0, 60)}...</td>
                          <td className="p-3">
                            {d.problemas
                              ? <Badge className="bg-red-100 text-red-700 text-xs">⚠ Sim</Badge>
                              : <Badge className="bg-green-100 text-green-700 text-xs">✓ Não</Badge>}
                          </td>
                          <td className="p-3">{fotos > 0 ? <Badge variant="outline">{fotos}</Badge> : '-'}</td>
                        </tr>
                      );
                    })}
                    {diariosFiltrados.length === 0 && (
                      <tr><td colSpan={6} className="p-8 text-center text-slate-400">Nenhum registro com os filtros selecionados</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
