...
export const gerarReciboMovimentacao = (movimentacao, empresa) => {
  ...
  // Número do recibo
  doc.setFont(undefined, 'bold');
  doc.text(`Recibo #${movimentacao.id?.substring(0, 8).toUpperCase()}`, 180, 35, { align: 'right' });
  
  // Data de emissão
  doc.setFont(undefined, 'normal');
  doc.setFontSize(9);
  doc.text(`Emitido em: ${format(new Date(), 'dd/MM/yyyy HH:mm', { locale: ptBR })}`, 180, 41, { align: 'right' });
  ...
  // Data
  doc.text(`Data da Movimentação: ${format(new Date(movimentacao.data_movimentacao), 'dd/MM/yyyy', { locale: ptBR })}`, 15, yPos);
  yPos += 5;
  ...
  if (movimentacao.tipo_movimentacao === 'Empréstimo' && movimentacao.data_prevista_devolucao) {
    doc.text(`Data Prevista de Devolução: ${format(new Date(movimentacao.data_prevista_devolucao), 'dd/MM/yyyy', { locale: ptBR })}`, 15, yPos);
    yPos += 5;
  }
