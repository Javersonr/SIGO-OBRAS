...
import CamposObrigatoriosCaminhao from '@/components/ferramental/CamposObrigatoriosCaminhao';
...
function CampoObrigatorioRow({ campo, ferramentasVinculadas, ferramentasMovimentacao, ferramentas, onToggle }) {
  ...
  const ferramentasDisponiveisParaCampo = ferramentasMovimentacao
    ...
    .filter(Boolean);
  const ferramentasFiltradas = ferramentasDisponiveisParaCampo.filter(f =>
  ...
  return (
    <div className="bg-white rounded-lg border border-amber-100 p-3">
      <div className="flex items-center justify-between mb-2">
        ...
        <Badge className={ferramentasVinculadas.length > 0 ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}>{ferramentasVinculadas.length} vinculada(s)</Badge>
      </div>
      <div className="relative mb-2">
        <Input placeholder="Buscar ferramenta para este campo..." value={buscaCampo} onChange={(e) => setBuscaCampo(e.target.value)} className="h-8 text-xs" />
        {buscaCampo && ferramentasFiltradas.length > 0 && (
          <div className="absolute z-50 left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
            {ferramentasFiltradas.map(f => {
              ...
              return (
                <div key={f.id} onClick={() => { onToggle(campo.id, f.id); setBuscaCampo(''); }}
      ...
      {ferramentasVinculadas.length > 0 && (
        <div className="space-y-1">
          {ferramentasVinculadas.map(ferrId => {
            ...
            if (!ferr) return null;
            return (
              <div key={ferrId} className="flex items-center justify-between bg-green-50 border border-green-100 rounded px-2 py-1 text-xs">
                <span className="font-medium text-green-800 truncate flex-1">{ferr.descricao}</span>
                <Button variant="ghost" size="sm" className="h-5 w-5 p-0 text-red-500 hover:text-red-700 flex-shrink-0" onClick={() => onToggle(campo.id, ferrId)}><X className="w-3 h-3" /></Button>
              </div>
            );
          })}
        </div>
...
export default function MovimentacaoModal({ open, onClose, empresaAtiva, onSave, movimentacao }) {
  ...
  const loadData = async () => {
    try {
      ...
      setFuncionarios(funcionariosUnificados.sort((a, b) => (a.nome_completo || '').localeCompare(b.nome_completo || '')));
    ...
    try {
      ...
      if (funcao.modelo_ferramentas) {
        try {
          ...
          modeloFerramentas = Array.isArray(parsed) ? parsed.map(i => ({ ...i, _tipo: 'Ferramenta' })) : [];
        } catch (e) {
          modeloFerramentas = [];
        }
      }
      ...
      const itensFuncao = todosItens.map(item => {
        ...
        const candidatos = todasFerramentas.filter(f =>
          f.status === 'Disponível' && (
  ...
  const handleSave = async () => {
    ...
    if ((form.tipo_movimentacao === 'Entrega para Funcionário' || form.tipo_movimentacao === 'Empréstimo') && !form.funcionario_id) {
    ...
    let destino = '';
    if (form.tipo_movimentacao === 'Entrega para Funcionário' || form.tipo_movimentacao === 'Empréstimo') {
      destino = form.funcionario_nome;
    ...
    try {
      ...
      if (form.tipo_movimentacao === 'Manutenção') {
        ...
        await base44.entities.ManutencaoFerramenta.create({
          ...
          ferramenta_descricao: ferramentasValidas.length === 1
      ...
      if (form.tipo_movimentacao === 'Movimentação para Caminhão' && form.almoxarifado_id) {
        ...
        await Promise.all(
          ferramentasMovimentacao.filter(f => f.ferramenta_id).map(f =>
  ...
  const handleCaminhaoSelect = async (caminhaoId) => {
    ...
    setCampoVinculacoes({});
    setForm(prev => ({ ...prev, almoxarifado_id: caminhaoId, almoxarifado_nome: caminhao?.placa || '' }));
    if (!caminhaoId) { setCamposObrigatorios([]); setFerramentasDoCaminhao([]); setFerramentasMovimentacao([]); return; }
    try {
      ...
      setCamposObrigatorios(campos.sort((a, b) => (a.nome_campo || '').localeCompare(b.nome_campo || '')));
      setFerramentasDoCaminhao(ferrsNoCaminhao.sort((a, b) => (a.descricao || '').localeCompare(b.descricao || '')));
  ...
  const adicionarFerramenta = () => {
    setFerramentasMovimentacao([...ferramentasMovimentacao, {
      ferramenta_id: '',
      codigo: '',
      descricao: '',
      numero_serie: '',
  ...
  const exportarParaExcel = async () => {
    try {
      ...
      xlsx.writeFile(wb, `ferramentas_${form.tipo_movimentacao}_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const exportarParaPDF = async () => {
    try {
      ...
      let y = 60;
      doc.setFontSize(9);
      doc.text('Código', 20, y); doc.text('Descrição', 60, y); doc.text('Qtd.', 150, y); doc.text('V. Unit.', 170, y);
      y += 10;
      ...
      try {
        ...
        reader.onload = (event) => {
          ...
          const novasFerramentas = dados.map(row => {
            ...
            const ferramentaMatching = ferramentas.find(f =>
    ...
    <SheetModalComponent
      ...
      footer={
        <div className="flex gap-2">
          ...
          <Button
            onClick={handleSave}
            disabled={saving || !form.tipo_movimentacao || (metodoEntrega !== 'funcao' && ferramentasMovimentacao.filter(f => f.ferramenta_id).length === 0) || (metodoEntrega === 'funcao' && Object.keys(selecoesFuncao).length === 0)}
            className="flex-1 bg-amber-500 hover:bg-amber-600"
          >
            {saving ? 'Salvando...' : (metodoEntrega === 'funcao' ? 'Entregar Ferramentas' : (movimentacao ? 'Atualizar' : 'Registrar Movimentação'))}
          </Button>
        </div>
      }
    >
      <div className="space-y-4">
        ...
        <div>
          ...
          <Select value={form.tipo_movimentacao} onValueChange={(v) => {
        ...
        {form.tipo_movimentacao === 'Entrega para Funcionário' && !movimentacao && (
          <div>
            ...
            <Select value={metodoEntrega} onValueChange={(v) => {
            setMetodoEntrega(v);
            setFerramentasPorFuncao([]);
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={form.almoxarifado_id} onValueChange={handleCaminhaoSelect}>
                <SelectTrigger className="mt-1.5"><SelectValue placeholder="Selecione um caminhão" /></SelectTrigger>
                <SelectContent>
                  {caminhoes.map(c => (
                    <SelectItem key={c.id} value={c.id}>
                      <span className="font-mono font-bold">{c.placa}</span>
                      {c.marca || c.modelo ? ` — ${[c.marca, c.modelo].filter(Boolean).join(' ')}` : ''}
            ...
            {caminhaoSelecionado && ferramentasDoCaminhao.length > 0 && (
              <Card className="border-blue-200 bg-blue-50">
                <CardContent className="p-3">
                  <p className="text-xs font-semibold text-blue-800 mb-2">🚛 Itens já neste caminhão ({ferramentasDoCaminhao.length})</p>
                  <div className="space-y-1 max-h-40 overflow-y-auto">
                    {ferramentasDoCaminhao.map(f => (
                      <div key={f.id} className="flex items-center justify-between bg-white rounded px-2 py-1 text-xs border border-blue-100">
                        <span className="font-medium text-slate-700 truncate flex-1">{f.descricao}</span>
                        <span className="font-mono text-slate-400 ml-2 flex-shrink-0">{f.codigo_secundario || f.codigo}</span>
                        <Badge className={`ml-2 flex-shrink-0 text-xs ${f.status === 'Disponível' ? 'bg-green-100 text-green-700' : f.status === 'Em Uso' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500'}`}>{f.status}</Badge>
                      </div>
                    ))}
                  </div>
                </CardContent>
            ...
            {caminhaoSelecionado && ferramentasDoCaminhao.length === 0 && (
              <p className="text-xs text-slate-400 italic">Nenhum item vinculado a este caminhão ainda.</p>
            )}
            {caminhaoSelecionado && camposObrigatorios.length > 0 && (
              <Card className="bg-amber-50 border-amber-200">
                <CardContent className="p-4">
                  <h4 className="font-semibold text-sm text-amber-900 mb-4">⚙️ Campos Obrigatórios — Busque e vincule ferramentas</h4>
            ...
            {caminhaoSelecionado && camposObrigatorios.length === 0 && (
              <Card className="bg-slate-50 border-slate-200">
                <CardContent className="p-3 text-xs text-slate-500 text-center">
                  Este caminhão não possui campos obrigatórios configurados.<br />Configure em Configurações → Caminhões.
                </CardContent>
              </Card>
            )}
          </div>
        ...
        {(form.tipo_movimentacao === 'Entrega para Funcionário' || form.tipo_movimentacao === 'Empréstimo') && (
          <div>
            <Label>Funcionário *</Label>
            <Select value={form.funcionario_id} onValueChange={(v) => {
              const func = funcionarios.find(f => f.id === v);
              setForm({ ...form, funcionario_id: v, funcionario_nome: func?.nome_completo || '' });
              if (metodoEntrega === 'funcao') {
                if (func?.funcao_id) handleFuncaoSelect(func.funcao_id);
                else if (func?.funcao_nome) {
                  const funcaoEncontrada = funcoes.find(f => f.nome === func.funcao_nome);
              ...
              <SelectContent>
                {funcionarios.map(f => (
                  <SelectItem key={f.id} value={f.id}>{f.nome_completo} {f.funcao_nome ? `- ${f.funcao_nome}` : ''}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
        ...
        {metodoEntrega === 'funcao' && form.funcionario_id && ferramentasPorFuncao.length > 0 && (
          <Card className="bg-blue-50 border-blue-200">
            <CardContent className="p-4">
              <h4 className="font-semibold text-sm text-blue-900 mb-3">Ferramentas da Função — Selecione cada item</h4>
              <div className="space-y-3">
                {ferramentasPorFuncao.map((item, itemIdx) => {
                  const slots = Array.from({ length: item.quantidade_necessaria }, (_, slotIdx) => {
                    ...
                    const candidatosFiltrados = item.candidatos.filter(f => {
                      const jaUsada = Object.entries(selecoesFuncao).some(([k, v]) => k !== globalIdx && v?.id === f.id);
                      if (jaUsada) return false;
                      if (!busca) return true;
                      return f.descricao?.toLowerCase().includes(busca.toLowerCase()) || f.codigo?.toLowerCase().includes(busca.toLowerCase());
                    });
                    ...
                    <div key={itemIdx} className="bg-white rounded-lg p-3 border border-blue-100">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          {item.tipo && <Badge className={item.tipo === 'EPI' ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-600'}>{item.tipo}</Badge>}
                        </div>
                        <Badge className={item.candidatos.length === 0 ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}>{item.candidatos.length} disponível(is)</Badge>
                      </div>
                      {item.candidatos.length === 0 ? (
                        <p className="text-xs text-red-600">⚠️ Nenhuma ferramenta disponível no estoque para este item</p>
                      ) : (
                              ...
                              {selecionada ? (
                                <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                                  <div className="flex-1">
                                    <span className="text-xs font-medium text-green-800">{selecionada.descricao}</span>
                                    <span className="text-xs text-slate-500 ml-2 font-mono">{selecionada.codigo}</span>
                                    {selecionada.numero_serie && <span className="text-xs text-blue-600 ml-2">SN: {selecionada.numero_serie}</span>}
                                  </div>
                                  <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-slate-400 hover:text-red-500" onClick={() => { const novas = { ...selecoesFuncao }; delete novas[globalIdx]; setSelecoesFuncao(novas); }}><X className="w-3 h-3" /></Button>
                                </div>
                              ) : (
                                <>
                                  <Input placeholder={`Buscar ferramenta para ${item.nome}...`} value={busca} onChange={(e) => setBuscaFuncao(prev => ({ ...prev, [globalIdx]: e.target.value }))} className="h-8 text-xs" />
                                  {busca && candidatosFiltrados.length > 0 && (
                                    <div className="absolute z-50 left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-xl max-h-48 overflow-y-auto">
                                      {candidatosFiltrados.map(f => (
                                        <div key={f.id} onClick={() => { setSelecoesFuncao(prev => ({ ...prev, [globalIdx]: f })); setBuscaFuncao(prev => ({ ...prev, [globalIdx]: '' })); }} className="px-3 py-2 hover:bg-slate-100 cursor-pointer border-b last:border-b-0">
                                          <div className="font-medium text-sm">{f.descricao}</div>
                                          <div className="flex gap-3 mt-0.5">
                                            <span className="text-xs text-slate-500 font-mono">{f.codigo}</span>
                                            {f.numero_serie && <span className="text-xs text-blue-600">SN: {f.numero_serie}</span>}
                                          </div>
                                        </div>
                                      ))}
                                    </div>
                                  )}
                                  {busca && candidatosFiltrados.length === 0 && <p className="text-xs text-slate-400 mt-1">Nenhuma ferramenta encontrada</p>}
                                </>
                              )}
                            </div>
                          ))}
        ...
        {metodoEntrega === 'funcao' && form.funcionario_id && ferramentasPorFuncao.length === 0 && (
          <Card className="bg-amber-50 border-amber-200">
            <CardContent className="p-4">
              <p className="text-sm text-amber-800">⚠️ Esta função não tem ferramentas padronizadas configuradas.</p>
            </CardContent>
          </Card>
        )}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <Input type="date" value={form.data_movimentacao} onChange={(e) => setForm({ ...form, data_movimentacao: e.target.value })} className="mt-1.5" />
          </div>
          {form.tipo_movimentacao === 'Empréstimo' && (
            <div>
              <Label>Data Prevista de Devolução</Label>
              <Input type="date" value={form.data_prevista_devolucao} onChange={(e) => setForm({ ...form, data_prevista_devolucao: e.target.value })} className="mt-1.5" />
            </div>
          )}
        </div>
        ...
        {form.tipo_movimentacao === 'Manutenção' && (
          <div>
            ...
            <Input value={form.motivo_manutencao} onChange={(e) => setForm({ ...form, motivo_manutencao: e.target.value })} placeholder="Ex: Calibração, Reparo, Inspeção" className="mt-1.5" />
          </div>
        )}
        ...
        {form.tipo_movimentacao === 'Baixa para Sucata' && (
          <div>
            ...
            <Input value={form.motivo_baixa} onChange={(e) => setForm({ ...form, motivo_baixa: e.target.value })} placeholder="Ex: Danificado, Obsoleto" className="mt-1.5" />
          </div>
        )}
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Button variant="outline" size="sm" className="text-xs" onClick={adicionarFerramenta}>+ Adicionar Ferramenta</Button>
          </div>
              ...
              <tbody>
                {ferramentasMovimentacao.map((ferr, idx) => {
                  ...
                  const ferramentasFiltradas = ferramentas.filter(f => {
                    ...
                    return f.descricao?.toLowerCase().includes(termo) ||
                  ...
                  return (
                    <React.Fragment key={idx}>
                      <tr className="border-b hover:bg-slate-50" style={{overflow: 'visible'}}>
                        <td className="px-3 py-2 text-slate-600 font-mono text-xs">{ferr.codigo}</td>
                        <td className="px-3 py-2 relative" style={{overflow: 'visible'}}>
                          <Input
                            ref={el => inputRefs.current[idx] = el}
                            placeholder="Digite para buscar..."
                            value={ferr.ferramenta_id ? ferr.descricao : (buscaFerramentas[idx] || '')}
                            onChange={(e) => {
                          ...
                          />
                          {!ferr.ferramenta_id && termo && ferramentasFiltradas.length > 0 && dropdownAberto === idx && (
                            <div
                              ref={dropdownRef}
                              onMouseDown={(e) => e.stopPropagation()}
                              style={{ position: 'absolute', top: '100%', left: 0, minWidth: 700, zIndex: 9999, background: 'white', border: '2px solid #cbd5e1', borderRadius: 8, boxShadow: '0 8px 32px rgba(0,0,0,0.25)', maxHeight: 320, overflowY: 'auto' }}
                            >
                              <table className="w-full text-xs">
                                <thead className="bg-slate-100 border-b border-slate-200 sticky top-0">
                                  <tr>
                                    <th className="px-3 py-2 text-left font-semibold text-slate-600 whitespace-nowrap">Código</th>
                                    <th className="px-3 py-2 text-left font-semibold text-slate-600">Descrição</th>
                                    <th className="px-3 py-2 text-left font-semibold text-slate-600 whitespace-nowrap">Localização</th>
                                    <th className="px-3 py-2 text-left font-semibold text-slate-600">Status</th>
                                    <th className="px-3 py-2 text-left font-semibold text-slate-600 whitespace-nowrap">N° Série</th>
                                    <th className="px-3 py-2 text-left font-semibold text-slate-600 whitespace-nowrap">N° Laudo</th>
                                    <th className="px-3 py-2 text-left font-semibold text-slate-600 whitespace-nowrap">Venc. Laudo</th>
                                    <th className="px-3 py-2 text-left font-semibold text-slate-600">CA</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {ferramentasFiltradas.slice(0, 15).map((f) => {
                                    const jaNosCaminhoes = ferramentasDoCaminhao.some(fc => fc.id === f.id);
                                    return (
                                      <tr
                                        key={f.id}
                                        onPointerDown={(e) => {
                                          e.preventDefault();
                                          setFerramentasMovimentacao(prev => prev.map((item, i) => i === idx ? {
                                            ...item,
                                            ferramenta_id: f.id,
                                            codigo: f.codigo,
                                            descricao: f.descricao,
                                      ...
                                      >
                                        <td className="px-3 py-2 font-mono text-slate-700 whitespace-nowrap">{f.codigo || '-'}</td>
                                        <td className="px-3 py-2 font-medium text-slate-800">
                                          {f.descricao}
                                          {jaNosCaminhoes && <span className="ml-2 text-xs bg-green-100 text-green-700 px-1.5 py-0.5 rounded">✓ neste caminhão</span>}
                                        </td>
                                        <td className="px-3 py-2 text-slate-500 whitespace-nowrap">{f.localizacao || '-'}</td>
                                        ...
                                        <td className="px-3 py-2 font-mono text-slate-600 whitespace-nowrap">{f.numero_serie || '-'}</td>
                                        <td className="px-3 py-2 font-mono text-slate-600 whitespace-nowrap">{f.numero_laudo || '-'}</td>
                                        <td className="px-3 py-2 text-slate-600 whitespace-nowrap">{f.data_vencimento_laudo ? new Date(f.data_vencimento_laudo).toLocaleDateString('pt-BR') : '-'}</td>
                                        <td className="px-3 py-2 text-slate-600">{f.ca || '-'}</td>
                                      </tr>
                                    );
                                  })}
                                </tbody>
                        ...
                        <td className="px-3 py-2 text-xs text-slate-600 font-mono whitespace-nowrap">{ferr.numero_serie || '-'}</td>
                        <td className="px-3 py-2">
                          <Input type="number" min="1" value={ferr.quantidade}
                            onChange={(e) => {
                              const novaQtd = parseInt(e.target.value) || 1;
                              atualizarFerramenta(idx, 'quantidade', novaQtd);
                              if (novaQtd > 0 && ferr.ferramenta_id && idx === ferramentasMovimentacao.length - 1) setTimeout(() => adicionarFerramenta(), 100);
                            }}
                            className="h-8 text-center w-16" />
                        </td>
                        <td className="px-3 py-2 text-right">
                          <Input type="number" step="0.01" min="0" value={ferr.valor_unitario}
                            onChange={(e) => atualizarFerramenta(idx, 'valor_unitario', parseFloat(e.target.value) || 0)}
                            className="h-8 text-right w-24" />
                        </td>
                        <td className="px-3 py-2 text-center">
                          <Button variant="ghost" size="sm" onClick={() => removerFerramenta(idx)} className="h-8 w-8 p-0">
        ...
        <div>
          ...
          <Textarea value={form.observacoes} onChange={(e) => setForm({ ...form, observacoes: e.target.value })} placeholder="Informações adicionais..." className="mt-1.5" rows={3} />
        </div>
      </div>
    </SheetModalComponent>
  );
