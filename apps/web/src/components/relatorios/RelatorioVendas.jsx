...
export default function RelatorioVendas({ dados }) {
  ...
  const kpis = useMemo(() => {
    ...
    const ganhas = oportunidades.filter(o => o.status_nome?.toLowerCase().includes('ganho') || o.status_nome?.toLowerCase().includes('aprovado')).length;
    const perdidas = oportunidades.filter(o => o.status_nome?.toLowerCase().includes('perdido') || o.status_nome?.toLowerCase().includes('cancelado')).length;
    const abertas = total - ganhas - perdidas;
    const valorTotal = oportunidades.reduce((s, o) => s + (o.valor_estimado || 0), 0);
    const valorGanho = oportunidades.filter(o => o.status_nome?.toLowerCase().includes('ganho')).reduce((s, o) => s + (o.valor_estimado || 0), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <KpiCard titulo="Total" valor={kpis.total} subtitulo="oportunidades" icon={Target} cor="blue" />
        <KpiCard titulo="Ganhas" valor={kpis.ganhas} subtitulo="oportunidades" icon={CheckCircle} cor="green" />
        <KpiCard titulo="Perdidas" valor={kpis.perdidas} subtitulo="oportunidades" icon={XCircle} cor="red" />
        <KpiCard titulo="Em Aberto" valor={kpis.abertas} subtitulo="oportunidades" icon={Clock} cor="amber" />
        <KpiCard titulo="Valor Total" valor={fmt(kpis.valorTotal)} subtitulo="estimado" icon={DollarSign} cor="purple" />
        <KpiCard titulo="Conversão" valor={`${kpis.txConversao}%`} subtitulo="taxa de ganho" icon={TrendingUp} cor="green" tendencia={kpis.txConversao >= 50 ? 'up' : 'down'} />
      </div>

      {/* Gráficos linha 1 */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle className="text-sm font-semibold text-slate-700">Oportunidades por Status</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={porStatus} layout="vertical" margin={{ left: 10 }}>
                ...
                <Tooltip formatter={(v, n) => [n === 'valor' ? fmt(v) : v, n === 'quantidade' ? 'Qtd' : 'Valor']} />
                <Legend />
        ...
        <Card>
          <CardHeader><CardTitle className="text-sm font-semibold text-slate-700">Distribuição por Valor</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie data={porStatus.slice(0, 6)} dataKey="valor" nameKey="nome" cx="50%" cy="50%" outerRadius={100} label={({ nome, percent }) => `${nome.length > 10 ? nome.slice(0,10)+'…' : nome} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle className="text-sm font-semibold text-slate-700">Evolução Mensal (Novas Oportunidades)</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={porMes}>
                ...
                <YAxis />
                <Tooltip />
                <Line type="monotone" dataKey="abertas" stroke="#3b82f6" strokeWidth={2} dot={{ r: 4 }} name="Novas" />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader><CardTitle className="text-sm font-semibold text-slate-700">Origem das Oportunidades</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={260}>
