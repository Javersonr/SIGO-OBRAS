-- ============================================================================
-- 0070_gerar_pedido_direto_fornecedor_nome.sql — corrige coluna inexistente
--
-- BUG LATENTE (plpgsql só valida referência de coluna em runtime): desde a
-- 0028, gerar_pedido_direto faz `select nome into v_fornecedor_nome from
-- fornecedor` — mas a tabela fornecedor NÃO tem coluna `nome` (só nome_razao,
-- nome_fantasia, contato_nome). A função foi criada sem erro, mas QUEBRA em
-- runtime ("column nome does not exist") na 1ª vez que alguém gera pedido
-- direto. A 0067 (identidade do emissor) preservou o mesmo erro.
--
-- FIX: `select nome_razao` — convenção do sistema inteiro (fornecedor_nome =
-- fornecedor.nome_razao em Despesas, Cotação, Recorrentes). Corpo idêntico ao
-- da 0067 (mantém identidade do emissor via token), só troca a coluna.
...
begin
  ...
  select nome_razao into v_fornecedor_nome
