...
export default function DashboardConsolidado({ grupoId, user }) {
  ...
  const carregarDadosConsolidados = async () => {
    try {
      ...
      const [vencimentosData, transacoesData, pedidosData, documentosData] = await Promise.all([
        ...
        base44.asServiceRole.entities.TransacaoFinanceira.filter({ empresa_id: { $in: empresaIds } }),
  ...
  const calcularKPIs = () => {
    ...
    const pedidosPendentes = dadosFiltrados.pedidos.filter(p => 
      p.status && (p.status.includes('Pendente') || p.status.includes('Processando'))
    ).length;
            ...
            <CardContent>
              <div className="space-y-3">
                ...
                {dadosFiltrados.vencimentos
                  ...
                  .slice(0, 5)
                  .map(v => (
                    <div key={v.id} className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-200">
                      <div className="flex-1">
                        <p className="font-medium text-slate-800">{v.titulo}</p>
                        <p className="text-xs text-slate-500">
                          {empresas.find(e => e.id === v.empresa_id)?.razao_social || v.empresa_nome}
                        </p>
                      </div>
                ...
                {dadosFiltrados.vencimentos
                  ...
                  .slice(0, 5)
                  .map(v => (
                    <div key={v.id} className="flex items-center justify-between p-3 bg-orange-50 rounded-lg border border-orange-200">
                      <div className="flex-1">
                        <p className="font-medium text-slate-800">{v.titulo}</p>
                        <p className="text-xs text-slate-500">
                          {empresas.find(e => e.id === v.empresa_id)?.razao_social || v.empresa_nome}
