...
Deno.serve(async (req) => {
  ...
  let { data: acesso } = await supabase
    ...
    .select("id, empresa_id, fornecedor_id, fornecedor_email, fornecedor_nome, senha_acesso, ativo")
  ...
  if (needsRehash) {
    try {
      ...
      console.error("[portal-fornecedor-login] rehash falhou (não-fatal):", (e as Error)?.message);
    }
  }
