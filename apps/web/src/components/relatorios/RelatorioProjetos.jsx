    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <KpiCard
          ...
          icon={AlertCircle}
          cor="red"
          tendencia={kpis.atrasados > 0 ? "down" : "up"}
        />
        <KpiCard
          titulo="Valor Total"
        ...
        />
        <KpiCard
          titulo="Valor Concluído"
          valor={fmt(kpis.valorConcluido)}
          subtitulo="entregue"
          icon={TrendingUp}
          cor="green"
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm font-semibold text-slate-700">Valor por Status</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie
                  data={porStatus.filter((d) => d.valor > 0)}
                  dataKey="valor"
                  nameKey="nome"
                  cx="50%"
                  cy="50%"
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={porCliente} layout="vertical">
                ...
                <XAxis
                  type="number"
                  tickFormatter={(v) => fmt(v).replace("R$", "R$").slice(0, 10)}
                  tick={{ fontSize: 10 }}
                />
