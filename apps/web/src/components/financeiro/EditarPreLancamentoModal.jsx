...
export default function EditarPreLancamentoModal({ open, onOpenChange, preLancamento, empresaId, contas, categorias, onSucesso }) {
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      await base44.entities.TransacaoFinanceira.update(preLancamento.id, {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {preLancamento?.comprovante_url && (
            <div className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-lg">
              {preLancamento.comprovante_url.toLowerCase().includes('.pdf') || preLancamento.comprovante_url.toLowerCase().includes('pdf') ? (
                ...
                <img src={preLancamento.comprovante_url} alt="Comprovante" className="w-12 h-12 object-cover rounded border border-slate-200 flex-shrink-0" />
              )}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-700">Comprovante anexado</p>
                <p className="text-xs text-slate-500 truncate">{preLancamento.comprovante_url.split('/').pop()}</p>
              </div>
              <Button
                variant="outline"
                size="sm"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <Label>Conta</Label>
              <Select value={form.conta_id} onValueChange={v => setForm(f => ({ ...f, conta_id: v }))}>
          ...
          <div className="flex gap-2 pt-2">
            <Button onClick={handleSalvar} disabled={loading} className="flex-1 bg-amber-600 hover:bg-amber-700">
              {loading ? <><Loader2 className="w-4 h-4 animate-spin mr-2" />Salvando...</> : 'Salvar'}
            </Button>
