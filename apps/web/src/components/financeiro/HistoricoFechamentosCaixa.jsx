...
import NovoPreLancamentoModal from './NovoPreLancamentoModal.jsx';
import AcrescentarPreLancamentoModal from './AcrescentarPreLancamentoModal.jsx';
import {
  Loader2, Package, CheckCircle2, Upload, FileText,
  ChevronDown, ChevronRight, AlertCircle, Undo2, DollarSign, ExternalLink, FileSpreadsheet, Pencil, Save, PlusCircle
} from 'lucide-react';
...
export default function HistoricoFechamentosCaixa({ empresaId, usuarioEmail, usuarioNome, podeAprovarPagamento, onReload }) {
  ...
  const carregarPreLancamentos = async (fechamento) => {
    ...
    const pls = await Promise.all(ids.map(id =>
      base44.entities.PreLancamento.filter({ id }).then(r => r[0]).catch(() => null)
    ));
  ...
  const handleImprimirComComprovantes = async (fechamento, pls) => {
    ...
    if (itens.length === 0) { alert('Nenhum item conciliado encontrado.'); return; }
    setGerandoPDFId(fechamento.id);
    try {
      const payload = {
        itens: itens.map((pl, i) => {
          const d = typeof pl.dados_extraidos === 'string' ? JSON.parse(pl.dados_extraidos || '{}') : (pl.dados_extraidos || {});
          return {
            idx: i,
            data: pl.data_competencia ? new Date(pl.data_competencia + 'T12:00:00').toLocaleDateString('pt-BR') : (d.data || '-'),
        ...
        cabecalho: {
          ...
          nomeResponsavel: fechamento.usuario_reposicao_nome || fechamento.usuario_reposicao_email || '',
          dataFormatada: fechamento.data_pagamento ? new Date(fechamento.data_pagamento + 'T12:00:00').toLocaleDateString('pt-BR') : '',
          empresa: '',
          saldoAnterior: 0,
          saldoAtual: 0
        }
  ...
  const handleImprimirPlanilha = (fechamento, pls) => {
    ...
    const dataFormatada = fechamento.data_fechamento ? new Date(fechamento.data_fechamento + 'T12:00:00').toLocaleDateString('pt-BR') : '';
    ...
    const linhasItens = itens.map((pl, i) => {
      const d = typeof pl.dados_extraidos === 'string' ? JSON.parse(pl.dados_extraidos || '{}') : (pl.dados_extraidos || {});
      const data = pl.data_competencia ? new Date(pl.data_competencia + 'T12:00:00').toLocaleDateString('pt-BR') : (d.data || '');
  ...
  const handleImprimir = (fechamento, pls) => {
    ...
    const linhas = itens.map((pl, i) => {
      const d = typeof pl.dados_extraidos === 'string' ? JSON.parse(pl.dados_extraidos || '{}') : (pl.dados_extraidos || {});
      const data = pl.data_competencia ? new Date(pl.data_competencia + 'T12:00:00').toLocaleDateString('pt-BR') : '-';
    ...
    <div className="space-y-3">
          ...
          {fechamentos.map(f => {
            ...
            return (
              <Card key={f.id} className="overflow-hidden">
                ...
                >
                  {expandido === f.id ? <ChevronDown className="w-4 h-4 text-slate-400" /> : <ChevronRight className="w-4 h-4 text-slate-400" />}
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      {editandoId === f.id ? (
                        ...
                        <span className="font-semibold text-slate-800">Fechamento #{f.numero || f.id.slice(0, 6)}</span>
                      )}
                    ...
                    <div className="text-xs text-slate-500 mt-1 flex items-center gap-2 flex-wrap">
                      <span>Fechado por {f.usuario_fechamento_nome || f.usuario_fechamento_email}</span>
                      {editandoId === f.id ? (
                        <input
                          type="date"
                          className="border rounded px-1 py-0.5 text-xs"
                          value={editValues.data_fechamento ?? f.data_fechamento ?? ''}
                          onChange={e => setEditValues(p => ({ ...p, data_fechamento: e.target.value }))}
                          onClick={e => e.stopPropagation()}
                        />
                      ) : (
                        <span>em {f.data_fechamento ? new Date(f.data_fechamento + 'T12:00:00').toLocaleDateString('pt-BR') : '-'}</span>
                      )}
                    </div>
                  </div>
                  <div className="text-right flex flex-col items-end gap-1">
                    <div className="font-bold text-red-600">
                      R$ {(f.valor_total || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </div>
                    ...
                    {f.status !== 'Pago' && editandoId !== f.id && (
                      <button
                        ...
                        onClick={e => { e.stopPropagation(); setEditandoId(f.id); setEditValues({ numero: f.numero, data_fechamento: f.data_fechamento }); }}
                      >
                    ...
                    {editandoId === f.id && (
                      <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                        ...
                        >
                          {salvandoEdicao ? <Loader2 className="w-3 h-3 animate-spin" /> : <Save className="w-3 h-3" />} Salvar
                        </button>
                        <button
                          className="text-xs text-slate-400 hover:text-slate-600"
                          onClick={() => { setEditandoId(null); setEditValues({}); }}
                        >
                          Cancelar
                        </button>
                      </div>
                ...
                {expandido === f.id && (
                  <div className="border-t bg-slate-50 px-4 pb-4 pt-3 space-y-3">
                    ...
                    <div>
                      <p className="text-xs font-semibold text-slate-500 mb-2">PRÉ-LANÇAMENTOS INCLUÍDOS</p>
                      {pls.length === 0 ? (
                        ...
                        <div className="space-y-1">
                          {pls.map(pl => {
                            ...
                            return (
                              <div key={pl.id} className={`flex items-center justify-between text-sm bg-white rounded border px-3 py-2 gap-2 ${conciliado ? 'border-green-200' : ''}`}>
                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                  {conciliado && <CheckCircle2 className="w-3.5 h-3.5 text-green-500 flex-shrink-0" />}
                                  <div className="min-w-0">
                                    <p className="text-slate-700 truncate">{d.fornecedor || pl.projeto_nome || '-'}</p>
                                    {pl.data_competencia && (
                                      <p className="text-xs text-slate-400">{new Date(pl.data_competencia + 'T12:00:00').toLocaleDateString('pt-BR')}</p>
                                    )}
                                  </div>
                                </div>
                                <div className="flex items-center gap-2 flex-shrink-0">
                                  {conciliado && <Badge className="bg-green-50 text-green-700 text-xs border-green-200">Conciliado</Badge>}
                                  ...
                                  <span className="font-semibold text-red-600">R$ {valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                  {pl.comprovante_url && (
                                    <a href={pl.comprovante_url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-700">
                    ...
                    {f.status === 'Pago' && (
                      <div className="bg-green-50 border border-green-200 rounded-lg p-3 space-y-1">
                        ...
                        <p className="text-xs text-green-600">
                          Por {f.usuario_pagamento_nome} em {f.data_pagamento ? new Date(f.data_pagamento + 'T12:00:00').toLocaleDateString('pt-BR') : '-'}
                        </p>
                        {f.observacoes_pagamento && <p className="text-xs text-green-600 italic">"{f.observacoes_pagamento}"</p>}
                        {f.comprovante_pagamento_url && (
                          <a href={f.comprovante_pagamento_url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 underline flex items-center gap-1">
                    ...
                    <div className="flex gap-2 flex-wrap">
                       ...
                       <>
                         {podeAprovarPagamento && (
                           <Button
                             ...
                             onClick={() => setAprovandoPagamentoFechamento({ fechamento: f, pls })}
                           >
                             <CheckCircle2 className="w-4 h-4" />
                             Aprovar Pagamento
                           </Button>
                     ...
                     {f.status === 'Pago' && (
                       <>
                         ...
                         >
                           {gerandoPDFId === f.id ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <FileText className="w-4 h-4 mr-1" />}
      ...
      {aprovandoPagamentoFechamento && (
        <FechamentoCaixaModal
          ...
          onOpenChange={(v) => { if (!v) setAprovandoPagamentoFechamento(null); }}
      ...
      {conciliarPL && (
        <ReconciliacaoComDespesaModal
          open={!!conciliarPL}
          onOpenChange={(v) => { if (!v) setConciliarPL(null); }}
      ...
      {acrescentandoEmFechamento && (
        <AcrescentarPreLancamentoModal
          ...
          onOpenChange={(v) => { if (!v) setAcrescentandoEmFechamento(null); }}
      ...
      {desfazendoConciliacaoPL && (
        <DesfazerConciliacaoModal
          ...
          onOpenChange={(v) => { if (!v) setDesfazendoConciliacaoPL(null); }}
          preLancamento={desfazendoConciliacaoPL}
          transacao={desfazendoConciliacaoPL.transacao_id ? { id: desfazendoConciliacaoPL.transacao_id } : null}
          onSucesso={() => {
  ...
  const handleDesfazer = async () => {
    ...
    try {
      ...
      for (const pl of plsFiltrados) {
        if (pl.transacao_id) {
          try {
            const anexos = await base44.entities.TransacaoAnexo.filter({ transacao_id: pl.transacao_id });
            await Promise.all(anexos.map(a => base44.entities.TransacaoAnexo.delete(a.id)));
          } catch {}
          try { await base44.entities.TransacaoFinanceira.delete(pl.transacao_id); } catch {}
        }
        await base44.entities.PreLancamento.update(pl.id, { status: 'Em Fechamento', transacao_id: null });
      }
      ...
      try {
        ...
        const reposicao = transacoesReposicao.find(t => t.descricao?.includes(`Fechamento #${fechamento.numero}`));
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-4">
          <p className="text-sm text-slate-600">
            Deseja desfazer o pagamento do fechamento #{fechamento.numero}? As transações financeiras geradas serão removidas e os pré-lançamentos voltarão para <strong>Em Fechamento</strong>.
          </p>
          <div className="bg-orange-50 border border-orange-200 rounded p-3 text-sm text-orange-800">
            ⚠️ Esta ação irá deletar todas as despesas e a receita de reposição geradas neste pagamento.
          </div>
          <div className="flex gap-2">
            <Button variant="outline" onClick={onClose} disabled={desfazendo} className="flex-1">Cancelar</Button>
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-4">
          <p className="text-sm text-slate-600">
            Deseja desfazer o fechamento #{fechamento.numero}? Os pré-lançamentos voltarão para a lista de pendentes.
          </p>
          <div className="flex gap-2">
            <Button variant="outline" onClick={onClose} disabled={desfazendo} className="flex-1">Cancelar</Button>
            <Button onClick={handleDesfazer} disabled={desfazendo} className="flex-1 bg-red-600 hover:bg-red-700">
