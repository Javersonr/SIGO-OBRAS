...
export default function ClientesTab({ empresaAtiva, clientes, loadData }) {
  ...
  const handleExportarClientesExcel = () => {
    const dados = clientes.map(c => [
      c.nome_razao || '', c.nome_fantasia || '', c.tipo_pessoa || '', c.documento || '',
      c.contato_email || '', c.contato_telefone || '', c.endereco || '', c.numero || '',
      c.complemento_bairro || '', c.cidade || '', c.estado || '', c.cep || '',
      c.contato_principal || '', c.observacoes || ''
    ]);
    const headers = ['Nome', 'Nome Fantasia', 'Tipo', 'CPF/CNPJ', 'Email', 'Telefone', 'Endereço', 'Número', 'Complemento Bairro', 'Cidade', 'Estado', 'CEP', 'Contato Principal', 'Observações'];
  ...
  const handleExportarClientesPDF = async () => {
    ...
    doc.setFontSize(16); doc.text('Lista de Clientes', 14, 15);
    doc.setFontSize(10); doc.text(`Gerado em: ${new Date().toLocaleDateString('pt-BR')}`, 14, 22);
    let y = 30; doc.setFontSize(8);
    doc.text('Nome', 14, y); doc.text('Tipo', 80, y); doc.text('CPF/CNPJ', 100, y);
    doc.text('Email', 140, y); doc.text('Telefone', 190, y); doc.text('Cidade', 230, y);
    y += 5; doc.line(14, y, 280, y); y += 5;
    clientes.forEach(c => {
      if (y > 190) { doc.addPage(); y = 20; }
      doc.text((c.nome_razao || '').substring(0, 30), 14, y); doc.text(c.tipo_pessoa || '-', 80, y);
      doc.text(c.documento || '-', 100, y); doc.text((c.contato_email || '-').substring(0, 25), 140, y);
      doc.text(c.contato_telefone || '-', 190, y); doc.text((c.cidade || '-').substring(0, 20), 230, y);
      y += 6;
    });
  ...
  const handleBaixarModeloClientes = () => {
    const csv = ['Nome;Nome Fantasia;Tipo;CPF/CNPJ;Email;Telefone;Endereço;Número;Complemento Bairro;Cidade;Estado;CEP;Contato Principal;Observações', 'EXEMPLO CLIENTE;;PJ;;;;;;;;;;;'].join('\n');
    const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a'); link.href = URL.createObjectURL(blob);
    link.download = 'modelo_importacao_clientes.csv'; link.click();
  };

  const handleLimparTodosClientes = async () => {
    if (!confirm('Tem certeza que deseja excluir TODOS os clientes? Esta ação não pode ser desfeita.')) return;
    const todos = await base44.entities.Cliente.filter({ empresa_id: empresaAtiva.id });
    for (const c of todos) { await base44.entities.Cliente.update(c.id, { ativo: false }); }
    ...
    reader.onload = async (event) => {
      try {
        ...
        const batch = data.map(v => ({
          ...
          documento: v[3] || '', contato_email: v[4] || '', contato_telefone: v[5] || '',
          endereco: v[6] || '', numero: v[7] || '', complemento_bairro: v[8] || '',
          cidade: v[9] || '', estado: v[10] || '', cep: v[11] || '',
          contato_principal: v[12] || '', observacoes: v[13] || '', ativo: true
        }));
        const BATCH_SIZE = 200; let importados = 0;
        for (let i = 0; i < batch.length; i += BATCH_SIZE) {
          await base44.entities.Cliente.bulkCreate(batch.slice(i, i + BATCH_SIZE));
  ...
  const handleSaveCliente = async () => {
    ...
    try {
      if (selectedCliente) {
        await base44.entities.Cliente.update(selectedCliente.id, clienteForm);
      } else {
  ...
  const handleDeleteCliente = async cliente => {
    ...
    await base44.entities.Cliente.update(cliente.id, { ativo: false });
    loadData();
  };
  ...
  const handleDeletarSelecionados = async () => {
    ...
    for (const id of selectedClienteIds) {
      await base44.entities.Cliente.update(id, { ativo: false });
    }
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <input id="fileInputClientes" type="file" className="hidden" accept=".csv" onChange={handleImportarClientes} />
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuItem onClick={() => document.getElementById('fileInputClientes')?.click()}>
                  <FileText className="w-4 h-4 mr-2" />
                  Importar Clientes
                </DropdownMenuItem>
                  ...
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => handleDeleteCliente(c)}>
          ...
          {clientes.filter(c =>
            ...
            <div className="flex justify-between items-center mt-4 pt-4 border-t">
              <p className="text-sm text-slate-600">
                Mostrando {(clientesPage - 1) * 50 + 1} a {Math.min(clientesPage * 50, clientes.filter(c => searchCliente === '' || c.nome_razao?.toLowerCase().includes(searchCliente.toLowerCase()) || c.nome_fantasia?.toLowerCase().includes(searchCliente.toLowerCase()) || c.contato_email?.toLowerCase().includes(searchCliente.toLowerCase()) || c.contato_telefone?.includes(searchCliente) || c.documento?.includes(searchCliente)).length)} de{' '}
                {clientes.filter(c => searchCliente === '' || c.nome_razao?.toLowerCase().includes(searchCliente.toLowerCase()) || c.nome_fantasia?.toLowerCase().includes(searchCliente.toLowerCase()) || c.contato_email?.toLowerCase().includes(searchCliente.toLowerCase()) || c.contato_telefone?.includes(searchCliente) || c.documento?.includes(searchCliente)).length} clientes
              </p>
              <div className="flex gap-2">
                <Button
                  variant="outline"
        ...
        <SheetContent side="right" className="h-full overflow-y-auto p-0 flex flex-col">
          ...
          <div className="space-y-4 py-4 px-6 flex-1">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label>Tipo *</Label>
                <Select value={clienteForm.tipo_pessoa} onValueChange={v => setClienteForm({ ...clienteForm, tipo_pessoa: v })}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <Input
                    ...
                    onChange={e => setClienteForm({ ...clienteForm, contato_principal: e.target.value })}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={e => setClienteForm({ ...clienteForm, contato_email: e.target.value })}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={e => setClienteForm({ ...clienteForm, contato_telefone: e.target.value })}
