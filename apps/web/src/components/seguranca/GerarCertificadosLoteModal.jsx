...
export default function GerarCertificadosLoteModal({
  open,
  onOpenChange,
  funcionarios,
  empresaAtiva,
  ...
  useEffect(() => {
    ...
    const carregarTreinamentos = async () => {
      setCarregando(true);
      try {
        const todos = await sigo.entities.Treinamento.filter({
  ...
  funcionarios?.forEach((f) => {
    const treinamentosDaFuncao = treinamentos.filter(
      (t) => t.funcao_id === f.funcao_id || t.usar_como_modelo
    );
      ...
      for (const item of itensParaGerar) {
        try {
          ...
          resultados.detalhes.push({
            funcionario: item.funcionario.nome_completo,
      ...
      if (resultados.sucesso > 0) {
        if (resultados.sucesso === 1) {
          ...
          if (unico) {
            ...
            doc.save(
              `certificado_${unico.funcionario.nome_completo.replace(/[^a-z0-9]/gi, "_")}.pdf`
            );
          }
        } else {
          ...
          a.href = url;
          a.download = `certificados_lote_${format(new Date(), "yyyy-MM-dd")}.zip`;
          ...
          <div className="space-y-4 overflow-y-auto flex-1">
            <div className="flex items-center gap-2 p-3 bg-slate-50 rounded-lg border">
              ...
              <span className="text-sm font-medium">
                Selecionar Todos ({totalSelecionados}/{itens.length})
              </span>
            </div>
            ...
            <div className="overflow-y-auto max-h-64 space-y-2 border rounded-lg p-3">
              {itens.length === 0 ? (
                <p className="text-sm text-slate-500 text-center py-4">
                  Nenhuma combinação funcionário/treinamento encontrada
                </p>
              ) : (
                itens.map((item) => (
                  <div
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        ...
                        if (checked) novos[item.id] = true;
                        else delete novos[item.id];
                        setSelecionados(novos);
                      }}
                      disabled={gerando}
                    />
            ...
            <div className="flex gap-2 justify-end">
              ...
              >
                {gerando ? (
                  ...
                  <>
                    <Download className="w-4 h-4" /> Gerar ({totalSelecionados})
                  </>
                )}
              </Button>
            </div>
