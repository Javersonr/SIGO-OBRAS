...
export default function IndicesFinanceiros({ transacoes, contas, versao = 'real' }) {
  ...
  const liquidezSeca = passivoCirculante > 0
    ? (ativoCirculante * 0.8) / passivoCirculante
    : 0;

  const liquidezImediata = passivoCirculante > 0
    ? (ativoCirculante * 0.6) / passivoCirculante
    : 0;

  // ========================================
  // ÍNDICES DE RENTABILIDADE
  // ========================================
  const margemLiquida = receitas > 0 
    ? (lucroLiquido / receitas) * 100 
    : 0;

  const roe = patrimonioLiquido > 0 
    ? (lucroLiquido / patrimonioLiquido) * 100 
    : 0;

  const roa = ativoTotal > 0 
    ? (lucroLiquido / ativoTotal) * 100 
    : 0;

  // ========================================
  // ÍNDICES DE ENDIVIDAMENTO
  // ========================================
  const grauEndividamento = ativoTotal > 0
    ? (passivoCirculante / ativoTotal) * 100
    : 0;

  const composicaoEndividamento = passivoCirculante > 0
    ? (passivoCirculante / passivoCirculante) * 100
    : 0;

  const imobilizacaoPatrimonio = patrimonioLiquido > 0
    ? ((ativoTotal - ativoCirculante) / patrimonioLiquido) * 100
    : 0;
  ...
  const prazoMedioRecebimento = receitas > 0
    ? (ativoCirculante * 0.4 / receitas) * 30
    : 0;
  ...
  const IndexCard = ({ icon: Icon, title, value, subtitle, status, description }) => (
    <Card className="hover:shadow-md transition-shadow">
      <CardContent className="p-4">
        ...
        {description && (
          <p className="text-xs text-slate-400 mt-2 pt-2 border-t">{description}</p>
        )}
      </CardContent>
    </Card>
  );
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <ul className="text-sm text-orange-800 space-y-1">
                  {liquidezCorrente < 1 && (
                    <li>• Liquidez Corrente abaixo do ideal - dificuldade em honrar compromissos</li>
                  )}
                  {margemLiquida < 0 && (
                    <li>• Margem Líquida negativa - empresa operando com prejuízo</li>
                  )}
                  {grauEndividamento > 70 && (
                    <li>• Endividamento elevado - mais de 70% dos ativos são financiados por terceiros</li>
                  )}
                </ul>
              </div>
            </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <IndexCard
            ...
            status={liquidezCorrente >= 1.5 ? 'good' : liquidezCorrente >= 1 ? 'warning' : 'critical'}
          ...
          <IndexCard
            ...
            status={liquidezImediata >= 0.5 ? 'good' : liquidezImediata >= 0.3 ? 'warning' : 'critical'}
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <IndexCard
            ...
            status={grauEndividamento <= 50 ? 'good' : grauEndividamento <= 70 ? 'warning' : 'critical'}
          ...
          <IndexCard
            ...
            status={composicaoEndividamento <= 40 ? 'good' : composicaoEndividamento <= 60 ? 'warning' : 'critical'}
          ...
          <IndexCard
            ...
            status={imobilizacaoPatrimonio <= 70 ? 'good' : imobilizacaoPatrimonio <= 90 ? 'warning' : 'critical'}
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <IndexCard
            ...
            status={prazoMedioRecebimento <= 30 ? 'good' : prazoMedioRecebimento <= 45 ? 'warning' : 'critical'}
      ...
      <Card className="bg-blue-50 border-blue-200">
        <CardContent className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-blue-800">
            ...
            <div>
              <strong>Rentabilidade:</strong> Mede o lucro gerado. Valores positivos e crescentes são ideais.
            </div>
            <div>
              <strong>Endividamento:</strong> Mede a dependência de capital de terceiros. Quanto menor, melhor.
            </div>
            <div>
              <strong>Atividade:</strong> Mede a eficiência operacional. Valores altos indicam boa gestão.
            </div>
          </div>
        </CardContent>
      </Card>
