...
export default function ConciliacaoBancaria({ empresaAtiva, contas, onReload }) {
  ...
  const extratosPendentes = extratosBancarios.filter((e) => e.status_conciliacao === "pendente");
  const extratosConciliados = extratosBancarios.filter(
    (e) => e.status_conciliacao === "conciliado"
  );
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold text-slate-900">{transacoesPendentes.length}</p>
              </CardContent>
            </Card>
          </div>
          ...
          <Tabs defaultValue="pendentes">
            <TabsList>
              ...
              <TabsTrigger value="conciliadas">
                Conciliadas ({extratosConciliados.length})
              </TabsTrigger>
            </TabsList>
                    ...
                    <tbody>
                      {extratosPendentes.map((e) => (
                        <tr key={e.id} className="border-b hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3 text-sm">
                            <span
                              className={e.tipo === "credito" ? "text-green-600" : "text-red-600"}
                            >
                            ...
                            <div className="flex gap-2 justify-center">
                              ...
                              {(() => {
                                ...
                                const sugestoes = transacoesPendentes.filter((t) => {
                                  const valorMatch = Math.abs(t.valor - e.valor) <= 0.1;
                                  const diasDiff = Math.abs(
                    ...
                    {extratosConciliados.map((e) => (
                      <tr key={e.id} className="border-b hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3 text-sm text-slate-600">
                          {e.data_conciliacao
                            ? new Date(e.data_conciliacao).toLocaleDateString("pt-BR")
                            : "-"}
                        </td>
                      </tr>
                    ))}
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <p className="font-semibold">
                        {new Date(itemSelecionado.data).toLocaleDateString("pt-BR")}
                      </p>
                    </div>
                    <div>
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {(() => {
                    ...
                    const scored = transacoesPendentes.map((t) => {
                      ...
                      else if (diffValor <= 0.1) score += 30;
                      ...
                      if (
                        descExtrato.includes(descTransacao) ||
                        descTransacao.includes(descExtrato)
                      ) {
                        score += 30;
                      } else {
                        const palavrasExtrato = descExtrato.split(" ");
                        const palavrasTransacao = descTransacao.split(" ");
                        const palavrasComuns = palavrasExtrato.filter(
                          (p) => palavrasTransacao.includes(p) && p.length > 3
                        );
                    ...
                    return sorted.map((t) => (
                      ...
                      >
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          {Math.abs(
                            (new Date(t.data) - new Date(itemSelecionado.data)) /
                              (1000 * 60 * 60 * 24)
                          ) <= 1 && (
