...
export default function AlertasVencimento() {
  ...
  const [dismissed, setDismissed] = useState(() => {
    try { return JSON.parse(sessionStorage.getItem('alertas_dismissed') || '[]'); } catch { return []; }
  });
  ...
  useEffect(() => {
    ...
    const mapa = {};
    empresas.forEach(e => { mapa[e.id] = e.razao_social || e.nome_fantasia || e.nome; });
    setEmpresasMap(mapa);
    ...
    <>
      <div className={cn(
        ...
        <div className={cn(
          ...
          >
            <AlertTriangle className={cn("w-5 h-5 shrink-0", urgentes.length > 0 ? "text-red-600" : "text-orange-600")} />
            <span className={cn("text-sm font-semibold", urgentes.length > 0 ? "text-red-800" : "text-orange-800")}>
              {alertasVisiveis.length} conta{alertasVisiveis.length > 1 ? 's' : ''} a pagar vencendo nos próximos 3 dias
              {urgentes.length > 0 && (
            ...
            {expandido ? <ChevronUp className="w-4 h-4 ml-auto text-slate-500" /> : <ChevronDown className="w-4 h-4 ml-auto text-slate-500" />}
          </button>
          <button
            onClick={dismissAll}
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-slate-800 truncate">{t.descricao || 'Sem descrição'}</p>
                      <p className="text-xs text-slate-500 truncate">
                        {[
                          isDespesa(t) ? (t.fornecedor_nome || t.categoria_nome) : (t.cliente_nome || t.categoria_nome),
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={cn("text-sm font-bold", isDespesa(t) ? "text-red-700" : "text-green-700")}>{formatCurrency(t.valor)}</span>
                    <button
