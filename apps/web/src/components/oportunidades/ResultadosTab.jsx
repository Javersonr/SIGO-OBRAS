...
export default function ResultadosTab({ oportunidadeId, empresaAtiva, orcamentoItens }) {
  ...
  const totalReceitas = receitas.filter(r => r.status === 'pago').reduce((s, r) => s + (r.valor || 0), 0);
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <Card className={resultado >= 0 ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}>
        ...
        <Card className={resultadoPrevisto >= 0 ? 'border-blue-200 bg-blue-50' : 'border-orange-200 bg-orange-50'}>
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-slate-700">Resultado Previsto</span>
              <DollarSign className="w-5 h-5 text-blue-600" />
            </div>
            <p className={`text-3xl font-bold ${resultadoPrevisto >= 0 ? 'text-blue-700' : 'text-orange-700'}`}>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-slate-800">{formatCurrency(totalReceitasPrevistas)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">Recebido:</span>
                  <span className="font-semibold text-green-700">{formatCurrency(totalReceitas)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">A Receber:</span>
                  <span className="font-semibold text-blue-700">{formatCurrency(totalReceitasPrevistas - totalReceitas)}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">Previsto:</span>
                  <span className="font-semibold text-slate-800">{formatCurrency(totalDespesasPrevistas)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">Pago:</span>
                  <span className="font-semibold text-red-700">{formatCurrency(totalDespesas)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">A Pagar:</span>
                  <span className="font-semibold text-blue-700">{formatCurrency(totalDespesasPrevistas - totalDespesas)}</span>
                </div>
              </div>
            </div>
          </div>
                ...
                {diferencaOrcamento > 0 ? (
                  <>
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-red-700">{formatCurrency(Math.abs(diferencaOrcamento))}</p>
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-green-700">{formatCurrency(Math.abs(diferencaOrcamento))}</p>
                    </div>
                  </>
                )}
              </div>
              <Badge className={diferencaOrcamento > 0 ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}>
                {diferencaOrcamento > 0 ? '+' : ''}{((diferencaOrcamento / totalOrcado) * 100).toFixed(1)}%
              </Badge>
            </div>
          </div>
        </CardContent>
