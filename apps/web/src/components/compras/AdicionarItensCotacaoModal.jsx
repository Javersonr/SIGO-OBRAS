...
export default function AdicionarItensCotacaoModal({ open, onOpenChange, empresaAtiva, onSave }) {
  ...
  // Itens manuais
  const [itensManual, setItensManual] = useState([
  ...
  const buscarCotacao = async () => {
    ...
    setBuscando(true);
    try {
      const results = await sigo.entities.Cotacao.filter({
        empresa_id: empresaAtiva.id,
        numero: numeroBusca.trim(),
      });
      if (results.length > 0) {
  ...
  const selecionarSolicitacao = async (sol) => {
    ...
    const itens = await sigo.entities.SolicitacaoCompraItem.filter({ solicitacao_id: sol.id });
  ...
  const selecionarCotacaoOrigem = async (cot) => {
    ...
    const itens = await sigo.entities.CotacaoItem.filter({ cotacao_id: cot.id });
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      if (abaAtiva === "manual") {
        ...
        if (itensSolSelecionados.length === 0) {
        ...
        if (itensCotSelecionados.length === 0) {
          alert("Selecione pelo menos um item");
          setSalvando(false);
          return;
        }
      ...
      <SheetContent side="right" className="h-full p-0 flex flex-col" data-fullscreen-modal>
              ...
              <Card className="border-blue-200 bg-blue-50">
                <CardContent className="p-4">
                  ...
                  <p className="text-sm text-blue-600">
                    {cotacaoEncontrada.projeto_nome || "Sem projeto"}
                  </p>
                    ...
                    <Card key={idx}>
                      <CardContent className="p-4 space-y-3">
                        <div className="flex items-start gap-2">
                          <span className="text-sm text-slate-500 mt-2 w-6 flex-shrink-0">
                            {idx + 1}.
                          </span>
                          <div className="flex-1 space-y-2">
                            ...
                            <div className="flex gap-2">
                              <Input
                                ...
                                onChange={(e) =>
                                  updateItemManual(idx, "quantidade", e.target.value)
                                }
                                className="w-24"
                              />
                              <Input
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  updateItemManual(idx, "especificacoes", e.target.value)
                                }
                                className="flex-1"
                              />
                            </div>
                  ...
                  {!solicitacaoSelecionada ? (
                    <div className="space-y-2">
                      ...
                      {solicitacoes.map((sol) => (
                        <button
                          key={sol.id}
                          onClick={() => selecionarSolicitacao(sol)}
                        ...
                        >
                          ...
                          <p className="text-xs text-slate-500">
                            {sol.projeto_nome || "Sem projeto"} • {sol.status}
                          </p>
                        </button>
                      ))}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2 mb-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setItensSolSelecionados(itensSolicitacao.map((i) => i.id))}
                        >
                          Selecionar Todos
                        </Button>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setItensSolSelecionados([])}
                        >
                          Limpar
                        </Button>
                      </div>
                      {itensSolicitacao.map((item) => (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            {itensSolSelecionados.includes(item.id) && (
                  ...
                  {!cotacaoOrigem ? (
                    <div className="space-y-2">
                      ...
                      {cotacoes.map((cot) => (
                        <button
                          key={cot.id}
                          onClick={() => selecionarCotacaoOrigem(cot)}
                          className="w-full text-left p-3 bg-slate-50 hover:bg-slate-100 rounded-lg border border-slate-200 transition-colors"
                        >
                          <p className="font-medium text-sm">{cot.numero}</p>
                          <p className="text-xs text-slate-500">
                            {cot.projeto_nome || "Sem projeto"} • {cot.status}
                          </p>
                        </button>
                      ))}
                      {cotacoes.length === 0 && (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2 mb-2">
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            setItensCotSelecionados(itensCotacaoOrigem.map((i) => i.id))
                          }
                        >
                          Selecionar Todos
                        </Button>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setItensCotSelecionados([])}
                        >
                          Limpar
                        </Button>
                      </div>
                      {itensCotacaoOrigem.map((item) => (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            {itensCotSelecionados.includes(item.id) && (
                              <Check className="w-4 h-4 text-blue-600 flex-shrink-0" />
                            )}
                            <div className="flex-1">
                              <p className="text-sm font-medium">{item.descricao}</p>
        ...
        <div className="sticky bottom-0 bg-white border-t p-6 flex-shrink-0 flex justify-end gap-3">
          ...
          <Button
            ...
            disabled={!cotacaoEncontrada || salvando}
