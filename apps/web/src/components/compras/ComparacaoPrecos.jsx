...
import { Trophy, Check, AlertCircle, Upload, Link2, Eye, Trash2, MoreVertical, Download, FileSpreadsheet, X, RefreshCw } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
function TotalRow({ itens, fornecedores, precos, totais, collapsedCols, formatMoeda }) {
  ...
  return (
    <tr className="bg-slate-100 font-bold">
      <td colSpan="3" className="px-2 py-2 border-r" style={{fontSize:10}}>TOTAL GERAL</td>
      <td className="px-2 py-2 border-r text-center text-blue-700 bg-blue-100" style={{fontSize:10}}>-</td>
      <td className="px-2 py-2 border-r text-center text-green-700 bg-green-100" style={{fontSize:10}}>
        ...
        return (
          <td key={f.id} className="px-2 py-2 border-r text-center" style={{fontSize:10}}>
            <span className={ehMenor ? 'text-green-700' : ''}>
              {formatMoeda(total)}{ehMenor && <Trophy className="w-3 h-3 inline ml-1" />}
            </span>
          </td>
        );
      })}
...
export default function ComparacaoPrecos({ 
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      if (solItemIds.length > 0 && cotacao.solicitacao_id) {
        try {
          const solItens = await base44.entities.SolicitacaoCompraItem.filter({ solicitacao_id: cotacao.solicitacao_id });
          solItens.forEach(si => { solItensMap[si.id] = si; });
        } catch(e) { /* ignora */ }
      }
  ...
  const salvarPreco = async (itemId, fornecedorCotacaoId, valor) => {
    ...
    try {
      ...
      if (cotacao.status === 'Enviada aos Fornecedores' || cotacao.status === 'Aguardando Respostas') {
  ...
  const navegarProximoCampo = (itemAtual, fornecedorId) => {
    ...
    if (idxAtual < itensOrdenados.length - 1) {
      ...
      setTimeout(() => {
        const input = document.querySelector(`input[data-campo="${proximoItem.id}_${fornecedorId}"]`);
        if (input) {
          input.focus();
          input.select();
        }
  ...
  const handleReabrirFornecedor = async (fornecedorCotId) => {
    ...
    if (!fornecedor || !confirm(`Reabrir cotação para ${fornecedor.fornecedor_nome}? O fornecedor poderá modificar os preços novamente.`)) return;
    ...
    if (!loading && itens.length > 0 && fornecedores.length > 0) {
      ...
      itens.forEach(item => {
        ...
        fornecedores.forEach(f => {
          ...
          if (p > 0 && p < menorPreco) { menorPreco = p; menorFornId = f.id; }
        });
  ...
  const handleAprovarPorItem = async () => {
    ...
    const nomesVencedores = fornecedoresVencedores.map(fId => fornecedores.find(f => f.id === fId)?.fornecedor_nome).filter(Boolean).join(', ');

    if (!confirm(`Confirmar aprovação por item?\n\nFornecedores selecionados: ${nomesVencedores}\nTotal: ${formatMoeda(totalAprovado)}`)) return;
    ...
    try {
      ...
      const contagem = {};
      Object.values(itensSelecionados).forEach(fId => { contagem[fId] = (contagem[fId] || 0) + 1; });
      ...
      await base44.entities.Cotacao.update(cotacao.id, {
        ...
        fornecedor_vencedor_nome: fornecedoresVencedores.length > 1 ? `Split: ${nomesVencedores}` : (principal?.fornecedor_nome || ''),
        valor_aprovado: totalAprovado
      });

      if (cotacao.solicitacao_id) {
        await base44.entities.SolicitacaoCompra.update(cotacao.solicitacao_id, { status: 'Cotação Aprovada' });
      }
  ...
  const handleSalvarTodos = async () => {
    ...
    try {
      ...
      Object.keys(precos).forEach(chave => {
        ...
        if (valorAtual !== '' && valorAtual !== null && valorAtual !== undefined && valorAtual > 0) {
  ...
  const exportarModelo = () => {
    ...
    const colunas = ['Código', 'Descrição', 'Quantidade', 'Unidade', ...fornecedores.map(f => f.fornecedor_nome)];
    ...
    return (
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent 
          ...
          style={{ position: 'fixed', inset: 0, width: '100vw', maxWidth: '100vw', height: '100vh', zIndex: 9999 }}
        >
  ...
  const startResize = (e, colKey) => {
    ...
    const onMove = (ev) => setColWidths(prev => ({ ...prev, [colKey]: Math.max(60, startW + ev.clientX - startX) }));
    const onUp = () => { window.removeEventListener('mousemove', onMove); window.removeEventListener('mouseup', onUp); };
    window.addEventListener('mousemove', onMove);
    window.addEventListener('mouseup', onUp);
  };

  const toggleCollapse = (colKey) => setCollapsedCols(prev => ({ ...prev, [colKey]: !prev[colKey] }));
    ...
    <>
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent 
          ...
          style={{ position: 'fixed', inset: 0, width: '100vw', maxWidth: '100vw', height: '100vh', maxHeight: '100vh', zIndex: 9999 }}
        >
          <div className="sticky top-0 bg-white border-b px-3 py-1 z-10 flex-shrink-0 flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-700">Comparação de Preços - {cotacao?.numero}</span>
            <button onClick={() => onOpenChange(false)} className="p-1 hover:bg-slate-100 rounded lg:hidden">
          ...
          <Tabs defaultValue="manual" className="w-full">
            <TabsList className="grid w-full grid-cols-3 mb-2 h-7">
              <TabsTrigger value="manual" className="text-xs py-0">Digitação Manual</TabsTrigger>
              <TabsTrigger value="excel" className="text-xs py-0">
                <Upload className="w-3 h-3 mr-1" />Importar Excel
              </TabsTrigger>
              <TabsTrigger value="links" disabled className="text-xs py-0">
                <Link2 className="w-3 h-3 mr-1" />Links Fornecedores
              </TabsTrigger>
            </TabsList>
              ...
              <div className="flex gap-2 items-center">
                ...
                <div className="bg-slate-50 border rounded px-3 py-1 flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-green-600">
                    {formatMoeda(itens.reduce((acc, item) => {
                      const precosItem = fornecedores.map(f => precos[`${item.id}_${f.id}`]).filter(p => p > 0);
                      const menor = precosItem.length > 0 ? Math.min(...precosItem) : 0;
                      return acc + menor * (item.quantidade || 1);
                    }, 0))}
              ...
              <Card>
                ...
                <CardContent className="p-0">
                  <div style={{overflowX:'auto', overflowY:'auto', maxHeight:'72vh'}}>
                    <table className="border-collapse" style={{fontSize:10, tableLayout:'fixed', width:'max-content', minWidth:'100%'}}>
                      <thead className="bg-slate-50 sticky top-0 z-20">
                        <tr>
                          {/* Código */}
                          <th style={{width: colWidths['cod']||70, minWidth:40, position:'relative', fontSize:10}} className="text-left px-2 py-1 font-semibold border-b border-r whitespace-normal break-words">
                            Código
                            <div onMouseDown={(e)=>startResize(e,'cod')} style={{position:'absolute',right:0,top:0,bottom:0,width:5,cursor:'col-resize',background:'transparent'}} />
                          </th>
                          {/* Item */}
                          <th style={{width: colWidths['item']||220, minWidth:80, position:'relative', fontSize:10}} className="text-left px-2 py-1 font-semibold border-b border-r whitespace-normal break-words">
                            Item
                            <div onMouseDown={(e)=>startResize(e,'item')} style={{position:'absolute',right:0,top:0,bottom:0,width:5,cursor:'col-resize',background:'transparent'}} />
                          </th>
                          {/* Qtd */}
                          <th style={{width: colWidths['qtd']||60, minWidth:40, position:'relative', fontSize:10}} className="text-center px-2 py-1 font-semibold border-b border-r whitespace-normal break-words">
                            Qtd
                            <div onMouseDown={(e)=>startResize(e,'qtd')} style={{position:'absolute',right:0,top:0,bottom:0,width:5,cursor:'col-resize',background:'transparent'}} />
                          </th>
                          {/* Menor Unitário */}
                          <th style={{width: colWidths['menorUnit']||80, minWidth:50, position:'relative', fontSize:10}} className="text-center px-2 py-1 font-semibold border-b border-r bg-blue-50 whitespace-normal break-words">
                            Menor Unit.
                            <div onMouseDown={(e)=>startResize(e,'menorUnit')} style={{position:'absolute',right:0,top:0,bottom:0,width:5,cursor:'col-resize',background:'transparent'}} />
                          </th>
                          {/* Menor Total */}
                          <th style={{width: colWidths['melhor']||90, minWidth:50, position:'relative', fontSize:10}} className="text-center px-2 py-1 font-semibold border-b border-r bg-green-50 whitespace-normal break-words">
                            Menor Total
                            <div onMouseDown={(e)=>startResize(e,'melhor')} style={{position:'absolute',right:0,top:0,bottom:0,width:5,cursor:'col-resize',background:'transparent'}} />
                          </th>
                          {fornecedores.map(f => {
                            const collapsed = collapsedCols[f.id];
                            const w = collapsed ? 28 : (colWidths[f.id] || 140);
                            return (
                              <th key={f.id} style={{width:w, minWidth: collapsed?28:60, position:'relative', fontSize:10}} className="text-center px-1 py-1 font-semibold border-b border-r whitespace-normal break-words">
                                {collapsed ? (
                                  <button onClick={()=>toggleCollapse(f.id)} title={f.fornecedor_nome} className="writing-mode-vertical text-xs rotate-180" style={{writingMode:'vertical-rl', fontSize:9}}>▶</button>
                                ) : (
                                  <div className="flex flex-col gap-0.5 items-center">
                                    <div className="flex items-center gap-1 justify-center w-full">
                                      <span className="break-words text-center" style={{fontSize:10}}>{f.fornecedor_nome}</span>
                                      <button onClick={()=>toggleCollapse(f.id)} title="Recolher coluna" className="text-slate-400 hover:text-slate-700 flex-shrink-0" style={{fontSize:9}}>◀</button>
                                    </div>
                                    <Badge variant={f.status === 'Respondida' ? 'default' : 'secondary'} className="text-xs" style={{fontSize:9}}>{f.status}</Badge>
                                    {(f.status === 'Respondida Totalmente' || f.status === 'Respondida Parcialmente' || f.status === 'Respondida') && (
                                      <button onClick={()=>handleReabrirFornecedor(f.id)} className="flex items-center gap-0.5 text-amber-600 hover:text-amber-700 underline" style={{fontSize:9}}>
                                        <RefreshCw className="w-2 h-2" /> Reabrir
                                      </button>
                                    )}
                                  </div>
                                )}
                                {!collapsed && <div onMouseDown={(e)=>startResize(e,f.id)} style={{position:'absolute',right:0,top:0,bottom:0,width:5,cursor:'col-resize',background:'transparent'}} />}
                              </th>
                            );
                          })}
                          <th style={{width:50, fontSize:10}} className="text-center px-2 py-1 font-semibold border-b">Ações</th>
                        </tr>
                      </thead>
                      <tbody>
                        {itensOrdenados.map(item => {
                          const precosItem = fornecedores.map(f => precos[`${item.id}_${f.id}`]).filter(p => p > 0);
                          const menorPrecoItem = precosItem.length > 0 ? Math.min(...precosItem) : null;
                          return (
                            <tr key={item.id} className="border-b hover:bg-slate-50">
                              <td className="px-2 py-1 border-r text-slate-600 break-words" style={{fontSize:10, wordBreak:'break-all'}}>
                                {getMaterialCodigo(item)}
                              </td>
                              <td className="px-2 py-1 border-r break-words" style={{fontSize:10, wordBreak:'break-word'}}>
                                {item.descricao}
                              </td>
                              <td className="px-2 py-1 border-r text-center break-words" style={{fontSize:10}}>
                                {item.quantidade} {item.unidade}
                              </td>
                              <td className="px-2 py-1 border-r text-center font-bold text-blue-700 bg-blue-50" style={{fontSize:10}}>
                                {menorPrecoItem ? formatMoeda(menorPrecoItem) : '-'}
                              </td>
                              <td className="px-2 py-1 border-r text-center font-bold text-green-700 bg-green-50" style={{fontSize:10}}>
                                {menorPrecoItem ? formatMoeda(menorPrecoItem * item.quantidade) : '-'}
                              </td>
                              {fornecedores.map(f => {
                               const collapsed = collapsedCols[f.id];
                               const chave = `${item.id}_${f.id}`;
                               const preco = precos[chave] || 0;
                               const ehMenor = preco > 0 && menorPrecoItem && preco === menorPrecoItem;
                               const ehSelecionado = itensSelecionados[item.id] === f.id;
                               const semPreco = !preco || preco <= 0;
                               const ehPerdendo = preco > 0 && menorPrecoItem && preco > menorPrecoItem;
                               if (collapsed) return <td key={f.id} className="border-r bg-slate-50" style={{width:28}} />;
                               return (
                                 <td key={f.id} className={`px-1 py-1 border-r ${ehSelecionado ? 'bg-green-50' : semPreco ? 'bg-yellow-50' : ehPerdendo ? 'bg-red-50' : ''}`}>
                                   <div className={`p-1 rounded border ${ehSelecionado ? 'border-green-600 bg-green-50' : ehMenor ? 'border-blue-400 bg-blue-50' : semPreco ? 'border-yellow-300 bg-yellow-50' : ehPerdendo ? 'border-red-300 bg-red-50' : 'border-slate-200'}`}>
                                      <Input
                                       type="text"
                                       placeholder="0,00"
                                       value={chave in valoresDigitando ? valoresDigitando[chave] : (preco > 0 ? preco.toFixed(2).replace('.', ',') : '')}
                                       disabled={salvando}
                                       data-campo={chave}
                                       style={{fontSize:10, height:24, textAlign:'center', padding:'0 4px'}}
                                       className={ehMenor ? 'font-bold text-green-700' : ''}
                                       onFocus={() => {
                                         setValoresDigitando(prev => ({ ...prev, [chave]: preco > 0 ? preco.toFixed(2).replace('.', ',') : '' }));
                                       }}
                                       onChange={(e) => {
                                         setValoresDigitando(prev => ({ ...prev, [chave]: e.target.value }));
                                       }}
                                       onKeyDown={async (e) => {
                                         if (e.key === 'Tab' || e.key === 'Enter') {
                                           e.preventDefault();
                                           const raw = (valoresDigitando[chave] || '').replace('R$','').replace(/\s/g,'').replace(',','.');
                                           if (raw && !isNaN(parseFloat(raw))) {
                                             const valorNum = parseFloat(raw);
                                             setPrecos(prev => ({ ...prev, [chave]: valorNum }));
                                             await salvarPreco(item.id, f.id, raw);
                                           }
                                           setValoresDigitando(prev => { const n = {...prev}; delete n[chave]; return n; });
                                      ...
                                      />
                                      {preco > 0 && (
                                        <p className={`mt-0.5 text-center cursor-pointer ${ehSelecionado ? 'text-green-700 font-bold' : 'text-slate-500'}`}
                                          style={{fontSize:9}}
                                          onClick={() => preco > 0 && setItensSelecionados(prev => ({ ...prev, [item.id]: f.id }))}
                                          title="Clique para selecionar como vencedor"
                                        >
                                          {formatMoeda((parseFloat(preco)||0) * item.quantidade)}
                                          {ehSelecionado && <Check className="w-2 h-2 inline ml-0.5 text-green-600" />}
                                          {!ehSelecionado && ehMenor && <Trophy className="w-2 h-2 inline ml-0.5 text-blue-600" />}
                                        </p>
                                      )}
                                    </div>
                                  </td>
                              ...
                              <td className="px-1 py-1 text-center">
                                <DropdownMenu>
                                  ...
                                  <DropdownMenuContent align="end">
                                    <DropdownMenuItem onClick={() => { setMaterialSelecionado(item.solicitacao_item_id); setVisualizarMaterialOpen(true); }}>
                                    ...
                                    {fornecedores.map(f => {
                                      ...
                                      const ehSelecionado = itensSelecionados[item.id] === f.id;
                                      return (
                                        <DropdownMenuItem key={f.id} onClick={() => setItensSelecionados(prev => ({ ...prev, [item.id]: f.id }))} className={ehSelecionado ? 'text-green-700 font-bold' : ''}>
                                          <Check className="w-3 h-3 mr-2" /> {ehSelecionado ? '✓ ' : ''}{f.fornecedor_nome}
                                        </DropdownMenuItem>
                                      );
                                    })}
                                    <DropdownMenuItem onClick={() => handleExcluirItem(item.id)} className="text-red-600">
                        ...
                        {/* Linha de Totais */}
                        <TotalRow itens={itensOrdenados} fornecedores={fornecedores} precos={precos} totais={totais} collapsedCols={collapsedCols} formatMoeda={formatMoeda} />
                      </tbody>
                    </table>
                  </div>
                </CardContent>
                  ...
                  <div className="flex items-start gap-4 p-4 bg-amber-50 rounded-lg border border-amber-200">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-slate-600">
                        Abra o arquivo Excel e preencha os preços unitários nas colunas de cada fornecedor
                      </p>
                    </div>
                  </div>
