...
export default function MovimentacoesTab({ empresaAtiva }) {
  ...
  const loadMovimentacoes = async () => {
    setLoading(true);
    try {
      const result = await sigo.entities.MovimentacaoFerramenta.filter({
        empresa_id: empresaAtiva.id,
      });
      setMovimentacoes(
        result.sort((a, b) => new Date(b.data_movimentacao) - new Date(a.data_movimentacao))
      );
    } catch (error) {
  ...
  const handleSave = async (formData) => {
    try {
      if (editingMovimentacao) {
        await sigo.entities.MovimentacaoFerramenta.update(editingMovimentacao.id, formData);
        ...
        if (formData.tipo_movimentacao === "Entrada Estoque") {
          ...
          const ferramentaModelo = await sigo.entities.Ferramenta.filter({
            empresa_id: empresaAtiva.id,
          ...
          for (let i = 0; i < quantidade; i++) {
            const novoItem = await sigo.entities.Ferramenta.create({
              empresa_id: empresaAtiva.id,
          ...
          toast.success(
            `${quantidade} ${quantidade === 1 ? "unidade criada" : "unidades criadas"} com sucesso`
          );
        } else {
          ...
          const criada = await sigo.entities.MovimentacaoFerramenta.create(formData);
          return criada;
        }
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await sigo.entities.MovimentacaoFerramenta.delete(id);
  ...
  const handleToggleSelect = (id) => {
    ...
    setSelectAll(
      newSelected.size === filteredMovimentacoes.length && filteredMovimentacoes.length > 0
    );
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    if (
      !confirm(
        `Deseja excluir ${selectedMovimentacoes.size} movimentação(ções)? Esta ação não pode ser desfeita.`
      )
    )
      return;
    ...
    try {
      await Promise.all(
        Array.from(selectedMovimentacoes).map((id) =>
          sigo.entities.MovimentacaoFerramenta.delete(id)
        )
      );
  ...
  const handleConcluirTodosPendentes = async () => {
    ...
    try {
      await Promise.all(
        pendentes.map((m) =>
          sigo.entities.MovimentacaoFerramenta.update(m.id, { status: "Realizada" })
        )
      );
  ...
  const handleImportarExcel = async (e) => {
    ...
    try {
      ...
      const { output } = await sigo.integrations.Core.ExtractDataFromUploadedFile({
        file_url,
        json_schema: {
          type: "object",
          properties: {
      ...
      for (const item of output) {
        try {
          ...
          const destino =
            item.destino || item.caminhao_placa || item.funcionario_nome || "Almoxarifado";
    ...
    <div className="space-y-6">
      ...
      {selectedMovimentacoes.size > 0 && (
        <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <Button size="sm" variant="destructive" onClick={handleDeleteSelected} className="gap-2">
          ...
          <TableHeader>
            <TableRow>
              <TableHead className="w-10">
                <Checkbox checked={selectAll} onCheckedChange={handleToggleSelectAll} />
              </TableHead>
          ...
          <TableBody>
            {loading ? (
              ...
              filteredMovimentacoes.map((mov) => (
                <TableRow
                  key={mov.id}
                  className={selectedMovimentacoes.has(mov.id) ? "bg-blue-50" : ""}
                >
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      ...
                      {[
                        ...
                        "Baixa para Sucata",
                        "Entrada Estoque",
                      ].includes(mov.tipo_movimentacao) && (
                        <Button
                          size="sm"
                          variant="ghost"
