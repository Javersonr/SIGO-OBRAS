...
export default function ConfirmarEntregaModal({
    ...
    if (open && entrega?.id) {
      const carregarDados = async () => {
        ...
        if (entrega.empresa_id) {
          ferrsDisp = await sigo.entities.Ferramenta.filter({
        ...
        const itensComDados = await Promise.all(
          dados.map(async (i) => {
            ...
            if (i.ferramenta_id) {
              const ferrs = await sigo.entities.Ferramenta.filter({ id: i.ferramenta_id }).catch(
                () => []
              );
              if (ferrs.length > 0) {
  ...
  const handleConfirmar = async () => {
    const itensEntreguesConferidos = itensEntrega.filter(
      (i) => i.conferido && (i.quantidade_entregue ?? 0) > 0
    );
    ...
    const itensComQuantidadeEntregada = itensEntrega.filter(
      (i) => (i.quantidade_entregue ?? 0) > 0
    );
    if (
      itensComQuantidadeEntregada.length > 0 &&
      itensComQuantidadeEntregada.length !== itensEntreguesConferidos.length
    ) {
      const naoConferidos = itensComQuantidadeEntregada.length - itensEntreguesConferidos.length;
      toast.error(
        `${naoConferidos} item(ns) entregue(s) ainda não foi(foram) conferido(s). Marque o checkbox para confirmar cada item.`
      );
      return;
    }
    ...
    try {
      ...
      const todasAsQuantidadesBatem =
        itensEntrega.length > 0 &&
        itensEntrega.every((item) => (item.quantidade_entregue ?? 0) === (item.quantidade ?? 1));

      const novoStatus = todasAsQuantidadesBatem
        ? "Entregue"
        : acaoEntregaParcial === "concluir"
          ? "Entregue"
          : "Pendente";
      ...
      if (movimentacoes.length > 0) {
        await sigo.entities.MovimentacaoFerramenta.bulkCreate(movimentacoes);
      }

      // Atualizar número de série, laudo e STATUS das ferramentas entregues → Em Uso
      const atualizacoes = itensEntrega.filter(
        (item) => item.ferramenta_id && item.conferido && item.quantidade_entregue > 0
      );
      await Promise.all(
        atualizacoes.map((item) => {
          ...
          if (entrega.funcionario_id) {
            updates.funcionario_id = entrega.funcionario_id;
            updates.funcionario_nome = entrega.funcionario_nome || "";
          }
          if (entrega.caminhao_id) {
            updates.localizacao = entrega.caminhao_placa || "";
          }
          return sigo.entities.Ferramenta.update(item.ferramenta_id, updates).catch(() => {});
        })
      );
  ...
  const episOrdenados = [...itensEntrega]
    .filter(
      (i) => i.tipo === "EPI" && i.conferido && (i.quantidade_entregue ?? i.quantidade ?? 1) > 0
    )
    ...
    >
        ...
        >
          <div style={{ borderBottom: "1px solid #000", paddingBottom: "2px" }}>
            ...
            <div style={{ marginTop: "2px" }}>
              {entrega.funcionario_nome || `Caminhão ${entrega.caminhao_placa}` || ""}
            </div>
          </div>
          <div
            style={{ borderBottom: "1px solid #000", paddingBottom: "2px", paddingLeft: "20px" }}
          >
            <div style={{ fontWeight: "bold" }}>Nº DE REGISTRO:</div>
            <div style={{ marginTop: "2px" }}>
              {entrega.funcionario_id || entrega.caminhao_id || ""}
            </div>
          </div>
          <div
            style={{ borderBottom: "1px solid #000", paddingBottom: "2px", paddingLeft: "20px" }}
          >
            <div style={{ fontWeight: "bold" }}>DATA DE EMISSÃO:</div>
            <div style={{ marginTop: "2px" }}>{format(new Date(), "dd/MM/yyyy")}</div>
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0" }}>
          <div style={{ borderBottom: "1px solid #000", paddingBottom: "2px" }}>
            <div style={{ fontWeight: "bold" }}>FUNÇÃO/TIPO:</div>
            <div style={{ marginTop: "2px" }}>
              {entrega.funcao_nome || entrega.caminhao_modelo || ""}
            </div>
          </div>
          <div
            style={{ borderBottom: "1px solid #000", paddingBottom: "2px", paddingLeft: "20px" }}
          >
            <div style={{ fontWeight: "bold" }}>SEÇÃO: OPERACIONAL</div>
          </div>
          <div
            style={{ borderBottom: "1px solid #000", paddingBottom: "2px", paddingLeft: "20px" }}
          >
      ...
      >
        <p style={{ textAlign: "justify", marginBottom: "8px", lineHeight: "1.4" }}>
          ...
          cumpri a utiliza-los corretamente durante o tempo que permanece ao meu dispor.
        </p>
      ...
      <table style={{ fontSize: "11px", marginBottom: "12px" }}>
        <thead>
          <tr style={{ backgroundColor: "#e5e5e5" }}>
            ...
            <th style={{ width: "23%", padding: "6px", textAlign: "center" }}>
              ASSINATURA DO FUNCIONÁRIO
            </th>
            <th style={{ width: "20%", padding: "6px", textAlign: "center" }}>
        ...
        <tbody>
          {episOrdenados.map((epi, idx) => (
            <tr key={idx} style={{ height: "20px" }}>
              ...
              <td style={{ padding: "6px", textAlign: "center" }}>
                {epi.quantidade_entregue || epi.quantidade || 1}
              </td>
              <td style={{ padding: "6px", fontSize: "10px" }}>
                {epi.descricao || epi.item || ""}
              </td>
          ...
          {episOrdenados.length === 0 && (
            <tr style={{ height: "20px" }}>
              <td colSpan={7} style={{ padding: "6px", textAlign: "center", fontSize: "10px" }}>
                Nenhum EPI entregue e conferido
              </td>
            </tr>
          )}
          {[...Array(Math.max(0, episOrdenados.length > 0 ? 30 - episOrdenados.length : 30))].map(
            (_, idx) => (
      ...
      <div style={{ marginTop: "24px", paddingTop: "8px", borderTop: "1px solid #1f2937" }}>
        ...
        >
          <div>
            <div
              style={{ borderBottom: "1px solid #1f2937", height: "35px", marginBottom: "2px" }}
            ></div>
    ...
    return (
      ...
      >
        <td className="px-3 py-2 text-center">
          <input
            type="checkbox"
            checked={item.conferido || false}
            onChange={(e) =>
              setItensEntrega((prev) =>
                prev.map((it, i) =>
                  i === originalIdx ? { ...it, conferido: e.target.checked } : it
                )
              )
            }
            ...
            >
              ...
              {ferramentasDisponiveis.map((f) => (
                <option key={f.id} value={f.id}>
                  {f.descricao}
                  {f.numero_serie ? ` - S/N: ${f.numero_serie}` : ""}
                </option>
              ))}
            </select>
          )}
        ...
        <td className="px-3 py-2 text-center">
          <span className="inline-flex items-center justify-center w-10 h-7 bg-slate-100 border border-slate-200 rounded text-slate-500 font-semibold text-xs">
            {item.quantidade || 1}
          </span>
        </td>
        <td className="px-3 py-2">
          <input
            ...
            onChange={(e) =>
              setItensEntrega((prev) =>
                prev.map((it, i) =>
                  i === originalIdx
                    ? { ...it, quantidade_entregue: parseInt(e.target.value) || 0 }
                    : it
                )
              )
            }
        ...
        <td className="px-3 py-2">
          <input
            ...
            onChange={(e) =>
              setItensEntrega((prev) =>
                prev.map((it, i) =>
                  i === originalIdx ? { ...it, numero_laudo: e.target.value } : it
                )
              )
            }
        ...
        <td className="px-3 py-2">
          <input
            ...
            onChange={(e) =>
              setItensEntrega((prev) =>
                prev.map((it, i) =>
                  i === originalIdx ? { ...it, numero_serie: e.target.value } : it
                )
              )
            }
      ...
      >
        <SheetHeader className="sticky top-0 bg-white z-10 space-y-3 border-b">
          <SheetTitle className="sr-only">
            Confirmar Entrega - {entrega.funcionario_nome || entrega.caminhao_placa}
          </SheetTitle>
          <div className="px-6 pt-4 flex items-center gap-2">
            <Search className="w-4 h-4 text-slate-400 flex-shrink-0" />
            <span className="text-sm font-semibold text-slate-800">
              {entrega.funcionario_nome || entrega.caminhao_placa || "Entrega"}
            </span>
          </div>
              ...
              <div className="text-xs space-y-1">
                <div className="text-slate-600">
                  <span className="font-semibold text-green-600">
                    {itensEntrega.filter((i) => i.conferido).length}
                  </span>{" "}
                  Entregues
                </div>
                <div className="text-slate-600">
            ...
            <div>
              <div className="sticky top-0 bg-white pt-4 pb-3 space-y-3 z-10">
                <div className="mx-1 flex gap-2 border-b">
                  <button
                    onClick={() => setAbaFerramentas("entregues")}
                    className={`px-4 py-2 font-medium text-sm border-b-2 transition-all ${abaFerramentas === "entregues" ? "border-green-600 text-green-700" : "border-transparent text-slate-600 hover:text-slate-800"}`}
                  >
                    ✓ Entregues (
                    {itensEntrega.filter((i) => i.tipo !== "EPI" && i.conferido).length})
                  </button>
                  <button
                    onClick={() => setAbaFerramentas("pendentes")}
                    className={`px-4 py-2 font-medium text-sm border-b-2 transition-all ${abaFerramentas === "pendentes" ? "border-red-600 text-red-700" : "border-transparent text-slate-600 hover:text-slate-800"}`}
                  >
                    ⏳ Pendentes (
                    {itensEntrega.filter((i) => i.tipo !== "EPI" && !i.conferido).length})
                  </button>
                ...
                <input
                  ...
                  value={buscaForramentas || ""}
                  onChange={(e) => setBuscaForramentas(e.target.value)}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
                />
                <p className="text-sm text-slate-500">
                  Itens sem ferramenta vinculada mostram um seletor para escolher uma ferramenta
                ...
                <div className="border border-slate-200 rounded-lg overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50">
                      <tr>
                        <th className="px-3 py-2 text-center text-xs font-semibold text-slate-600 w-10">
                          ✓
                        </th>
                        ...
                        <th className="px-3 py-2 text-center text-xs font-semibold text-slate-600 w-32">
                          Nº de Laudo
                        </th>
                        <th className="px-3 py-2 text-center text-xs font-semibold text-slate-600 w-32">
                          Nº de Série
                    ...
                    <tbody>
                      {(() => {
                        const ferramentas = itensEntrega
                          .filter((i) => i.tipo !== "EPI")
                          .filter(
                            (i) =>
                              !buscaForramentas ||
            ...
            <div>
              <div className="space-y-4">
                <div className="flex gap-2 border-b mt-4">
                  <button
                    onClick={() => setAbaEpis("entregues")}
                    className={`px-4 py-2 font-medium text-sm border-b-2 transition-all ${abaEpis === "entregues" ? "border-green-600 text-green-700" : "border-transparent text-slate-600 hover:text-slate-800"}`}
                  >
                    ✓ Entregues (
                    {itensEntrega.filter((i) => i.tipo === "EPI" && i.conferido).length})
                  </button>
                  <button
                    onClick={() => setAbaEpis("pendentes")}
                    className={`px-4 py-2 font-medium text-sm border-b-2 transition-all ${abaEpis === "pendentes" ? "border-red-600 text-red-700" : "border-transparent text-slate-600 hover:text-slate-800"}`}
                  >
                    ⏳ Pendentes (
                    {itensEntrega.filter((i) => i.tipo === "EPI" && !i.conferido).length})
                  </button>
                ...
                <input
                  ...
                  value={buscaEpis || ""}
                  onChange={(e) => setBuscaEpis(e.target.value)}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
                />
                <p className="text-sm text-slate-500">
                  Informe a quantidade entregue, o Nº do C.A. e Nº de Série antes de confirmar.
                </p>
                <div className="border border-slate-200 rounded-lg overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50">
        ...
        {(() => {
          const algumPendente = itensEntrega.some(
            (i) => (i.quantidade_entregue ?? 0) < (i.quantidade ?? 1)
          );
          if (!algumPendente) return null;
          return (
            <div className="px-6 py-3 bg-amber-50 border-t border-amber-200">
              <p className="text-xs font-semibold text-amber-800 mb-2">
                ⚠️ Entrega parcial detectada — o que deseja fazer?
              </p>
              <div className="flex gap-3">
                <label
                  className={`flex-1 flex items-center gap-2 p-2 rounded-lg border-2 cursor-pointer transition-all ${acaoEntregaParcial === "pendente" ? "border-amber-500 bg-amber-100" : "border-slate-200 bg-white"}`}
                >
                  <input
                    type="radio"
                    name="acao_parcial"
                    value="pendente"
                    checked={acaoEntregaParcial === "pendente"}
                    onChange={() => setAcaoEntregaParcial("pendente")}
                    className="accent-amber-600"
                  />
                  <div>
                    <div className="text-xs font-semibold text-amber-800">Deixar Pendente</div>
                    <div className="text-xs text-amber-600">
                      Salva progresso, entrega continua aberta
                    </div>
                  </div>
                </label>
                <label
                  className={`flex-1 flex items-center gap-2 p-2 rounded-lg border-2 cursor-pointer transition-all ${acaoEntregaParcial === "concluir" ? "border-green-600 bg-green-50" : "border-slate-200 bg-white"}`}
                >
                  <input
                    type="radio"
                    name="acao_parcial"
                    value="concluir"
                    checked={acaoEntregaParcial === "concluir"}
                    onChange={() => setAcaoEntregaParcial("concluir")}
                    className="accent-green-600"
                  />
                  <div>
                    <div className="text-xs font-semibold text-green-800">Concluir Entrega</div>
                    <div className="text-xs text-green-600">
                      Marca como entregue mesmo incompleta
                    </div>
                  </div>
                </label>
              </div>
