...
import { Upload, X, Image as ImageIcon, Loader2, Link2, Sparkles, CheckCircle2, AlertCircle, FileWarning } from 'lucide-react';
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
                  ...
                  <>
                    ...
                    {caminhoes.map((cam) => (
                      <SelectItem key={cam.id} value={`Caminhão - ${cam.placa}`}>
                        {cam.placa}{cam.modelo ? ` · ${cam.modelo}` : ''}{cam.marca ? ` ${cam.marca}` : ''}
                      </SelectItem>
                    ))}
                  </>
                )}
            ...
            {caminhoes.length === 0 && (
              <p className="text-xs text-slate-400 mt-1">Nenhum caminhão cadastrado em Configurações.</p>
            )}
          </div>
        </div>
                  ...
                  {buscaCampo && sugestoesIA.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
                      {sugestoesIA.map((campo) => (
                        ...
                        >
                          <div className="text-sm font-medium text-slate-800">{campo.nome_campo}</div>
                          {campo.quantidade_obrigatoria > 1 && (
                            <div className="text-xs text-slate-500">Quantidade: {campo.quantidade_obrigatoria}</div>
                          )}
                        </button>
                      ))}
                    </div>
        ...
        <div className="border border-slate-200 rounded-lg p-4 space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Marque se esta ferramenta exige laudo técnico</p>
            </div>
            <Switch
            ...
            <>
              {formData.laudo_url ? (
                <div className="flex items-center gap-2 p-2 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <a href={formData.laudo_url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 underline">Ver</a>
                  <button onClick={() => setFormData({ ...formData, laudo_url: '' })} className="ml-1 text-slate-400 hover:text-red-500">
              ...
              <div>
                ...
                <label className="mt-1 flex items-center gap-3 cursor-pointer border-2 border-dashed border-slate-300 rounded-lg px-4 py-3 hover:border-amber-400 transition-colors">
                  ...
                  <span className="text-xs text-slate-500">{uploadingLaudo ? 'Enviando...' : 'Clique para anexar laudo'}</span>
                  <input type="file" accept=".pdf,image/*" className="hidden" onChange={handleLaudoUpload} disabled={uploadingLaudo} />
                </label>
              </div>
            </>
          )}
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) => setFormData({ ...formData, intervalo_manutencao_dias: parseInt(e.target.value) || 0 })}
