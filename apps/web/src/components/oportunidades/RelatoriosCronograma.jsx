  ...
  const handleGerarPDF = async () => {
    ...
    try {
      ...
      tarefasOrganizadas.forEach((tarefa, idx) => {
        ...
        x += 80;
        if (incluirStatus) {
          doc.text((tarefa.status || "-").substring(0, 15), x, y);
          x += 35;
        }
        if (incluirDatas) {
          doc.text(
            tarefa.data_inicio || tarefa.data_inicio_planejada
              ? new Date(
                  (tarefa.data_inicio || tarefa.data_inicio_planejada) + "T00:00:00"
          ...
          doc.text(
            tarefa.data_conclusao || tarefa.data_fim_planejada
              ? new Date(
                  (tarefa.data_conclusao || tarefa.data_fim_planejada) + "T00:00:00"
                ).toLocaleDateString("pt-BR")
              : "-",
            x,
            y
        ...
        if (incluirPercentual) {
          doc.text(`${tarefa.progresso || tarefa.percentual_conclusao || 0}%`, x, y);
          x += 25;
        }
        if (incluirResponsaveis) {
        ...
        if (incluirSubtarefas && tarefa.subtarefas && tarefa.subtarefas.length > 0) {
          ...
          tarefa.subtarefas.forEach((sub, subIdx) => {
            ...
            x += 75;
            if (incluirStatus) {
              doc.text((sub.status || "-").substring(0, 12), x, y);
              x += 35;
            }
            if (incluirDatas) {
              doc.text(
                sub.data_inicio
                  ? new Date(sub.data_inicio + "T00:00:00").toLocaleDateString("pt-BR")
                  : "-",
                x,
                y
              );
              ...
              doc.text(
                sub.data_fim
                  ? new Date(sub.data_fim + "T00:00:00").toLocaleDateString("pt-BR")
                  : "-",
                x,
                y
              );
      ...
      if (incluirDetalhes && tarefasOrganizadas.length > 0) {
        ...
        tarefasOrganizadas.forEach((tarefa, idx) => {
          ...
          if (incluirDatas && (tarefa.data_inicio || tarefa.data_inicio_planejada)) {
            ...
            doc.text(
              `Período: ${new Date(dataInicio + "T00:00:00").toLocaleDateString("pt-BR")} até ${dataConclusao ? new Date(dataConclusao + "T00:00:00").toLocaleDateString("pt-BR") : "-"}`,
              18,
              y
            );
            y += 5;
    ...
    try {
      ...
      doc.text(
        empresaAtiva?.endereco
          ? `${empresaAtiva.endereco}${empresaAtiva.numero ? ", " + empresaAtiva.numero : ""}`
          : "",
        40,
        19
      );
      ...
      const todasDatas = tarefasOrganizadas
        ...
        .flatMap((t) => [
          ...
          t.data_fim || t.data_fim_planejada
            ? new Date((t.data_fim || t.data_fim_planejada) + "T12:00:00")
            : new Date((t.data_inicio || t.data_inicio_planejada) + "T12:00:00"),
        ]);
      ...
      const desenharCabecalho = (yPos) => {
        ...
        meses.forEach((mes) => {
          ...
          doc.setFontSize(10);
          doc.setFont(undefined, "bold");
          const mesNome = new Date(mes.ano, mes.mes)
      ...
      tarefasOrganizadas.forEach((tarefa, idx) => {
        ...
        if (tarefa.data_inicio || tarefa.data_inicio_planejada) {
          const dataInicioTarefa = new Date(
            (tarefa.data_inicio || tarefa.data_inicio_planejada) + "T12:00:00"
          );
          const dataFimTarefa =
            tarefa.data_fim || tarefa.data_fim_planejada
          ...
          const duracao = Math.max(
            1,
            Math.ceil((dataFimTarefa - dataInicioTarefa) / (1000 * 60 * 60 * 24)) + 1
          );
          ...
          if (tarefa.progresso > 0 && tarefa.status !== "Concluída") {
            ...
            doc.roundedRect(
              xBarra,
              yTarefa + 1,
              (larguraBarra * tarefa.progresso) / 100,
              8,
              2,
              2,
              "F"
          ...
          doc.setFontSize(7);
          doc.setFont(undefined, "bold");
          const dataInicioFormatada = dataInicioTarefa.toLocaleDateString("pt-BR", {
            day: "2-digit",
            month: "2-digit",
            year: "2-digit",
          });
          const dataFimFormatada = dataFimTarefa.toLocaleDateString("pt-BR", {
            day: "2-digit",
            month: "2-digit",
            year: "2-digit",
          });
        ...
        if (incluirSubtarefas && tarefa.subtarefas && tarefa.subtarefas.length > 0) {
          ...
          tarefa.subtarefas.forEach((sub, subIdx) => {
            ...
            if (sub.data_inicio) {
              ...
              const dataFimSub = sub.data_fim
                ? new Date(sub.data_fim + "T12:00:00")
                : new Date(dataInicioSub);
              ...
              const duracao = Math.max(
                1,
                Math.ceil((dataFimSub - dataInicioSub) / (1000 * 60 * 60 * 24)) + 1
              );
              ...
              doc.setFontSize(6);
              doc.setFont(undefined, "bold");
              const dataInicioFormatada = dataInicioSub.toLocaleDateString("pt-BR", {
                day: "2-digit",
                month: "2-digit",
                year: "2-digit",
              });
              const dataFimFormatada = dataFimSub.toLocaleDateString("pt-BR", {
                day: "2-digit",
                month: "2-digit",
                year: "2-digit",
              });
      ...
      doc.text(
        `${empresaAtiva?.razao_social || empresaAtiva?.nome || ""} - Documento gerado em ${new Date().toLocaleDateString("pt-BR")} às ${new Date().toLocaleTimeString("pt-BR")}`,
        12,
        198
      );
    ...
    <div className="space-y-6">
      ...
      <div className="print:block hidden">
        <div className="p-8">
          ...
          <p className="mb-2">
            <strong>Obra:</strong> {nomeObra || "Não informado"}
          </p>
          <p className="mb-2">
              ...
              {tarefasOrganizadas.map((tarefa, idx) => (
                <React.Fragment key={tarefa.id}>
                  <tr className="font-bold bg-slate-50">
                    ...
                    {incluirStatus && (
                      <td className="border border-slate-300 p-2">{tarefa.status}</td>
                    )}
                    {incluirDatas && (
                      <>
                  ...
                  {incluirSubtarefas &&
                    tarefa.subtarefas &&
                    tarefa.subtarefas.map((sub, subIdx) => (
                      <tr key={sub.id} className="bg-blue-50/30">
                        ...
                        {incluirStatus && (
                          <td className="border border-slate-300 p-2 text-sm">{sub.status}</td>
                        )}
                        {incluirDatas && (
                          <>
                            <td className="border border-slate-300 p-2 text-sm">
                              {sub.data_inicio
                                ? new Date(sub.data_inicio + "T00:00:00").toLocaleDateString(
                                    "pt-BR"
                                  )
                                : "-"}
                            </td>
