...
import { 
  Plus, Upload, Edit, Eye, X, FileText, Clock, CheckCircle2, Trash2, PackageCheck
} from 'lucide-react';
...
export default function TSTTab({
  ...
  const handleAbrirAnexoAssinado = (treinamento) => {
    ...
    console.log('[TSTTab] setTreinamentoAssinado chamado, agora chamando setShowCertificadoAssinado(true)');
  ...
  return (
    <div className="space-y-4 mt-4 pb-4">
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-600 mt-1">Anexe certificados de cursos não obrigatórios na função</p>
            </div>
            <label>
              <Button variant="outline" size="sm" className="border-amber-400 hover:bg-amber-100" disabled={uploadingDoc} asChild>
              ...
              <input
                ...
                onChange={async (e) => {
                  ...
                  if (!file) return;
                  const nomeArquivo = prompt('Nome do treinamento/diploma:', file.name.replace(/\.[^/.]+$/, ""));
                  if (!nomeArquivo) { e.target.value = ''; return; }
                  setUploadingDoc(true);
                  try {
                    const { file_url } = await base44.integrations.Core.UploadFile({ file });
                    const anexos = JSON.parse(funcionarioForm.treinamentos_anexos || '[]');
                    anexos.push({ nome: nomeArquivo, url: file_url, treinamento_id: null, treinamento_nome: nomeArquivo, tipo: 'extra', data_upload: new Date().toISOString() });
                    const novoForm = { ...funcionarioForm, treinamentos_anexos: JSON.stringify(anexos) };
                    setFuncionarioForm(novoForm);
                    handleAutoSave(novoForm);
                    toast.success('Treinamento extra anexado');
                  } catch { toast.error('Erro ao anexar'); } finally { setUploadingDoc(false); }
                  e.target.value = '';
                }}
      ...
      {/* Extras anexados */}
      {JSON.parse(funcionarioForm.treinamentos_anexos || '[]').filter(a => !a.treinamento_id || a.tipo === 'extra').length > 0 && (
        <Card className="border-green-200 bg-green-50">
          <CardHeader className="pb-3"><CardTitle className="text-sm text-green-800">Treinamentos Extras Anexados</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            {JSON.parse(funcionarioForm.treinamentos_anexos || '[]').filter(a => !a.treinamento_id || a.tipo === 'extra').map((anexo, idx) => (
              <div key={idx} className="flex items-center justify-between p-2 bg-white rounded-lg border border-green-200">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">{anexo.data_upload ? format(new Date(anexo.data_upload), 'dd/MM/yyyy') : ''}</p>
                  </div>
                </div>
                <div className="flex gap-1 ml-2">
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => window.open(anexo.url, '_blank')}>
                  ...
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => {
                    ...
                    const novoForm = { ...funcionarioForm, treinamentos_anexos: JSON.stringify(novos) };
                    setFuncionarioForm(novoForm);
                    handleAutoSave(novoForm);
      ...
      {!funcionarioForm.funcao_id ? (
        <div className="text-center py-8 bg-slate-50 rounded-lg">
          <p className="text-sm text-slate-500">Selecione uma função na aba "Dados Pessoais" para ver os treinamentos obrigatórios</p>
        </div>
      ) : treinamentosDaFuncao.length === 0 ? (
        <div className="text-center py-8 bg-slate-50 rounded-lg">
          <p className="text-sm text-slate-500">Nenhum treinamento obrigatório cadastrado para esta função</p>
          <p className="text-xs text-slate-400 mt-1">Cadastre treinamentos em Configurações → Funções</p>
        </div>
      ) : (
        <>
          <div className="border-t pt-4">
            <h4 className="text-sm font-medium text-slate-700 mb-3">Treinamentos Obrigatórios da Função</h4>
          </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          {treinamento.codigo && <p className="text-xs text-slate-500">Código: {treinamento.codigo}</p>}
                          {treinamento.carga_horaria && <p className="text-xs text-slate-500">Carga Horária: {treinamento.carga_horaria}h</p>}

                          {/* Datas salvas */}
                          {!editandoDatasTreinamento[treinamento.id] && (treinamento.data_inicio || treinamento.data_fim) && (
                            <div className="mt-2">
                              {treinamento.data_inicio && treinamento.data_fim && (
                                <p className="text-xs text-green-700 font-medium">
                                  📅 {format(new Date(treinamento.data_inicio + 'T00:00:00'), 'dd/MM/yyyy')} a {format(new Date(treinamento.data_fim + 'T00:00:00'), 'dd/MM/yyyy')}
                                </p>
                              )}
                              {treinamento.aproveitamento != null && (
                                <p className="text-xs text-green-700 font-medium">📊 Aproveitamento: {treinamento.aproveitamento}%</p>
                              )}
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex gap-1 ml-2">
                          ...
                          {editandoDatasTreinamento[treinamento.id] ? (
                            <>
                              <input 
                                ...
                                onChange={e => setEditandoDatasTreinamento({ ...editandoDatasTreinamento, [treinamento.id]: { ...editandoDatasTreinamento[treinamento.id], data_inicio: e.target.value } })} 
                                className="text-xs px-2 py-1 border rounded" 
                              />
                              <input 
                                type="date" 
                                value={editandoDatasTreinamento[treinamento.id]?.data_fim || ''} 
                                onChange={e => setEditandoDatasTreinamento({ ...editandoDatasTreinamento, [treinamento.id]: { ...editandoDatasTreinamento[treinamento.id], data_fim: e.target.value } })} 
                                className="text-xs px-2 py-1 border rounded" 
                              />
                              <input 
                                type="number" 
                                min="0" 
                                max="100" 
                                value={editandoDatasTreinamento[treinamento.id]?.aproveitamento !== undefined && editandoDatasTreinamento[treinamento.id]?.aproveitamento !== '' ? editandoDatasTreinamento[treinamento.id].aproveitamento : ''} 
                                onChange={e => setEditandoDatasTreinamento({ ...editandoDatasTreinamento, [treinamento.id]: { ...editandoDatasTreinamento[treinamento.id], aproveitamento: e.target.value } })} 
                                className="text-xs px-2 py-1 border rounded w-16" 
                                placeholder="%" 
                              />
                              <Button variant="outline" size="sm" className="bg-green-600 hover:bg-green-700 text-white" onClick={async () => {
                                try {
                                  const data_inicio = editandoDatasTreinamento[treinamento.id].data_inicio;
                                  const data_fim = editandoDatasTreinamento[treinamento.id].data_fim;
                                  const aproveitamento = editandoDatasTreinamento[treinamento.id].aproveitamento !== '' ? parseInt(editandoDatasTreinamento[treinamento.id].aproveitamento) : undefined;
                                  ...
                                  const updateData = {};
                                  if (data_inicio !== undefined && data_inicio !== null) updateData.data_inicio = data_inicio || null;
                                  if (data_fim !== undefined && data_fim !== null) updateData.data_fim = data_fim || null;
                                  if (aproveitamento !== undefined) updateData.aproveitamento = aproveitamento;

                                  const result = await base44.entities.Treinamento.update(treinamento.id, updateData);
                              ...
                              <Button variant="ghost" size="sm" onClick={() => { const n = { ...editandoDatasTreinamento }; delete n[treinamento.id]; setEditandoDatasTreinamento(n); }}>
                                <X className="w-3 h-3 text-red-500" />
                              </Button>
                            </>
                          ) : (
                            <Button variant="ghost" size="sm" onClick={() => setEditandoDatasTreinamento({ ...editandoDatasTreinamento, [treinamento.id]: { data_inicio: treinamento.data_inicio || '', data_fim: treinamento.data_fim || '', aproveitamento: treinamento.aproveitamento || '100' } })} title="Editar datas">
                              <Edit className="w-3 h-3 text-blue-500" />
                            </Button>
                          )}

                          {/* Visualizar certificado gerado */}
                          <Button variant="outline" size="sm" onClick={() => handleAbrirCertificado(treinamento)} title="Visualizar certificado">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            className={cn(temAnexo ? "border-green-500 bg-green-50 hover:bg-green-100 text-green-700" : "border-slate-300 hover:bg-slate-50")}
                            onClick={() => handleAbrirAnexoAssinado(treinamento)}
                            title="Gerenciar certificados assinados"
                          >
                            <CheckCircle2 className={cn("w-3.5 h-3.5", temAnexo ? "text-green-600" : "text-slate-400")} />
                            {temAnexo && <span className="ml-1 text-xs font-medium">{anexosDeste.length}</span>}
                          </Button>
                        </div>
                      </div>
      ...
      {showImportarCertificados && (
        <ImportarCertificadosTSTModal
          ...
          funcionarioAtual={selectedFuncionario || { id: funcionarioForm.id, nome_completo: funcionarioForm.nome_completo, funcao_nome: funcionarioForm.funcao_nome, funcao_id: funcionarioForm.funcao_id, ativo: true }}
          empresaAtiva={empresaAtiva}
