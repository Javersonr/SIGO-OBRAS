...
import { Truck, Wrench, CheckCircle2, AlertCircle } from 'lucide-react';
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const relatorio = caminhoes.map(caminhao => {
        ...
        const comLaudo = ferramentasCaminhao.filter(f => f.laudo_url && f.laudo_url.trim()).length;
        ...
        return {
          ...
          valorTotal,
          comLaudo,
          ferramentas: ferramentasCaminhao.sort((a, b) => (a.descricao || '').localeCompare(b.descricao || ''))
        };
      });
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="pt-6 text-center text-slate-500">Nenhum caminhão encontrado</CardContent>
        </Card>
      ) : (
        <div className="space-y-6">
            ...
            <Card key={caminhao.id}>
              ...
              <CardContent>
                {caminhao.ferramentas.length === 0 ? (
                  <p className="text-sm text-slate-500 text-center py-4">Nenhuma ferramenta vinculada</p>
                ) : (
                  <div className="overflow-x-auto">
                       ...
                       <TableBody>
                         {caminhao.ferramentas.map(ferr => (
                           <TableRow key={ferr.id} className="text-xs">
                             <TableCell className="font-mono font-semibold text-amber-600">{ferr.codigo}</TableCell>
                             <TableCell>
                               <div className="flex items-center gap-2">
                                 {ferr.descricao}
                                 {ferr.laudo_url && <Badge className="bg-blue-100 text-blue-700 text-xs">📄</Badge>}
                               </div>
                             </TableCell>
                             <TableCell>
                               <Badge variant={ferr.tipo === 'EPI' ? 'default' : 'outline'} className="text-xs">
                                 {ferr.tipo}
                               </Badge>
                             </TableCell>
                             ...
                             <TableCell>
                               {ferr.data_vencimento_laudo ? (
                                 <span className={ferr.data_vencimento_laudo < new Date().toISOString().split('T')[0] ? 'text-red-600 font-semibold' : ''}>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-200 flex justify-end">
                      <div className="text-right">
                        ...
                        <p className="text-lg font-bold text-slate-800">R$ {caminhao.valorTotal.toFixed(2)}</p>
                      </div>
                    </div>
                  </div>
                )}
