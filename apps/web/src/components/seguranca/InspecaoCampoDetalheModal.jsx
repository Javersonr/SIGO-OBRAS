...
import { Camera, Check, X, Bot, AlertCircle, CheckCircle2, Clock, Image, Download, FileText, FileSpreadsheet, Flag } from 'lucide-react';
import { toast } from 'sonner';
import { cn } from '@/lib/utils';
import { format } from 'date-fns';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from '@/components/ui/dropdown-menu';

export default function InspecaoCampoDetalheModal({ open, onOpenChange, inspecao, empresaAtiva, onSaved }) {
  ...
  const handleUploadFoto = async (e, idx) => {
    ...
    try {
      ...
      novosItens[idx] = { ...novosItens[idx], foto_inspecao_url: file_url, status: 'pendente', resultado_ia: '' };
      setItens(novosItens);
    ...
    finally { setUploadingFoto(null); if (e.target) e.target.value = ''; }
  };
...
Responda SOMENTE em JSON: {"status": "conforme" ou "nao_conforme", "descricao": "breve justificativa em até 2 frases"}`;
  ...
  const salvarItens = async (itensAtual) => {
    ...
    const status = inspecionados === 0 ? 'Em Andamento' : naoConformes > 0 ? 'Não Conforme' : inspecionados === total ? 'Concluída' : 'Em Andamento';
  ...
  const handleExportExcel = async () => {
    ...
    const resumo = [
      ...
      { 'Campo': 'Data', 'Valor': inspecao.data_inspecao ? format(new Date(inspecao.data_inspecao), 'dd/MM/yyyy') : '-' },
    ...
    XLSX.writeFile(wb, `inspecao_campo_${inspecao.id}_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const handleExportPDF = async () => {
    ...
    doc.text(`Data: ${inspecao.data_inspecao ? format(new Date(inspecao.data_inspecao), 'dd/MM/yyyy') : '-'} | Local: ${inspecao.local || '-'}`, 14, 29);
    doc.text(`Responsável: ${inspecao.responsavel_nome || '-'}`, 14, 35);

    let y = 45;
    itens.forEach((item, i) => {
      if (y > 270) { doc.addPage(); y = 20; }
      const statusLabel = item.status === 'conforme' ? 'CONFORME' : item.status === 'nao_conforme' ? 'NÃO CONFORME' : 'PENDENTE';
  ...
  const itensFiltrados = itens.filter(it => filtroStatus === 'todos' || it.status === filtroStatus);
        ...
        <DialogHeader className="px-6 py-4 border-b flex-shrink-0">
          ...
          <div className="mt-3 flex items-center gap-4">
            <div className="flex-1 bg-slate-200 rounded-full h-2">
              <div className="bg-blue-500 h-2 rounded-full transition-all" style={{ width: `${total > 0 ? (inspecionados / total) * 100 : 0}%` }} />
            </div>
            <span className="text-sm text-slate-600 whitespace-nowrap">{inspecionados}/{total}</span>
          ...
          <div className="mt-3 flex gap-2 flex-wrap">
            {[
              ...
              { key: 'nao_conforme', label: `Não Conformes (${naoConformes})`, cls: 'text-red-700' },
            ].map(f => (
              <button
                key={f.key}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="space-y-4 max-w-3xl mx-auto">
            {itensFiltrados.length === 0 && (
              <div className="text-center py-8 text-slate-400 text-sm">Nenhum item neste filtro</div>
            )}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-shrink-0 text-center">
                    ...
                    {item.foto_referencia_url ? (
                      <img src={item.foto_referencia_url} alt="Ref" className="w-20 h-20 object-cover rounded-lg border" />
                    ) : (
                  ...
                  <div className="flex-shrink-0 text-center">
                    ...
                    {item.foto_inspecao_url ? (
                      <div className="relative w-20 h-20">
                        <img src={item.foto_inspecao_url} alt="Insp" className="w-20 h-20 object-cover rounded-lg border" />
                        <button
                          onClick={() => { const u = [...itens]; u[idx] = { ...u[idx], foto_inspecao_url: '', status: 'pendente', resultado_ia: '' }; setItens(u); salvarItens(u); }}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        {item.obrigatorio && <Badge className="text-xs bg-red-100 text-red-700 mt-1">Obrigatório</Badge>}
                      </div>
                    ...
                    {item.resultado_ia && analisando !== idx && (
                      <div className={cn("mt-2 p-2 rounded-lg flex items-start gap-2 text-xs",
                        item.status === 'conforme' ? 'bg-green-50' : 'bg-red-50')}>
                        {item.status === 'conforme' ? <CheckCircle2 className="w-3.5 h-3.5 text-green-600 flex-shrink-0 mt-0.5" /> : <AlertCircle className="w-3.5 h-3.5 text-red-500 flex-shrink-0 mt-0.5" />}
                        <span className={item.status === 'conforme' ? 'text-green-700' : 'text-red-700'}>
                    ...
                    <div className="mt-2 flex gap-2">
                      <Button
                        ...
                        className={cn("gap-1 h-7 text-xs", item.status === 'conforme' ? 'bg-green-600 hover:bg-green-700' : '')}
                      ...
                      <Button
                        ...
                        className={cn("gap-1 h-7 text-xs", item.status === 'nao_conforme' ? 'bg-red-600 hover:bg-red-700' : '')}
                  ...
                  <Button onClick={handleSalvarObservacoes} disabled={saving} className="bg-amber-500 hover:bg-amber-600">
