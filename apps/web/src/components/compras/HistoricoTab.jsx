...
export default function HistoricoTab({ empresaAtiva, projetos }) {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const cotacoesData = await base44.entities.Cotacao.filter({ 
  ...
  const getStatusBadge = useCallback((cotacao) => {
    const statusMap = {
      ...
      'Cancelada': { bg: 'bg-red-100', text: 'text-red-700', icon: XCircle, label: 'Perdida/Cancelada' },
  ...
  const { cotacoesFiltradas, stats } = useMemo(() => {
    ...
    const filtradas = cotacoes.filter(cot => {
      if (busca && !cot.numero?.toLowerCase().includes(buscaLower) && 
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {(busca || filtroDataInicio || filtroDataFim || filtroProjeto !== 'todos' || filtroStatus !== 'todos') && (
           <Button
             variant="outline"
             onClick={() => {
               setBusca('');
                          ...
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              ...
                              {forn.fornecedor_email && (
                                <p className="text-xs text-slate-500 mt-1">{forn.fornecedor_email}</p>
                              )}
                            </div>
                            <Badge className={
