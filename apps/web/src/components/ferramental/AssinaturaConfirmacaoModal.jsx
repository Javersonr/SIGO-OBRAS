...
import { PenLine, Camera, RotateCcw, Check, X, MapPin, Loader2, CheckCircle2 } from 'lucide-react';
...
export default function AssinaturaConfirmacaoModal({ open, onClose, onConfirm, funcionarioNome, itens = [] }) {
  ...
  const confirmar = async () => {
    if (!fotoUrl) { toast.error('Foto de confirmação obrigatória'); setStep('foto'); return; }
    if (!localizacao) { toast.error('Localização obrigatória'); setStep('localizacao'); return; }
    ...
    try {
      ...
      const { file_url: fotoUploadUrl } = await base44.integrations.Core.UploadFile({ file: fotoFile });
            ...
            <div className="space-y-0.5">
              {itens.map((item, i) => (
                <div key={i} className="text-xs text-slate-700 flex gap-2">
                  ...
                  <span>{item.descricao || item.nome}{item.codigo ? <span className="font-mono text-slate-500 ml-1">({item.codigo})</span> : ''}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {STEPS.map((s) => (
            <button
              ...
              className={`flex-1 py-2.5 text-xs font-medium flex items-center justify-center gap-1.5 transition-colors relative ${
                step === s ? 'border-b-2 border-slate-800 text-slate-800' : 'text-slate-400 hover:text-slate-600'
              }`}
            >
              {stepIcon(s)}
              {stepLabel(s)}
            ...
            <div className="text-center">
              {!cameraAtiva && !fotoUrl && (
                <div className="py-4">
                  ...
                  <p className="text-xs text-slate-400 mb-4">Fotografe o funcionário/local para registrar a entrega</p>
              ...
              {cameraAtiva && (
                <div>
                  <video ref={videoRef} autoPlay playsInline className="w-full rounded-xl border-2 border-slate-200 max-h-56 object-cover" />
                  <div className="flex gap-2 mt-3 justify-center">
                    <Button onClick={tirarFoto} className="bg-slate-800 hover:bg-slate-700 gap-2">
                      <Camera className="w-4 h-4" /> Capturar
                    </Button>
                    <Button onClick={pararCamera} variant="outline" size="sm">Cancelar</Button>
                  </div>
                </div>
              )}
              ...
              {fotoUrl && !cameraAtiva && (
                <div>
                  <img src={fotoUrl} alt="Confirmação" className="w-full rounded-xl border-2 border-green-200 max-h-48 object-cover mb-3" />
                  <div className="flex gap-2 justify-center mb-3">
                    <Button onClick={() => { setFotoUrl(null); abrirCamera(); }} variant="outline" size="sm" className="gap-1">
            ...
            <div className="text-center py-2">
              {!localizacao && !buscandoLocalizacao && (
                <>
                  ...
                  <p className="text-xs text-slate-400 mb-4">Registre o local onde a movimentação está sendo confirmada</p>
                  <Button onClick={buscarLocalizacao} className="bg-slate-800 hover:bg-slate-700 gap-2">
                ...
                <div>
                  <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-left mb-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-green-800 mb-1">Localização capturada</p>
                        {enderecoFormatado && (
                          <p className="text-xs text-slate-600 mb-2 leading-relaxed">{enderecoFormatado}</p>
                        )}
                        ...
                        {localizacao.accuracy && (
                          <p className="text-xs text-slate-400 mt-1">Precisão: ±{Math.round(localizacao.accuracy)}m</p>
                        )}
                      </div>
                    </div>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => { setLocalizacao(null); setEnderecoFormatado(''); buscarLocalizacao(); }}
