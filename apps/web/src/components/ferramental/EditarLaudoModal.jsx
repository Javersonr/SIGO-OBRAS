...
import { Upload, Download, FileText, Trash2, Eye, Clock, Plus, X } from 'lucide-react';

export default function EditarLaudoModal({ open, onClose, ferramenta, onSaved }) {
  const [form, setForm] = useState({ numero_serie: '', numero_laudo: '', data_vencimento_laudo: '', laudo_url: '' });
  ...
  const loadHistorico = async (ferramentaId) => {
    ...
    try {
      const laudos = await base44.entities.LaudoFerramenta.filter({ ferramenta_id: ferramentaId }, '-data_laudo');
      setHistorico(laudos);
    } catch {
      setHistorico([]);
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (form.numero_laudo && form.laudo_url) {
        const jaExiste = historico.some(h => h.numero_laudo === form.numero_laudo && h.foto_laudo_url === form.laudo_url);
        if (!jaExiste) {
  ...
  const isVencendoBreve = form.data_vencimento_laudo && !isVencido &&
        ...
        <div className="space-y-5 pt-2">
          ...
          <div className="space-y-3 p-4 border rounded-lg bg-slate-50">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={e => setForm(p => ({ ...p, data_vencimento_laudo: e.target.value }))}
                  className="mt-1.5"
                />
              </div>
            </div>
            ...
            {form.data_vencimento_laudo && (
              <Badge className={isVencido ? 'bg-red-100 text-red-700' : isVencendoBreve ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}>
            ...
            {form.laudo_url ? (
              <div className="flex items-center gap-2 p-2 border rounded bg-white">
                ...
                <a href={form.laudo_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-700" title="Visualizar">
                  <Eye className="w-4 h-4" />
                </a>
                <a href={form.laudo_url} download className="text-slate-500 hover:text-slate-700" title="Baixar">
                  <Download className="w-4 h-4" />
                </a>
                <button onClick={() => setForm(p => ({ ...p, laudo_url: '' }))} className="text-red-500 hover:text-red-600" title="Remover">
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
            ) : (
              <label className={`flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${uploading ? 'opacity-50 cursor-not-allowed' : 'border-slate-300 hover:border-slate-400 hover:bg-white'}`}>
                ...
                {uploading
                  ...
                  : <><Upload className="w-4 h-4 text-slate-500" /><span className="text-sm text-slate-600">Anexar novo laudo (PDF/imagem)</span></>
                }
              </label>
            )}
          </div>
          ...
          <div>
            ...
            {loadingHistorico ? (
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                ...
                return (
                <div key={h.id} className="flex items-center justify-between p-2 border rounded bg-white text-xs gap-2">
                  <div className="flex-1 min-w-0">
                    <span className="font-mono font-semibold text-slate-700">{h.numero_laudo || 'S/N'}</span>
                    {h.data_laudo && <span className="text-slate-400 ml-2">emissão: {new Date(h.data_laudo + 'T00:00:00').toLocaleDateString('pt-BR')}</span>}
                    {h.data_vencimento && (
                      <span className={`ml-2 ${venc ? 'text-red-600' : 'text-green-600'}`}>
                        · venc: {new Date(h.data_vencimento + 'T00:00:00').toLocaleDateString('pt-BR')}
                      </span>
                    )}
                    {h.resultado && <Badge className={`ml-2 text-xs ${h.resultado === 'Aprovado' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{h.resultado}</Badge>}
                  </div>
                  {h.foto_laudo_url && (
                    <div className="flex gap-1 flex-shrink-0">
                      <a href={h.foto_laudo_url} target="_blank" rel="noopener noreferrer" className="p-1 text-blue-600 hover:bg-blue-50 rounded" title="Visualizar">
                        <Eye className="w-3.5 h-3.5" />
                      </a>
                      <a href={h.foto_laudo_url} download className="p-1 text-slate-500 hover:bg-slate-50 rounded" title="Baixar">
