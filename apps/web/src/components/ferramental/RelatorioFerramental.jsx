...
import { Calendar, Download, Filter, AlertTriangle, MapPin } from "lucide-react";
...
export default function RelatorioFerramental({
  ferramentas,
  movimentacoes,
  almoxarifados,
  empresaAtiva,
  ...
  const ferramentasVencimento = useMemo(() => {
    ...
    return dadosFiltrados.ferrFiltradas
      .filter((f) => {
  ...
  const stats = useMemo(() => {
    ...
    const manutencao = dadosFiltrados.ferrFiltradas.filter(
      (f) => f.status === "Em Manutenção"
    ).length;
    const valorTotal = dadosFiltrados.ferrFiltradas.reduce(
      (sum, f) => sum + (f.valor_unitario || 0),
      0
    );
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="localizacao" className="w-full">
        ...
        <TabsList className="grid w-full grid-cols-5">
          <TabsTrigger value="localizacao" className="text-xs sm:text-sm">
            Por Local
          </TabsTrigger>
          <TabsTrigger value="status" className="text-xs sm:text-sm">
            Por Status
          </TabsTrigger>
          <TabsTrigger value="vencimento" className="text-xs sm:text-sm">
            Vencimento
          </TabsTrigger>
          <TabsTrigger value="funcao" className="text-xs sm:text-sm">
            Por Função
          </TabsTrigger>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      handleExportarCSV(ferramentasPorLocalizacao, "relatorio_localizacao")
                    }
                  >
            ...
            <CardContent>
              {ferramentasVencimento.length === 0 ? (
                <p className="text-center text-slate-500 py-8">
                  Nenhuma ferramenta com vencimento próximo
                </p>
              ) : (
                        ...
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <p className="font-medium">
                              {mov.ferramenta_descricao || mov.tipo_movimentacao}
                            </p>
