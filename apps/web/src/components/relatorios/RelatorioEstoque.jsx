...
import { Package, AlertTriangle, TrendingDown, TrendingUp, DollarSign, Archive } from 'lucide-react';
...
export default function RelatorioEstoque({ empresaId }) {
  ...
  useEffect(() => {
    ...
    base44.entities.Material.filter({ empresa_id: empresaId, ativo: true })
      .then(setMateriais).finally(() => setLoading(false));
  }, [empresaId]);

  const kpis = useMemo(() => {
    const total = materiais.length;
    const abaixoMinimo = materiais.filter(m => (m.estoque || 0) <= (m.estoque_minimo || 0) && (m.estoque_minimo || 0) > 0).length;
    const semEstoque = materiais.filter(m => (m.estoque || 0) === 0).length;
    const valorTotal = materiais.reduce((s, m) => s + ((m.estoque || 0) * (m.preco_medio || m.preco || 0)), 0);
  ...
  const abaixoMinimo = useMemo(() =>
    materiais.filter(m => (m.estoque || 0) <= (m.estoque_minimo || 0) && (m.estoque_minimo || 0) > 0)
      .sort((a, b) => (a.estoque || 0) - (b.estoque || 0)).slice(0, 10),
  [materiais]);

  const handleCSV = () => exportarCSV(
    ['Nome', 'Categoria', 'Estoque', 'Estoque Mínimo', 'Preço Médio', 'Valor Total'],
    materiais.map(m => [m.nome, m.categoria || '', m.estoque || 0, m.estoque_minimo || 0, m.preco_medio || m.preco || 0, (m.estoque || 0) * (m.preco_medio || m.preco || 0)]),
    'relatorio_estoque'
  );
  ...
  const handlePDF = () => exportarPDF(
    ...
    materiais.slice(0, 50).map(m => [m.nome, m.categoria || '-', m.estoque || 0, m.estoque_minimo || 0, fmt((m.estoque || 0) * (m.preco_medio || m.preco || 0))]),
    'relatorio_estoque'
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        <KpiCard titulo="Materiais" valor={kpis.total} subtitulo="cadastrados" icon={Package} cor="blue" />
        <KpiCard titulo="Total Itens" valor={kpis.totalItens.toLocaleString('pt-BR')} subtitulo="em estoque" icon={Archive} cor="green" />
        <KpiCard titulo="Abaixo Mínimo" valor={kpis.abaixoMinimo} subtitulo="materiais" icon={AlertTriangle} cor="amber" tendencia={kpis.abaixoMinimo > 0 ? 'down' : 'up'} />
        <KpiCard titulo="Sem Estoque" valor={kpis.semEstoque} subtitulo="materiais" icon={TrendingDown} cor="red" />
        <KpiCard titulo="Valor Total" valor={fmt(kpis.valorTotal)} subtitulo="em estoque" icon={DollarSign} cor="purple" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle className="text-sm font-semibold text-slate-700">Valor por Categoria</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={porCategoria} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" horizontal={false} />
                <XAxis type="number" tickFormatter={(v) => fmt(v).slice(0,10)} tick={{ fontSize: 10 }} />
        ...
        <Card>
          <CardHeader><CardTitle className="text-sm font-semibold text-slate-700">Qtd Materiais por Categoria</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie data={porCategoria} dataKey="quantidade" nameKey="nome" cx="50%" cy="50%" outerRadius={100}
                ...
                <tbody>
                  {abaixoMinimo.map(m => (
                    <tr key={m.id} className="border-b border-slate-100 hover:bg-amber-50">
                      ...
                      <td className="py-2 text-right font-bold text-red-600">{m.estoque || 0} {m.unidade}</td>
                      <td className="py-2 text-right text-slate-600">{m.estoque_minimo || 0} {m.unidade}</td>
                      <td className="py-2 text-right text-amber-700 font-medium">{(m.estoque_minimo || 0) - (m.estoque || 0)} {m.unidade}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
