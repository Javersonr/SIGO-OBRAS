...
export default function SolicitacaoModal({ 
  ...
  const carregarMateriais = React.useCallback(async () => {
    ...
    try {
      ...
      const matsMapeados = mats.map(m => ({ id: m.id, nome: m.nome, tipo: 'Material', preco: m.preco || null, unidade: m.unidade || 'UN', codigo: m.codigo || '' }));
      const ferrMapeadas = ferramentas.map(f => ({ id: f.id, nome: f.descricao || f.nome, tipo: 'Ferramenta', preco: null, unidade: 'UN', codigo: f.codigo || '' })).filter(f => f.nome);
      const todos = [...matsMapeados, ...ferrMapeadas].sort((a, b) => (a.nome || '').localeCompare(b.nome || '', 'pt-BR'));
      setMateriais(todos);
    } catch (error) {
  ...
  const addItem = (focusIndex = null) => {
    const novoItem = { descricao: '', quantidade: 1, unidade: 'UN', data_necessidade: '', ultimo_preco: null };
    setForm(prev => {
  ...
  const carregarOrcamentoProjeto = async (projetoId) => {
    ...
    try {
      const [orcamentoItens, todosPedidosItems, reservasAtivas] = await Promise.all([
        base44.entities.OrcamentoItem.filter({ empresa_id: empresaAtiva.id, projeto_id: projetoId }),
        base44.entities.PedidoCompraItem.filter({ empresa_id: empresaAtiva.id }),
        base44.entities.ReservaMaterial.filter({ empresa_id: empresaAtiva.id, projeto_id: projetoId, status: 'Ativa' })
      ]);
      ...
      if (orcamentoItens.length > 0) {
        const itensFormatados = orcamentoItens.map(item => {
          ...
          const reserva = reservasAtivas.find(r => r.material_id && r.material_id === item.material_id);
        ...
        setForm(prev => ({ 
          ...
          itens: itensFormatados.sort((a, b) => (a.descricao || '').localeCompare(b.descricao || '', 'pt-BR'))
        }));
      } else {
        ...
        setForm(prev => ({ 
          ...
          itens: [{ descricao: '', quantidade: 1, unidade: 'UN', data_necessidade: '', ultimo_preco: null, qtd_reservada: 0 }]
        }));
      }
    } catch (error) {
  ...
  const carregarOrcamentosMultiProjetos = async (projetoIds) => {
    ...
    try {
      const [todosPedidosItems, todasReservas, ...todosOrcamentos] = await Promise.all([
        ...
        ...projetoIds.map(pid => base44.entities.OrcamentoItem.filter({ empresa_id: empresaAtiva.id, projeto_id: pid }))
      ]);
      ...
      const itens = Object.values(itensMap).map(item => {
        const reserva = reservasDosProjetos.find(r => r.material_id && r.material_id === item.material_id);
        const qtdReservada = reserva?.quantidade_reservada || 0;
        return {
          ...item,
          qtd_reservada: qtdReservada,
      ...
      setForm(prev => ({
        ...
        itens: itens.length > 0 ? itens : [{ descricao: '', quantidade: 1, unidade: 'UN', data_necessidade: '', ultimo_preco: null, qtd_reservada: 0 }]
      }));
    } catch (error) {
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                onClick={() => {
                  setMultiProjeto(v => !v);
                  setForm(prev => ({ ...prev, projetos_ids: '', projetos_nomes: '', projeto_id: '', projeto_nome: '' }));
                }}
              ...
              <>
                <Select
                  ...
                  onValueChange={(v) => {
                    ...
                    setForm(prev => ({ ...prev, projeto_id: v, projeto_nome: projeto?.nome || '' }));
                ...
                {loadingOrcamento && <p className="text-xs text-slate-500">Carregando itens do orçamento...</p>}
                {!loadingOrcamento && form.itens.length > 0 && form.itens[0].descricao && (
                  <p className="text-xs text-blue-600">{form.itens.filter(i => i.descricao).length} itens do orçamento carregados</p>
                )}
              </>
            ) : (
              <>
                <div className="border border-slate-200 rounded-lg p-3 space-y-2 max-h-48 overflow-y-auto">
                  {projetos.map(p => {
                    const selecionados = (() => { try { return JSON.parse(form.projetos_ids || '[]'); } catch { return []; } })();
                    ...
                    return (
                      <label key={p.id} className="flex items-center gap-2 cursor-pointer hover:bg-slate-50 p-1 rounded">
                        <input
                          ...
                          checked={marcado}
                          onChange={() => {
                            const novos = marcado ? selecionados.filter(id => id !== p.id) : [...selecionados, p.id];
                ...
                {(() => {
                  const selecionados = (() => { try { return JSON.parse(form.projetos_ids || '[]'); } catch { return []; } })();
                  const nomes = projetos.filter(p => selecionados.includes(p.id)).map(p => p.nome);
                  return nomes.length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {nomes.map(n => <Badge key={n} className="text-xs bg-blue-100 text-blue-700">{n}</Badge>)}
                    </div>
                  ) : null;
                })()}
                {loadingOrcamento && <p className="text-xs text-slate-500">Carregando itens dos orçamentos...</p>}
                {!loadingOrcamento && form.itens.length > 0 && form.itens[0].descricao && (
                  <p className="text-xs text-blue-600">{form.itens.filter(i => i.descricao).length} itens agrupados carregados</p>
                )}
              </>
            )}
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex gap-2">
                <Button type="button" variant="outline" size="sm" onClick={() => setShowImportar(true)}>
                  ...
                  <div className="flex flex-col md:flex-row gap-3 items-start">
                    <div className="flex-1 relative">
                      ...
                      <Input
                       ...
                       onBlur={() => setTimeout(() => { setSugestoesAberta(null); setSugestaoFocada(-1); }, 200)}
                       onKeyDown={(e) => {
                         const filtrados = sugestoesAberta === index && item.descricao.length >= 1
                           ? materiais.filter(m => m.nome?.toLowerCase().includes(item.descricao.toLowerCase())).slice(0, 10)
                           : [];
                         ...
                         if (e.key === 'Enter' && sugestaoFocada >= 0 && filtrados[sugestaoFocada]) {
                           e.preventDefault();
                            ...
                            {filtrados.map((m, mIdx) => (
                              <button
                                ...
                                >
                                  ...
                                  <span className={`text-xs px-1.5 py-0.5 rounded flex-shrink-0 ${m.tipo === 'Ferramenta' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
                                    {m.tipo}
                                  </span>
                                </button>
                            ))}
                    ...
                    <div className="w-28 hidden sm:block">
                      ...
                      <Input
                        type="text"
                        value={item.qtd_reservada > 0 ? `${item.qtd_reservada} ${item.unidade || ''}` : '—'}
                        disabled
            ...
            <p className="text-xs text-slate-500 mt-2">
              * Campos obrigatórios. Digite as últimas palavras para adicionar novos itens automaticamente.
            </p>
          </div>
