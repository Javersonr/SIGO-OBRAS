...
import { Upload, Download, FileSpreadsheet, ChevronDown, Camera, MessageSquare, FileText, Archive } from 'lucide-react';
...
export default function PreLancamentosTab({ empresaAtiva, contas = [], categorias = [], onReload = () => {}, usuarioEmail, usuarioNome, verTodos, verProprios, podeAprovar, podePagar }) {
  ...
  const { online, itemsPendentes, sincronizando, marcarSincronizado, deletarOffline } = useOfflineSync();

  const handleSincronizar = async (itens) => {
    const response = await base44.functions.invoke('sincronizarPreLancamentosOffline', { itensOffline: itens });
  ...
  const handleExportarExcel = async () => {
    ...
    try {
      ...
      if (todos.length === 0) { alert('Nenhum pré-lançamento encontrado'); return; }
      const dados = todos.map(p => {
        const d = typeof p.dados_extraidos === 'string' ? JSON.parse(p.dados_extraidos || '{}') : (p.dados_extraidos || {});
        return {
      ...
      ws['!cols'] = [{ wch: 25 }, { wch: 35 }, { wch: 14 }, { wch: 14 }, { wch: 20 }, { wch: 12 }, { wch: 30 }, { wch: 50 }];
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Pré-Lançamentos');
      XLSX.writeFile(wb, `Pre_Lancamentos_${new Date().toLocaleDateString('pt-BR').replace(/\//g, '-')}.xlsx`);
    } catch (e) {
  ...
  const handleBaixarModelo = () => {
    if (!XLSX) return;
    const dados = [{ 'Data Competência': '2026-01-07', 'Data Vencimento': '2026-01-15', 'Descrição': 'Descrição do pré-lançamento', 'Tipo': 'Receita ou Despesa', 'Fornecedor': 'Nome do Fornecedor (opcional)', 'Cliente': 'Nome do Cliente (opcional)', 'Categoria': 'Nome da Categoria', 'Conta': 'Nome da Conta', 'Projeto': 'Título do Projeto (opcional)', 'Centro de Custo': 'Nome do Centro de Custo (opcional)', 'Valor': 1000.00, 'Observações': 'Observações adicionais' }];
    const ws = XLSX.utils.json_to_sheet(dados);
    const wb = XLSX.utils.book_new();
  ...
  const handleImportarExcel = async (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const [todosFornecedores, todosClientes, todosProjetos, todasCategorias, todasContas] = await Promise.all([
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          try {
            ...
            if (!conta) { erros++; setImportacao(prev => ({ ...prev, processados: i + 1, erros: prev.erros + 1 })); continue; }
            await base44.entities.TransacaoFinanceira.create({
              ...
              fornecedor_id: todosFornecedores.find(f => f.nome_razao === row.Fornecedor)?.id || null,
              fornecedor_nome: todosFornecedores.find(f => f.nome_razao === row.Fornecedor)?.nome_razao || null,
              cliente_id: todosClientes.find(c => c.nome_razao === row.Cliente)?.id || null,
              cliente_nome: todosClientes.find(c => c.nome_razao === row.Cliente)?.nome_razao || null,
          ...
          } catch { erros++; setImportacao(prev => ({ ...prev, processados: i + 1, erros: prev.erros + 1 })); }
        }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap justify-between items-center gap-2">
        ...
        <div className="flex gap-2 flex-wrap items-center">
          ...
          {verTodos && (
            <Select value={filtroUsuario || ''} onValueChange={(v) => setFiltroUsuario(v || '')}>
              ...
              <SelectContent>
                ...
                {usuarios.map(u => (
                  <SelectItem key={u.id} value={u.usuario_email}>{u.nome_completo || u.usuario_email}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          )}
          ...
          <a href={base44.agents.getWhatsAppConnectURL('prelancamentos_whatsapp')} target="_blank" rel="noopener noreferrer">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-52">
              ...
              <DropdownMenuItem onClick={() => document.getElementById('importar-excel-pre').click()}>
                <Upload className="w-4 h-4 mr-2" />
                Importar Excel
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          <input type="file" id="importar-excel-pre" accept=".xlsx,.xls" onChange={handleImportarExcel} className="hidden" />
        </div>
      </div>
      ...
      <PreLancamentosAReconciliar
        ...
        onReload={() => { setReloadTrigger(prev => prev + 1); onReload(); }}
      ...
      <NovoPreLancamentoModal
        ...
        verTodos={verTodos}
        isAdmin={verTodos}
        onSucesso={() => { setReloadTrigger(prev => prev + 1); onReload(); }}
      />
    </div>
  );
}