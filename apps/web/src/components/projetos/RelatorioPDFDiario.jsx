...
export function gerarRelatorioDiarioPDF(diario, empresaAtiva) {
  ...
  if (diario.mao_de_obra) {
    try {
      const maoObra = typeof diario.mao_de_obra === 'string' 
  ...
  if (diario.fotos) {
    try {
      const fotos = typeof diario.fotos === 'string' 
        ? JSON.parse(diario.fotos) 
        : diario.fotos;
    ...
    doc.save(`RDO_${diario.obra_nome?.replace(/\s/g, '_')}_${new Date(diario.data).toLocaleDateString('pt-BR').replace(/\//g, '-')}.pdf`);
  } catch (error) {
