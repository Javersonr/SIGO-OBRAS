  ...
  const carregarDados = async () => {
    ...
    try {
      const [projsData, contasData, usuariosData] = await Promise.all([
        base44.entities.Projeto.filter({ empresa_id: empresaId }),
        base44.entities.ContaFinanceira.filter({ empresa_id: empresaId }),
        base44.entities.UsuarioEmpresa.filter({ empresa_id: empresaId, ativo: true })
      ]);
      ...
      if (!podeVerTodos) {
        ...
        projsAtivos = projsAtivos.filter(p => {
          ...
          if (p.responsaveis_emails) {
            try {
              ...
              if (Array.isArray(emails) && emails.some(e => String(e).toLowerCase().trim() === emailLower)) {
                return true;
              }
            } catch (e) {
          ...
          if (p.responsavel_email && String(p.responsavel_email).toLowerCase().trim() === emailLower) {
            return true;
          }
          ...
          if (p.responsaveis_ids) {
            try {
              ...
              if (Array.isArray(ids)) {
                ...
                const matchingUsers = usuariosData.filter(u => ids.includes(u.id) && u.usuario_email.toLowerCase().trim() === emailLower);
        ...
        contasAtivas = contasAtivas.filter(c => 
          c.tipo === 'Fundo Fixo' && 
          c.responsavel_email && 
      ...
      <CardContent className="space-y-6">
        ...
        <div>
          <Label className="text-sm font-medium mb-2 block">
            Projeto ({projetos.length} disponível{projetos.length !== 1 ? 'is' : ''}{!podeVerTodos ? ' — apenas seus projetos' : ''})
          </Label>
          <select
            value={projetoId}
            onChange={e => { setProjetoId(e.target.value); setErro(null); }}
        ...
        <div>
          <Label className="text-sm font-medium mb-2 block">
            Conta * ({contas.length} disponível{contas.length !== 1 ? 'is' : ''}{!podeVerTodos ? ' — apenas seu Fundo Fixo' : ''})
          </Label>
          <select
            value={contaId}
        ...
        <Alert className="border-amber-200 bg-amber-50">
          ...
          <AlertDescription className="text-amber-800 text-sm">
            O pré-lançamento será criado como pendente. Você precisará conciliá-lo antes de gerar a despesa final.
          </AlertDescription>
        </Alert>
