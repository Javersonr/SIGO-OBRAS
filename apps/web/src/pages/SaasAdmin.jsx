...
import { 
  Building2, Users, DollarSign, CreditCard, Plus, Edit, Trash2,
  Search, Filter, MoreHorizontal, AlertCircle, CheckCircle2, Clock,
  Ban, TrendingUp, Package, FileText, Receipt, Download, Lock, Eye, EyeOff, KeyRound,
  Copy, CheckCircle, Loader2, UserPlus
} from 'lucide-react';
...
export default function SaasAdmin() {
  ...
  const [criarAdminForm, setCriarAdminForm] = useState({ nome_completo: '', email: '', senha: '', confirmar_senha: '' });
  ...
  // Form data
  const [empresaForm, setEmpresaForm] = useState({ nome: '', razao_social: '', cnpj: '', email: '', telefone: '', is_super_admin_empresa: false });
  ...
  const [assinaturaForm, setAssinaturaForm] = useState({ empresa_id: '', plano_id: '', status: 'Ativa', forma_pagamento: 'Boleto', data_inicio: '', data_vencimento: '', observacoes: '' });
  const [propostaForm, setPropostaForm] = useState({ empresa_nome: '', empresa_cnpj: '', contato_nome: '', contato_email: '', plano_id: '', desconto_percentual: 0, vigencia_meses: 12, observacoes: '' });
  const [boletoForm, setBoletoForm] = useState({ empresa_id: '', valor: 0, data_vencimento: '', instrucoes: '' });
  const [pagamentoForm, setPagamentoForm] = useState({ empresa_id: '', valor: 0, data_vencimento: '', status: 'Pendente', forma_pagamento: 'Boleto', assinatura_id: '' });
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [empresasList, planosList, assinaturasList, propostasList, boletosList, pagamentosList, usuariosList, superAdminsList] = await Promise.all([
  ...
  const handleOpenEmpresaModal = (empresa = null) => {
    if (empresa) {
      ...
      setEmpresaForm({ nome: '', razao_social: '', cnpj: '', email: '', telefone: '', is_super_admin_empresa: false });
  ...
  const handleOpenAssinaturaModal = (assinatura = null) => {
    if (assinatura) {
      ...
      setAssinaturaForm({ empresa_id: '', plano_id: '', status: 'Ativa', forma_pagamento: 'Boleto', data_inicio: '', data_vencimento: '', observacoes: '' });
    }
  ...
  const handleOpenPropostaModal = (proposta = null) => {
    if (proposta) {
      ...
      setPropostaForm({ empresa_nome: '', empresa_cnpj: '', contato_nome: '', contato_email: '', plano_id: '', desconto_percentual: 0, vigencia_meses: 12, observacoes: '' });
    }
  ...
  const handleOpenPagamentoModal = () => {
    setPagamentoForm({ empresa_id: '', valor: 0, data_vencimento: '', status: 'Pendente', forma_pagamento: 'Boleto', assinatura_id: '' });
    ...
    <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {empresas.filter(e => e.nome?.includes(searchTerm)).map(empresa => (
                    <TableRow key={empresa.id}>
                      <TableCell className="font-medium">{empresa.nome}</TableCell>
                      <TableCell>{empresa.cnpj || '-'}</TableCell>
                      <TableCell>{empresa.email}</TableCell>
                      <TableCell>
                        <Button variant="ghost" size="sm" onClick={() => handleOpenEmpresaDetalheModal(empresa)}>
                          Ver
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleOpenEmpresaModal(empresa)}>
                          Editar
                        </Button>
                      </TableCell>
                    </TableRow>
                ...
                {planos.map(plano => (
                  <Card key={plano.id} className="border-2">
                    <CardContent className="p-6">
                      ...
                      <p className="text-2xl font-bold text-amber-600 my-2">{formatCurrency(plano.valor_mensal)}</p>
                      ...
                      <div className="flex gap-2 mt-4">
                        <Button variant="ghost" size="sm" onClick={() => handleOpenPlanoModal(plano)}>Editar</Button>
                        <Button variant="ghost" size="sm" onClick={() => handleDeletePlano(plano)} className="text-red-600">Deletar</Button>
                      </div>
                    </CardContent>
                  </Card>
                ))}
                ...
                <TableBody>
                  {assinaturas.map(a => (
                    <TableRow key={a.id}>
                      ...
                      <TableCell>
                        <Button variant="ghost" size="sm" onClick={() => handleOpenAssinaturaModal(a)}>Editar</Button>
                        <Button variant="ghost" size="sm" onClick={() => handleDeleteAssinatura(a)} className="text-red-600">Deletar</Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
                ...
                <TableBody>
                  {propostas.map(p => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell>
                        <Button variant="ghost" size="sm" onClick={() => handleOpenPropostaModal(p)}>Editar</Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
                ...
                <TableBody>
                  {boletos.map(b => (
                    <TableRow key={b.id}>
                      ...
                      <TableCell>{new Date(b.data_vencimento).toLocaleDateString('pt-BR')}</TableCell>
                      <TableCell><Badge className={statusColors[b.status]}>{b.status}</Badge></TableCell>
                      <TableCell>
                        <Button variant="ghost" size="sm" onClick={() => handleOpenBoletoModal(b)}>Editar</Button>
                        <Button variant="ghost" size="sm" onClick={() => handleDeleteBoleto(b)} className="text-red-600">Deletar</Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
                ...
                <TableBody>
                  {pagamentos.map(p => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell><Badge className={pagamentoColors[p.status]}>{p.status}</Badge></TableCell>
                      <TableCell>
                        {p.status === 'Pendente' && (
                          <Button size="sm" onClick={() => handleMarcarPago(p)} className="bg-green-500">Marcar Pago</Button>
                        )}
                      </TableCell>
                    </TableRow>
                  ))}
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              <Card>
                <CardContent className="p-6">
                  ...
                  <p className="text-2xl font-bold text-green-700">{formatCurrency(stats.receitaMensal)}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-6">
                ...
                <TableBody>
                  {superAdmins.map(admin => (
                    <TableRow key={admin.id}>
                      ...
                      <TableCell>
                        <Button variant="ghost" size="sm" onClick={() => handleRemoveSuperAdmin(admin)} className="text-red-600">Remover</Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
      ...
      <Sheet open={showEmpresaModal} onOpenChange={setShowEmpresaModal}>
        <SheetContent side="right" className="w-full overflow-y-auto" data-fullscreen-modal>
          ...
          <div className="space-y-4 py-4">
            <div>
              <Label>Nome *</Label>
              <Input value={empresaForm.nome} onChange={(e) => setEmpresaForm({...empresaForm, nome: e.target.value})} className="mt-1.5" />
            </div>
            <div>
              <Label>Razão Social</Label>
              <Input value={empresaForm.razao_social} onChange={(e) => setEmpresaForm({...empresaForm, razao_social: e.target.value})} className="mt-1.5" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>CNPJ</Label>
                <Input value={empresaForm.cnpj} onChange={(e) => setEmpresaForm({...empresaForm, cnpj: e.target.value})} className="mt-1.5" />
              </div>
              <div>
                <Label>Email *</Label>
                <Input type="email" value={empresaForm.email} onChange={(e) => setEmpresaForm({...empresaForm, email: e.target.value})} className="mt-1.5" />
              </div>
            </div>
          </div>
          <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={() => setShowEmpresaModal(false)}>Cancelar</Button>
            <Button onClick={handleSaveEmpresa} className="bg-amber-500">Salvar</Button>
          </div>
        </SheetContent>
      </Sheet>
      ...
      <Sheet open={showPlanoModal} onOpenChange={setShowPlanoModal}>
        <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
          ...
          <div className="space-y-4 py-4">
            <div>
              <Label>Nome *</Label>
              <Input value={planoForm.nome} onChange={(e) => setPlanoForm({...planoForm, nome: e.target.value})} className="mt-1.5" />
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <Label>Valor Mensal</Label>
                <Input type="number" value={planoForm.valor_mensal} onChange={(e) => setPlanoForm({...planoForm, valor_mensal: parseFloat(e.target.value)})} className="mt-1.5" />
              </div>
              <div>
                <Label>Max. Usuários</Label>
                <Input type="number" value={planoForm.max_usuarios} onChange={(e) => setPlanoForm({...planoForm, max_usuarios: parseInt(e.target.value)})} className="mt-1.5" />
              </div>
              <div>
                <Label>Max. Projetos</Label>
                <Input type="number" value={planoForm.max_projetos} onChange={(e) => setPlanoForm({...planoForm, max_projetos: parseInt(e.target.value)})} className="mt-1.5" />
              </div>
            </div>
                ...
                {Object.keys(planoForm.modulos_liberados).map(modulo => (
                  <div key={modulo} className="flex items-center gap-2">
                    <input
                      ...
                      className="rounded"
                    />
                    <Label htmlFor={modulo} className="cursor-pointer">{modulo}</Label>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button onClick={handleSavePlano} className="bg-amber-500">Salvar</Button>
          </div>
        </SheetContent>
      </Sheet>
        ...
        <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
          ...
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Empresa *</Label>
                <Select value={assinaturaForm.empresa_id} onValueChange={(v) => setAssinaturaForm({...assinaturaForm, empresa_id: v})}>
              ...
              <div>
                <Label>Plano *</Label>
                <Select value={assinaturaForm.plano_id} onValueChange={(v) => setAssinaturaForm({...assinaturaForm, plano_id: v})}>
                  <SelectTrigger className="mt-1.5">
                    <SelectValue placeholder="Selecione" />
                  </SelectTrigger>
                  <SelectContent>
          ...
          <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={() => setShowAssinaturaModal(false)}>Cancelar</Button>
            <Button onClick={handleSaveAssinatura} className="bg-amber-500">Salvar</Button>
          </div>
        </SheetContent>
      </Sheet>
      ...
      <Sheet open={showPropostaModal} onOpenChange={setShowPropostaModal}>
        <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
          ...
          <div className="space-y-4 py-4">
            <Input placeholder="Empresa" value={propostaForm.empresa_nome} onChange={(e) => setPropostaForm({...propostaForm, empresa_nome: e.target.value})} />
            <Select value={propostaForm.plano_id} onValueChange={(v) => setPropostaForm({...propostaForm, plano_id: v})}>
              <SelectTrigger>
          ...
          <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={() => setShowPropostaModal(false)}>Cancelar</Button>
            <Button onClick={handleSaveProposta} className="bg-amber-500">Salvar</Button>
          </div>
        </SheetContent>
      </Sheet>
      ...
      <Sheet open={showBoletoModal} onOpenChange={setShowBoletoModal}>
        <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
          ...
          <div className="space-y-4 py-4">
            <Select value={boletoForm.empresa_id} onValueChange={(v) => setBoletoForm({...boletoForm, empresa_id: v})}>
              <SelectTrigger>
            ...
            <Input type="number" placeholder="Valor" value={boletoForm.valor} onChange={(e) => setBoletoForm({...boletoForm, valor: parseFloat(e.target.value)})} />
            <Input type="date" value={boletoForm.data_vencimento} onChange={(e) => setBoletoForm({...boletoForm, data_vencimento: e.target.value})} />
          </div>
          <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={() => setShowBoletoModal(false)}>Cancelar</Button>
            <Button onClick={handleSaveBoleto} className="bg-amber-500">Salvar</Button>
          </div>
        </SheetContent>
      </Sheet>
      ...
      <Sheet open={showPagamentoModal} onOpenChange={setShowPagamentoModal}>
        <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
          ...
          <div className="space-y-4 py-4">
            <Select value={pagamentoForm.empresa_id} onValueChange={(v) => setPagamentoForm({...pagamentoForm, empresa_id: v})}>
              <SelectTrigger>
                <SelectValue placeholder="Selecione empresa" />
              </SelectTrigger>
              <SelectContent>
            ...
            <Input type="number" placeholder="Valor" value={pagamentoForm.valor} onChange={(e) => setPagamentoForm({...pagamentoForm, valor: parseFloat(e.target.value)})} />
          </div>
          <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={() => setShowPagamentoModal(false)}>Cancelar</Button>
            <Button onClick={handleSavePagamento} className="bg-amber-500">Salvar</Button>
          </div>
        </SheetContent>
      </Sheet>
      ...
      <Sheet open={showSuperAdminModal} onOpenChange={setShowSuperAdminModal}>
        <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
          ...
          <div className="space-y-4 py-4">
            <Input placeholder="Nome" value={superAdminForm.nome_completo} onChange={(e) => setSuperAdminForm({...superAdminForm, nome_completo: e.target.value})} />
            <Input type="email" placeholder="Email" value={superAdminForm.email} onChange={(e) => setSuperAdminForm({...superAdminForm, email: e.target.value})} />
            <Input type="password" placeholder="Senha" value={superAdminForm.senha} onChange={(e) => setSuperAdminForm({...superAdminForm, senha: e.target.value})} />
          </div>
          <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={() => setShowSuperAdminModal(false)}>Cancelar</Button>
            <Button onClick={handleSaveSuperAdmin} className="bg-amber-500">Criar</Button>
          </div>
        </SheetContent>
      </Sheet>
          ...
          {selectedEmpresa && (
            <Tabs defaultValue="info" className="mt-6">
              <TabsList className="grid w-full grid-cols-3">
                ...
                <TabsTrigger value="usuarios">Usuários ({usuariosEmpresaFiltrados.length})</TabsTrigger>
                <TabsTrigger value="assinatura">Assinatura</TabsTrigger>
              </TabsList>

              <TabsContent value="info" className="space-y-4 py-4">
                <div><p className="text-sm text-slate-500">Nome</p><p className="font-medium">{selectedEmpresa.nome}</p></div>
                <div><p className="text-sm text-slate-500">CNPJ</p><p className="font-medium">{selectedEmpresa.cnpj}</p></div>
                <div><p className="text-sm text-slate-500">Email</p><p className="font-medium">{selectedEmpresa.email}</p></div>
              </TabsContent>

              <TabsContent value="usuarios" className="space-y-4 py-4">
                <div className="flex gap-2">
                  <Button onClick={() => { setCriarAdminForm({ nome_completo: '', email: '', senha: '', confirmar_senha: '' }); setShowCriarAdminModal(true); }} className="bg-amber-500">
                  ...
                  <TableBody>
                    {usuariosEmpresaFiltrados.map(u => (
                      <TableRow key={u.id}>
                        ...
                        <TableCell><Badge className={u.ativo ? 'bg-green-100' : 'bg-red-100'}>{u.ativo ? 'Ativo' : 'Inativo'}</Badge></TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
              ...
              <TabsContent value="assinatura">
                {assinaturas.find(a => a.empresa_id === selectedEmpresa.id) ? (
                  <div className="py-4 space-y-4">
                    <div><p className="text-sm text-slate-500">Plano</p><p className="font-medium">{assinaturas.find(a => a.empresa_id === selectedEmpresa.id)?.plano_nome}</p></div>
                    <div><p className="text-sm text-slate-500">Valor</p><p className="font-medium">{formatCurrency(assinaturas.find(a => a.empresa_id === selectedEmpresa.id)?.valor_mensal)}</p></div>
                    <div><p className="text-sm text-slate-500">Status</p><Badge className={statusColors[assinaturas.find(a => a.empresa_id === selectedEmpresa.id)?.status]}>{assinaturas.find(a => a.empresa_id === selectedEmpresa.id)?.status}</Badge></div>
                  </div>
                ) : (
      ...
      <Sheet open={showCriarAdminModal} onOpenChange={setShowCriarAdminModal}>
        <SheetContent side="right" className="w-full sm:max-w-md overflow-y-auto">
          <SheetHeader>
            ...
            <p className="text-sm text-slate-500">Empresa: <strong>{selectedEmpresa?.nome}</strong></p>
          </SheetHeader>
          <div className="space-y-4 py-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setCriarAdminForm({ ...criarAdminForm, nome_completo: e.target.value })}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setCriarAdminForm({ ...criarAdminForm, confirmar_senha: e.target.value })}
          ...
          <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={() => setShowCriarAdminModal(false)}>Cancelar</Button>
            <Button onClick={handleCriarAdmin} disabled={salvandoAdmin} className="bg-amber-500">
              {salvandoAdmin ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Criando...</> : 'Criar Administrador'}
            </Button>
          </div>
        </SheetContent>
      </Sheet>
