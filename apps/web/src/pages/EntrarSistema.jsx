...
import { HardHat, Loader2, Building2, Shield, BarChart3, Zap, Mail, Lock, Eye, EyeOff, WifiOff } from 'lucide-react';
...
export default function EntrarSistema() {
  ...
  const [lembrarEmail, setLembrarEmail] = useState(() => !!localStorage.getItem('login_email_salvo'));
    ...
    try {
      ...
      try {
        ...
        if (fornResp.data.success) {
          sessionStorage.setItem('custom_auth', JSON.stringify({
      ...
      if (response.data.success) {
        ...
        if (lembrarEmail) { localStorage.setItem('login_email_salvo', email.trim()); }
        else { localStorage.removeItem('login_email_salvo'); }
        ...
        <Card className="w-full max-w-3xl bg-white">
          <CardHeader className="text-center border-b border-slate-200 p-6">
            ...
            <CardDescription className="text-slate-600">
              Olá, {usuarioBase?.nome_completo}! Você tem acesso a {empresasDisponiveis.length} empresa(s).
            </CardDescription>
          </CardHeader>
                  ...
                  {empresasDisponiveis.map(empresa => (
                    ...
                    >
                      <div className="w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0 bg-slate-100">
                        {empresa.logo_url ? (
                          <img src={empresa.logo_url} alt={empresa.nome} className="w-full h-full object-contain rounded-lg" />
                        ) : (
                  ...
                  <div className="grid gap-3">
                    {grupos.map(grupo => (
                      ...
                      >
                        <div className="w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0 bg-slate-100">
                          {grupo.logo_url ? (
                            <img src={grupo.logo_url} alt={grupo.nome} className="w-full h-full object-contain rounded-lg" />
                          ) : (
                            <Building2 className="w-6 h-6 text-slate-500" />
                          )}
                        </div>
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-slate-500">{grupo.cnpj_principal || 'Sem CNPJ'}</p>
                        </div>
                        {loading && (
                          <Loader2 className="w-5 h-5 animate-spin text-blue-600" />
                        )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {!isOnline && (
                <div className="flex items-center gap-2 bg-orange-50 border border-orange-200 rounded-lg p-3">
                  ...
                  <p className="text-sm text-orange-700">Sem conexão — conecte-se para fazer login</p>
                </div>
              )}
        ...
        <div className="max-w-lg space-y-8 text-white">
          ...
          <div className="space-y-4">
            <h2 className="text-4xl font-bold leading-tight">
              Gestão profissional<br />para suas obras
            </h2>
            <p className="text-lg text-white/70">
              Plataforma completa para gerenciar projetos, equipes, orçamentos e muito mais com total controle e eficiência.
            </p>
          </div>
