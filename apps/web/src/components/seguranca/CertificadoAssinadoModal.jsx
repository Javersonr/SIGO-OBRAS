...
export default function CertificadoAssinadoModal({
  open,
  onOpenChange,
  treinamento,
  funcionario,
  ...
  React.useEffect(() => {
    console.log(
      "[CertificadoAssinadoModal] open mudou para:",
      open,
      "| treinamento:",
      treinamento?.id,
      treinamento?.nome,
  ...
  const anexosDeste = treinamento
    ...
    : [];
  console.log(
    "[CertificadoAssinadoModal] render | open:",
    open,
    "| treinamento:",
    treinamento?.id,
    "| funcionario:",
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      let resultData = {
        status: "incompleto",
        mensagem: "Análise não disponível. Certificado salvo.",
        campos_atualizados: {},
      };
      try {
      ...
      if (resultData.status === "válido")
        toast.success(`✅ Certificado anexado! ${resultData.mensagem}`);
      else if (resultData.status === "incompleto")
        toast.warning(`⚠️ Certificado incompleto. ${resultData.mensagem}`);
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent
          className="p-0 flex flex-col h-[90vh] max-w-2xl w-full"
          aria-describedby={undefined}
        >
          <input
            ref={fileInputRef}
          ...
          <div className="px-6 py-4 border-b flex-shrink-0 flex items-start justify-between">
            ...
            <button
              ...
              className="p-2 hover:bg-slate-100 rounded-lg ml-4"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {analiseResultado && (
              <div
                className={`p-4 rounded-lg border-l-4 ${
                  analiseResultado.status === "válido"
                    ? "bg-green-50 border-green-500"
                    : analiseResultado.status === "incompleto"
              ...
              >
                <div className="flex gap-3">
                  ...
                  {(analiseResultado.status === "incompleto" ||
                    analiseResultado.status === "erro") && (
                    <AlertCircle
                      className={`w-5 h-5 flex-shrink-0 mt-0.5 ${analiseResultado.status === "incompleto" ? "text-yellow-600" : "text-red-600"}`}
                    />
                  )}
                  <div className="flex-1">
                    <p
                      className={`font-semibold text-sm ${analiseResultado.status === "válido" ? "text-green-800" : analiseResultado.status === "incompleto" ? "text-yellow-800" : "text-red-800"}`}
                    >
                    ...
                    {analiseResultado.campos_atualizados &&
                      Object.keys(analiseResultado.campos_atualizados).length > 0 && (
                        <div className="mt-2 text-xs space-y-1">
                          <p className="font-medium">Campos atualizados automaticamente:</p>
                          {analiseResultado.campos_atualizados.data_inicio && (
                            <p>
                              • Data de início: {analiseResultado.campos_atualizados.data_inicio}
                            </p>
                          )}
                          {analiseResultado.campos_atualizados.data_fim && (
                            <p>• Data de término: {analiseResultado.campos_atualizados.data_fim}</p>
                          )}
                          {analiseResultado.campos_atualizados.aproveitamento && (
                            <p>
                              • Aproveitamento: {analiseResultado.campos_atualizados.aproveitamento}
                              %
                            </p>
                          )}
                        </div>
            ...
            >
              ...
              {uploading
                ...
                : analisando
                  ? "Analisando..."
                  : "Anexar Novo Certificado Assinado"}
            </Button>
              ...
              {anexosDeste.length === 0 ? (
                ...
                <div className="space-y-2">
                  {anexosDeste.map((anexo, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between p-3 bg-white border border-green-200 rounded-lg hover:bg-green-50 transition-colors"
                    >
