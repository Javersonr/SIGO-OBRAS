-- ============================================================================
-- 0062_compras_identidade_backend.sql — Compras no padrão 0061
--
-- aprovar/rejeitar_solicitacao_compra recebiam email/perfil do CLIENTE — um
-- usuário logado podia se declarar 'Admin' e aprovar fora do seu nível, ou
-- driblar o bloqueio de auto-aprovação trocando o e-mail.
--
-- Mesmo fix da 0061: com JWT presente, e-mail e perfil são SOBRESCRITOS pelos
-- valores reais do token (current_user_email/current_user_perfil); sem vínculo
-- ativo na empresa → erro. Sem JWT (smoke/serviço), os parâmetros seguem
-- valendo. Corpo idêntico ao vigente (0054). Grants preservados (0056 já
-- revogou anon; create or replace mantém).
-- ============================================================================

-- 1. aprovar_solicitacao_compra ----------------------------------------------
