...
import { AlertTriangle, User, Truck, FileWarning, ShieldAlert, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function AlertasConformidadeFerramental({ empresaAtiva, compact = false }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const laudosFuncIds = new Set(laudos.filter(l => l.foto_laudo_url && l.funcionario_id).map(l => l.funcionario_id));
      const funcsSemLaudo = funcionarios.filter(f => !laudosFuncIds.has(f.id));

      // Laudos de caminhões sem foto_laudo_url
      const laudosCamIds = new Set(laudos.filter(l => l.foto_laudo_url && l.caminhao_id).map(l => l.caminhao_id));
      ...
      funcoes.forEach(f => {
        let itens = [];
        try { itens = f.modelo_ferramentas ? JSON.parse(f.modelo_ferramentas) : []; } catch {}
      ...
      ferramentas.forEach(ferr => {
        if (ferr.funcionario_id) {
          if (!ferramentasPorFunc[ferr.funcionario_id]) ferramentasPorFunc[ferr.funcionario_id] = [];
  ...
  const totalAlertas = data.funcsSemLaudo.length + data.caminhoeSemLaudo.length + data.funcsFaltandoFerramentas.length;
    ...
    return (
      <Card className="border-green-300">
        <CardContent className="p-4 flex items-center gap-3">
          ...
          <span className="text-sm text-green-700 font-medium">Conformidade OK — Nenhum alerta de laudo ou ferramentas obrigatórias</span>
        </CardContent>
      </Card>
    );
  }
  ...
  const sections = [
    ...
    },
    {
      ...
      cor: 'yellow',
      itens: data.funcsFaltandoFerramentas.map(f => `${f.funcionario} (${f.funcao}): falta ${f.faltando.join(', ')}`),
    },
  ].filter(s => s.count > 0);

  const corMap = {
    red: { bg: 'bg-red-50', border: 'border-red-200', badge: 'bg-red-100 text-red-700', text: 'text-red-800', sub: 'text-red-600' },
    orange: { bg: 'bg-orange-50', border: 'border-orange-200', badge: 'bg-orange-100 text-orange-700', text: 'text-orange-800', sub: 'text-orange-600' },
    yellow: { bg: 'bg-yellow-50', border: 'border-yellow-200', badge: 'bg-yellow-100 text-yellow-700', text: 'text-yellow-800', sub: 'text-yellow-600' },
  };
    ...
    <Card className="border-red-200">
      <CardHeader className="pb-2 pt-3 px-4">
        <CardTitle className="text-sm flex items-center gap-2">
          ...
          <Badge className="bg-red-100 text-red-700 border-red-200 ml-auto">{totalAlertas} pendências</Badge>
        </CardTitle>
      </CardHeader>
              ...
              {expanded && (
                <div className={`px-3 pb-2 space-y-1 border-t ${c.border}`}>
                  {sec.itens.map((item, i) => (
                    <p key={i} className={`text-xs ${c.sub} py-0.5`}>• {item}</p>
                  ))}
                </div>
              )}
            </div>
